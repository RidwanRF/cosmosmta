...
function dxDrawMetroButtonWithBorder(key, label, x, y, w, h, buttonColor, activeColor, textColor, font, textAlign1, textAlign2, imagePath, imageW, imageH, imageColor)
    ...
    if activeDirectX ~= lastActiveDirectX and activeDirectX then
        playSound(":cosmo_assets/audio/interface/hover.ogg")
    end
end
...
function dxDrawMetroBox(x, y, w, h, color, imagePath, imageW, imageH, imageColor, imageAlign, titleText, titleTextColor, titleTextFont, titleTextAlign, downText, downTextColor, downTextFont, downTextAlign, bg)   
    ...
    if bg then 
        ...
        dxDrawRectangle(x, y, bg, h, tocolor(223, 97, 23, 255 * fadeAmount))
    end
...
function drawScrollbar(key, x, y, w, h, visibleItems, currentItems)
    ...
    dxDrawImage(x, trackY - w, w, w, ":cosmo_groups/files/icons/arrow.png", 0, 0, 0, colorOfArrow)
    ...
    dxDrawImage(x, trackY + trackHeight, w, w, ":cosmo_groups/files/icons/arrow.png", 180, 0, 0, colorOfArrow)
...
function drawButtonSlider(key, state, x, y, h, offColor, onColor)
    ...
    dxDrawImage(x, y, 64, 32, ":cosmo_groups/files/toggleSwitch/off.png", 0, 0, 0, tocolor(255, 255, 255, (255 - alphaDifference) * fadeAmount))
    dxDrawImage(x, y, 64, 32, ":cosmo_groups/files/toggleSwitch/on.png", 0, 0, 0, tocolor(buttonColor[1], buttonColor[2], buttonColor[3], alphaDifference * fadeAmount))
    dxDrawImage(x + buttonSlider.offsets[key], y, 64, 32, ":cosmo_groups/files/toggleSwitch/circle.png", 0, 0, 0, tocolor(255, 255, 255, 255 * fadeAmount))
end
