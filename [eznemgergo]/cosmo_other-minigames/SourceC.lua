...
function setBalanceQTEState(state,difficulty, resource)
...
function balanceQTEFail()
	...
	--outputChatBox("[ExternalGaming - Minigame]#ffffff Sajnálom de nem sikerült.",255, 255, 255, true)
	if qte.balance.resource == "Diesel Life_inventory" then 
		exports[qte.balance.resource]:givePenzkaziVisszajelzes(1)
	--elseif qte.balance.resource == "mta_fishing" then 
		--exports[qte.balance.resource]:giveFishing(1)	
	--elseif qte.balance.resource == "mta_dead" then 
		--exports[qte.balance.resource]:wastedBro(1)
	--elseif qte.balance.resource == "CP" then 
		--exports[qte.balance.resource]:giveATMVIsszajelzes(1)
	end
end
...
function createProgressBar(progressTime)
	...
	qte.balance.timer = setTimer(function()
		removeEventHandler("onClientRender",getRootElement(),renderProgressGui)
		setBalanceQTEState(false)
		--outputChatBox("[ExternalGaming - Minigame]#ffffff Sikeresen megcsináltad grat",255, 255, 255, true)
		if qte.balance.resource == "Diesel Life_inventory" then 
			exports[qte.balance.resource]:givePenzkaziVisszajelzes(2)
		--elseif qte.balance.resource == "mta_fishing" then 
			--exports[qte.balance.resource]:giveFishing(2)
		--elseif qte.balance.resource == "mta_dead" then 
			--exports[qte.balance.resource]:wastedBro(2)
		--elseif qte.balance.resource == "CP" then 
			--exports[qte.balance.resource]:giveATMVIsszajelzes(2)
		end
...
function renderProgressGui()
	...
	dxDrawImageSection(currentX,currentY,fuelFullWidth,qte.balance.progressGui.height,0,0,fuelFullWidth,qte.balance.progressGui.height,"files/pasekon.png",0,0,0,tocolor(255,255,255,alpha))
	dxDrawImageSection (currentX+fuelEmptyX,currentY,fuelEmptyWidth,qte.balance.progressGui.height,fuelEmptyX,0,fuelEmptyWidth,qte.balance.progressGui.height,"files/pasekoff.png",0,0,0,tocolor(255,255,255,alpha))
end
