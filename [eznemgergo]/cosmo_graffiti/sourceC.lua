...
local brushData = {
	isActive = false,
	barColor = tocolor(20, 102, 255, 230),
...
local panelMenu = {
	{"Új", tocolor(217, 83, 79), "new"},
	{"Visszalépés", tocolor(240, 173, 78, 230), "undo"},
	{"Előrelépés", tocolor(240, 173, 78, 230), "redo"},
	{"Mentés", tocolor(21, 219, 114), "save"},
	{"Színkeverő", tocolor(20, 102, 255, 230), "palette", false,
		activeWhen = function()
	...
	{"Ecset vastagsága", tocolor(20, 102, 255, 230), "brushsize", false,
		activeWhen = function()
	...
	{"Téma (sötét/világos)", tocolor(20, 102, 255, 230), "theme"},
	{"Mentett graffitik", tocolor(21, 219, 114), "savedgraffitis", 10,
		activeWhen = function()
	...
	--{"Kép betöltése URL-ből", tocolor(244, 184, 235), "url_image", 11,
	--	activeWhen = function()
...
addCommandHandler("graffiti",
	function ()
		print("fasz")
		if exports.cosmo_groups:isPlayerHavePermission(localPlayer, "roadmarks") then
			showTheEditor()
		end
	end
)
...
addCommandHandler("gsize",
	function (command, size)
		if not tonumber(size) or tonumber(size) <= 0 or tonumber(size) > 5 then
			outputChatBox("#ff9428[CosmoMTA - Használat]:#ffffff /" .. command .. " [Size (1 - 5)]", 255, 255, 255, true)
		else
			graffitiSize = tonumber(size)
			outputChatBox("#ff9428[CosmoMTA - Graffiti]:#ffffff A graffiti mérete sikeresen megváltozott #ff9428" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)
	...
	function ()
		if exports.cosmo_groups:isPlayerHavePermission(localPlayer, "cuff") then
			...
			if graffitiCleanMode then
				outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffGraffiti tisztítás mód bekapcsolva.", 255, 255, 255, true)
			else
				outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffGraffiti tisztítás mód kikapcsolva.", 255, 255, 255, true)
			end
		end
	end
)
...
--							outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffElkezdted a kép letöltését. Kérlek várj!", 255, 255, 255, true)
--						else
--							outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffA kiválasztott URL nem létezik!", 255, 255, 255, true)
--						end
--					else
--						outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffElőször írd be a kép URL címét!", 255, 255, 255, true)
--					end
--				else
--					outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffKérlek várj #ff9428" .. 3 - math.floor(deltaTime / 1000) .. " másodpercet#ffffff a következő letöltésig.", 255, 255, 255, true)
--				end
--			else
--				outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffEgy letöltés jelenleg folyamatban!", 255, 255, 255, true)
--			end
--		end
--	end
--)
...
-- 			outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffA kiválasztott kép sikeresen mentve.", 255, 255, 255, true)
-- 			outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffA képet elérheted a 'Mentett graffitik' panelban.", 255, 255, 255, true)
-- 		else
-- 			outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffA kiválasztott kép nem létezik!", 255, 255, 255, true)
-- 		end
	...
	function (fromClient, pixels, data)
		...
		if graffitiFile then
			...
			if fromClient == localPlayer then
				...
				outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffA graffiti felfújása sikerült.", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (weapon, _, _, hitX, hitY, hitZ)
		...
		if lastDrawedPixels and not startedGraffiti.isActive and not startedGraffiti.actionDisabled and #drawingSteps >= 2 and getTickCount() >= nextSprayingAfterTick then
			if getTickCount() >= tryToSprayGraffitiTick then
				...
				outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffElkezdted a graffiti felfújását!", 255, 255, 255, true)
			end
						...
						if screenX and screenY then
							...
							if distanceBetweenGraffiti <= 15 then
								...
								if isCursorWithinArea(cursorX, cursorY, iconPosX, iconPosY, iconSize, iconSize) then
									...
									dxDrawImage(iconPosX, iconPosY, iconSize, iconSize, "files/images/clean.png", 0, 0, 0, tocolor(20, 102, 255, alphaMultiplier))
								end
							end
						end
					end
				...
				if button == "left" and state == "up" then
					if string.find(selected3DButton, "trash|") then
						...
						if serverGraffitis[fileName].isProtected then
							outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffNem törölheted ezt a graffitit mivel 'védett'.", 255, 255, 255, true)
							return
						end
					...
					elseif string.find(selected3DButton, "clean|") then
						...
						if not getPedOccupiedVehicle(localPlayer) then
							if not isTimer(cleaningGraffitiTimer) then
								if serverGraffitis[fileName].isProtected then
									outputChatBox("#ff9428[CosmoMTA - Graffiti]: #fffffffNem tisztíthatod ezt a graffitit mivel 'védett'.", 255, 255, 255, true)
									return
								end
								...
								outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffJelenleg már takarítasz egy graffitit.", 255, 255, 255, true)
							end
						else
							outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffNem használhatod járműben ülve.", 255, 255, 255, true)
						end
					end
				end
			end
...
addEventHandler("onClientPreRender", getRootElement(),
	function ()
		...
		if startedGraffiti.isActive and startedGraffiti.texture and startedGraffiti.data then
			if playerInterior == startedGraffiti.data[14] and playerDimension == startedGraffiti.data[15] then
				...
				if getDistanceBetweenPoints3D(playerX, playerY, playerZ, startedGraffiti.data[10], startedGraffiti.data[11], startedGraffiti.data[12]) <= 3 then
					...
					dxDrawText("Felfújás:", progressbarPosX, progressbarPosY - progressbarHeight - 20, progressbarPosX + progressbarWidth, progressbarPosY + progressbarHeight, tocolor(255, 255, 255), 0.75, mainFont, "left", "center")
		...
		if cleaningGraffitiTimer then
			...
			dxDrawRectangle(progressbarPosX - 2, progressbarPosY - 2, progressbarWidth + 4, progressbarHeight + 4, tocolor(0, 0, 0, 160))
			dxDrawRectangle(progressbarPosX, progressbarPosY, progressbarWidth * barValue / 100, progressbarHeight, tocolor(20, 102, 255, 220))
			dxDrawText("Tisztítás:", progressbarPosX, progressbarPosY - progressbarHeight - 20, progressbarPosX + progressbarWidth, progressbarPosY + progressbarHeight, tocolor(255, 255, 255), 0.75, mainFont, "left", "center")
...
function cleanGraffiti(fileName)
	...
	outputChatBox("#ff9428[CosmoMTA - Graffiti]: #ffffffSikeresen befejezted a graffiti tisztítását.", 255, 255, 255, true)
end
...
function renderTheEditor()
	...
	if colorData.isActive then
		dxDrawText("Graffiti szerkesztő - #ccccccColorpicker", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	elseif savedGraffitisMenu.isActive then
		dxDrawText("Graffiti szerkesztő - #ccccccSaved graffitis", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	--elseif loadImagePanel.isActive then
	--	dxDrawText("Graffiti szerkesztő - #ccccccLoad image from URL", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	--else
		dxDrawText("Graffiti szerkesztő", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	end
	...
	--** Drawing surface
	if not colorData.isActive and not savedGraffitisMenu.isActive then
	...
	--** Colorpicker
	if colorData.isActive and not savedGraffitisMenu.isActive  then
		...
		for k, v in ipairs({"red", "green", "blue"}) do
			...
			if activecolorInput == v then
				dxDrawRectangle(rowX, paintAreaPosY + colorData.paletteHeight + marginOffsetHalf, colorData.colorInputs.rgb.width, 25, tocolor(20, 102, 255, 125))
			else
		...
		if activecolorInput == "hex" then
			dxDrawRectangle(paintAreaPosX + paintAreaWidth - colorData.colorInputs.hex.width, paintAreaPosY + colorData.paletteHeight + marginOffsetHalf, colorData.colorInputs.hex.width, 25, tocolor(20, 102, 255, 125))
		else
	...
	if promptDialog.isActive then
		...
		dxDrawText(promptDialog.text .. "\n\n\nApply #ff9428[ENTER]#ffffff - Cancel #ff9428[BACKSPACE]", panelPosX, panelPosY, panelPosX + panelWidth, panelPosY + panelHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
	end
end
