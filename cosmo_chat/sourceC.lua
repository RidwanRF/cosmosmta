...
addCommandHandler("say",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Üzenet]", 255, 255, 255, true)
			else
	...
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				...
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Cselekvés]", 255, 255, 255, true)
			else
	...
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				...
				sendLocalMeLowAction(localPlayer, string.gsub(table.concat({...}, " "), "#%x%x%x%x%x%x", ""))
			end
		end
	end
)
...
addCommandHandler("do",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Történés]", 255, 255, 255, true)
			else
	...
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				...
				sendLocalDoLowAction(localPlayer, string.gsub(table.concat({...}, " "), "#%x%x%x%x%x%x", ""))
			end
		end
	end
)
...
addCommandHandler("ame",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Vizuális leírás]", 255, 255, 255, true)
			else
	...
	if string.len(message) > 0 then
		...
		if not adminDuty then
			...
			if canLaugh == "szakad a röhögéstől" or canLaugh == "nevet" then
				if not getElementData(localPlayer, "player.Cuffed") and not getElementData(localPlayer,"player.Tazed") then
					triggerServerEvent("laughAnim", player)
				end
			end
			
			if not getElementData(localPlayer, "player.Cuffed") and not getElementData(localPlayer,"player.Tazed") then
				if getElementData(localPlayer, "sayAnim") then
					triggerServerEvent("sayAnimServer", player, message)
				end
			end
		end
		...
		if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
			...
			if occupiedVehicle and not (getElementData(occupiedVehicle, "vehicle.window.0") or getElementData(occupiedVehicle, "vehicle.window.1") or getElementData(occupiedVehicle, "vehicle.window.2") or getElementData(occupiedVehicle, "vehicle.window.3")) then
			...
			local adminTitle = exports.cosmo_core:getPlayerAdminTitleByLevel(adminLevel)
			local adminTitleColor = exports.cosmo_core:getAdminLevelColor(adminLevel)
...
function sendLocalMessageInPhone(player, message)
	if string.len(message) > 0 then
		...
		if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
			...
			if not adminDuty or player ~= localPlayer then
				outputChatBox("#FFFFFF" .. visibleName .. " mondja (telefonba)" .. additionalStr .. ": " .. firstToUpper(message), 16, 16, 16, true)
			else
				outputChatBox(adminTitleColor .. "[" .. adminTitle .. "] " .. visibleName .. " mondja (telefonba)" .. additionalStr .. ": " .. firstToUpper(message), 16, 16, 16, true)
			end
			...
			if pendingCount > 0 then
				triggerServerEvent("onLocalMessageInPhone", localPlayer, pendingPlayers, visibleName, firstToUpper(message), additionalStr, adminLevel, adminTitle, adminTitleColor, adminDuty)
			end
		end
	end
end
addEvent("onClientPlayerLocalMessageInPhone", true)
addEventHandler("onClientPlayerLocalMessageInPhone", getRootElement(), sendLocalMessageInPhone)
...
function sendLocalMeLowAction(player, message)
	if string.len(message) > 0 then
		...
		for k, v in ipairs(getElementsByType("player", getRootElement(), true)) do
			if v ~= (getElementType(player) == "ped" and localPlayer or player) and getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, getElementPosition(v)) <= 5 then
				table.insert(pendingPlayers, v)
				pendingCount = pendingCount + 1
			end
		end

		outputChatBox("#77499a[LOW] " .. visibleName .. " #77499a" .. message, 32, 32, 32, true)
...
addEvent("onClientPlayerLocalMeLow", true)
addEventHandler("onClientPlayerLocalMeLow", getRootElement(), sendLocalMeLowAction)
	...
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") then
			if not (...) then
				...
				local currentStandingMarker = exports.cosmo_interiors:getCurrentStandingMarker()

				if currentStandingMarker then
					local interiorData = exports.cosmo_interiors:getInteriorData(currentStandingMarker)
					...
					if interiorData then
						...
						exports.cosmo_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
					end
				else
					exports.cosmo_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
				end
...
function sendLocalDoLowAction(player, message)
	if string.len(message) > 0 then
		...
		outputChatBox("#c50e30[LOW]* " .. firstToUpper(message) .. " ((#c50e30" .. visibleName .. "))", 64, 64, 64, true)
...
addEvent("onClientPlayerLocalDoLow", true)
addEventHandler("onClientPlayerLocalDoLow", getRootElement(), sendLocalDoLowAction)
				...
				if currentStandingMarker then
					...
					if interiorData then
						if not isPedInVehicle(localPlayer) then
							...
							exports.cosmo_hud:showInfobox("error", "Járműben ülve nem ordíthatsz be az ingatlanba!")
						end
					else
						exports.cosmo_hud:showInfobox("error", "A parancs használatához egy interior markerben kell lenned!")
					end
...
function onOOCChat(command, ...)
	...
	if #message > 0 and utf8.len(message) > 0 and utf8.len(message) <= 128 then
		local visibleName = getElementData(localPlayer, "visibleName"):gsub("_", " ")
		local playerPosX, playerPosY, playerPosZ = getElementPosition(localPlayer)
		--print(visibleName, message)
		--print(message)
		--exports.cosmo_dclog:sendDiscordMessage("**[OOC]** :", "ooclog")
		local pendingPlayers = {}
		local pendingCount = 0
			...
			if occupiedVehicle and not (getElementData(occupiedVehicle, "vehicle.window.0") or getElementData(occupiedVehicle, "vehicle.window.1") or getElementData(occupiedVehicle, "vehicle.window.2") or getElementData(occupiedVehicle, "vehicle.window.3")) then
				...
				for k, v in ipairs(getElementsByType("player", getRootElement(), true)) do
					if v ~= localPlayer then
						...
						if distance <= 5 then
							table.insert(pendingPlayers, {v, string.format("#%.2X%.2X%.2X", interpolateBetween(255, 255, 255, 50, 50, 50, distance / 5, "Linear"))})
						end
					end
				end
			end
...
addCommandHandler("m",
	function (command, ...)
		if getElementData(localPlayer, "loggedIn") and exports.cosmo_groups:isPlayerHavePermission(localPlayer, "megaPhone") then
			if not (...) then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Üzenet]", 255, 255, 255, true)
			else
				local occupiedVehicle = getPedOccupiedVehicle(localPlayer)
				...
				if #message > 0 and utf8.len(message) > 0 then
					if utf8.len((utf8.gsub(message, " ", "") or 0)) > 0 then
						...
						outputChatBox("((" .. visibleName .. ")) Megaphone <O: " .. firstToUpper(message), 246, 255, 0, true)
...
addCommandHandler("acceptsell",
	function()
		if tradeContract then
			...
			-- ingatlan
			elseif tonumber(tradeContract[2][1]) then
				triggerServerEvent("acceptInteriorBuy", localPlayer, tradeContract[1], tradeContract[2][1], tradeContract[3])
			end
...
addEventHandler("sellInteriorNotification", getRootElement(),
	function(seller, inti, price, data)
		if not tradeContract then
			...
			exports.cosmo_hud:showInfobox("info", sellerName .. " el akar adni neked egy ingatlant " .. formatNumber(price) .. " $-ért!")

			outputChatBox(exports.cosmo_core:getServerTag("info") .. "#84FF00" .. sellerName .. " #ffffffel akar adni neked egy ingatlant #FF9600" .. formatNumber(price) .. " $#ffffff-ért!", 0, 0, 0, true)
			
			outputChatBox("    * #FF9600Ingatlan: #ffffff" .. data.name, 255, 255, 255, true)

			outputChatBox(exports.cosmo_core:getServerTag("info") .. "#FF96005 #ffffffperced van elfogadni az ingatlant a #84FF00/acceptsell #ffffffparanccsal.", 0, 0, 0, true)
...
addEventHandler("sellVehicleNotification", getRootElement(),
	function(seller, veh, price)
		if not tradeContract then
			...
			exports.cosmo_hud:showInfobox("info", sellerName .. " el akar adni neked egy járművet " .. formatNumber(price) .. " $-ért!")

			outputChatBox(exports.cosmo_core:getServerTag("info") .. "#84FF00" .. sellerName .. " #ffffffel akar adni neked egy járművet #FF9600" .. formatNumber(price) .. " $#ffffff-ért!", 0, 0, 0, true)
			
			outputChatBox("    * #FF9600Típus: #ffffff" .. exports.cosmo_mods_veh:getVehicleName(veh), 255, 255, 255, true)

			outputChatBox(exports.cosmo_core:getServerTag("info") .. "#FF96005 #ffffffperced van elfogadni a járművet a #84FF00/acceptsell #ffffffparanccsal.", 0, 0, 0, true)
...
addEventHandler("failedToSell", getRootElement(),
	function(errno)
		if errno then
			exports.cosmo_hud:showInfobox("error", errno)
		end
...
addCommandHandler("sell",
	function(cmd, targetPlayer, amount)
		...
		if not (targetPlayer and amount) then
			outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. cmd .. " [Játékos név / ID] [Összeg]", 0, 0, 0, true)
		else
			targetPlayer = exports.cosmo_core:findPlayer(localPlayer, targetPlayer)
				...
				if targetPlayer ~= localPlayer then
					...
					if dist <= 5 and pi == ti and pd == td then
						...
						if amount >= 0 then
							...
							local inti = exports.cosmo_interiors:getCurrentStandingInterior()
							...
							if isElement(pedveh) then
								if not gotSelling then
									...
									outputChatBox(exports.cosmo_core:getServerTag("error") .. "Egy adásvételi szerződés már folyamatban van!", 0, 0, 0, true)
								end
							elseif inti then
								local intidata = exports.cosmo_interiors:getInteriorData(inti[1])
								...
								if intidata then
									if intidata.type == "house" or intidata.type == "garage" then
										if intidata.ownerId == getElementData(localPlayer, "char.ID") then
											...
											outputChatBox(exports.cosmo_core:getServerTag("error") .. "Ez az ingatlan nem a tiéd.", 0, 0, 0, true)
										end
									end
								end
							end
						else
							outputChatBox(exports.cosmo_core:getServerTag("error") .. "Maradjunk a nullánál nagyobb egész számoknál.", 0, 0, 0, true)
						end
					else
						outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled.", 0, 0, 0, true)
					end
				else
					outputChatBox(exports.cosmo_core:getServerTag("error") .. "Magadnak nem adhatod el.", 0, 0, 0, true)
				end
			end
		end
	end)