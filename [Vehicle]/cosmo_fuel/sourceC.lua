...
function loadFonts()
	local fonts = {
		Roboto = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 13, false, "antialiased"),
		CounterFont = exports.cosmo_assets:loadFont("counter2.ttf", 45 * (lcdRTSize / 400), false, "antialiased"),
		CounterPriceFont = exports.cosmo_assets:loadFont("counter.ttf", 24, false, "antialiased"),
		CounterPriceFont2 = exports.cosmo_assets:loadFont("counter2.ttf", 24, false, "antialiased"),
		RobotoL = exports.cosmo_assets:loadFont("Roboto-Light.ttf", 16, false, "cleartype"),
		ThermalPrinter = exports.cosmo_assets:loadFont("ThermalPrinter.ttf", 18, false, "antialiased")
	}
...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedResource)
		if getResourceName(startedResource) == "cosmo_hud" then
		...
		elseif source == getResourceRootElement() then
			...
			for k, v in ipairs(availableStations) do
				if v.stationDetails then
					...
					setObjectScale(v.stationElement, 0.0001)
			...
			if availableStations[stationId] then
				...
				if fuelPos then
					...
					if fuelPos.pistolObjects[fuelType] then
						exports.cosmo_boneattach:attachElementToBone(fuelPos.pistolObjects[fuelType], v, 12, 0.05, 0.05, 0.05, 180, 0, 0)

						if v == localPlayer then
							exports.cosmo_controls:toggleControl({"fire", "enter_exit", "aim_weapon"}, false)
			...
			if pistolHolder then
				...
				if availableStations[stationId] then
					...
					if fuelPos then
						...
						if fuelPos.pistolObjects[fuelType] then
							exports.cosmo_boneattach:attachElementToBone(fuelPos.pistolObjects[fuelType], source, 12, 0.05, 0.05, 0.05, 180, 0, 0)
			...
			elseif oldValue then
				...
				if availableStations[stationId] then
					...
					if fuelPos then
						...
						if fuelPos.pistolObjects[fuelType] then
							exports.cosmo_boneattach:detachElementFromBone(fuelPos.pistolObjects[fuelType])
							...
							if source == localPlayer then
								exports.cosmo_controls:toggleControl({"fire", "enter_exit", "aim_weapon"}, true)
			...
			if stationId and positionId then
				...
				if pistolHolder then
					if pistolHolder[1] ~= stationId or pistolHolder[2] ~= positionId then
						exports.cosmo_hud:showInfobox("error", "Ezt a kutat jelenleg nem használhatod!")
						return
					end
...
function generatePriceTable()
	if activeSyncedStation and isElement(availableStations[activeSyncedStation].priceTable.renderTarget) then
		...
		for i = 0, 0 do
			...
			dxDrawText("5000", 150 + x, 121, 150 + 64 + x, 121 + 40, tocolor(255, 255, 255, 150), 1, CounterPriceFont, "left", "center")
			...
			dxDrawText("3000", 150 + x, 189, 150 + 64 + x, 189 + 40, tocolor(255, 255, 255, 150), 1, CounterPriceFont, "left", "center")
				...
				if not press or getTickCount() - lastPressTick >= 1000 then
					...
					if press and hoverVehicle then
						if getElementData(hoverVehicle, "vehicle.engine") == 1 then
							exports.cosmo_hud:showInfobox("error", "A jármű motorja jár!")
							return
						end
						...
						if vehicleFuelLeft[hoverVehicle] + vehicleAddFuel[hoverVehicle] >= (fuelTankSize[getElementModel(hoverVehicle)] or 50) then
							exports.cosmo_hud:showInfobox("error", "A jármű tankja tele van!")
							return
						end
					end
...
addEventHandler("payFuel", getRootElement(),
	function (success)
		if not success then
			exports.cosmo_hud:showInfobox("error", "Nincs nálad elég pénz!")
		else
			...
			exports.cosmo_hud:showInfobox("success", "A tankolt üzemanyag sikeresen kifizetve.")
		end
	end
)
...
addEventHandler("onClientClick", getRootElement(),
	function (button, state, _, _, worldX, worldY, worldZ, clickedWorld)
		if clickedWorld and getElementType(clickedWorld) == "ped" and state == "down" and not showReciept and getElementData(clickedWorld, "stationId") and exports.cosmo_core:inDistance3D(clickedWorld, localPlayer, 4) then
...
function onClientClick(button, state, _, _, worldX, worldY, worldZ, clickedWorld)
	if button == "right" and state == "down" then
		...
		if isElement(clickedWorld) and getElementData(clickedWorld, "isFuelPump") and exports.cosmo_core:inDistance3D(clickedWorld, localPlayer, 3) then
			local pistolHolder = getElementData(localPlayer, "pistolHolder")
			local stationId = activePump[1]
			local positionId = activePump[2]
			...
			if pistolHolder then
				if pistolHolder[1] == stationId and pistolHolder[2] == positionId then
					interactionTable = {{"Pisztoly visszarakása", ":cosmo_fuel/files/images/resetPistol.png", "resetPumpPistol"}}
				else
					interactionTable = {}
					exports.cosmo_hud:showInfobox("error", "Ezt a kutat jelenleg nem használhatod!")
				end
				...
				if canUse then
					interactionTable = {
						{"\"Diesel\" pisztoly levétele", ":cosmo_fuel/files/images/pickupPistol.png", "requestPumpPistol"},
						{"\"Regular 87\" pisztoly levétele", ":cosmo_fuel/files/images/pickupPistol.png", "requestPumpPistol"}
					}
				else
					interactionTable = {}
					exports.cosmo_hud:showInfobox("error", "Ezt a kutat jelenleg nem használhatod!")
...
function onClientRender()
	if activeSyncedStation then
		...
		if showReciept and activeStation then
			...
			dxDrawText("FUEL NYUGTA\n---------------------", x, y, x + sx, y + 90, tocolor(0, 0, 0), 0.8, ThermalPrinter, "center", "center")

			dxDrawText("Üzemanyag:", x + 10, y + 110, 0, 0, tocolor(0, 0, 0), 0.6, ThermalPrinter, "left", "top")
			
			dxDrawText(math.floor(availableStations[stationId].fuelPositions[positionId].currentAmount * 10) / 10 .. " Liter", x, y + 110, x + sx - 10, 0, tocolor(0, 0, 0), 0.5, ThermalPrinter, "right", "top")
		
			dxDrawText("Liter ár:", x + 10, y + 135, 0, 0, tocolor(0, 0, 0), 0.6, ThermalPrinter, "left", "top")
			...
			drawButton("pay", "Kifizetés", x + sx / 2, y + sy, sx / 2, 35, 255, 148, 40, 1, Roboto, 1)
		...
		for k = 1, #fuelPositions do
			...
			if v.renderTarget then
				...
				dxDrawText(math.floor(v.currentAmount * 10) / 10 .. " Liter", 0, 41, lcdRTSize, 42 + 30, tocolor(0, 0, 0, 200), 1, CounterFont, "right", "center")
			...
			if v.fillingStarted then
				...
				if havePistol and selectedVehicle and havePistol[1] == activeSyncedStation and havePistol[2] == k then
					...
					if vehicleFuelLeft[selectedVehicle] + vehicleAddFuel[selectedVehicle] >= (fuelTankSize[getElementModel(selectedVehicle)] or 50) then
						...
						exports.cosmo_hud:showInfobox("info", "Teletankoltad a járművet.")
					end
				end
			end
		end
		...
		if havePistol and not pistolTaken then
			...
			if fuelPos then
				...
				if fuelPos.checkRotation(angle) then
					...
					exports.cosmo_hud:showInfobox("warning", "Itt nem tud átmenni a cső!")
				...
				if getDistanceBetweenPoints3D(lineStartX, lineStartY, lineStartZ, playerPosX, playerPosY, playerPosZ) >= 3 then
					...
					exports.cosmo_hud:showInfobox("warning", "Túl messzire mentél a csővel!")
...
addEventHandler("onClientVehicleEnter", getRootElement(),
	function (player, seat)
		local playerAuto = getPedOccupiedVehicle(localPlayer)
	end
)
...
setTimer(function()
	if isPedInVehicle(localPlayer) then 
		--if tonumber(getPedOccupiedVehicle(localPlayer):getData("vehicle.fuel") or 0) > 0 then
		if getElementData(playerAuto,"vehicle.fuel") > 0 then
			--if getPedOccupiedVehicle(localPlayer):getData("veh:motor") then
			if getElementData(playerAuto,"vehicle.engine") then
				local fogyaszt = 0
				...
				--getPedOccupiedVehicle(localPlayer):setData("vehicle.fuel",getPedOccupiedVehicle(localPlayer):getData("vehicle.fuel") - fogyaszt)
				setElementData(playerAuto, "vehicle.fuel", getElementData(playerAuto,"vehicle.fuel") - fogyaszt)
			end
		else
			setElementData(playerAuto, "vehicle.fuel", 0)
			setElementData(playerAuto, "vehicle.engine", false)
			setVehicleEngineState(playerAuto, false)
		end
	end
end,1000*30,0)
