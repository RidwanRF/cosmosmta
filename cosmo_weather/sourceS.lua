...
if #availableWeathers > 1 then
	...
	fetchRemote("https://api.openweathermap.org/data/2.5/weather?q=Miskolc&appid=3ac308c24bbfa0322a12665715b7a6c0",
		function(data)
			local result = fromJSON(data)
			local temp = tonumber((result["main"]["temp"])/17)
		    local weather = tostring(result["main"]["description"]) or "Overcast"
			...
			outputDebugString("Időjárás: " .. weatherNames[weather] .. ", Hőmérséklet: " .. temp .. " °C")
		end,
	nil, true)
end
...
exports.cosmo_admin:addAdminCommand("settime", 8, "Szerver idő módosítása")
addCommandHandler("settime",
	function (player, command, hour, minute)
		if getElementData(player, "acc.adminLevel") >= 8 then
			if not hour then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [óra (* = valós idő)] [perc]", player, 0, 0, 0, true)
			elseif hour == "*" then
				setServerTime(false, false, false)
				outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Az idő sikeresen visszaállítva a valós időhöz.", player, 0, 0, 0, true)
			else
				hour = tonumber(hour) or 12
				minute = tonumber(minute) or 0

				if hour < 0 or hour > 23 then
					outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Az óra nem lehet kisebb mint #ff46460 #ffffffés nem lehet nagyobb mint #ff464623#ffffff.", player, 0, 0, 0, true)
					return
				end

				if minute < 0 and minute > 59 then
					outputChatBox(exports.cosmo_core:getServerTag("admin") .. "A perc nem lehet kisebb mint #ff46460 #ffffffés nem lehet nagyobb mint #ff464659#ffffff.", player, 0, 0, 0, true)
					return
				end

				setServerTime(hour, minute, true)
				outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Az idő sikeresen átállítva! #ffa600(" .. string.format("%.2i:%.2i", hour, minute) .. ")", player, 0, 0, 0, true)
			end
		end
	end
)

exports.cosmo_admin:addAdminCommand("setweather", 8, "Szerver időjárás módosítása")
addCommandHandler("setweather",
	function (player, command, weatherId)
		if getElementData(player, "acc.adminLevel") >= 8 then
			if not weatherId then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [időjárás id (* = random)]", player, 0, 0, 0, true)
			elseif weatherId == "*" then
				setServerWeather(false, false)
			else
				weatherId = tonumber(weatherId) or 0
				setServerWeather(weatherId, true)
				outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Az időjárás sikeresen átállítva! #ffa600(" .. weatherId .. ")", player, 0, 0, 0, true)
			end
		end
	end
)