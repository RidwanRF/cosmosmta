	...
	function (vehicle)
		if isElement(vehicle) then
			if getElementData(localPlayer, "loggedIn") then
				...
				local soundElement = playSound3D("files/airride.mp3", x, y, z)
...
function renderAirRidePanel()
	...
	dxDrawImage(panelPosX, panelPosY, 160, 310, "files/panel.png")

	if memorySet then
		dxDrawImage(panelPosX, panelPosY, 160, 310, "files/ledorange.png")
	elseif getTickCount() - clickTick <= 520 then
		dxDrawImage(panelPosX, panelPosY, 160, 310, "files/led" .. ledColor .. ".png")
	end
	...
	if 16 - airRideLevel > 0 then
		dxDrawImage(panelPosX + 34 + arrowSize * (16 - airRideLevel - 1), panelPosY + 94, arrowSize, 12, "files/arrow.png", 180, 0, 0, tocolor(0, 0, 0, 160))
	end

	if 16 - airRideLevel < 16 then
		dxDrawImage(panelPosX + 34 + arrowSize * (16 - airRideLevel + 1), panelPosY + 94, arrowSize, 12, "files/arrow.png", 0, 0, 0, tocolor(0, 0, 0, 160))
	end
end
...
addCommandHandler("airride",
	function ()
		if isPedInVehicle(localPlayer) and getPedOccupiedVehicleSeat(localPlayer) == 0 then
			...
			if not getElementData(currVeh, "danihe->tuning->airride") or getElementData(currVeh, "danihe->tuning->airride") == 0 then
				return
			end
