...
local nonVisibleComponent = false

--[[
fonts = {}
fonts["default_10"] = dxCreateFont("fonts/roboto.ttf",10)
fonts["default_10"] = dxCreateFont("fonts/roboto.ttf",11)
fonts["default_10"] = dxCreateFont("fonts/roboto_bold.ttf",10)
fonts["default_10"] = dxCreateFont("fonts/roboto_bold.ttf",11)
...
function loadFonts()
	fonts = {
		default_10 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 10, false, "antialiased"),
		default_bold_10 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 10, false, "antialiased"),
		default_bold_11 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 11, false, "antialiased"),
		bebasnue_14 = exports.cosmo_assets:loadFont("BebasNeue.otf", 14, false, "antialiased"),
		fontawsome_15 = exports.cosmo_assets:loadFont("FontAwesome.ttf", 15, false, "antialiased")
	}
end
...
addEventHandler("onClientRender",root,
	function()
		...
		if upgrade["state"] then
			...
			dxDrawCircleRectangle(ppos[1]+1,ppos[2]+1,(psize[1]-2)*animation,psize[2]-2,tocolor(255, 148, 40,125),4,true)
			dxDrawText(upgrade["text"],ppos[1]+5,ppos[2]-20,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"left","top",false,false,true,true)
			dxDrawText(roundNumber(animation*100) .. "%",ppos[1]+psize[1]-5,ppos[2]-20,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"right","top",false,false,true,true)
		...
		if promptbox["state"] then
			...
			dxDrawText("Valóban szeretnéd megvásárolni a kiválasztott element a járművedre?",s[1]/2,s[2]/2-100+anim_y,nil,nil,tocolor(222,222,222,222*alpha),1,fonts["default_10"],"center","top",false,false,true,true)
			...
			dxDrawCircleRectangle(s[1]/2-350/2,s[2]/2-30-30/2+anim_y,350,35,tocolor(124, 197, 118,150*alpha),9,true)
			dxDrawText("Vásárlás (Enter)",s[1]/2,s[2]/2-30-30/2+30/2+2+anim_y,nil,nil,tocolor(185,185,185,200*alpha),1,fonts["default_10"],"center","center",false,false,true,true)

			dxDrawCircleRectangle(s[1]/2-350/2,s[2]/2+20-30/2+anim_y,350,35,tocolor(214,76,69,150*alpha),9,true)
			dxDrawText("Mégse (Backspace)",s[1]/2,s[2]/2+20-30/2+30/2+2+anim_y,nil,nil,tocolor(185,185,185,200*alpha),1,fonts["default_10"],"center","center",false,false,true,true)
		end
		if tuning then
				...
				if k <= max_box then
					...
					if loopedTable[box_id].handling and hovered == k then
						local perf_data = {
							...
							[4] = {text="Fékezés",handlingData={"brakeDeceleration","brakeBias"},maxAmount=700},
						}
						...
						for i,row in ipairs(perf_data) do
							...
							dxDrawText(row.text,descP[1],descP[2]-20+(i-1)*40,nil,nil,tocolor(200,200,200,255),1,fonts["default_10"],"left","top")
							...
							if handlingWhitout > 0 then
								if handlingWhitout+loopedTable[box_id].handling[2] > handlingCount then
									dxDrawRectangle(descP[1],descP[2]+(i-1)*40,(row_width/row.maxAmount)*(handlingWhitout+loopedTable[box_id].handling[2]),12,tocolor(124, 197, 118,255))
									...
									dxDrawCircleRectangle(descP[1]+descS[1]-30,descP[2]+(i-1)*40-30+12,30,30,tocolor(124, 197, 118,200),5)
									dxDrawText(perfNumber,descP[1]+descS[1]-30/2,descP[2]+(i-1)*40-30+12+31/2,nil,nil,tocolor(30,30,30,222),1,fonts["default_10"],"center","center",false,false,false,true)
					...
					if loopedTable[box_id].price then
							...
							if data == box_id then
								...
								if loopedTable[box_id].price == 0 then
									...
									if loopedTable[box_id].priceType == "Money" then
										dxDrawText("#74b347" .. format(loopedTable[box_id].price) .. "$",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
									else
										dxDrawText("#5490c4" .. format(loopedTable[box_id].price) .. "PP",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
									end
								end
							end
						...
						elseif loopedTable.matrica then
							if storedTuning == box_id-1 then
								dxDrawText("Felszerelve",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
							else
								if loopedTable[box_id].price == 0 then
						...
						elseif loopedTable.facelift then
							if storedTuning == box_id-1 then
								dxDrawText("Felszerelve",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
							else
								if loopedTable[box_id].price == 0 then
						...
						elseif loopedTable.wheeltexture then
							if storedTuning == box_id-1 then
								dxDrawText("Felszerelve",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
							else
								if loopedTable[box_id].price == 0 then
					...
					elseif loopedTable[box_id].stickerEditing then
						...
						dxDrawText("#74b347" .. format(getStickerPrice(stickers[#stickers][5])) .. "PP",boxP[1]+boxS[1]/2,boxP[2]+boxS[2]-10,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"center","bottom",false,false,false,true)
					end		
				end
			end
			...
			local vehicle_name = exports.cosmo_mods_veh:getVehicleName(enteredVehicle)
			local width = dxGetTextWidth(vehicle_name,1,fonts["default_10"],false)+30
			dxDrawCircleRectangle(pos[1],40,width,30,tocolor(20,20,20,140),5)
			dxDrawText(vehicle_name,pos[1]+width/2,40+30/2,nil,nil,tocolor(200,200,200,200),1,fonts["default_10"],"center","center",false,false,false,true)

			local money = format(getElementData(localPlayer,"char.Money")) .. "$"
			local width = dxGetTextWidth(money,1,fonts["default_10"],false)+30
			dxDrawCircleRectangle(s[1]-80-width,40,width,30,tocolor(20,20,20,140),5)
			dxDrawText("#74b347" .. money,s[1]-80-width/2,40+30/2,nil,nil,tocolor(200,200,200,200),1,fonts["default_10"],"center","center",false,false,false,true)

			local pp = format(getElementData(localPlayer,"char.PP") or 0) .. "PP"
			local width = dxGetTextWidth(pp,1,fonts["default_10"],false)+30
			dxDrawCircleRectangle(s[1]-80-width,80,width,30,tocolor(20,20,20,140),5)
			dxDrawText("#5490c4" .. pp,s[1]-80-width/2,80+30/2,nil,nil,tocolor(200,200,200,200),1,fonts["default_10"],"center","center",false,false,false,true)
			...
			if loopedTable.paintID then
				if anySliderMoving() then
					...
					if loopedTable.paintID == 1 then
						...
						setVehicleColor(enteredVehicle, r,g,b)
			...
			--	dxDrawText("Törléshez nyomd meg a '[DELETE]' billentyűt!",pos[1]+10+1,pos[2]-30+1,nil,nil,tocolor(0,0,0,100),1,fonts["default_10"],"left","top",false,false,false,true)
			--	dxDrawText("Törléshez nyomd meg a '[DELETE]' billentyűt!",pos[1]+10-1,pos[2]-30+1,nil,nil,tocolor(0,0,0,100),1,fonts["default_10"],"left","top",false,false,false,true)
			--	dxDrawText("Törléshez nyomd meg a '[DELETE]' billentyűt!",pos[1]+10+1,pos[2]-30-1,nil,nil,tocolor(0,0,0,100),1,fonts["default_10"],"left","top",false,false,false,true)
			--	dxDrawText("Törléshez nyomd meg a '[DELETE]' billentyűt!",pos[1]+10-1,pos[2]-30-1,nil,nil,tocolor(0,0,0,100),1,fonts["default_10"],"left","top",false,false,false,true)
			--	dxDrawText("Törléshez nyomd meg a #d64c45'[DELETE]' billentyűt!",pos[1]+10,pos[2]-30,nil,nil,tocolor(222,222,222,222),1,fonts["default_10"],"left","top",false,false,false,true)
			--end
		end
)
	...
	function(k,p)
		if tuning then
				...
				if not promptbox["state"] then
					if #loopedTable[hovered+scroll] > 0 then
						if loopedTable[hovered+scroll].wheelID then
							if loopedTable[hovered+scroll].wheelID == storedTuning.id then
								--exports.cosmo_hud:showInfobox("error","A kiválasztott elem, jelenleg is fel van szerelve a járművedre.")
								--return
							end
						end
						...
						if loopedTable[hovered+scroll].upgradeSlot then
							...
							if #compatibleUpgrades > 0 then
								...
								exports.cosmo_hud:showInfobox("error","A kiválasztott elem, nem kompatibilis a járműveddel.")
								return
							end
						end
						...
						if loopedTable[hovered+scroll].matrica then
							storedTuning = getElementData(enteredVehicle,"danihe->vehicles->matrica") or 0
							setElementData(enteredVehicle,"danihe->vehicles->matrica",0)
						end
						if loopedTable[hovered+scroll].facelift then
							storedTuning = getElementData(enteredVehicle,"danihe->vehicles->facelift") or 0
							setElementData(enteredVehicle,"danihe->vehicles->facelift",0)
						end
						if loopedTable[hovered+scroll].wheeltexture then
							storedTuning = getElementData(enteredVehicle,"danihe->vehicles->wheeltexture") or 0
							setElementData(enteredVehicle,"danihe->vehicles->wheeltexture",0)
						end
						...
						if loopedTable.cameraSettings then
							...
							if cameraSettings[5] then
								...
								nonVisibleComponent = cameraSettings[1]
							end
						end
							...
							if data == hovered+scroll then
								...
								if loopedTable[hovered+scroll].price == 0 then
									...
									if loopedTable[hovered+scroll].priceType == "Money" then
										promptbox["priceText"] = "#74b347" .. format(loopedTable[hovered+scroll].price) .. "$"
									else
										promptbox["priceText"] = "#5490c4" .. format(loopedTable[hovered+scroll].price) .. "PP"
									end
								end
						...
						elseif loopedTable.matrica then
							if getElementData(enteredVehicle,"danihe->vehicles->matrica") == storedTuning then
								exports.cosmo_hud:showInfobox("error","A kiválasztott elem, jelenleg is fel van szerelve a járművedre.")
							else
								promptbox["state"] = true
								promptbox["tick"] = getTickCount()
						...
						elseif loopedTable.facelift then
							if getElementData(enteredVehicle,"danihe->vehicles->facelift") == storedTuning then
								exports.cosmo_hud:showInfobox("error","A kiválasztott elem, jelenleg is fel van szerelve a járművedre.")
							else
								promptbox["state"] = true
								promptbox["tick"] = getTickCount()
						...
						elseif loopedTable.wheeltexture then
							if getElementData(enteredVehicle,"danihe->vehicles->wheeltexture") == storedTuning then
								exports.cosmo_hud:showInfobox("error","A kiválasztott elem, jelenleg is fel van szerelve a járművedre.")
							else
								promptbox["state"] = true
								promptbox["tick"] = getTickCount()
					...
					if getElementData(localPlayer,"char." .. loopedTable[hovered+scroll].priceType) >= price then
						if loopedTable[hovered+scroll].exhaust_id then --// Egyedi kipufogó
							...
							exports.cosmo_hud:showInfobox("info","Sikeresen megvásároltad a kiválasztott elemet.(" .. promptbox["text"] .. ")")
						...
						elseif loopedTable.variant then --// Variáns
							...
							hovered = 1
							scroll = 0
						elseif loopedTable.matrica then --// Variáns
							toggleUpgradeLine("Felszerelés..","sounds/upgrade.ogg")

							exports.cosmo_hud:showInfobox("info","Sikeresen megvásároltad a kiválasztott elemet.(" .. promptbox["text"] .. ")")
							triggerServerEvent("buyTuning",resourceRoot,localPlayer,enteredVehicle,price,loopedTable[hovered+scroll].priceType,"danihe->vehicles->matrica",hovered+scroll-1)
							category = 4
							sub_category = 0
							sub_sub_category = 0
							loopedTable = tuning_options[category]
							...
							hovered = 1
							scroll = 0
						elseif loopedTable.facelift then --// Variáns
							toggleUpgradeLine("Felszerelés..","sounds/upgrade.ogg")

							exports.cosmo_hud:showInfobox("info","Sikeresen megvásároltad a kiválasztott elemet.(" .. promptbox["text"] .. ")")
							triggerServerEvent("buyTuning",resourceRoot,localPlayer,enteredVehicle,price,loopedTable[hovered+scroll].priceType,"danihe->vehicles->facelift",hovered+scroll-1)
							category = 4
							sub_category = 0
							sub_sub_category = 0
							loopedTable = tuning_options[category]
							...
							hovered = 1
							scroll = 0
						elseif loopedTable.wheeltexture then --// kerékmatrica
							toggleUpgradeLine("Felszerelés..","sounds/upgrade.ogg")

							exports.cosmo_hud:showInfobox("info","Sikeresen megvásároltad a kiválasztott elemet.(" .. promptbox["text"] .. ")")
							triggerServerEvent("buyTuning",resourceRoot,localPlayer,enteredVehicle,price,loopedTable[hovered+scroll].priceType,"danihe->vehicles->wheeltexture",hovered+scroll-1)
							category = 4
							sub_category = 0
							sub_sub_category = 0
							loopedTable = tuning_options[category]
						...
						elseif loopedTable.data == "danihe->tuning->nitro" and hovered+scroll == 3 then --// Nitro feltöltés
							if getElementData(enteredVehicle,loopedTable.data) == 1 then
								...
								if nitroprecent < 100 then
									...
									exports.cosmo_hud:showInfobox("error","A nitro palack teljesen felvan töltve!")
								end
							else
								exports.cosmo_hud:showInfobox("error","Nincs nitro palack szerelve a járművedbe!")
							end
						...
						if loopedTable[hovered+scroll].priceType == "Money" then
							exports.cosmo_hud:showInfobox("error","Nincs elegendő készpénzed a vásárláshoz!")
						else
							exports.cosmo_hud:showInfobox("error","Nincs elegendő prémium pontod a vásárláshoz!")
						end
					end
				...
				if not promptbox["state"] then
					...
					if category > 0 then
						if storedTuning then
							...
							if loopedTable.matrica then
								setElementData(enteredVehicle,"danihe->vehicles->matrica",storedTuning)
							end
							if loopedTable.facelift then
								setElementData(enteredVehicle,"danihe->vehicles->facelift",storedTuning)
							end
							if loopedTable.wheeltexture then
								setElementData(enteredVehicle,"danihe->vehicles->wheeltexture",storedTuning)
							end
			...
			elseif k == "arrow_r" and p and not stickerSelect then
				...
				if loopedTable.matrica then
					setElementData(enteredVehicle,"danihe->vehicles->matrica",hovered+scroll-1)
				end
				if loopedTable.facelift then
					setElementData(enteredVehicle,"danihe->vehicles->facelift",hovered+scroll-1)
				end
				if loopedTable.wheeltexture then
					setElementData(enteredVehicle,"danihe->vehicles->wheeltexture",hovered+scroll-1)
				end
				if #compatibleUpgrades > 0 then
...
function exitTuning()
	...
	triggerServerEvent("savePerformanceTuning",resourceRoot,localPlayer,enteredVehicle)

	triggerServerEvent("exitTuning",resourceRoot,localPlayer,enteredVehicle)
	
	if enteredTuning ~= 0 then
				...
				if getPedOccupiedVehicle(localPlayer) then
					if getPedOccupiedVehicleSeat(localPlayer) == 0 then
						...
						if (getElementData(enteredVehicle,"vehicle.owner") == getElementData(localPlayer,"char.ID")) or getElementData(localPlayer,"acc.adminLevel") >= 9 then
							...
							exports.cosmo_hud:showInfobox("error","Ezt a járművet nem tuningolhatod, mert nem a tied!")
						end
					end
				end
			end
	...
	function(cmd)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if enteredVehicle then
				...
				triggerServerEvent("enterTuning", resourceRoot, localPlayer, enteredVehicle, enteredTuning, true)

				local cameraRotation = 110
...
function moveCameraToDefaultPosition()
	...
	if nonVisibleComponent then
		--setVehicleComponentVisible(enteredVehicle, nonVisibleComponent, true)
		nonVisibleComponent = false
	end
		...
		for vehicle,data in pairs(blowOff) do
			...
			if getVehicleCurrentGear(vehicle) ~= data.old_gear then
				if getElementData(vehicle,"danihe->tuning->turbo") >= 4 then
					...
					elseif getElementData(vehicle,"danihe->tuning->turbo") == 5 then
						local sound = playSound3D("sounds/charger.mp3",p)
						attachElements(sound,vehicle)
						setSoundVolume(sound, 0.7) -- set the sound volume to 50%
					end	
				end
			-- 	if getElementData(vehicle,"danihe->tuning->backfire") == 1 then
			...
			-- 			--[[if exports.cosmo_handling:getDefaultHandling(getElementModel(vehicle))["modelFlags"] == "0x2000" or exports.cosmo_handling:getDefaultHandling(getElementModel(vehicle))["modelFlags"] == "double_exhaust" then
			-- 				local offset = {getVehicleModelExhaustFumesPosition(getElementModel(vehicle))}
			-- 				local x,y,z = getPositionFromElementOffset(vehicle,-offset[1],offset[2],offset[3])
			...
			-- 			if doubleBackfireVehicles[model] then
			-- 				local offset = {getVehicleModelExhaustFumesPosition(getElementModel(vehicle))}
			-- 				local x,y,z = getPositionFromElementOffset(vehicle,-offset[1],offset[2],offset[3])
			-- 				data.effect2 = createEffect("gunflash",x,y,z, 90, 0, 180)
			-- 				setEffectSpeed(data.effect2, 1.5)
