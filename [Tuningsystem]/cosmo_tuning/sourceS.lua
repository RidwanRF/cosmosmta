mysql = exports.cosmo_database:getConnection()
...
function loadTunedHandling(data,old,new)
		if getElementType(source) == "vehicle" then
			local vehicle = source
			if findPerformanceByData(data,1) then
				if not old then old = 0 end
				--outputChatBox(data..", "..old..", "..new,root,255,255,255,true)
...
addEventHandler("onElementDataChange",root,loadTunedHandling)
...
addEventHandler("enterTuning", resourceRoot,
	function(player, vehicle, tuning_id, adminCommand)
		if player and vehicle then
...
addCommandHandler("fulltuning", function(player, vehicle)
	if getElementData(player, "acc.adminLevel") >= 6 then
		local vehicle = getPedOccupiedVehicle(player)
		outputChatBox("Sikeresen fullosra tuningoltad a járgányt haver", player, 255, 255, 255, true)
		...
		setElementData(vehicle, "danihe->tuning->turbo", 4)
		...
		setElementData(vehicle, "danihe->tuning->backfire", 1)
