...
addEventHandler("onClientElementStreamOut", getRootElement(),
	function ()
		if getElementType(source) == "vehicle" then
			local paintjobId = getElementData(source, "danihe->vehicles->matrica") or 0
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		for k, v in ipairs(getElementsByType("vehicle", getRootElement(), true)) do
			local paintjobId = getElementData(v, "danihe->vehicles->matrica") or 0
...
addEventHandler("onClientElementDataChange", getRootElement(),
	function (dataName, oldValue, newValue)
		if dataName == "danihe->vehicles->matrica" then
			if not newValue or newValue == 0 then
				removeVehiclePaintjob(source)
				--print("newPaintjobApplied")
			else
