...
function loadVehicle(data)
	if data then
		...
		if vehicle then
			...
			exports.cosmo_handling:loadHandling(vehicle)
			...
			setElementData(vehicle, "danihe->tuning->supercharger", data["supercharger"])
			...
			setElementData(vehicle,"danihe->vehicles->customhorn", tonumber(data.customhorn))
			setElementData(vehicle,"danihe->tuning->supercharger", tonumber(data["supercharger"]))
			...
			if tuningData["customhorn"] then
                setElementData(vehicle, "danihe->vehicles->customhorn", tuningData["customhorn"] or 0)
            end
	...
	if vehicleID > 0 then
		do
			...
			local datas = {
				...
				customhorn = getElementData(vehicle,"danihe->vehicles->customhorn") or 0,
				supercharger = getElementData(vehicle,"danihe->tuning->supercharger") or 0,
...
addCommandHandler("park",
	function (player)
		...
		dbExec(connection, "UPDATE vehicles SET parkedPosition = ? WHERE vehicleID = ? ", toJSON(currentPosition), vehicleID)
