...
boxCols[1] = createColRectangle(1547.51953125, -1278.5123291016, 53, 50)
...
function renderService()
    if startTick then
        ...
        dxDrawRectangle(barX + 2, barY + 2, (barW - 4) * barFill, barH - 4, tocolor(255, 148, 40, 240))  
...
local panelX, panelY = 1600, (screenY - panelH) *0.5
    ...
    if selectedVehicle then
        if isElementInServiceZone(selectedVehicle) then
            ...
            if exports.cosmo_core:inDistance3D(localPlayer, selectedVehicle, 4) and not isServicing then
                                ...
                                if components[k] then
                                    ...
                                    if doors[k] then
                                        if getVehicleDoorState(selectedVehicle, doors[k]) > 1 then 
                                            dxDrawMetroButtonWithBorder("component:" .. k, components[k], boxX, boxY, boxW, boxH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, fonts.Roboto11, "center", "center", nil, nil, nil, nil)
                ...
                dxDrawImage(math.floor(panelX + 3), math.floor(panelY + 3), 24, 24, ":cosmo_hud/files/logo.png", 0, 0, 0, tocolor(255, 255, 255))
                ...
                if getElementHealth(selectedVehicle) < 800 then
                    dxDrawMetroButtonWithBorder("component:engine", "Motor", panelX + (panelW-buttonW)*0.5, (panelY + 40) + (buttonH + 10) * num, buttonW, buttonH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, fonts.Roboto11, "center", "center", nil, nil, nil, nil)
                    num = num + 1
                end

                if getVehicleLightState(selectedVehicle, 0) == 1 then
                    dxDrawMetroButtonWithBorder("component:left_light", "Bal első lámpa", panelX + (panelW-buttonW)*0.5, (panelY + 40) + (buttonH + 10) * num, buttonW, buttonH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, fonts.Roboto11, "center", "center", nil, nil, nil, nil)
                    num = num + 1
                end

                if getVehicleLightState(selectedVehicle, 1) == 1 then
                    dxDrawMetroButtonWithBorder("component:right_light", "Jobb első lámpa", panelX + (panelW-buttonW)*0.5, (panelY + 40) + (buttonH + 10) * num, buttonW, buttonH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, fonts.Roboto11, "center", "center", nil, nil, nil, nil)
                    num = num + 1
                end
                ...
                if kilometersToChangeOil <= 0 then
                    dxDrawMetroButtonWithBorder("component:oil", "Olajcsere", panelX + (panelW-buttonW)*0.5, (panelY + 40) + (buttonH + 10) * num, buttonW, buttonH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, fonts.Roboto11, "center", "center", nil, nil, nil, nil)
                end
