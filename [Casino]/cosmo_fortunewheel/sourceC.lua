...
local removeCasinoModels = {
	{2188, 300, 1955.3712158203, 1017.6467285156, 992.46875, 10}, -- Blackjack asztal
	{2785, 300, 1955.3712158203, 1017.6467285156, 992.46875, 10}, -- Slot machines
	{2786, 300, 1955.3712158203, 1017.6467285156, 992.46875, 10}, -- Slot machines LOD
	{1978, 300, 1955.3712158203, 1017.6467285156, 992.46875, 10}, -- Rulett asztalok
	{2773, 15, 1961.1485595703, 1018.1159667969, 992.46875, 10}, -- Kordonok
}

local casinoMusics = {
	[10] = {"https://jazz-wr01.ice.infomaniak.ch/jazz-wr01-128.mp3", 1955.3712158203, 1017.6467285156, 992.46875, 10, 443},
}
	...
	function (commandName)
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			if not placedForuneTable then
				...
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffKerék létrehozás mód #ff9428bekapcsolva!", 255, 255, 255, true)
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffA kerék #ff9428lerakásához #ffffffnyomd meg az #ff9428BAL CTRL #ffffffgombot.", 255, 255, 255, true)
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffA #d75959kilépéshez #ffffffírd be a #d75959/" .. commandName .. " #ffffffparancsot.", 255, 255, 255, true)
			else
				...
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffKerék létrehozás mód #d75959kikapcsolva!", 255, 255, 255, true)
			end
		end
	end)
...
addCommandHandler("nearbyfortune",
	function (commandName, maxDistance)
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			...
			if #nearby > 0 then
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffKözeledben lévő kerekek (" .. maxDistance .. " yard):", 255, 255, 255, true)

				for i, v in ipairs(nearby) do
					outputChatBox("    * #ff9428Azonosító: #ffffff" .. v[1] .. " - " .. math.floor(v[2] * 500) / 500 .. " yard", 255, 255, 255, true)
				end
			else
				outputChatBox("#d75959[CosmoMTA - Szerencsekerék]: #ffffffNincs egyetlen kerék sem a közeledben.", 255, 255, 255, true)
			end
		end
	end)
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		...
		for k, v in pairs(removeCasinoModels) do
			removeWorldModel(unpack(v))
		end

		for k, v in pairs(casinoMusics) do
			local casinoSound = playSound3D(v[1], v[2], v[3], v[4])
			setElementInterior(casinoSound, v[5])
			setElementDimension(casinoSound, v[6])
			setSoundMaxDistance(casinoSound, 1000)
			setSoundVolume(casinoSound, 0.3)
		end

		engineImportTXD(engineLoadTXD("files/models/kb_wheel1.txd"), 1895)
		engineReplaceModel(engineLoadDFF("files/models/wheel_o_fortune.dff"), 1895)
end)
	...
	function ()
		for element, obj in pairs(clickers) do
			...
			if clickerRot[obj] > 4 then
				...
				local sound = playSound3D("files/sounds/wheel.mp3", x, y, z)
...
local panelWidth = respc(430)
local panelHeight = respc(330)
...
local Roboto12 = false
local Roboto15 = false
local Roboto18 = false
local Roboto13 = false
local Roboto12 = false
...
addEventHandler("openFortuneWheel", getRootElement(),
	function (tableId, element)
		...
		Roboto12 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(12), false, "antialiased")
		Roboto15 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased")
		Roboto18 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(18), false, "antialiased")
...
function closeTheTable(spin)
	...
	if isElement(Roboto12) then
		destroyElement(Roboto12)
	end
	Roboto12 = nil
	...
	if isElement(Roboto18) then
		destroyElement(Roboto18)
	end
	Roboto18 = nil

	if isElement(Roboto13) then
		destroyElement(Roboto13)
	end
	Roboto13 = nil
		...
		if state == "down" then
			if activeButton == "quitGame" then
				if creditAll == 0 then
					...
					exports.cosmo_hud:showInfobox("error", "Előbb pörgess, vagy vedd le a tétjeidet!")
				end
				...
				if creditAll > 0 and not movedChip then
					if getTickCount() - clickTick > 2000 then
						if creditAll > 8000 then
							exports.cosmo_hud:showInfobox("error", "Maximum 8000 Coin lehet a tét. A pörgetéshez vegyél le belőle!")
						else
							closeTheTable(true)
						end
					end
				...
				if selected[1] == "field" then
					if movedChip then
						if creditAll + movedChip > 8000 then
							exports.cosmo_hud:showInfobox("error", "Maximum 8000 Coin lehet a tét.")
						else
...
function renderFortuneWheel()
	...
	-- ** Cím
	dxDrawRectangle(panelPosX + 3, panelPosY + 3, panelWidth - 6, respc(40), tocolor(0, 0, 0, 120))
	dxDrawText("#ff9428Cosmo#ffffffMTA - Szerencsekerék", panelPosX + respc(10), panelPosY + 3, 0, panelPosY + 3 + respc(40), tocolor(255, 255, 255, 230), 1, Roboto12, "left", "center", false, false, false, true)

	-- ** Kilépés
	local closeLength = dxGetTextWidth("X", 0.8, Roboto15)

	buttons.quitGame = {panelPosX + panelWidth - respc(10) - closeLength, panelPosY, closeLength, respc(40)}

	if activeButton == "quitGame" then
		dxDrawText("X", buttons.quitGame[1], buttons.quitGame[2] + 7, buttons.quitGame[1] + buttons.quitGame[3], buttons.quitGame[2] + buttons.quitGame[4], tocolor(215, 89, 89), 0.8, Roboto15, "left", "center")
	else
		dxDrawText("X", buttons.quitGame[1], buttons.quitGame[2] + 7, buttons.quitGame[1] + buttons.quitGame[3], buttons.quitGame[2] + buttons.quitGame[4], tocolor(255, 255, 255), 0.8, Roboto15, "left", "center")
	end
	...
	for i = 0, 5 do
		...
		drawButton3("chip_" .. x .. "," .. y, fieldNames[x][y], x2, y2, sx, sy, {255, 148, 40}, false, Roboto18, true)
		...
		if topBets[field] then
			...
			if absX >= chipX and absY >= chipY and absX <= chipX + chipSize and absY <= chipY + chipSize then
				if movedChip and fieldCredits[field] and (fieldCredits[field] or 0) + movedChip > 8000 then
					...
					createTooltip("Téted: #ff9428" .. (fieldCredits[field] or 0) .. " #ffffffZseton")
				end
	...
	for i = 1, #availableCoins do
		...
		if mySlotCoins >= availableCoins[i] then
			...
			if not movedChip and absX >= x and absY >= y and absX <= x + chipSize and absY <= y + chipSize then
				...
				createTooltip("Zseton: #ff9428" .. availableCoins[i] .. "")
			end
		else
	...
	dxDrawText("#ffffffZseton: #ff9428" .. formatNumber(mySlotCoins) .. " #ffffffDB", panelPosX, panelPosY + respc(10) + respc(42.5), panelPosX + respc(10) + panelWidth, 0, tocolor(255, 255, 255), 1, Roboto13, "center", "top", false, false, false, true)

	-- ** Pörgetés
	buttons.startGame = {panelPosX, panelPosY + panelHeight - respc(40), panelWidth, respc(40)}

	drawButton3("startGame", "Pörgetés", buttons.startGame[1] + 3, buttons.startGame[2] + 3, buttons.startGame[3] - 6, buttons.startGame[4] - 6, {255, 148, 40}, false, Roboto15, true)
...
function createTooltip(text)
	...
	if text then
        ...
        dxDrawRectangle(cx - textWidth / 2, cy - (height / 2) - respc(50), textWidth + respc(32), height, tocolor(0, 0, 0, 190), true)
        dxDrawText(text, cx - textWidth / 2, cy - (height / 2) - respc(100), textWidth + cx - textWidth / 2 + respc(32), height + cy - (height / 2) , tocolor(255, 255, 255, 230), 0.8, Roboto12, "center", "center", false, false, true, true)	
	end
end
