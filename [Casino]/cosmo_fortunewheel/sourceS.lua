				...
				dbQuery(
					function (qh, sourcePlayer)
						...
						if result then
							...
							exports.cosmo_hud:showInfobox(sourcePlayer, "success", "Szerencsekerék sikeresen létrehozva! ID: " .. result.id)
						end
					end,
		...
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			...
			if not tableId then
				...
				if fortuneTables[tableId] then
					...
					outputChatBox("#d75959[CosmoMTA - Szerencsekerék]: #ffffffA kiválasztott asztal nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
					...
					if not getElementData(source, "playerUsingFortune") then
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) < 2 then
							...
							if fortuneTable.gameStage == 1 then
								exports.cosmo_hud:showInfobox(source, "error", "Várd meg a pörgetés végét!")
								return
							end
...
function payoutTheWinner(tableId)
	...
	if fortuneTable then
		...
		if isElement(fortuneTable.currentPlayer) then
			...
			local currentBalance = getElementData(fortuneTable.currentPlayer, "char.ucoin") or 0

			setElementData(fortuneTable.currentPlayer, "char.ucoin", currentBalance + profit)

			outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffKapott szám: #ff9428" .. winnerNumber .. "x", fortuneTable.currentPlayer, 0, 0, 0, true)

			if profit > lost then
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffNyertél #ff9428" .. profit .. " Coin#ffffff-t.", fortuneTable.currentPlayer, 0, 0, 0, true)
				setElementData(fortuneTable.currentPlayer, "playerIcons", {"plus", profit})
			else
				outputChatBox("#ff9428[CosmoMTA - Szerencsekerék]: #ffffffNem nyertél semmit.", fortuneTable.currentPlayer, 0, 0, 0, true)
