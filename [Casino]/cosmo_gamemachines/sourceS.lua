	...
	if databaseConnection then
		for machineId, objectElement in pairs(gameMachineObjects) do
			if isElement(objectElement) then
				local machineCredit = getElementData(objectElement, "char.ucoin") or 0
	...
	if machineInfo then
		...
		if machineId then
			...
			if isElement(gameMachineObjects[machineId]) then
				...
				setElementData(gameMachineObjects[machineId], "char.ucoin", machineInfo.credit, false)
						...
						if queryResult then
							...
							if lastInsertId then
								if isElement(sourcePlayer) then
									outputChatBox("#ff9428[CosmoMTA]: #ffffffA játékgép sikeresen létrehozva. #32b3ef(ID: " .. lastInsertId .. ")", sourcePlayer, 255, 255, 255, true)
								end
								...
								if isElement(sourcePlayer) then
									outputChatBox("#d75959[CosmoMTA]: #ffffffA játékgép létrehozása meghiúsult.", sourcePlayer, 255, 255, 255, true)
								end
							end
						else
							if isElement(sourcePlayer) then
			...
			if not machineId then
				...
				if gameMachineObjects[machineId] then
					...
					if executionResult then
						outputChatBox("#ff9428[CosmoMTA]: #ffffffA játékgép sikeresen törölve. #32b3ef(" .. machineId .. ")", sourcePlayer, 255, 255, 255, true)
					...
					outputChatBox("#d75959[CosmoMTA]: #ffffffA kiválasztott játékgép nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
				...
				if creditAmount then
					...
					if machineObject then
						...
						if not isElement(machineUser) then
							local characterCoins = getElementData(client, "char.ucoin") or 0
							local machineCredits = getElementData(machineObject, "char.ucoin") or 0
							...
							if actionType == "deposit" then
								if characterCoins >= creditAmount then
									...
									setElementData(machineObject, "char.ucoin", machineCredits, false)
									setElementData(client, "char.ucoin", characterCoins, true)

									exports.cosmo_hud:showInfobox(client, "success", "Sikeres befizetés! Kellemes játékot :)")
									exports.cosmo_chat:sendLocalMeAction(client, "befizet egy összeget a játékgépbe.")
								else
									exports.cosmo_hud:showInfobox(client, "error", "Nincs ennyi zsetonod.")
								end
							elseif actionType == "withdraw" then
								if machineCredits >= creditAmount then
									...
									exports.cosmo_hud:showInfobox(client, "success", "Sikeres kifizetés!")
									exports.cosmo_chat:sendLocalMeAction(client, "kifizet egy összeget a játékgépből.")
								else
									exports.cosmo_hud:showInfobox(client, "error", "Nincs ennyi zsetonod a játékgépben.")
								end
							end
						else
							exports.cosmo_hud:showInfobox(client, "error", "Ez a játékgép használatban van!")
						end
					end
				end
			end
						...
						if gameType then
							...
							--if gameResource then
								...
								--if gameResourceRoot then
									local currentCredit = getElementData(machineObject, "char.ucoin") or 0
								...
								--else
									--exports.cosmo_hud:showInfobox(client, "error", "Ez a játékgép jelenleg nem üzemel!")
								--end
							--else
								--exports.cosmo_hud:showInfobox(client, "error", "Ez a játékgép jelenleg nem üzemel!")
							--end
		...
		if isElement(client) then
			if machineId then
				...
				if machineObject then
					...
					setElementData(machineObject, "char.ucoin", machineCredit, false)
					setElementData(machineObject, "slotMachineInUse", false)
