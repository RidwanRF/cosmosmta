				...
				dbQuery(
					function (qh, sourcePlayer)
						...
						if result then
                            ...
                            outputChatBox(exports.cosmo_core:getServerTag("info").."Rulett asztal sikeresen létrehozva! ID: " .. result.id, sourcePlayer, 255, 255, 255, true)
						end
					end,
...
exports.cosmo_admin:addAdminCommand("deleteroulette", 9, "Roulette asztal törlés")
addCommandHandler("deleteroulette",
	function (sourcePlayer, commandName, tableId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			tableId = tonumber(tableId)

			if not tableId then
                outputChatBox("#32b3ef>> Használat: #ffffff/" .. commandName .. " [ID]", sourcePlayer, 0, 0, 0, true)
                return
            else
				if rouletteTables[tableId] then
                    ...
                    outputChatBox(exports.cosmo_core:getServerTag("error").."A kiválasztott asztal nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end)
					...
					if not getElementData(source, "playerUsingRoulette") then
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) < 2 then
							...
							if rouletteTable.gameStage == 2 then
                                exports.cosmo_hud:showInfobox(source, "error", "Várd meg a kör végét")
								return
							end 
...
function loadRouletteTable(id, data)
	...
	rouletteTables[id].fixNumber = false
	rouletteTables[id].fixedNumber = 0
...
function pickWinningNumber(tableId)
	...
	if rouletteTable then
		...
		if rouletteTable.fixNumber then
			newNum = wheelNumbers[rouletteTable.fixedNumber]
		else
			newNum = wheelNumbers[math.random(#wheelNumbers)]
		end
		...
		rouletteTable.spinTimer = setTimer(finishTheRound, 11000, 1, tableId)
	end
end
	...
	function (field, datas, players)
		if isElement(source) then
			...
			if tableId then
				...
				if rouletteTable then
					local myCharId = getElementData(source, "acc.ID")
					local myCharName = getElementData(source, "visibleName"):gsub("_", " ")
					...
					if currentBalance - betAmount >= 0 then
						setElementData(source, "char.ucoin", currentBalance - betAmount)
					else
						setElementData(source, "char.ucoin", 0)
					end
			...
			if tableId then
				...
				if rouletteTable then
					...
					if not rouletteTable.spinTimer then
						...
						setElementData(source, "char.ucoin", (getElementData(source, "char.ucoin") or 0) + creditAll)
...
function payoutTheWinners(tableId)
	...
	if rouletteTable then
		...
		for charId, data in pairs(winnersTable) do
			if data[1] > 0 then
				table.insert(rouletteTable.roundHistory, 1, "#D6D6D6" .. data[2] .. " nyereménye: " .. data[1] .. " ZSETON")
				winners = winners + 1
			end

			if isElement(data[3]) then
				setElementData(data[3], "char.ucoin", (getElementData(data[3], "char.ucoin") or 0) + data[1])
...
function finishTheRound(tableId)
	...
	if rouletteTable then
        ...
        rouletteTable.fixNumber = false
        rouletteTable.fixedNumber = 0
