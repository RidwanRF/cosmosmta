...
addEventHandler("onResourceStart", resourceRoot,
	function ()
		databaseConnection = exports.cosmo_database:getConnection()
						...
						if queryResult then
							...
							if lastInsertId then
								if isElement(sourcePlayer) then
									outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffA pókerasztal sikeresen létrehozva. #32b3ef(ID: " .. lastInsertId .. ")", sourcePlayer, 255, 255, 255, true)
								end
								...
								if isElement(sourcePlayer) then
									outputChatBox("#d75959[CosmoMTA]: #ffffffA pókerasztal létrehozása meghiúsult.", sourcePlayer, 255, 255, 255, true)
								end
							end
						else
							if isElement(sourcePlayer) then
	...
	function (sourcePlayer, commandName, tableId, smallBlind, bigBlind, minEntry, maxEntry)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			...
			if not (tableId and smallBlind and bigBlind and minEntry and maxEntry) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [ID] [Kisvak] [Nagyvak] [Min. beülõ] [Max. beülõ]", sourcePlayer, 255, 255, 255, true)
			else
				...
				if selectedPokerTable then
					...
					if executionResult then
						outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffA pókerasztal sikeresen módosítva.", sourcePlayer, 255, 255, 255, true)
					...
					outputChatBox(exports.cosmo_core:getServerTag("error") .. " #ffffffA kiválasztott pókerasztal nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
...
addCommandHandler("deletepoker",
	function (sourcePlayer, commandName, tableId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			...
			if not tableId then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [ID]", sourcePlayer, 255, 255, 255, true)
			else
				if tablePositions[tableId] then
					...
					if executionResult then
						...
						outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffA pókerasztal sikeresen törölve. #32b3ef(" .. tableId .. ")", sourcePlayer, 255, 255, 255, true)
					end
				else
					outputChatBox(exports.cosmo_core:getServerTag("error") .. " #ffffffA kiválasztott pókerasztal nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			...
			if pokerTableId then
				...
				if pokerTableSeat then
					...
					if pokerTable then
						...
						if buyInAmount then
							local currentCoins = getElementData(client, "char.ucoin") or 0
							...
							if currentCoins >= buyInAmount then
								...
								if buyInAmount >= minimumBuyIn and buyInAmount <= maximumBuyIn then
									...
									setElementData(client, "char.ucoin", currentCoins)
	...
	if pokerTable then
		...
		if isElement(playerElement) then
			...
			if chipsInPlay > 0 then
				local slotCoins = getElementData(playerElement, "char.ucoin") or 0

				slotCoins = slotCoins + chipsInPlay

				setElementData(playerElement, "char.ucoin", slotCoins)
			end
		end
		...
		if isElement(playerElement) then
			...
			if not forcedLeave then
				...
				if playerName then
					addPokerChatLine("#ff9428" .. playerName .. " #ffffffkiszállt a játékból!", tableId)
				end
			-- Állítsuk fel a játékost az asztaltól ha elfogyott a zsetonja
				...
				outputChatBox("#d75959[CosmoMTA - Póker]: #ffffffNincs több zsetonod a folytatáshoz.", playerElement, 255, 255, 255, true)
			end
		end
		...
		if currentPlayer == seatId then
			...
			if #pokerCards > 0 then
				...
				if isElement(playerElement) then
					...
					if isTimeout then
						addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffFold! (TimeOut)", tableId)
					else
						addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffFold!", tableId)
					end
				end
					...
					if playerName then
						...
						if betAmount > playerCoins then
							...
							if highestCall == 0 then
								addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffBet! (All-In)", tableId)
							-- Ellenkező esetben, emelünk
							else
								addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffRaise! (All-In)", tableId)
							end
						-- Ha a jelenlegi licitkörben még senki nem nyitott
						elseif highestCall == 0 then
							addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffBet! (" .. thousandsStepper(betAmount) .. " Zseton)", tableId)
						-- Ellenkező esetben, emelünk
						else
							addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffRaise! (" .. thousandsStepper(betAmount) .. " Zseton)", tableId)
						end
					end
			...
			-- Ha a felrakni kívánt tét kisebb vagy egyenlő mint a kötelező tét, passzolunk vagy hívunk
				...
				if isElement(playerElement) then
					...
					if amountToCall == 0 then
						if playerName then
							addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffCheck!", tableId)
						end
					...
					-- Ellenkező esetben, megadjuk a minimális tétet
						...
						if playerName then
							...
							if amountToCall > playerCoins then
								addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffCall! (All-In)", tableId)
							else
								addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffCall!", tableId)
							end
						end
			...
			for winnerSeat, rewardAmount in pairs(winnedRewards) do
				...
				if isElement(winnerPlayer) then
					...
					if playerName then
						...
						if evaluatedHand then
							addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffNyeremény: " .. thousandsStepper(rewardAmount) .. " Zseton (" .. evaluatedHand[1] .. ")", tableId)
						else
							addPokerChatLine("#ff9428[" .. playerName .. "]: #ffffffNyeremény: " .. thousandsStepper(rewardAmount) .. " Zseton", tableId)
						end
					end
