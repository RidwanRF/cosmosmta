...
local Roboto = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(13), false, "antialiased")
local Roboto2 = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(13), false, "antialiased")
local Roboto3 = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(15), false, "antialiased")
local Roboto4 = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(15), false, "antialiased")

local panelState = false
local panelWidth = respc(500)
local panelHeight = respc(290)
...
addEventHandler("onClientResourceStart", resourceRoot,
	function ()
        ...
        if engineLoadCOL("files/model/nagypoker.col") then
            engineReplaceCOL(engineLoadCOL("files/model/nagypoker.col"), 4334)
            if engineLoadTXD("files/model/pokertextura.txd") then
                engineImportTXD(engineLoadTXD("files/model/pokertextura.txd"), 4334)
                if engineLoadDFF("files/model/nagypoker.dff") then
                    engineReplaceModel(engineLoadDFF("files/model/nagypoker.dff"), 4334)
                end
            end
        end
	end
		...
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			if not pokerPlacement then
				...
				if not (smallBlind and bigBlind and minEntry and maxEntry) then
					outputChatBox(exports.cosmo_core:getServerTag("usage") .. " #FFFFFF/" .. commandName .. " [Kisvak] [Nagyvak] [Min. beülõ] [Max. beülõ]", 255, 255, 255, true)
				else
					...
					outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffPókerasztal létrehozás #ffffffmód #ff9428bekapcsolva!", 255, 255, 255, true)
					outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffAz asztal #ff9428lerakásához #ffffffnyomd meg az #ff9428BAL ALT #ffffffgombot!", 255, 255, 255, true)
					outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffA #d75959kilépéshez #ffffffírd be a #d75959/createpoker #ffffffparancsot!", 255, 255, 255, true)
				end
			else
				...
				outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffPókerasztal létrehozás #ffffffmód #d75959kikapcsolva!", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (commandName)
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			...
			if #nearbyPokerTables > 0 then
				outputChatBox("#ff9428[CosmoMTA]: #ffffffA közeledben lévõ pókerasztalok (15 yard):", 255, 255, 255, true)
				...
				outputChatBox(exports.cosmo_core:getServerTag("info") .. " #ffffffA közeledben nem található egyetlen pókerasztal sem.", 255, 255, 255, true)
			end
		end
	end
)
...
function createFonts()
	...
	if not isHudControlled then
		...
		exports.cosmo_hud:setPokerMode(true)
...
function destroyFonts(state)
	if not state then
		if isHudControlled then
			...
			exports.cosmo_hud:setPokerMode(false)
		end
	end
end
		...
		if source == localPlayer then
			if dataName == "pokerTableId" then
				...
				if pokerTableId then
					...
					addPokerChatLine("#ff9428" .. getElementData(localPlayer, "visibleName"):gsub("_", " ") .. " #ffffffcsatlakozott a játékhoz!")
							...
							if pokerTableId then
								if pokerTableId == joinedTableId then
									...
									if pokerTableSeat then
										...
										addPokerChatLine("#ff9428" .. getElementData(source, "visibleName"):gsub("_", " ") .. " #ffffffcsatlakozott a játékhoz!")
									end
								end
							end
						end
									...
									if buttonParts[1] == "join" then
										...
										if not nearbyOccupiedSeats[seatId] then
											if tablePositions[tableId][9] <= mySlotCoins then
												...
												outputChatBox("#d75959[CosmoMTA - Póker]: #ffffffNincs elég coinod.", 255, 255, 255, true)
											end
										end
									end
								end
...
addEventHandler("onClientRender", root,
	function ()
			...
			if joinedTableId then
						...
						if playerInfo[2] > 0 then
							...
							if isElement(objectElement) then
								...
								if guiPosX and guiPosY then
									local visibleText = thousandsStepper(playerInfo[2]) .. " #ff9428Coin"

									local guiSizeX = dxGetTextWidth(visibleText, 0.9, Roboto, true) + respc(5)
									...
									dxDrawText(thousandsStepper(playerInfo[2]).." Coin", guiPosX + 1, guiPosY + 1, guiPosX + guiSizeX + 1, guiPosY + guiSizeY + 1, tocolor(0, 0, 0, 200), 0.9, Roboto, "center", "center", false, false, false, true)
									dxDrawText(visibleText, guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.9, Roboto, "center", "center", false, false, false, true)
								end
							end
						end
							...
							if pokerCoins > 0 then
								if realCoinObjectsEx[joinedTableId] then
									...
									if isElement(objectElement) then
										...
										if guiPosX and guiPosY then
											local visibleText = thousandsStepper(pokerCoins) .. " #ff9428Coin"
											...
											--dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(0, 0, 0, 160))
											dxDrawText(thousandsStepper(pokerCoins) .. " Coin", guiPosX + 1, guiPosY + 1, guiPosX + guiSizeX + 1, guiPosY + guiSizeY + 1, tocolor(0, 0, 0, 200), 0.9, Roboto, "center", "center", false, false, false, true)
											dxDrawText(visibleText, guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.9, Roboto, "center", "center", false, false, false, true)
										end
									end
								end
				...
				if realCoinAmounts[joinedTableId] then
					...
					if pokerCoins > 0 then
						...
						-- Képernyő teteje
						local guiSizeX = dxGetTextWidth("Pot: " .. pokerCoins .. " Coin", 0.6, Roboto) + respc(20)
						...
						dxDrawText("Pot: #ff9428" .. pokerCoins .. " Coin", guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.6, Roboto, "center", "center", false, false, false, true)
						...
						if theCoinPosition then
							...
							if guiPosX and guiPosY then
								local guiSizeX = dxGetTextWidth(pokerCoins .. " Coin", 0.9, Roboto) + respc(5)
								local guiSizeY = respc(20)
								...
								--dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(0, 0, 0, 160))
								dxDrawText(pokerCoins .. " Coin", guiPosX + 1, guiPosY + 1, guiPosX + guiSizeX + 1, guiPosY + guiSizeY + 1, tocolor(0, 0, 0, 200), 0.9, Roboto, "center", "center", false, false, false, true)
								dxDrawText(pokerCoins .. " #ff9428Coin", guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.9, Roboto, "center", "center", false, false, false, true)
							end
						end
					end
				end
								...
								if elapsedProgress < 1.1 then
									...
									if thePedPosition then
										...
										if guiPosX and guiPosY then
											...
											dxDrawImage(guiPosX, guiPosY, guiSizeY, guiSizeY, "files/fonts/clock.png", 0, 0, 0, tocolor(255, 148, 40))
											...
											local progressColor = {255, 148, 40}

											if elapsedTime >= coloringTime then
												progressColor[1], progressColor[2], progressColor[3] = interpolateBetween(255, 148, 40, 215, 89, 89, (elapsedTime - coloringTime) / coloringDuration, "Linear")
											end
		...
		if panelState then
			...
			if panelStage == 1 then
				-- ** Háttér
				dxDrawRectangle(panelPosX, panelPosY, panelWidth, panelHeight, tocolor(0, 0, 0, 170), false)

				-- ** Cím
				dxDrawRectangle(panelPosX + 3, panelPosY + 3, panelWidth - 6, respc(40) - 6, tocolor(0, 0, 0, 150), false)
				dxDrawText("#ff9428Cosmo#ffffffMTA - Póker", panelPosX + respc(5) + 3, panelPosY, 3, panelPosY + respc(40), tocolor(255, 255, 255, 230), 1, Roboto, "left", "center", false, false, false, true)
				...
				--drawButton3("startGame", "Játék", startButton[1], startButton[2], startButton[3], startButton[4], {255, 148, 40}, false, Roboto, true)
				...
				dxDrawText("X", panelPosX + panelWidth - respc(10) - 3, panelPosY + respc(20), nil, nil, quitImageColor, 1, Roboto, "center", "center")

				-- ** Slider
				dxDrawText("Mennyi zsetonnal szeretnél beülni?", panelPosX, panelPosY + respc(40), panelPosX + panelWidth, 0, tocolor(255, 255, 255, 230), 1, Roboto3, "center", "top")
				...
				dxDrawRectangle(sliderBaseX - respc(5), sliderBaseY, sliderBaseWidth + respc(10), sliderKnobHeight, tocolor(0, 0, 0, 130))
				dxDrawRectangle(sliderBaseX - respc(5) + 3, sliderBaseY + 3, (sliderBaseWidth * sliderValue + respc(10)) - 6, sliderKnobHeight - 6, tocolor(255, 148, 40, 150))
				...
				dxDrawText("ZSETON: #ff9428"..thousandsStepper(sliderAmount) .. " #ffffffDB", panelPosX, panelPosY + respc(170) - respc(10), panelPosX + panelWidth, 0 - respc(10), tocolor(255, 255, 255, 230), 1, Roboto3, "center", "top", false, false, false, true)
				...
				--if activeButton == "startGame" then
				--	dxDrawRectangle(startButton[1], startButton[2], startButton[3], startButton[4], tocolor(255, 148, 40, 220), false)
				--else
				--	dxDrawRectangle(startButton[1], startButton[2], startButton[3], startButton[4], tocolor(255, 148, 40, 160), false)
				--end

				drawButton3("startGame", "Játék", startButton[1], startButton[2], startButton[3], startButton[4], {255, 148, 40}, false, Roboto, true)

				--dxDrawText("Játék", startButton[1], startButton[2], startButton[1] + startButton[3], startButton[2] + startButton[4], tocolor(255, 255, 255, 255), 1, Roboto, "center", "center")
			elseif panelStage == 2 then
				...
				if utf8.len(visibleText) > 0 then
					...
					if countdownProgress < 1.1 then
						local guiSizeX = respc(20) + dxGetTextWidth(visibleText, 0.85, Roboto, true)
						...
						if countdownData[1] == localPlayer then
							dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(25, 25, 25))
							dxDrawText(visibleText, guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.85, Roboto, "center", "center")
						else
							dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(25, 25, 25))
							dxDrawText(visibleText, guiPosX, guiPosY, guiPosX + guiSizeX, guiPosY + guiSizeY, tocolor(255, 255, 255, 230), 0.85, Roboto, "center", "center", false, false, false, true)
						end
					end
				end
				...
				if #pokerChatLines > pokerChatMaxLine then
					...
					dxDrawRectangle(rectanglePosX, chatPosY + chatHeight - rectanglePosY - rectangleHeight, rectangleWidth, rectangleHeight, tocolor(255, 148, 40, 225))
				end
				...
				for i = 1, pokerChatMaxLine do
					...
					if textMessage then
						dxDrawText(textMessage, chatPosX + respc(5), chatPosY + chatLineHeight * (pokerChatMaxLine - i), 0, chatPosY + chatLineHeight * (pokerChatMaxLine + 1 - i), tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center", false, false, false, true)
					end
				end
				...
				for i = 0, buttonCount do
					...
					if buttonName == "quitGame" then
						dxDrawText("Kilépés", buttonPosX, buttonPosY, buttonPosX + buttonWidth, buttonPosY + buttonHeight, tocolor(0, 0, 0), 0.85, Roboto, "center", "center")
					else
						dxDrawText(buttonCaptions[i + 1], buttonPosX, buttonPosY, buttonPosX + buttonWidth, buttonPosY + buttonHeight, tocolor(255, 255, 255), 0.85, Roboto, "center", "center")
					end]]

					buttons[buttonName] = {buttonPosX, buttonPosY, buttonWidth, buttonHeight}

					if buttonName == "quitGame" then
						drawButton3(buttonName, "Kilépés", buttonPosX, buttonPosY, buttonWidth, buttonHeight, {215, 89, 89}, false, Roboto, true)
					else
						drawButton3(buttonName, buttonCaptions[i + 1], buttonPosX, buttonPosY, buttonWidth, buttonHeight, {255, 148, 40}, false, Roboto, true)
					end
				end
				...
				dxDrawRectangle(sliderKnobX, sliderKnobY, sliderKnobWidth, sliderKnobHeight, tocolor(255, 148, 40, 240))
				...
				dxDrawText(sliderInputText, inputPosX, inputPosY, inputPosX + inputWidth, inputPosY + inputHeight, tocolor(0, 0, 0, 225), 0.85, Roboto, "center", "center")

				if sliderInputInUse then
					local elapsedTime = getTickCount() - sliderInputState[2]
					local textWidth = dxGetTextWidth(tostring(sliderInputText), 0.85, Roboto) + respc(2)
				...
				drawButton3("fold", "Fold", guiPosX, guiPosY, buttonWidth, buttonHeight, {215, 89, 89}, false, Roboto, true)
				...
				if canDoAction then
					...
					drawButton3("check", buttonText, guiPosX, guiPosY, buttonWidth, buttonHeight, {122, 188, 61}, false, Roboto, true)
				else
				...
				if buttonText == "Check" then
					--dxDrawText(buttonText, guiPosX, guiPosY, guiPosX + buttonWidth, guiPosY + buttonHeight, tocolor(255, 255, 255), 1, Roboto, "center", "center")
				else
					local callAmount = thousandsStepper(amountToCall)
					...
					--dxDrawText(buttonText .. "\n(" .. callAmount .. ")", guiPosX, guiPosY, guiPosX + buttonWidth, guiPosY + buttonHeight, tocolor(255, 255, 255), 1, Roboto, "center", "center")
				end
				...
				drawButton3("raise", buttonText .. "\n(" .. raiseAmount .. ")", guiPosX, guiPosY, buttonWidth, buttonHeight, {89, 142, 215}, false, Roboto, true)
			end
		else
			if nearbyTableId then
				if not joinedTableId then
						...
						for i = 1, #realGuiPositions[nearbyTableId] do
							...
							if not nearbyOccupiedSeats[i] then
								...
								if guiPosX and guiPosY then
									...
									dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(0, 0, 0, 110))
									drawButton3(buttonName, "LEÜLÖK", guiPosX + 3, guiPosY + 3, guiSizeX - 6, guiSizeY - 6, {255, 148, 40}, false, Roboto, true)
								end
							end
						end
					end
					...
					if theTablePosition then
						...
						if guiPosX and guiPosY then
							...
							-- Háttér
							dxDrawRectangle(guiPosX, guiPosY, guiSizeX, guiSizeY, tocolor(0, 0, 0, 170))

							-- Minimum beülő
							dxDrawText("Min. beülő:", guiPosX + respc(5), guiPosY, 0, guiPosY + guiLineHeight, tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center")
							dxDrawText(thousandsStepper(theTablePosition[9]) .. " Zseton", 0, guiPosY, guiPosX + guiSizeX - respc(5), guiPosY + guiLineHeight, tocolor(255, 148, 40, 200), 0.8, Roboto, "right", "center")

							-- Maximum beülő
							guiPosY = guiPosY + guiLineHeight
							dxDrawText("Max. beülő:", guiPosX + respc(5), guiPosY, 0, guiPosY + guiLineHeight, tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center")
							dxDrawText(thousandsStepper(theTablePosition[10]) .. " Zseton", 0, guiPosY, guiPosX + guiSizeX - respc(5), guiPosY + guiLineHeight, tocolor(255, 148, 40, 200), 0.8, Roboto, "right", "center")

							-- Kisvak értéke
							guiPosY = guiPosY + guiLineHeight
							dxDrawText("Kisvak:", guiPosX + respc(5), guiPosY, 0, guiPosY + guiLineHeight, tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center")
							dxDrawText(thousandsStepper(theTablePosition[7]) .. " Zseton", 0, guiPosY, guiPosX + guiSizeX - respc(5), guiPosY + guiLineHeight, tocolor(255, 148, 40, 200), 0.8, Roboto, "right", "center")

							-- Nagyvak értéke
							guiPosY = guiPosY + guiLineHeight
							dxDrawText("Nagyvak:", guiPosX + respc(5), guiPosY, 0, guiPosY + guiLineHeight, tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center")
							dxDrawText(thousandsStepper(theTablePosition[8]) .. " Zseton", 0, guiPosY, guiPosX + guiSizeX - respc(5), guiPosY + guiLineHeight, tocolor(255, 148, 40, 200), 0.8, Roboto, "right", "center")
						end
					end
				end
			end
		...
		if pokerTableId then
			if pokerTableSeat then
				...
				if #pokerCards ~= 0 then
					...
					triggerEvent("playPokerSound", resourceRoot, pokerTableId, "dealear2lap")
					setTimer(triggerEvent, 750, 1, "playPokerSound", resourceRoot, pokerTableId, "dealear2lap")
				else
...
function drawFakeInputText(inputname, details, placeholder, x, y, sx, sy, color, ...)
	if not fakeInputValue[inputname] then
		fakeInputValue[inputname] = ""
	end
	...
	if activename == inputname then
		...
		if utf8.len(fakeInputValue[inputname]) == 0 then
			if placeholder then
				dxDrawText(placeholder, x + marginSize, y, sx - marginSize * 2, sy, tocolor(150, 150, 150), ...)
			end
		end

		if progress > 0.5 then
			dxDrawText(fakeInputValue[inputname] .. "|", x + marginSize, y, sx - marginSize * 2, sy, color, ...)
		else
			dxDrawText(fakeInputValue[inputname], x + marginSize, y, sx - marginSize * 2, sy, color, ...)
		end

		drawOutline(x, y, sx - x, sy - y, tocolor(0, 150, 255))
	else
		if utf8.len(fakeInputValue[inputname]) > 0 then
			dxDrawText(fakeInputValue[inputname], x + marginSize, y, sx - marginSize * 2, sy, color, ...)
		else
	...
	buttons["fakeInput:" .. inputname .. "|" .. details] = {x, y, sx - x, sy - y}
end

function drawButton3(k, text, x, y, sx, sy, hoverColor)
	dxDrawRectangle(x, y, sx, sy, tocolor(0, 0, 0, 160))

	local borderColor = tocolor(0, 0, 0, 170)
	if activeButton == k then
		borderColor = hoverColor
	end

	drawOutline(x, y, sx, sy, borderColor)
	dxDrawText(text, x, y, x + sx, y + sy, -1, 0.9, Roboto2, "center", "center")

	buttons[k] = {x, y, sx, sy}
end

function drawOutline(x, y, sx, sy, color)
	dxDrawRectangle(x, y, 1, sy, tocolor(0, 0, 0, 250)) -- bal
	dxDrawRectangle(x + sx - 1, y, 1, sy, tocolor(0, 0, 0, 250)) -- jobb
	dxDrawRectangle(x, y, sx, 1, tocolor(0, 0, 0, 250)) -- felső
	dxDrawRectangle(x, y + sy - 1, sx, 1, tocolor(0, 0, 0, 250)) -- alsó
end