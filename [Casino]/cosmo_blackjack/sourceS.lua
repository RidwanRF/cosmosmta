				...
				dbQuery(
					function (qh, sourcePlayer)
						...
						if result then
							...
							exports.cosmo_hud:showInfobox(sourcePlayer, "success", "Blackjack sikeresen létrehozva! ID: " .. result.id)
						end
					end,
		...
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			...
			if not tableIdB then
				...
				if blackjackTables[tableIdB] then
					...
					outputChatBox("#d75959[CosmoMTA - Blackjack]: #ffffffA kiválasztott asztal nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end)
					...
					if not getElementData(source, "playerUsingBlackjack") then
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) < 2 then
							...
							if isElement(blackjackTable.data.currentPlayer) then
								exports.cosmo_hud:showInfobox(source, "error", "Már valaki más használja ezt az asztalt!")
								return
							end
							...
							if currentBalance < blackjackTable.minEntry then
								exports.cosmo_hud:showInfobox(source, "error", "Nincs elegendő zsetonod az asztalhoz való beálláshoz! Az asztal minimuma: " .. formatNumber(blackjackTable.minEntry) .. " zseton")
								return
							end
	...
	if blackjackTable then
		...
		if newround then
			if isElement(currentPlayer) then
				local currentBalance = getElementData(currentPlayer, "char.ucoin") or 0

				if currentBalance < blackjackTable.minEntry then
					exports.cosmo_hud:showInfobox(currentPlayer, "error", "Nem volt elég zsetonod a folytatáshoz!")
					currentPlayer = nil
				end
			end
		end
				...
				if blackjackTable then
					...
					if action == "pot" then
						if currentBalance >= credit then
							...
							setElementData(source, "char.ucoin", currentBalance - credit)
							setElementData(blackjackTable.element, "blackjackData", blackjackTable.data)
						end
					...
					elseif action == "double" then
						if #blackjackTable.data.playerCards == 2 then
							...
							if currentBalance - blackjackTable.credit >= 0 then
								...
								setElementData(source, "char.ucoin", currentBalance - blackjackTable.credit)
								setPedAnimation(blackjackTable.dealer, "CASINO", "dealone", 800, false, false, false, false)
								...
								triggerClientEvent(source, "addBlackjackHistory", source, "#d75959Nem rendelkezel elég zsetonnal a duplázáshoz!")
							end
					...
					elseif action == "surrender" then
						if #blackjackTable.data.playerCards == 2 then
							...
							setElementData(source, "char.ucoin", currentBalance + blackjackTable.credit / 2)
							setElementData(source, "playerIcons", {"minus", blackjackTable.credit / 2})

							outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffFeladtad a kört, így vesztetted a tét felét.", source, 0, 0, 0, true)
		...
		if isElement(currentPlayer) then
			...
			if state == "newcard" then
				...
				elseif state == "loses" then
					triggerClientEvent(currentPlayer, "addBlackjackHistory", currentPlayer, "#ffffffVesztettél #d75959" .. blackjackTable.credit .. " #ffffffzsetont!")
					triggerClientEvent("playBlackjackSound", resourceRoot, tableIdB, "dealerwin")
				elseif state == "win" then
					win = blackjackTable.credit * 2
					triggerClientEvent(currentPlayer, "addBlackjackHistory", currentPlayer, "#ffffffNyertél #ff9428" .. blackjackTable.credit .. " #ffffffzsetont!")
				...
				elseif state == "blackjack" then
					...
					triggerClientEvent(currentPlayer, "addBlackjackHistory", currentPlayer, "#ffffffNyertél #ff9428" .. math.floor(blackjackTable.credit * 1.5) .. " #ffffffzsetont! (Blackjack)")
					triggerClientEvent("playBlackjackSound", resourceRoot, tableIdB, "blackjack")
				end

				setElementData(currentPlayer, "char.ucoin", (getElementData(currentPlayer, "char.ucoin") or 0) + win)
				...
				triggerClientEvent(currentPlayer, "addBlackjackHistory", currentPlayer, "#ffffffÚj kör kezdéséig #ff94285 másodperc!")
