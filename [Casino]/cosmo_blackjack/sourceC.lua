		...
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			if not placedTable then
				...
				if not minEntry or not maxEntry then
					outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Min. tét] [Max. tét]", 255, 255, 255, true)
				else
					...
					outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffBlackjack létrehozás mód #ff9428bekapcsolva!", 255, 255, 255, true)
					outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffAz asztal #ff9428lerakásához #ffffffnyomd meg az #ff9428BAL CTRL #ffffffgombot.", 255, 255, 255, true)
					outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffA #d75959kilépéshez #ffffffírd be a #d75959/" .. commandName .. " #ffffffparancsot.", 255, 255, 255, true)
				end
			else
				...
				outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffBlackjack létrehozás mód #d75959kikapcsolva!", 255, 255, 255, true)
			end
		end
	end)
...
addCommandHandler("nearbyblackjack",
	function (commandName, maxDistance)
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			...
			if #nearby > 0 then
				outputChatBox("#ff9428[CosmoMTA - Blackjack]: #ffffffKözeledben lévő asztalok (" .. maxDistance .. " yard):", 255, 255, 255, true)

				for i, v in ipairs(nearby) do
					outputChatBox("    * #ff9428Azonosító: #ffffff" .. v[1] .. " - " .. math.floor(v[2] * 500) / 500 .. " yard", 255, 255, 255, true)
				end
			else
				outputChatBox("#d75959[CosmoMTA - Blackjack]: #ffffffNincs egyetlen asztal sem a közeledben.", 255, 255, 255, true)
			end
		end
	end)
...
local actionButtons = {
	{"hit", "Lapkérés", 124, 197, 118},
	{"stay", "Megállás", 215, 89, 89},
	{"double", "Duplázás", 124, 197, 118},
...
function addChatHistory(text, showtime)
	if showtime then
		...
		text = "#ff9428[" .. string.format("%02d:%02d", currentTime.hour, currentTime.minute) .. "]: " .. text
	end
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		...
		engineImportTXD(engineLoadTXD("files/models/kbblackjack.txd"), 2188)
	end)
		...
		if state == "down" then
			...
			elseif activeButton == "startGame" then
				if sliderAmount > mySlotCoins then
					outputChatBox("#ff9428[CosmoMTA]: #ffffffNincs elég zsetonod!", 255, 255, 255, true)
				else
					if tonumber(sliderAmount) < tablePositionsB[blackjackId][7] or tonumber(sliderAmount) > tablePositionsB[blackjackId][8] then
						outputChatBox("#d75959[CosmoMTA]: #ffffffHiba történt, próbáld újra.", 255, 255, 255, true)
					else
...
function renderBlackjack()
	...
	if blackjackData.gameStage > 0 then
		...
		dxDrawRectangle(startX - respc(15), buttonPosY - respc(15), allWidth + respc(15), buttonHeight + respc(30), tocolor(0, 0, 0, 170))

		for i = 1, #actionButtons do
			local dat = actionButtons[i]
			
			buttonPosX = startX + (i - 1) * (buttonWidth + respc(15))
			drawButton3(dat[1], dat[2], buttonPosX, buttonPosY, buttonWidth, buttonHeight, {dat[3], dat[4], dat[5]}, false, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), true)
		...
		if playerCardsNum ~= 0 then
			...
			dxDrawText("Játékos: " .. value, x, y - respc(50), x + cardSizeX, 0, tocolor(255, 255, 255, 230), 1, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased"), "center", "top", false, false, false, true)
		end
		...
		if dealerCardsNum ~= 0 then
			...
			dxDrawText("Dealer: " .. value, x, y - respc(50), x + cardSizeX, 0, tocolor(255, 255, 255, 230), 0.85, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased"), "center", "top", false, false, false, true)
		end

		-- ** Chat history
		buttons.chatscroll = {chatPosX, chatPosY, chatWidth, chatHeight}

		dxDrawRectangle(chatPosX - respc(5), chatPosY - respc(5), chatWidth + respc(10), chatHeight + respc(10), tocolor(0, 0, 0, 170))
		dxDrawRectangle(chatPosX, chatPosY, chatWidth, chatHeight, tocolor(0, 0, 0, 150))

		local lines = #chatLines

		if lines > 6 then
			local gripHeight = (chatHeight / lines) * 7
			dxDrawRectangle(chatPosX + chatWidth, chatPosY - (chatHeight / lines) * chatOffset + chatHeight - gripHeight, respc(5), gripHeight, tocolor(255, 148, 40, 225))
		end
		...
		for i = 1, 7 do
			...
			if text then
				dxDrawText(text, chatPosX + 5, chatPosY + respc(20) * (7 - i), 0, chatPosY + respc(20) * (8 - i), tocolor(255, 255, 255, 230), 0.6, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased"), "left", "center", false, false, false, true)
			end
		end
	elseif blackjackData.gameStage == 0 then
		...
		-- ** Cím
		dxDrawRectangle(x + 3, y + 3, sx - 6, respc(40) - 6, tocolor(0, 0, 0, 150))
		dxDrawText("#ff9428Cosmo#ffffffMTA - Blackjack", x + respc(10), y, 0, y + respc(40), tocolor(255, 255, 255, 230), 1, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(12), false, "antialiased"), "left", "center", false, false, false, true)

		-- ** Kilépés
		local closeLength = dxGetTextWidth("X", 0.8, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"))

		buttons.quitGame = {x + sx - respc(10) - closeLength, y, closeLength, respc(40)}

		if activeButton == "quitGame" then
			dxDrawText("X", buttons.quitGame[1], buttons.quitGame[2], buttons.quitGame[1] + buttons.quitGame[3], buttons.quitGame[2] + buttons.quitGame[4], tocolor(215, 89, 89), 0.8, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), "left", "center")
		else
			dxDrawText("X", buttons.quitGame[1], buttons.quitGame[2], buttons.quitGame[1] + buttons.quitGame[3], buttons.quitGame[2] + buttons.quitGame[4], tocolor(255, 255, 255, 230), 0.8, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), "left", "center")
		end

		--  ** Content
		dxDrawText("Mennyi zsetont szeretnél feltenni a körre?", x, y + respc(45), x + sx, 0, tocolor(255, 255, 255, 230), 0.75, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), "center", "top")
		...
		dxDrawRectangle(sliderBaseX, sliderY, sliderWidth, sliderHeight, tocolor(0, 0, 0, 150))
		dxDrawRectangle((sliderX - sliderValue * (sliderWidth - respc(15))) + 3, sliderY + 3, (respc(15) + sliderValue * (sliderWidth - respc(15))) - 6, sliderHeight - 6, tocolor(255, 148, 40, 150))
		...
		dxDrawText("ZSETON: #ff9428"..formatNumber(sliderAmount) .. " #ffffffDB", x, sliderY + sliderHeight, x + sx, y + sy - respc(40), tocolor(255, 255, 255, 230), 1, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(13), false, "antialiased"), "center", "center", false, false, false, true)

		-- Játék
		buttons.startGame = {x, y + sy - respc(40), sx, respc(40)}

		drawButton3("startGame", "Játék indítása", buttons.startGame[1] + 3, buttons.startGame[2] + 3, buttons.startGame[3] - 6, buttons.startGame[4] - 6, {255, 148, 40}, false, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), true)
	end
		...
		for tableIdB, board in pairs(blackjackBoard) do
				...
				if dat then
					...
					if dist < 10 then
						...
						if x and y then
							...
							dxDrawText("Min. tét/kör:", x + respc(5) * scale, y + respc(5) * scale, 0, y + respc(27.5) * scale, tocolor(255, 255, 255, 230), 0.675 * scale, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased"), "left", "center")
							dxDrawText(formatNumber(dat[7]) .. " ZSETON", 0, y + respc(5) * scale, x + sx - respc(5) * scale, y + respc(27.5) * scale, tocolor(255, 148, 40, 200), 0.675 * scale, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), "right", "center")
							
							dxDrawText("Max. tét/kör:", x + respc(5) * scale, y + respc(27.5) * scale, 0, y + respc(55) * scale, tocolor(255, 255, 255, 230), 0.675 * scale, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased"), "left", "center")
							dxDrawText(formatNumber(dat[8]) .. " ZSETON", 0, y + respc(27.5) * scale, x + sx - respc(5) * scale, y + respc(55) * scale, tocolor(255, 148, 40, 200), 0.675 * scale, exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(15), false, "antialiased"), "right", "center")
						end
					end
				end
			end
			...
			if boardCards then
				...
				if dat then
					...
					dxDrawMaterialLine3D(x1, y1, z1, x2, y2, z2, cardTextures[dat.texture], 0.1, tocolor(255, 255, 255, 230), x1, y1, z1 + 1)
						...
						for i = 1, #board.dealerCards do
							...
							if dat then
								...
								dxDrawMaterialLine3D(
									...
									cardTextures[texture], 0.1, tocolor(255, 255, 255, 230),
