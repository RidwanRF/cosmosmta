...
addEventHandler ( "onClientRender", root,
	function()
		...
		for player in pairs(voicePlayers) do
			...
			dxDrawVoiceLabel ( player, index, color, getElementData(player, "char.voice") )
			index = index + 1
			...
			while true do
				...
				dxDrawVoice ( absX, absY, color, getDistanceBetweenPoints3D(camX, camY, camZ, headX, headY, headZ), getElementData(player, "char.voice") )
				break
			end
		end
	end
...
function dxDrawVoice ( posX, posY, color, distance, type )
	distance = 1/distance

    if type == "local" then
        ICON_PATH = "images/voice.png"
    elseif type == "radio" then
        ICON_PATH = "images/radiol.png"
    end
...
function dxDrawVoiceLabel ( player, index, color, type )
