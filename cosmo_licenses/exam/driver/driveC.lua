...
function renderDrivePanel()
	...
	--dxDrawImage(math.floor(driveX + 3), math.floor(driveY + 3), 24, 24, ":cosmo_hud/files/logo.png", 0, 0, 0, tocolor(255, 255, 255))
    dxDrawText("Vizsga", driveX + 30, driveY, 0, driveY + 30, tocolor(255, 255, 255), 1, BebasNeue, "left", "center")
    ...
    if page == "main" then
        ...
        dxDrawText("X", closeTextPosX, driveY, 0, driveY + 30, closeColor, 1, BebasNeue, "left", "center")

        --> Tartalom
        dxDrawText("Üdvözöljük", driveX, driveY + 50, driveW + driveX, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center")

        dxDrawText("Út a jogosítványhoz", driveX + 10, driveY + 40, driveW + driveX - 10, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center", "center", false, true)

        local buttonW, buttonH = respc(220), respc(40)

        dxDrawMetroButtonWithBorder("exam:exercise:start", "Gyakorlati vizsga elkezdése", driveX + (driveW - buttonW) * 0.5, driveY + driveH - 10 - buttonH, buttonW, buttonH, {255, 148, 40, 125}, {255, 148, 40, 175}, {255, 255, 255}, BebasNeue, "center", "center", nil, nil, nil, nil)
    ...
    elseif page == "result" then
            ...
            if driveSuccess then
                dxDrawText("Átment!", driveX, driveY + 50, driveW + driveX, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center")
        
                dxDrawText("Az ön ponszáma elérte a szükséges pontszámot! Az igazoló papírt átadtuk önnek. A továbbiakban keressen fel a Városházán egy okmányost.", driveX + 10, driveY + 40, driveW + driveX - 10, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center", "center", false, true)
            else
                dxDrawText("Megbukott!", driveX, driveY + 50, driveW + driveX, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center")
        
                dxDrawText("Az ön ponszáma nem érte el a szükséges pontszámot! Az igazoló papírt átadtuk önnek. A továbbiakban felkereshet minket a vizsga újrapróbálásáért.", driveX + 10, driveY + 40, driveW + driveX - 10, driveH + driveY, tocolor(255, 255, 255, 255), 1, BebasNeue, "center", "center", false, true)
            end
    end
...
function startTestDrive()
    ...
    exports.cosmo_alert:showAlert("info", "Kövesse az útvonalat")
    triggerServerEvent("cosmo_licenesesS:createTestVehicle", localPlayer, localPlayer)
	activeButton = false
end
...
function createNextCheckpoint()
    ...
    if currentCheckpoint > #drivePositions then
        exports.cosmo_alert:showAlert("info", "Visga vége")
        destroyCheckpoint()
        endOfTestDrive()
        return
    end
...
function createCheckpoint(x, y, z, speedlimit)
    ...
    if speedlimit then
        outputChatBox(exports.cosmo_core:getServerTag("info") .. "Sebesség korlát #ff4646" .. currentSpeedLimit .. "#ffffffmph lett.", 0, 0, 0, true)
    end
end
...
function endOfTestDrive(failTest)
    if failTest then
        ...
        triggerServerEvent("cosmo_licenesesS:destroyTestVehicle", localPlayer, localPlayer)
        return
    end
...
addEventHandler("onClientVehicleStartEnter", root, function(player, seat)
    if player == localPlayer then 
        if getElementData(source, "vehicle.test") and player ~= getElementData(source, "vehicle.testOwner") then
            --if seat == 0 then
                exports.cosmo_hud:showAlert("error", "Ez nem a te oktató járműved")
                cancelEvent()
            --end
        end
    end
...
addEventHandler("onClientVehicleExit", getRootElement(), function(player, seat)
    if player == localPlayer then
        if getElementData(source, "vehicle.test") and player == getElementData(source, "vehicle.testOwner") then
            exports.cosmo_hud:showAlert("info", "Az oktató járműved törlődni fog.", "Ha nem szálsz vissza a járművedbe, akkor " .. destroyTime .. " perc után törlődik")
            destroyTimer = setTimer(function()
                endOfTestDrive(true)
                exports.cosmo_hud:showAlert("info", "A oktató járműved törlődött")
