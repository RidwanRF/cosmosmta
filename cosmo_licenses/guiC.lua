...
function loadFonts()
    fonts = {
        ...
        SignPainter = exports.cosmo_assets:loadFont("SignPainter-HouseScript-Regular.ttf", respc(24), false, "cleartype"),
        lunabar = exports.cosmo_assets:loadFont("lunabar.ttf", respc(26), false, "antialiased"),
    }
end
...
function Panel.show()
	...
	widgets.logo = UI:createImage({
		...
		texture = ":cosmo_hud/files/logo.png",
...
local function showBuyButton(data)
    ...
    table.insert(buttons, {"Kiváltás (" .. data.price .. "$)", "buyDocument", {255, 148, 40, 175}})
...
addEventHandler("sarpUI.click", resourceRoot, function (widget, button)
	if button == "left" then
		...
		elseif widget == widgets.buyDocument then
			...
			triggerServerEvent("cosmo_licensesS:giveDocument", localPlayer, data.itemID, docData, data.price)
		end
	end
end)
...
addEventHandler("onClientClick", root, function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
    if button == "left" and state == "down" then
        if clickedElement then
            if exports.cosmo_core:inDistance3D(getLocalPlayer(), clickedElement, 3) then
                if tonumber(getElementData(clickedElement, "ped.type")) == 4 then
                    Panel.show()
                elseif tonumber(getElementData(clickedElement, "ped.type")) == 5 and tonumber(getElementData(clickedElement, "ped.subtype")) == 1 and not getElementData(localPlayer, "startedQuest") then
                    showExamPanel("main")
                elseif tonumber(getElementData(clickedElement, "ped.type")) == 5 and tonumber(getElementData(clickedElement, "ped.subtype")) == 2 then
                    local itemData = exports.cosmo_inventory:hasItemWithData(113, "data2", "vezetés-elmélet")
                    if itemData and (tonumber(itemData.data1) == 1 or itemData.data1) and itemData.data2 == "vezetés-elmélet" and tonumber(itemData.data3) == getElementData(localPlayer, "char.ID") then
                        ...
                        exports.cosmo_hud:showInfobox("error", "Névre szóló vizsga papírok", "nélkül nem kezdheted meg a vizsgát")
                    end
                end
            end
        end
