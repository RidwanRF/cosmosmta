...
local robotoBoldFont12 = exports.cosmo_assets:loadFont("Raleway.ttf", resp(12), false)
local panelFont = exports.cosmo_assets:loadFont("Raleway.ttf", resp(17), false)
robotoBoldFont10 = exports.cosmo_assets:loadFont("Raleway.ttf", resp(10), false)
robotoBoldFont9 = exports.cosmo_assets:loadFont("Raleway.ttf", resp(9), false)
	...
	for i = #actions, 1, -1 do
		...
		if dat ~= "new" then
			if not tableCoinBets[dat[1]] then
				tableCoinBets[dat[1]] = dat[5] .. " tétje: #ff9428" .. dat[3] .. " #ffffffCoin"
			else
				tableCoinBets[dat[1]] = tableCoinBets[dat[1]] .. "\n" .. dat[5] .. " tétje: #ff9428" .. dat[3] .. " #ffffffCoin"
			end
	...
	if not getKeyState("mouse1") then
		if #sectorsHovered > 0 and coinMoving > 0 and remainingTime ~= 0 and getTickCount() - lastBetTick > 1275 then
			...
			if (coinsWorth + availableCoins[coinMoving].worth) > maximumBet then
				exports.cosmo_hud:showInfobox("error", "A maximum tét $500.000, amelyet nem léphetsz túl!")
			else
...
function drawCosmoPanelBottomExit(x, y, h, w, panelFont, buttonFontScale)

	dxDrawRectangle(x, y, h, w, tocolor(0, 0, 0, 170))
	dxDrawRectangle(x + 3, y + 3, h - 6, respc(40) - 6, tocolor(0, 0, 0, 200))
	dxDrawText("#ff9428Cosmo#ffffffMTA - Rulett", x + respc(5), y + respc(40) / 2, nil, nil, tocolor(255, 255, 255, 230), 1, panelFont, "left", "center", false, false, false, true)

	drawButton3("exitFromPanel", "Kilépés", x + 3, y + w - respc(40), h - 6, respc(40) - 3, {255, 148, 40}, false, panelFont, true, buttonFontScale)
end
...
function renderRouletteBar()
	...
	drawCosmoPanelBottomExit(renderData.rouletteBarX, renderData.rouletteBarY, renderData.rouletteBarW, respc(200), robotoBoldFont10, 1)
	dxDrawText("Pörgetésig hátralévő idő: " .. minutesLeft .. ":" .. secondsLeft, renderData.rouletteBarX + renderData.rouletteBarW / 2, renderData.rouletteBarY + respc(45), nil, nil, tocolor(255, 255, 255, 230), 1, robotoBoldFont10, "center", "top", false, false, false, true)
	dxDrawText("Előző nyerőszám: " .. tostring(previousNumber), renderData.rouletteBarX + renderData.rouletteBarW / 2, renderData.rouletteBarY + respc(65), nil, nil, tocolor(255, 255, 255, 230), 1, robotoBoldFont10, "center", "top", false, false, false, true)

	for i = 1, 6 do
		dxDrawImage(renderData.rouletteBarX + respc(35) + (i - 1) * respc(40) - respc(35) + respc(275) / 2, renderData.rouletteBarY + respc(100), respc(30), respc(30), "/files/chips/" .. availableCoins[tonumber(i)].worth .. ".png")
	...
	if string.find(renderData.activeDirectX, "coin_") then
		...
		showTooltip(cx * screenX, cy * screenY, "#ff9428" .. availableCoins[tonumber(index)].worth .. " #ffffffCoin")
	end
	...
	if coinMoving > 0 then
		if isCursorShowing() then
			...
			dxDrawImage(relX-respc(35)/2, relY-respc(35)/2, respc(35), respc(35), "files/chips/" .. availableCoins[tonumber(coinMoving)].worth .. ".png")
		end
	end
		...
		if clickedElement and getElementModel(clickedElement) == 1978 and getElementData(clickedElement, "casino:rouletteTable") and not isElement(renderData.clickedRoulette) then
			...
			if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) < 2 then
				...
				if not getElementData(clickedElement, "rouletteAnimating") then
					...
					exports.cosmo_hud:showInfobox("error", "A pörgetés már elkezdődőtt ennél az asztalnál. Várd meg, hogy csatlakozhass a játékhoz!")
				end
			end
		end
...
function closeRouletteTable()
	if table.empty(ownCoinBets) then
		...
		exports.cosmo_hud:showInfobox("error", "Várd meg a kör végét, vagy vedd le az eddig feltett téteidet, hogy kiszálhass!")
	end
end
...
function renderRoulettePreview()
	...
	dxDrawCorrectText("A rulett asztal lerakásához használd a kurzorod.\nForgatás: #ff9428Görgő #ffffff| Létrehozás: #ff9428Bal klikk #ffffff| Visszavonás: #ff9428Jobb klikk", 0, screenY-respc(200), screenX, 0, tocolor(255,255,255,255), 1, robotoBoldFont12, "center", "top", false, false, false, true)
...
addEventHandler("chipSound", getRootElement(),
	function (x, y, z, num)
		local soundEffect = playSound3D("files/sounds/chip1.mp3", x, y, z)
...
function showTooltip(x, y, text, text2)
	...
	dxDrawText(text, x, y, x + sx, y + sy, tocolor(255, 255, 255, 230), 1, robotoBoldFont12, "center", "center", false, false, true, true)
end