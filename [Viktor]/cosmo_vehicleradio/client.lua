...
local fonts = {
    ["condensed-11"] = font:loadFont("Roboto-Regular.ttf", 11),
    ["bebasneue-13"] = font:loadFont("Roboto-Regular.ttf", 13),
    ["condensed-16"] = font:loadFont("Roboto-Regular.ttf", 16),
    ["fontawesome-16"] = font:loadFont("FontAwesome.ttf", 16),
}
    ...
    for k, v in pairs(playedMusics) do 
        if isElement(v) then 
            if isElementStreamedIn(k) then 
                ...
                local distance = getDistance(k, localPlayer) 
...
function renderRadio()
    ...
    dxDrawRectangle(positions["big"][1], positions["big"][2], sx*0.25, sy*0.2, tocolor(0, 0, 0, 170*a))
    dxDrawRectangle(positions["big"][1]+2/myX*sx, positions["big"][2]+2/myY*sy, sx*0.215, sy*0.161, tocolor(0, 0, 0, 150*a))

    dxDrawRectangle(positions["big"][1]+110/myX*sx, positions["big"][2]+148/myY*sy, sx*0.18, sy*0.033, tocolor(0, 0, 0, 150*a))

    dxDrawRectangle(positions["big"][1]+2/myX*sx, positions["big"][2]+148/myY*sy, sx*0.067, sy*0.033, tocolor(0, 0, 0, 150*a))
    ...
    dxDrawRectangle(positions["big"][1]+357/myX*sx - 4/myX*sx*volumeBarAnimation[2], positions["big"][2]+10/myY*sy - 4/myY*sy*volumeBarAnimation[2], sx*0.02 + 8/myX*sx*volumeBarAnimation[2], sy*0.143 + 8/myY*sy*volumeBarAnimation[2], tocolor(0, 0, 0, 150*a))
    ...
    if radioStatus then 
        ...
        if audioVizualizerShowing then 
            ...
            dxDrawImage(positions["big"][1]+120/myX*sx, positions["big"][2]+10/myY*sy, 110/myX*sx, 110/myY*sy, "files/logo.png", 0, 0, 0, tocolor(255, 255, 255, 100*a))
        ...
        dxDrawRectangle(positions["big"][1], positions["big"][2]+sy*0.203, sx*0.25, sy*0.1, tocolor(0, 0, 0, 170, 255*a))

        local startY = positions["big"][2]+sy*0.205
    
        for i = 1, 3 do 
            local barColor = tocolor(0, 0, 0, 170*a)
            local barColor2 = tocolor(r, g, b, 255*a)

            if (i+radioScroll) % 2 == 0 then 
                barColor = tocolor(0, 0, 0, 150*a)
        ...
        dxDrawImage(positions["big"][1]+120/myX*sx, positions["big"][2]+2/myY*sy, 110/myX*sx, 110/myY*sy, "files/logo.png", 0, 0, 0, tocolor(255, 255, 255, 100*a))
...
addEventHandler("onClientResourceStop", resourceRoot, function()
    ...
    exports.cosmo_json:saveDataToJSONFile(favouriteChannels, "radio_FavouriteChannels", true)
    exports.cosmo_json:saveDataToJSONFile(positions, "radio_Positions", true)
    exports.cosmo_json:saveDataToJSONFile(audioVizualizerShowing, "radio_VizualizerShowing", true)
end)

addEventHandler("onClientResourceStart", resourceRoot, function()
    local favouriteChannels = {}

    for k, v in ipairs(exports.cosmo_json:loadDataFromJSONFile("radio_FavouriteChannels", true)) do 
        table.insert(favouriteChannels, v)
    end

    for k, v in ipairs(stations) do 
        if tableContains(favouriteChannels, v[1]) then 
            v[3] = 1
        end
    end
    ...
    positions = exports.cosmo_json:loadDataFromJSONFile("radio_Positions", true) or {
        ["big"] = {sx*0.375, sy*0.39},
        --["small"] = {sx*0.375, sy*0.75},
    }
    ...
    --if exports.cosmo_json:loadDataFromJSONFile("radio_VizualizerShowing", true) then 
        audioVizualizerShowing =  exports.cosmo_json:loadDataFromJSONFile("radio_VizualizerShowing", true)
    --end
end)
