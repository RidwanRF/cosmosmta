...
function adminClothes(commandName, partialNick, mode, slotName)
	if getElementData(localPlayer, "acc.adminLevel") >= 3 then
		if not partialNick then
			outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Név / ID]", 255, 255, 255, true)
		else
			local targetPlayer, targetName = exports.cosmo_core:findPlayer(localPlayer, partialNick)
			...
			if isElement(targetPlayer) then
				...
				if mode ~= "levesz" or not slotName then
					if mode == "levesz" and not slotName then
						outputChatBox("#ff9428[CosmoMTA - Adminruha]:#ffffff Használd a #32b3ef/" .. commandName .. " " .. partialNick .. " levesz [slot név]", 255, 255, 255, true)
					else
						outputChatBox("#ff9428[CosmoMTA - Adminruha]:#ffffff Használd a #32b3ef/" .. commandName .. " " .. partialNick .. " levesz [slot név] #FFFFFFparancsot egy ruhadarab levételéhez!", 255, 255, 255, true)
					end

					outputChatBox("#ff9428[CosmoMTA - Adminruha]:#ffffff " .. targetName .. " ruhadarabjai:", 255, 255, 255, true)

					for k, v in pairs(baseCategories) do
						local slot = v[2]

						if currentClothes[slot] and currentClothes[slot][1] then
							outputChatBox("  - " .. v[1] .. ": #ff9428" .. itemNames[currentClothes[slot][1]] .. "#ffffff (slot név: #ff9428" .. slot .. "#ffffff)", 255, 255, 255, true)
						else
				...
				if mode == "levesz" and slotName then
					if getTickCount() - delClothTick >= 5000 then
						if currentClothes[slotName] then
							...
							exports.cosmo_hud:showInfobox("error", "Ezen a sloton (" .. slotName .. ") nem található ruhadarab!")
						end
					else
						exports.cosmo_hud:showInfobox("error", "Ilyen gyorsan nem kéne!")
					end
				end
			end
		end
...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedResource)
		if getResourceName(startedResource) == "cosmo_boneattach" then
			...
			for i = 1, #players do
				...
				if player then
					...
					--print("clothe 3 anyad")
				end
			end
		elseif getResourceName(startedResource) == "cosmo_clothesshop" then
			shopColshape = createColSphere(198.56469726562, -135.88250732422, 1003.5078125, 1)

			if isElement(shopColshape) then
				shopMarker = createMarker(198.56469726562, -135.88250732422, 1003.5078125-1, "cylinder", 1.5, 255, 148, 40, 155)

				if isElement(shopMarker) then
					setElementData(shopMarker, "3DText", "Kiegészítők", false)
					setElementInterior(shopMarker, 3)
					setElementDimension(shopMarker, 219)
				end

				setElementInterior(shopColshape, 3)
				setElementDimension(shopColshape, 219)
			end
...
addEventHandler("onClientElementDataChange", getRootElement(),
	function (dataName)
		...
		if dataName == "currentClothes" then
			...
			--print("clothe 5 anyad")
		elseif dataName == "adminDuty" then
			processClothes(source)
			--print("clothe 6 anyad")
		end
	end
)
...
addEventHandler("clothBuyProcessed", localPlayer,
	function (infoType, infoText)
		if infoType and infoText then
			exports.cosmo_hud:showInfobox(infoType, infoText)
		end
...
function checkGroups()
	...
	for k, v in pairs(groupClothes) do
		...
		for i, groupId in ipairs(v) do
			if exports.cosmo_groups:isPlayerInGroup(localPlayer, groupId) then
				inGroup = true
				break
			end
		end
		...
		if getPedWeapon(localPlayer) == 34 then
			if getControlState("aim_weapon") then
				if not sniperAim then
					...
					--print("clothe 8 anyad")
				end
			elseif sniperAim then
				sniperAim = false
				processClothes(localPlayer)
				--print("clothe 9 anyad")
			end
		elseif sniperAim then
			sniperAim = false
			processClothes(localPlayer)
			--print("clothe 10 anyad")
		end
	end
)
...
function processClothes(player, category)
	...
	if isElementStreamedIn(player) then
		...
		local adminDuty = getElementData(player, "adminDuty", true)

		if adminDuty == false then
					...
					for k, v in pairs(currentClothes) do
						if k ~= category and tonumber(v[11]) and clothesLimit >= v[11] then
							...
							if isElement(objectElement) then
								...
								exports.cosmo_boneattach:attachElementToBone(objectElement, player, boneCategories[k], v[2], v[3], v[4], v[5], v[6], v[7])
...
function renderPreview()
	...
	dxDrawText(itemNames[previewModel], x, y - respc(40), x + sx, y, tocolor(255, 255, 255, 230), 1, Roboto, "center", "center")

	dxDrawRectangle(x - 5, y - 5, sx + 10, sy + 10, tocolor(0, 0, 0, 170))
	...
	dxDrawText("Forgatás: egér | Kilépés: Backspace | Nagyítás: görgő", x, y + sy, x + sx, y + sy + respc(40), tocolor(255, 255, 255, 230), 0.75, Roboto, "center", "center")
...
function renderClothesShop()
	...
	--dxDrawImage(x, y + respc(40), sx, sy - respc(40), ":cosmo_hud/files/images/vin.png")

	-- ** Cím
	dxDrawRectangle(x + 4, y + 4, sx - 8, respc(40) - 4, tocolor(0, 0, 0, 130))
	dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff- Ruha bolt", x + respc(7.5), y, x + sx, y + respc(40), tocolor(255, 255, 255, 230), 1, Roboto, "left", "center", false, false, false, true)
	...
	if activeButton == "exit" then
		...
		dxDrawImage(buttons.exit[1], buttons.exit[2], buttons.exit[3], buttons.exit[4], "files/images/close.png", 0, 0, 0, tocolor(255, 255, 255, 230))
	end
			...
			if item[1] == "cat" then
				...
				if baseCategories[cat] then
					if activeButton == "openCategory:" .. cat then
						dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY, tocolor(255, 148, 40, 120))
					end

					if openCategories[cat] then
						dxDrawImage(itemX + respc(10), itemY + itemSizeY / 2 - respc(8), respc(16), respc(16), "files/images/arrow.png", 90, 0, 0, tocolor(255, 255, 255))
					else
						dxDrawImage(itemX + respc(10), itemY + itemSizeY / 2 - respc(8), respc(16), respc(16), "files/images/arrow.png", 0, 0, 0, tocolor(255, 255, 255))
					end

					dxDrawText(baseCategories[cat][1], itemX + respc(10) + respc(20), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")

					buttons["openCategory:" .. cat] = {itemX, itemY, itemSizeX, itemSizeY}
				end
			else
				if item[1] == "sub" then
					dxDrawText(item[2], itemX + respc(20), itemY, 0, itemY + itemSizeY, tocolor(255, 148, 40, 200), 0.75, Roboto, "left", "center")
				else
					local model = item[2]
					...
					if itemMods[model] then
						...
						if subCategories[cat] then
							dxDrawText(itemNames[model], itemX + respc(30), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
						else
							dxDrawText(itemNames[model], itemX + respc(20), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
						end
						...
						if myBoughtClothes[model] then
							...
							drawButton3(btnName, "Előnézet", buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], {255, 148, 40}, false, Roboto, true, 0.7)
							-- Információ
							dxDrawText("Megvéve", itemX, itemY, buttons[btnName][1] - respc(5), itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.7, Roboto, "right", "center")
						else
							-- Előnézet
							...
							drawButton3(previewName, "Előnézet", buttons[previewName][1], buttons[previewName][2], buttons[previewName][3], buttons[previewName][4], {255, 148, 40}, false, Roboto, true, 0.7)
							-- Vásárlás
							...
							drawButton3(buyName, "Megvétel", buttons[buyName][1], buttons[buyName][2], buttons[buyName][3], buttons[buyName][4], {255, 148, 40}, false, Roboto, true, 0.7)

							-- Ár
							dxDrawText("Ár: " .. itemPrices[item[2]] .. "$", itemX, itemY, buttons[buyName][1] - respc(5), itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.7, Roboto, "right", "center")
						end
					end
				end
			end
	...
	if visibleItems > 15 then
		...
		dxDrawRectangle(x + sx - respc(10), y + offsetItems * contentRatio, respc(5), contentRatio * 15, tocolor(255, 148, 40, 150))
	end
...
function renderMyClothes()
	...
	if delClothPrompt then
		...
		-- ** Háttér
		dxDrawRectangle(x, y, sx, sy + respc(50), tocolor(0, 0, 0, 170))
		--dxDrawImage(x, y + respc(40), sx, sy - respc(40), ":cosmo_hud/files/images/vin.png")

		-- ** Content
		dxDrawText("Ténylegesen szeretnéd törölni a következő kiegészítőt?\n#ADADAD" .. itemNames[delClothPrompt], x, y, x + sx, y + sy, tocolor(255, 255, 255, 230), 0.85, Roboto, "center", "center", false, false, false, true)

		-- Elfogadás
		buttons.acceptDelCloth = {x + respc(3), y + sy - resp(3), sx / 2 - resp(6), respc(50)}

		drawButton3("acceptDelCloth", "Igen", buttons.acceptDelCloth[1], buttons.acceptDelCloth[2], buttons.acceptDelCloth[3], buttons.acceptDelCloth[4], {215, 89, 89}, false, Roboto, true, 0.7)
		--drawButton3(acceptDelCloth, "Átnevezés", buttons[renameName][1], buttons[renameName][2], buttons[renameName][3], buttons[renameName][4], {255, 148, 40}, false, Roboto, true, 0.7)
		-- Elutasítás
		buttons.cancelDelCloth = {x + sx / 2 + respc(3), y + sy - resp(3), sx / 2 - resp(6), respc(50)}

		drawButton3("cancelDelCloth", "Nem", buttons.cancelDelCloth[1], buttons.cancelDelCloth[2], buttons.cancelDelCloth[3], buttons.cancelDelCloth[4], {255, 148, 40}, false, Roboto, true, 0.7)
	...
	elseif not selectedSlot then
		...
		if outfitsPanel then
			dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff- Öltözékek", x + respc(7.5), y + 2, x + sx, y + 2 + respc(40), tocolor(255, 255, 255, 230), 1, Roboto, "left", "center", false, false, false, true)
		else
			dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff- Kiegészítőid", x + respc(7.5), y + 2, x + sx, y + 2 + respc(40), tocolor(255, 255, 255, 230), 1, Roboto, "left", "center", false, false, false, true)
		end
		...
		if outfitsPanel then
			...
			for i = 1, 8 do
				...
				if not myOutfits[i] then
					dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY - respc(10), tocolor(0, 0, 0, 180))

					if activeButton == "saveOutfitTo:" .. i then
						dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY - respc(10), tocolor(255, 148, 40, 100))
					end

					dxDrawText("Jelenlegi öltözék mentése", itemX, itemY, itemX + itemSizeX, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "center", "center")
					...
					if activeFakeInput == "outfit:" .. i then
						if cursorState then
							dxDrawText(fakeInputText .. "|", itemX + respc(10), itemY, 0, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "left", "center")
						else
							dxDrawText(fakeInputText, itemX + respc(10), itemY, 0, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "left", "center")
						end
						...
						--	dxDrawRectangle(buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], tocolor(255, 148, 40))
						--else
						--	dxDrawRectangle(buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], tocolor(255, 148, 40, 180))
						--end
						drawButton3(btnName, "Átnevezés", buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], {255, 148, 40}, false, Roboto, true, 0.7)
						...
						drawButton3(delName, "Törlés", buttons[delName][1], buttons[delName][2], buttons[delName][3], buttons[delName][4], {215, 89, 89}, false, Roboto, true, 0.7)
						-- Átnevezés
						...
						drawButton3(renameName, "Átnevezés", buttons[renameName][1], buttons[renameName][2], buttons[renameName][3], buttons[renameName][4], {255, 148, 40}, false, Roboto, true, 0.7)
						-- Item
						dxDrawText(myOutfits[i][1], itemX + respc(10), itemY, 0, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "left", "center")
			...
			for i = 1, categoryNum do
				...
				if i == myClothesLimit + 1 then
					dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY - respc(10), tocolor(0, 0, 0, 130))

					if unlockSlotPrompt then
						dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY - respc(10), tocolor(0, 0, 0, 180))
						dxDrawText("Megveszed a slotot #4aabd01000 PP#ffffff-ért?", itemX + respc(10), itemY, 0, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "left", "center", false, false, false, true)
						...
						drawButton3("cancelUnlockSlot", "Nem", buttons.cancelUnlockSlot[1], buttons.cancelUnlockSlot[2], buttons.cancelUnlockSlot[3], buttons.cancelUnlockSlot[4], {215, 89, 89}, false, Roboto, true, 0.7)
						...
						drawButton3("unlockSlot", "Igen", buttons.unlockSlot[1], buttons.unlockSlot[2], buttons.unlockSlot[3], buttons.unlockSlot[4], {255, 148, 40}, false, Roboto, true, 0.7)
						...
						dxDrawText("#ADADADZárolt slot - #ffffffSlot megvásárlása #4aabd01000 PP#ADADAD-ért", itemX, itemY, itemX + itemSizeX, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "center", "center", false, false, false, true)
				...
				elseif i > myClothesLimit + 1 then
					...
					if clothSlotInUse[i] then
						...
						dxDrawText(itemNames[clothSlotInUse[i]], itemX + respc(10), itemY, 0, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "left", "center")
						...
						drawButton3(removeName, "Levétel", buttons[removeName][1], buttons[removeName][2], buttons[removeName][3], buttons[removeName][4], {215, 89, 89}, false, Roboto, true, 0.7)
						...
						drawButton3(moveName, "Mozgatás", buttons[moveName][1], buttons[moveName][2], buttons[moveName][3], buttons[moveName][4], {255, 148, 40}, false, Roboto, true, 0.7)
					else
						if activeButton == "selectSlot:" .. i then
							dxDrawRectangle(itemX, itemY, itemSizeX, itemSizeY - respc(10), tocolor(255, 148, 40, 100))
						end

						dxDrawText("Üres slot", itemX, itemY, itemX + itemSizeX, itemY + itemSizeY - respc(10), tocolor(255, 255, 255, 230), 0.7, Roboto, "center", "center")
		...
		if outfitsPanel then
			dxDrawText("Kiegészítőim", buttons.outfits[1], buttons.outfits[2], buttons.outfits[1] + buttons.outfits[3], buttons.outfits[2] + buttons.outfits[4], tocolor(255, 255, 255, 230), 0.7, Roboto, "center", "center")
		else
			dxDrawText("Öltözékek", buttons.outfits[1], buttons.outfits[2], buttons.outfits[1] + buttons.outfits[3], buttons.outfits[2] + buttons.outfits[4], tocolor(255, 255, 255, 230), 0.7, Roboto, "center", "center")
		end
	else
		local sx = respc(550)
		local sy = respc(550)
		...
		-- ** Cím
		dxDrawRectangle(x + 3, y + 3, sx - 6, respc(40) - 3, tocolor(0, 0, 0, 170))
		dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff- Ruházat", x + respc(7.5), y, x + sx, y + respc(40), tocolor(255, 255, 255, 230), 1, Roboto, "left", "center", false, false, false, true)
			...
			if item then
				if item[1] == "cat" then
					...
					elseif item[1] == "haveitem" then
						...
						if baseCategories[cat] then
							dxDrawText("Már van felvéve ebből a kategóriából.", itemX + respc(20) + respc(16), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
							...
							drawButton3(btnName, "Levétel", buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], {215, 89, 89}, false, Roboto, true, 0.7)
						end
					elseif item[1] == "sub" then
						dxDrawText(item[2], itemX + respc(20), itemY, 0, itemY + itemSizeY, tocolor(255, 148, 40, 160), 0.75, Roboto, "left", "center")
					else
						local model = item[2]
						...
						if itemMods[model] and itemIndexes[model] then
							...
							if subCategories[cat] then
								if not groupClothes[model] then
									...
									dxDrawText(itemNames[model], itemX + respc(30) + respc(16), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
								else
									dxDrawText(itemNames[model], itemX + respc(30), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
								end
							else
								if not groupClothes[model] then
									...
									dxDrawText(itemNames[model], itemX + respc(20) + respc(16), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
								else
									dxDrawText(itemNames[model], itemX + respc(20), itemY, 0, itemY + itemSizeY, tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center")
								end
							end
							...
							drawButton3(addName, "Kiválaszt", buttons[addName][1], buttons[addName][2], buttons[addName][3], buttons[addName][4], {255, 148, 40}, false, Roboto, true, 0.7)
...
function openTheEditor(category, datas, slot)
	...
	exports.cosmo_boneattach:attachElementToBone(editorMainObject, localPlayer, boneCategories[category], datas[2] or 0, datas[3] or 0, datas[4] or 0, datas[5] or 0, datas[6] or 0, datas[7] or 0)
	--exports.cosmo_core:toggleHeadMove(0)
	...
	if button == "left" then
		if state == "up" then
			...
			elseif activeButton == "moveObject:reset" then
				if editorType then
					local ped, bone, x, y, z, rx, ry, rz = exports.cosmo_boneattach:getElementBoneAttachmentDetails(editorMainObject)

					myLastPresets[editorType] = {getElementModel(editorMainObject), x, y, z, rx, ry, rz, 1, 1, 1}

					exports.cosmo_boneattach:setElementBonePositionOffset(editorMainObject, 0, 0, 0)
					exports.cosmo_boneattach:setElementBoneRotationOffset(editorMainObject, 0, 0, 0)
			...
			elseif activeButton == "moveObject:save" then
				...
				if editorType then
					...
					editorType = false
					--exports.cosmo_core:toggleHeadMove(1)
			...
			elseif activeButton == "preset:loadTheDef" then
				if presetsPanel then
					...
					exports.cosmo_boneattach:setElementBonePositionOffset(editorMainObject, clothData[2] or 0, clothData[3] or 0, clothData[4] or 0)
					exports.cosmo_boneattach:setElementBoneRotationOffset(editorMainObject, clothData[5] or 0, clothData[6] or 0, clothData[7] or 0)
...
function moveObjectRender()
	...
	if menu2dX and menu2dY and not activeAxis then
		...
		-- Mozgatás
		if activeMode ~= "move" then
			dxDrawRectangle(menu2dX - 8, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		else
		...
		-- Forgatás
		if activeMode ~= "rotate" then
			dxDrawRectangle(menu2dX - 8 + 20, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		else
		...
		-- Méretezés
		if activeMode ~= "scale" then
			dxDrawRectangle(menu2dX - 8 + 40, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		else
		...
		-- Alapállapot
		dxDrawRectangle(menu2dX - 8 + 60, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		dxDrawImage(menu2dX - 8 + 60, menu2dY - 8, 16, 16, "files/images/reset.png", 0, 0, 0, tocolor(0, 0, 0))

		-- Mentés
		dxDrawRectangle(menu2dX - 8 + 80, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		dxDrawImage(menu2dX - 8 + 80, menu2dY - 8, 16, 16, "files/images/save.png", 0, 0, 0, tocolor(0, 0, 0))

		-- Mentések
		dxDrawRectangle(menu2dX - 8 + 100, menu2dY - 8, 16, 16, tocolor(255, 148, 40))
		...
		if presetsPanel then
			...
			dxDrawText("Mentett elrendezések", x, y, x + sx, y + respc(22), tocolor(255, 255, 255, 230), 1, editorFont, "center", "center")
		...
		if tooltipText then
			...
			dxDrawText(tooltipText, absX + 5, absY, absX + 5 + textWidth, absY + textHeight, tocolor(255, 255, 255, 230), 0.95, editorFont, "center", "center")
		end
	end
	...
	if activeAxis == "x" and cursorX and cursorY then
		...
		if activeMode == "move" then
			exports.cosmo_boneattach:setElementBonePositionOffset(editorMainObject, attachmentDetails[1] + moveOffset, attachmentDetails[2], attachmentDetails[3])
		elseif activeMode == "rotate" then
			if attachmentDetails[1] + moveOffset > 360 then
				moveOffset = 0
			end

			exports.cosmo_boneattach:setElementBoneRotationOffset(editorMainObject, attachmentDetails[1] + moveOffset, attachmentDetails[2], attachmentDetails[3])
	...
	if activeAxis == "y" and cursorX and cursorY then
		...
		if activeMode == "move" then
			exports.cosmo_boneattach:setElementBonePositionOffset(editorMainObject, attachmentDetails[1], attachmentDetails[2] + moveOffset, attachmentDetails[3])
		elseif activeMode == "rotate" then
			if attachmentDetails[1] + moveOffset > 360 then
				moveOffset = 0
			end

			exports.cosmo_boneattach:setElementBoneRotationOffset(editorMainObject, attachmentDetails[1], attachmentDetails[2] + moveOffset, attachmentDetails[3])
		elseif activeMode == "scale" then
	...
	if obj2dX and obj2dY then
		...
		if axis2dX and axis2dY then
			...
			dxDrawLine(obj2dX, obj2dY, axis2dX, axis2dY, tocolor(255, 148, 40))
			dxDrawRectangle(math.floor(axis2dX - 8), math.floor(axis2dY - 8), 16, 16, tocolor(255, 148, 40))
			dxDrawImage(math.floor(axis2dX - 8), math.floor(axis2dY - 8), 16, 16, "files/images/" .. activeMode .. ".png", 0, 0, 0, tocolor(0, 0, 0))
		end
	end
	...
	if activeAxis == "z" and cursorX and cursorY then
		...
		if activeMode == "move" then
			exports.cosmo_boneattach:setElementBonePositionOffset(editorMainObject, attachmentDetails[1], attachmentDetails[2], attachmentDetails[3] + moveOffset)
		elseif activeMode == "rotate" then
			if attachmentDetails[1] + moveOffset > 360 then
				moveOffset = 0
			end

			exports.cosmo_boneattach:setElementBoneRotationOffset(editorMainObject, attachmentDetails[1], attachmentDetails[2], attachmentDetails[3] + moveOffset)
		elseif activeMode == "scale" then
