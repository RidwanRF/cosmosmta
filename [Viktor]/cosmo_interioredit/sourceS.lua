...
function loadInterior(thePlayer, interiorId, editInterior, forceLoad)
	outputDebugString(inspect(thePlayer) .. " | " .. inspect(interiorId) .. " | " .. inspect(editInterior) .. " | " .. inspect(forceLoad))
	local currentInterior = getElementData(thePlayer, "currentCustomInterior2") or 0

	if not forceLoad and currentInterior > 0 and currentInterior == interiorId then
		return
	end

	setElementData(thePlayer, "currentCustomInterior2", interiorId)
	...
	if editInterior then
		setElementData(thePlayer, "editingInterior2", interiorId)
	else
		setElementData(thePlayer, "editingInterior2", false)
		setCameraTarget(thePlayer, thePlayer)
	end

	local editable = exports.cosmo_interiors:getInteriorEditable(interiorId)
...
registerEvent("interiorEdit:exitInterior", getRootElement(),
	function (normalexit)
		if isElement(source) then
			local interiorId = getElementData(source, "currentCustomInterior2") or 0

			if interiorId > 0 then
				setElementData(source, "editingInterior2", false)
				setElementData(source, "currentCustomInterior2", false)

				setCameraTarget(source, source)
				setElementFrozen(source, false)

				if normalexit then
					local entrance = exports.cosmo_interiors:getInteriorEntrance(interiorId)

					exports.cosmo_interiors:warpElement(source, entrance.position[1], entrance.position[2], entrance.position[3], entrance.rotation[1], entrance.rotation[2], entrance.rotation[3], entrance.interior, entrance.dimension)
	...
	function (interiorId, interiorData, costs)
		if isElement(source) then
			if interiorId and interiorData then
				...
				if currCosts > 0 then
					exports.cosmo_core:takeMoney(source, currCosts, eventName)
				else
					exports.cosmo_core:giveMoney(source, currCosts, eventName)
				end
...
registerEvent("interiorEdit:placeSafe", getRootElement(),
	function (dat)
		if source == client and dat then
			local interiorId = getElementData(source, "editingInterior2") or 0

			if interiorId > 0 then
				local currBalance = getElementData(source, "char.PP") or 0

				currBalance = currBalance - prices.furnitures[2332]

				if currBalance >= 0 then
					setElementData(source, "char.PP", currBalance)

					triggerEvent("createSafe", source, 0, dat[3], dat[4], dat[5], dat[6], 1, interiorId)

					dbExec(connection, "UPDATE accounts SET premium = ? WHERE accountId = ?", currBalance, getElementData(source, "char.accID"))
				else
					exports.cosmo_hud:showInfobox(source, "error", "Nincs elég prémium pontod!")
				end
			end
		end
	end)
	...
	function (url, interiorId, fromMovieList)
		if isElement(source) and url and interiorId then
			...
			if url and id then
				...
				exports.cosmo_hud:showInfobox(client, "error", "Érvénytelen URL! Kérlek próbálj másikat.")
			end
		end
	end)