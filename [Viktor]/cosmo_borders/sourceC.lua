...
function isOfficer(player)
	return exports.cosmo_groups:isPlayerInGroup(player, 1) or exports.cosmo_groups:isPlayerInGroup(player, 57) or exports.cosmo_groups:isPlayerInGroup(player, 4) -- példa: or exports.cosmo_groups:isPlayerInGroup(player, 2) or exports.cosmo_groups:isPlayerInGroup(player, 2) or exports.cosmo_groups:isPlayerInGroup(player, 2)
end
	...
	function (commandName, mode)
		if isOfficer(localPlayer) then
			...
			if not (mode and mode >= 1 and mode <= 3) then
				outputChatBox("#ff9428[Használat]: #ffffff/" .. commandName .. " [Mód (1/2/3)]", 255, 255, 255, true)
				outputChatBox("#ff9428[Módok]: #ffffffAutomatikus(1), Nyitva(2), Zárva(3)", 255, 255, 255, true)
			else
				...
				if borderId then
					...
					outputChatBox("#ff9428[Határ]: #ffffffCsak a határ közelében használhatod ezt a parancsot! (ColShape)", 255, 255, 255, true)
				end
			end
		end
	end)
	...
	function (playerName, borderId, set)
		if isOfficer(localPlayer) or getElementData(localPlayer, "acc.adminLevel") >= 1 then
			if set == 2 or set == 3 then
				...
				outputChatBox("#ff9428[CosmoMTA - Határ]: #ffffff" .. playerName:gsub("_", " ") .. " átállított egy határt manuális nyitásra. (#" .. borderId .. " - " .. state .. ")", 255, 255, 255, true)
			elseif set == 1 then
				outputChatBox("#ff9428[CosmoMTA - Határ]: #ffffff" .. playerName:gsub("_", " ") .. " átállított egy határt automatikus nyitásra. (#" .. borderId .. ")", 255, 255, 255, true)
			end
		end
	end)
...
function toggleBorderMessage()
	if isOfficer(localPlayer) then
		if borderWarns then
			outputChatBox("#ff9428[CosmoMTA - Határ]: #ffffffSikeresen kikapcsoltad az határ értesítéseket!", 255, 255, 255, true)
			borderWarns = false
		else
			outputChatBox("#ff9428[CosmoMTA - Határ]: #ffffffSikeresen bekapcsoltad az határ értesítéseket!", 255, 255, 255, true)
			borderWarns = true
		end
	end
end
addCommandHandler("toghatar", toggleBorderMessage)
addCommandHandler("togborder", toggleBorderMessage)
addCommandHandler("togbordermessage", toggleBorderMessage)
addCommandHandler("toghataruzenet", toggleBorderMessage)
	...
	function (vehicle)
		if isElement(vehicle) and isOfficer(localPlayer) and borderWarns then
			...
			if plateText then
				...
				outputChatBox("#ff9428[CosmoMTA - Határ]:#ffffff Egy jármű átlépte a határt! Rendszáma: #ff9428" .. table.concat(plateText, "-"), 255, 255, 255, true)

				local r1, g1, b1, r2, g2, b2 = getVehicleColor(vehicle, true)

				outputChatBox("#ff9428[CosmoMTA - Határ]:#ffffff Típus: #598ed7" .. exports.cosmo_mods_veh:getVehicleNameFromModel(getElementModel(vehicle)) .. "#FFFFFF Színek: " .. rgbToHex(r1, g1, b1) .. "szín1 " .. rgbToHex(r2, g2, b2) .. "szín2", 255, 255, 255, true)
			end
		end
	end)
...
function togglePassport(itemId, data1, data2, data3)
	if passportData1 then
		...
		exports.cosmo_chat:localActionC(localPlayer, "elrak egy útlevelet.")
	else
		...
		passportShader = dxCreateShader(":cosmo_inventory/files/monochrome.fx")
		dxSetShaderValue(passportShader, "screenSource", passportRT)

		exports.cosmo_chat:localActionC(localPlayer, "elővesz egy útlevelet.")
	end
end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		if passportData1 then
			...
			dxDrawImage(x, y, sx, sy, ":cosmo_accounts/files/passport.png")

			if fileExists(":cosmo_binco/files/skins/" .. passportData1.skin .. ".png") then
				dxDrawImage(x + 20, y + 43, 120, 120, ":cosmo_binco/files/skins/" .. passportData1.skin .. ".png")
			end
			...
			if passportIsCopy then
				...
				dxDrawRoundedRectangle(x, y, 800, 500, tocolor(255, 255, 255))
				dxDrawImage(x + passportIsCopy[1], y + passportIsCopy[2], 481, 481, passportShader, -passportIsCopy[3], 210, 210)
				dxDrawImage(x, y, 800, 500, ":cosmo_inventory/files/paper.png")
			end
		end
		...
		if currentBorderId then
			...
			dxDrawRoundedRectangle(x, y, sx, sy, tocolor(0, 0, 0, 170))
			dxDrawRoundedRectangle(x, y, sx, respc(30), tocolor(0, 0, 0, 130))
			
			dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff - Határ", x + respc(7.5), y, 0, y + respc(30), tocolor(255, 255, 255), 0.75, Roboto, "left", "center", false, false, false, true)
			dxDrawText("Szeretnél átlépni a határon?\nA határátlépés ára #ff9428100.000$#ffffff.", x, y + respc(30), x + sx, y + sy - respc(45) * 2, tocolor(255, 255, 255), 0.75, Roboto, "center", "center", false, false, false, true)
			
			if activeButton == "yes" then
				dxDrawRoundedRectangle(x + respc(5), y + sy - respc(45) * 2, sx - respc(10), respc(40), tocolor(124, 197, 118, 190))
			else
				dxDrawRoundedRectangle(x + respc(5), y + sy - respc(45) * 2, sx - respc(10), respc(40), tocolor(124, 197, 118, 150))
			end
			...
			if activeButton == "no" then
				dxDrawRoundedRectangle(x + respc(5), y + sy - respc(45), sx - respc(10), respc(40), tocolor(215, 89, 89, 190))
			else
				dxDrawRoundedRectangle(x + respc(5), y + sy - respc(45), sx - respc(10), respc(40), tocolor(215, 89, 89, 150))
			end
