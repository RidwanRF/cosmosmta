	...
	function (commandName, gameInterior, interiorType, buyPrice, ...)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (gameInterior and interiorType and buyPrice and (...)) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Belső Azonosító (0: DUMMY)] [Típus] [Ár] [Név]", 255, 255, 255, true)
				outputChatBox("#ff9428[Típusok]: #ffffffHáz (1), Passzív Biznisz (2), Aktív Biznisz (3), Középület (4), Garázs (5), Zárható középület (6), Bérelhető (7), Műhely (8)", 255, 255, 255, true)
			else
				...
				if gameInteriors[gameInterior] or gameInterior == 0 then
					if interiorType >= 1 and interiorType <= 8 then
						...
						if buyPrice >= 0 and buyPrice <= 10000000000 then
							...
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffAz ár nem lehet kisebb mint #ff94280 #ffffffés nem lehet nagyobb mint #ff942810 000 000 000#ffffff.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffÉrvénytelen interior típus.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffÉrvénytelen interior azonosító.", 255, 255, 255, true)
				end
			end
		end
	end
	...
	function (commandName, interiorType)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorType then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Típus (1: Kicsi, 2: Közepes, 3: Nagy)]", 255, 255, 255, true)
			else
				if interiorType >= 1 and interiorType <= 3 then
					...
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffÉrvénytelen garázs típus.", 255, 255, 255, true)
				end
			end
		end
	end
	...
	function (commandName, interiorId)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID]", 255, 255, 255, true)
			else
				...
				elseif deletedInteriors[interiorId] then
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior már törölve van.", 255, 255, 255, true)
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffAz interior visszaállításához használd a #ff9428'/resetinterior' #ffffffparancsot.", 255, 255, 255, true)
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
			end
		end
	end
	...
	function (commandName, interiorId, ...)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and (...)) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Név]", 255, 255, 255, true)
			else
				if availableInteriors[interiorId] then
					local name = table.concat({...}, " ")
	...
	function (commandName, interiorId)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				...
				if availableInteriors[interiorId] then
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior neve #ff9428" .. availableInteriors[interiorId]["name"] .. ".", 255, 255, 255, true)
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
			end
	...
	function (commandName, interiorId, buyPrice)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and buyPrice) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Ár]", 255, 255, 255, true)
			else
				buyPrice = math.floor(buyPrice)
	...
	function (commandName, interiorId)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				...
				if availableInteriors[interiorId] then
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior ára #ff9428" .. formatNumber(availableInteriors[interiorId]["price"]) .. " $.", 255, 255, 255, true)
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
			end
	...
	function (commandName, interiorId, interiorType)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and interiorType) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Típus]", 255, 255, 255, true)
				outputChatBox("#ff9428[Típusok]: #ffffffHáz (1), Passzív Biznisz (2), Aktív Biznisz (3), Középület (4), Garázs (5), Zárható középület (6), Bérelhető (7)", 255, 255, 255, true)
			else
				if availableInteriors[interiorId] then
		...
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				...
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior típusa #ff9428" .. interiorType .. ".", 255, 255, 255, true)
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
			end
	...
	function (commandName)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if #nearbyList > 0 then
				outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffKözeledben lévő interiorok (15 yard):", 255, 255, 255, true)

				for k, v in ipairs(nearbyList) do
					outputChatBox("    * #ff9428Azonosító: #ffffff" .. v[1] .. " | #ff9428Név: #ffffff" .. utf8.gsub(v[2], "_", " "), 255, 255, 255, true)
				end
			else
				outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNincs egyetlen interior sem a közeledben.", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (commandName, interiorId, gameInterior)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and gameInterior) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Belső]", 255, 255, 255, true)
			else
				if availableInteriors[interiorId] then
					if gameInteriors[gameInterior] then
						if availableInteriors[interiorId]["editable"] ~= "N" and getElementData(localPlayer, "acc.adminLevel") < 7 then
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffSzerkeszthető interiorok belsejét csak SA kezelheti!", 255, 255, 255, true)
							return
						end
						...
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott belső nem létezik.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
	...
	function (commandName, interiorId, x, y)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not (interiorId and x and y) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [x] [y]", 255, 255, 255, true)
			else
				if x > 13 or y > 13 or x < 2 or y < 2 then
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA méret 2 és 13 között lehet!", 255, 255, 255, true)
					return
				end

				if availableInteriors[interiorId] then
					if availableInteriors[interiorId]["editable"] ~= "N" then
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior már szerkeszthető.", 255, 255, 255, true)
						return
					end
			...
			if not interiorId then
				...
				if availableInteriors[interiorId] then
					if availableInteriors[interiorId]["editable"] and string.find(availableInteriors[interiorId]["editable"], "x") then
						outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior szerkeszthető, mérete: " .. availableInteriors[interiorId]["editable"] .. ".", 255, 255, 255, true)
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem szerkeszthető.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
	...
	function (commandName, interiorId, colShapeType)
		if getElementData(localPlayer, "acc.adminLevel") >= 9 then
			...
			if not (interiorId and colShapeType) then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [< 0 = bejárat | 1 = kijárat >]", 255, 255, 255, true)
			else
				if availableInteriors[interiorId] then
					if not isPedInVehicle(localPlayer) then
						...
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffElőbb szállj ki a járműből!", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", 255, 255, 255, true)
				end
