	...
	if resultRows then
		...
		if resultRow then
			...
			if isElement(sourcePlayer) then
				outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffSikeresen létrehoztál egy interiort. #ff9428(" .. resultRow.interiorId .. ")", sourcePlayer, 255, 255, 255, true)
			...
			if interiorId then
				if availableInteriors[interiorId] then
					if not deletedInteriors[interiorId] then
						...
						outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior sikeresen törölve. #ff9428(" .. interiorId .. ")", source, 255, 255, 255, true)
							
						--exports.sm_logs:logCommand(source, "deleteInterior", {interiorId})
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior már törölve van.", source, 255, 255, 255, true)
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffAz interior visszaállításához használd a #d75959/resetinterior#ffffff parancsot.", source, 255, 255, 255, true)
					end
				elseif deletedInteriors[interiorId] then
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior már törölve van.", source, 255, 255, 255, true)
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffAz interior visszaállításához használd a #ff9428'/resetinterior' #ffffffparancsot.", source, 255, 255, 255, true)
				end
			end
		end
	end
...
function resetInteriorCallback(queryHandle, sourcePlayer)
	...
	if resultRows then
		...
		if resultRow then
			...
			if isElement(sourcePlayer) then
				outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior sikeresen visszaállítva. #ff9428(" .. resultRow.interiorId .. ")", sourcePlayer, 255, 255, 255, true)
	...
	function (sourcePlayer, commandName, interiorId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID]", sourcePlayer, 255, 255, 255, true)
			else
				...
				elseif availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior sikeresen visszaállítva. #ff9428(" .. interiorId .. ")", sourcePlayer, 255, 255, 255, true)
						
					-- exports.sm_logs:logCommand(sourcePlayer, "resetInterior", {interiorId})
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
		...
		if isElement(source) then
			if interiorId and newName then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior sikeresen átnevezve. #ff9428(" .. newName .. ")", source, 255, 255, 255, true)
		...
		if isElement(source) then
			if interiorId and newPrice then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior ára sikeresen módosítva. #ff9428(" .. formatNumber(newPrice) .. "$)", source, 255, 255, 255, true)
		...
		if isElement(source) then
			if interiorId and newType then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior típusa sikeresen módosítva.", source, 255, 255, 255, true)
		...
		if isElement(source) then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior bejárata sikeresen áthelyezve.", source, 255, 255, 255, true)
			...
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					if source == client then
						outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior kijárata sikeresen áthelyezve.", source, 255, 255, 255, true)
		...
		if isElement(source) then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior belső kinézete sikeresen lecserélve.", source, 255, 255, 255, true)
		...
		if isElement(source) then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior szerkeszthetővé alakítva. (" .. newDatas["editable"] .. ")", source, 255, 255, 255, true)
...
addCommandHandler("setinteriorowner",
	function (sourcePlayer, commandName, interiorId, partialNick)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				outputChatBox("#ff9428[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Játékos Név / ID]", sourcePlayer, 255, 255, 255, true)
			else
				local int = availableInteriors[interiorId]

				if int then
					if int.type ~= "building" and int.type ~= "rentable" then
						local targetPlayer, targetName = exports.cosmo_core:findPlayer(sourcePlayer, partialNick)
						...
						if isElement(targetPlayer) then
							...
							if characterId then
								...
								if not exports.cosmo_inventory:hasItemWithData(targetPlayer, 2, interiorId) then
									exports.cosmo_inventory:giveItem(targetPlayer, 2, 1, false, interiorId)
								end
						...
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffKözépületnek és bérlakásnak nem lehet tulajdonost beállítani.", sourcePlayer, 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
...
function getInteriorOwnerCallback(queryHandle, sourcePlayer)
	if isElement(sourcePlayer) then
		...
		if resultRows then
			...
			if resultRow then
				outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffA kiválasztott interior tulajdonosa #ff9428" .. resultRow.name .. " [Karakter ID: " .. resultRow.charID .. " - Account ID: " .. resultRow.accID .. "].", sourcePlayer, 255, 255, 255, true)
			end
		end
	end
end
			...
			if not interiorId then
				...
				if int then
					if int.ownerId > 0 then
						dbQuery(getInteriorOwnerCallback, {sourcePlayer}, connection, "SELECT characters.name, characters.charID, characters.accID FROM characters LEFT JOIN interiors ON interiors.ownerId = characters.charID WHERE interiors.interiorId = ? LIMIT 1", interiorId)
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interiornak nincs tulajdonosa.", sourcePlayer, 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
	...
	function (posX, posY, posZ, rotX, rotY, rotZ, interior, dimension)
		if isElement(source) then
			if posX and posY and posZ and rotX and rotY and rotZ and interior and dimension then
				...
				outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffSikeresen elteleportáltál a kiválasztott interiorhoz.", source, 255, 255, 255, true)
