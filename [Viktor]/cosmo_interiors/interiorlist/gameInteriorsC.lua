...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedResource)
		...
		elseif getResourceName(startedResource) == "cosmo_interiors" then
			gameInteriors = exports.cosmo_interiors:getGameInteriors()
		elseif startedResource == getThisResource() then
			...
			local cosmo_interiors = getResourceFromName("cosmo_interiors")

			if cosmo_interiors and getResourceState(cosmo_interiors) == "running" then
				gameInteriors = exports.cosmo_interiors:getGameInteriors()
			end
		end
	end
...
function createFonts()
	...
	Roboto = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(15), false, "antialiased")
end
...
exports.cosmo_admin:addAdminCommand("gameintlist", 6, "Interior lista megnézése")
function interiorlista()
...
addCommandHandler("gameintlist", interiorlista)
addCommandHandler("gameinteriors", interiorlista)
addCommandHandler("interiorlist", interiorlista)
							...
							if getTickCount() - clickTick >= 1000 then
								if not isPedInVehicle(localPlayer) then
									if gameInteriors[id] then
										...
										exports.cosmo_hud:showInfobox("error", "Nem létező interior!")
									end
								else
									exports.cosmo_hud:showInfobox("error", "Előbb szállj ki a járműből!")
								end
							else
...
function renderThePanel()
	...
	dxDrawText("#ff9428Cosmo#ffffffMTA #ffffff- Interior belsők", x + respc(7), y, x + sx, y + respc(40), tocolor(255, 255, 255, 230), 0.8, Roboto, "left", "center", false, false, false, true)
	...
	for i = 1, 15 do
		...
		if v then
			dxDrawText("#ff9428[" .. v[1] .. "] #ffffff" .. v[2].name, x + respc(5), y2, x + sx, y2 + oneButtonHeight, tocolor(255, 255, 255, 230), 0.65, Roboto, "left", "center", false, true, false, true)
			...
			drawButton3(btnName, "Megtekint", buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], {255, 148, 40}, false, Roboto, true)
		end
	end
	...
	if #interiorList > 15 then
		...
		dxDrawRectangle(x + sx - respc(5), y + respc(40) + visibleItem * contentRatio, respc(5), contentRatio * 15, tocolor(255, 148, 40, 150))
	end

	-- Kereső mező
	dxDrawRectangle(x, y + sy - respc(40), sx, respc(40), tocolor(0, 0, 0, 175))
	
	buttons.searchInput = {x + respc(5), y + sy - respc(40) + respc(5), sx - respc(10), respc(40) - respc(10)}

	dxDrawRectangle(buttons.searchInput[1], buttons.searchInput[2], buttons.searchInput[3], buttons.searchInput[4], tocolor(0, 0, 0, 50))

	if utf8.len(searchInputText) < 1 and not activeSearchInput then
		dxDrawText("Keresés...", buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center", true)
	else
		if activeSearchInput and cursorState then
			dxDrawText(searchInputText .. "|", buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center", true)
		else
			dxDrawText(searchInputText, buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(255, 255, 255, 230), 0.75, Roboto, "left", "center", true)
		end
	end
