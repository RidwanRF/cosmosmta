	...
	if interiorId then
		if availableInteriors[interiorId] then
			...
			if standingMarker and standingMarker[1] == interiorId then
				exports.cosmo_hud:endInteriorBox()
			end
		end
	end
end
	...
	function (hitElement, dimensionMatch)
		if hitElement == localPlayer and dimensionMatch then
			...
			if interiorId and colShapeType then
				...
				if int.ownerId == 0 then
					if int.type ~= "building" and int.type ~= "building2" then
						if int.type == "rentable" then
							outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffEz az ingatlan kiadó! A bérléshez használd a #ff9428/rent #ffffffparancsot!", 255, 255, 255, true)
							outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffBérleti díj: #ff9428" .. formatNumber(int.price) .. " $/hét.#ffffff Kaució: #ff9428" .. formatNumber(int.price * 4) .. " $", 255, 255, 255, true)
						else
							outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffEz az ingatlan eladó! A vásárláshoz használd a #ff9428/buy #ffffffparancsot! Ár: #ff9428" .. formatNumber(int.price) .. " $", 255, 255, 255, true)
						end
					...
					if int.ownerId == tonumber(getElementData(localPlayer, "char.ID")) then
						if int.editable ~= "N" then
							if colShapeType == "entrance" then
								outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffEz az ingatlan szerkeszthető!", 255, 255, 255, true)
								outputChatBox("#ff9428[CosmoMTA - Interior]: #ffffffHasználd a #ff9428[Z]#ffffff gombot vagy a '#ff9428/edit#ffffff' parancsot a szerkesztéshez!", 255, 255, 255, true)
								--exports.cosmo_hud:showInfobox("i", "Ez egy szerkeszthető interior! Részletek a chatboxban.")
							end
						end
					end
				end
				...
				if colShapeType == "entrance" then
					exports.cosmo_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj #ff9428[E] #ffffffgombot a belépéshez.", int.locked, int.type, showPreview and int.gameInterior or false)
				elseif colShapeType == "exit" then
					exports.cosmo_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj #ff9428[E] #ffffffgombot a kilépéshez.", int.locked, int.type)
				end
			end
		end
	end
	...
	if standingMarker and standingMarker[1] == interiorId then
		...
		if int then
			...
			if standingMarker[2] == "entrance" then
				exports.cosmo_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj [E] gombot a belépéshez.", int.locked, int.type, showPreview and int.gameInterior or false)
			else
				exports.cosmo_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj [E] gombot a kilépéshez.", int.locked, int.type)
			end
		else
			exports.cosmo_hud:endInteriorBox()
		end
				...
				if int.type == "rentable" then
					...
					if int.ownerId == characterId then
						if getTickCount() >= lastInteriorBuy + 5000 then
							...
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffCsak 5 másodpercenként használhatod ezt a parancsot.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEzt az ingatlant nem te bérled.", 255, 255, 255, true)
					end
				end
			end
		end
		...
		if standingMarker then
			...
			if int then
				if int.type == "rentable" then
					...
					if int.ownerId == 0 then
						if exports.cosmo_core:getMoney(localPlayer) >= int.price * 5 then
							local interiorLimit = getElementData(localPlayer, "char.interiorLimit") or 5
							...
							if interiorCount < interiorLimit then
								...
								outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNincs szabad ingatlan slotod.", 255, 255, 255, true)
							end
						else
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat és kauciót.", 255, 255, 255, true)
						end
					elseif int.ownerId == characterId then
						if exports.cosmo_core:getMoney(localPlayer) >= int.price then
							if getTickCount() >= lastInteriorBuy + 5000 then
							...
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEz a bérlakás nem kiadó.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEzt az ingatlant nem lehet kibérelni.", 255, 255, 255, true)
				end
			end
		end
	end)
...
addCommandHandler("buy",
	function ()
		if standingMarker then
			...
			if int then
				if int.type == "rentable" then
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffBérelhető lakást nem vásárolhatsz meg.", 255, 255, 255, true)
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffBérléshez használd a #598ed7/rent#ffffff parancsot.", 255, 255, 255, true)
					return
				end
				...
				if int.type ~= "building" and int.type ~= "building2" then
					if int.ownerId == 0 then
						if exports.cosmo_core:getMoney(localPlayer) >= int.price then
							...
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNincs elég pénzed az ingatlan megvásárlásához. (#ff9428" .. formatNumber(int.price) .. " $#ffffff)", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEz az ingatlan nem eladó.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffKözépületet nem vásárolhatsz meg.", 255, 255, 255, true)
				end
			end
		end
	end)
			...
			if int and int.type ~= "building" then
				...
				if characterId then
					if getTickCount() >= lastInteriorLock + 5000 then
						...
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffCsak 5 másodpercenként #ff9428nyithatod #ffffff/ #d75959zárhatod #ffffffaz ajtót.", 255, 255, 255, true)
					end
				end
			end
		end
	...
	function (interiorId, locked)
		if interiorId then
			...
			if int then
				...
				if standingMarker and standingMarker[1] == interiorId then
					-- exports.cosmo_hud:setInteriorDoorState(int.locked, int.type)
				end
			end
		end
	end)
	...
	function ()
		if standingMarker then
			if getElementData(localPlayer, "editingInterior") then
				exports.cosmo_hud:showInfobox("error", "Nem hagyhatod el az interiort szerkesztő módban.")
				return
			end
					...
					if not int.dummy or int.dummy == "N" then
						if not getElementData(localPlayer, "cuffed") and not getElementData(localPlayer, "tazed") then
							if getTickCount() >= lastInteriorEnter + 3000 then
								...
								outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffVárj egy kicsit, mielőtt újra használod az ajtót.", 255, 255, 255, true)
							end
						else
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffMegbilincselve/lesokkolva nem használhatod az ajtót.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEz az interior nem rendelkezik tényleges belső térrel.", 255, 255, 255, true)
					end
				end
			end
		end
...
function useDoorRammer()
	if exports.cosmo_groups:isPlayerHavePermission(localPlayer, "doorRammer") then
		if standingMarker then
			local interiorId = tonumber(standingMarker[1])
			...
			if interiorId then
				...
				if int then
					if int.type ~= "building" then
						...
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffItt nem használhatod a faltörő kost.", 255, 255, 255, true)
					end
				end
			end
		else
		...
		outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffNem használhatod a faltörő kost.", 255, 255, 255, true)
	end
end
		...
		if interiorId then
			...
			if int then
				...
				if lastKnockTick + 5000 <= getTickCount() then
					...
					exports.cosmo_chat:sendLocalMeAction(localPlayer, "kopogtat egy ajtón.")

					lastKnockTick = getTickCount()
				else
					exports.cosmo_hud:showInfobox("error", "Csak 5 másodpercenként használhatod ezt a parancsot.")
				end
			end
			end
		end
	else
		outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffItt nem kopogtathatsz.", 255, 255, 255, true)
	end
end
	...
	if standingMarker then
		...
		if interiorId then
			...
			if int then
				if int.type == "house" or int.type == "garage" or int.type == "rentable" then
					if getElementDimension(localPlayer) ~= 0 then
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffCsak kintről csengethetsz be.", 255, 255, 255, true)
					else
					if lastBellTick + 5000 <= getTickCount() then
						triggerServerEvent("useDoorBell", localPlayer, interiorId)

						exports.cosmo_chat:sendLocalMeAction(localPlayer, "megnyomja a csengőt.")
					...
					outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffCsak házba vagy garázsba csengethetsz.", 255, 255, 255, true)
				end
			end
		end
	else
		outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffItt nem csengethetsz.", 255, 255, 255, true)
	end
end
		...
		if interiorId then
			...
			if int then
				...
				if source == localPlayer then
					exports.cosmo_chat:sendLocalMeAction(localPlayer, "betöri az ajtót.")
				end
			end
		end
	end)
...
function createCoolMarker(posX, posY, posZ, markerType, state)
	...
	local markerElement = createMarker(posX, posY, posZ - 1, "cylinder", 0.9, iconColors[markerType][1], iconColors[markerType][2], iconColors[markerType][3], 135)
		...
		if source == localPlayer then
			if dataName == "loggedIn" then
				...
				if interiorId and interiorId > 0 then
					triggerServerEvent("interiorEdit:loadInterior", localPlayer, interiorId, false)
				end
			end
		end
	end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		local size = 0.1 -- * getEasingValue(math.abs(getTickCount() % 5000 - 2500) / 2500, "SineCurve") ezt is kivettem ha kell vissza
		...
		for k = 1, #nearbyMarkers do
			...
			if isElement(v[3]) then
				if getElementInterior(v[3]) == cameraInterior then
					...
					--setMarkerSize(v[3], 1 + size) ezt vettem ki ha érdekel
			...
			if interiorId and colShapeType == "entrance" then
				...
				if int then
					if int.editable ~= "N" then
						...
						if int.ownerId == characterId or getElementData(localPlayer, "acc.adminLevel") >= 9 then
							...
							outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEz az ingatlan nem a te tulajdonod.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[CosmoMTA - Interior]: #ffffffEz nem szerkeszthető interior.", 255, 255, 255, true)
					end
				end
			end
		end
