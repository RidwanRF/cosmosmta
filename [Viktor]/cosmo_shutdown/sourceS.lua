	...
	function (sourcePlayer, commandName, seconds, ...)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			...
			if not (seconds and (...)) then
				outputChatBox("#ff9428[Használat]: #ffffff/" .. commandName .. " [Másodperc] [Indok]", sourcePlayer, 0, 0, 0, true)
			else
	...
	function (sourcePlayer, commandName)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 9 then
			if shutdownTimer then
				...
				outputChatBox("#d75959[CosmoMTA - Shutdown]: #ffffffNincs leállítás folyamatban.", sourcePlayer, 0, 0, 0, true)
			end
		end
	end)
