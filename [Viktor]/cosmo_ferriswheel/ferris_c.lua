...
local tstgs = {
  text = "Nyomj ["..bdKey[ 1 ].."] gombot, hogy beülj a kabinba!",       -- Text that will show up when cabin gets closer to a player
  ...
  for i, col in ipairs( t ) do
    addEventHandler( 'onClientColShapeHit', col, function( player, dim )
      if player == localPlayer and dim then
        ...
        tstgs.text = "Nyomj ["..bdKey[ 1 ].."] gombot, hogy beülj a kabinba!";
...
function getInside( key, state, col )
  ...
  tstgs.text = "Nyomj ["..bdKey[ 2 ].."] gombot, hogy kiszállj a kabinból!";
  unbindKey( bdKey[ 1 ], 'down', getInside );
