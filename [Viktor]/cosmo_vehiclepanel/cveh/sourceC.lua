					...
					if currentSeat ~= 0 then
						...
						if getTickCount() - windowTick[window] > 4000 then
							...
							if getElementData(currentVehicle, "vehicle.window." .. window) then
								exports.cosmo_chat:sendLocalMeAction(localPlayer, "lehúzza a " .. utf8.lower(windowNames[window - 1]) .. "ot.")
							else
								exports.cosmo_chat:sendLocalMeAction(localPlayer, "felhúzza a " .. utf8.lower(windowNames[window - 1]) .. "ot.")
							end
...
function onRender()
	...
	--dxDrawImage(panelPosX, panelPosY + titleHeight, panelWidth, panelHeight - titleHeight, ":cosmo_hud/files/images/vin.png")

	-- ** Cím
	dxDrawRectangle(panelPosX+2, panelPosY+2, panelWidth-4, titleHeight-4, tocolor(0, 0, 0, 150))
	dxDrawImage(panelPosX, panelPosY, titleHeight-4, titleHeight-4, ":cosmo_hud/files/logo.png")


	if panelStage == "window" then
		dxDrawText("#ff9428Cosmo#ffffffMTA", panelPosX + respc(30), panelPosY+respc(3), panelPosX + panelWidth, panelPosY-respc(3) + titleHeight, tocolor(255, 255, 255, 230), 0.9, Raleway, "left", "center", false, false, false, true)
	...
	if panelStage == "window" then
		...
		for i = 1, 4 do
			...
			if i % 2 == 1 then
				dxDrawRectangle(panelPosX, buttonY, panelWidth, oneButtonHeight, tocolor(0, 0, 0,120))
			else
				dxDrawRectangle(panelPosX, buttonY, panelWidth, oneButtonHeight, tocolor(0, 0, 0,130))
			end

			dxDrawText(windowNames[i], panelPosX + respc(10), buttonY, 0, buttonY + oneButtonHeight, tocolor(255, 255, 255, 230), 0.75, Raleway, "left", "center")
			...
			if isVehicleWindowOpen(currentVehicle, i + 1) then
				drawButton3(buttonName, "Felhúz", buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], {215, 89, 89, 170}, false, Raleway)
			else
				drawButton3(buttonName, "Lehúz", buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], {255, 148, 40, 170}, false, Raleway)
			end
		end
	elseif panelStage == "cveh" then
		...
		for i = 1, 6 do
			...
			dxDrawText(doorNames[i], panelPosX + respc(10), buttonY, 0, buttonY + oneButtonHeight, tocolor(255, 255, 255, 230), 1, Raleway, "left", "center")

			buttons[buttonName] = {panelPosX + panelWidth - respc(100), buttonY + respc(10), respc(90), oneButtonHeight - respc(20)}

			if getVehicleDoorOpenRatio(currentVehicle, i - 1) > 0.1 then
				drawButton3(buttonName, "Becsuk", buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], {215, 89, 89, 170}, false, Raleway)
			else
				drawButton3(buttonName, "Kinyit", buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], {255, 148, 40, 170}, false, Raleway)
			end
		end
	end
								...
								if currentVehicle then
									...
									if getTickCount() - doorTick[door] >= 750 then
										...
										if getVehicleDoorOpenRatio(currentVehicle, door - 1) > 0 then
											exports.cosmo_chat:sendLocalMeAction(localPlayer, "becsukja a " .. utf8.lower(doorNames[door]) .. "t.")
										else
											exports.cosmo_chat:sendLocalMeAction(localPlayer, "kinyitja a " .. utf8.lower(doorNames[door]) .. "t.")
										end
		...
		if doorRatio > 0 then
			setTimer(
				function (sourceElement)
					local soundPath = exports.cosmo_vehiclepanel:getDoorCloseSound(getElementModel(sourceElement))
			...
			local soundPath = exports.cosmo_vehiclepanel:getDoorOpenSound(getElementModel(source))
