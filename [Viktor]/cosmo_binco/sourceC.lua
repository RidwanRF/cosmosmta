	...
	function ()
		for k, v in pairs(availableShops) do
			if v[1] and v[2] and v[3] and v[4] then
				local markerElement = createMarker(v[1], v[2], v[3], "cylinder", 2, 255, 148, 40, 155)
					...
					if currentMarkerType then
						...
						if markerShopId then
							if availableShops[markerShopId] then
								...
								exports.cosmo_hud:showInfobox("info", "Nyomj [E] gombot a kínálat megtekintéséhez.")
								inColShape = true
							end
						end
					end
...
addEventHandler("onClientKey", getRootElement(),
	function (key, press)
		...
		if key == "backspace" and not press then
			if panelState and inColShape then
				closePanel()
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		if panelState then
			...
			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(0, 0, 0, 150)) -- háttér
			drawButton3("elozo", "Előző", buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {255, 148, 40}, false, panelFont)
			...
			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(0, 0, 0, 150)) -- háttér
			drawButton3("kovetkezo", "Következő", buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {255, 148, 40}, false, panelFont)
			...
			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(0, 0, 0, 150)) -- háttér
			drawButton3("buy", buttonText, buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {255, 148, 40}, false, panelFont)
					...
					elseif absoluteX >= screenX / 2 - 105 and absoluteX <= screenX / 2 + 105 and absoluteY >= screenY - 180 and absoluteY <= screenY - 120 then
						if skinsTable[selectedSkin] then
							if buySkinTick + 1000 <= getTickCount() then
								...
								outputChatBox("#d75959[CosmoMTA]: #ffffffMásodpercenként csak egyszer.", 0, 0, 0, true)
							end
						end
					end
				end
...
addEventHandler("bincoBuy", getRootElement(),
	function (skinID)
		outputChatBox("#ff9428[CosmoMTA]: #ffffffVettél egy új ruhát." .. " Skin ID: #ff9428".. skinID, 0, 0, 0, true)
		exports.cosmo_hud:toggleHUD(true)
		showChat(true)
		closePanel()
	end
