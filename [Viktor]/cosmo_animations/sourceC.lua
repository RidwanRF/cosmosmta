...
local panelWidth = respc(480)
local panelHeight = respc(530) + titleHeight
...
function renderThePanel()
	...
	dxDrawRectangle(panelPosX, panelPosY, panelWidth, panelHeight - 3 + respc(50), tocolor(0, 0, 0, 150))
	...
	dxDrawRectangle(panelPosX + 3, panelPosY + 3, panelWidth - 6, titleHeight - 6, tocolor(0, 0, 0, 170))
	dxDrawText("#ff9428Cosmo#ffffffMTA - Animációk", panelPosX + respc(7), panelPosY, panelPosX + panelWidth, panelPosY + titleHeight, tocolor(255, 255, 255), 0.8, panelFont, "left", "center", false, false, false, true)
	...
	for i = 2, 14 do
		...
		if i % 2 ~= visibleItem % 2 then
			dxDrawRectangle(panelPosX + 4, y, panelWidth - 8, oneButtonHeight - 2, tocolor(0, 0, 0, 125))
		else
			dxDrawRectangle(panelPosX + 4, y, panelWidth - 8, oneButtonHeight - 2, tocolor(0, 0, 0, 125))
		end
		...
		if v then
			...
			elseif v[2] then
				if v[2] == "stopanim" then
					...
					dxDrawText(v[1], panelPosX + respc(15), y, panelPosX + panelWidth, y + oneButtonHeight, tocolor(255, 255, 255, 200), 0.65, panelFont, "left", "center", true)

					if v[7] then
						local buttonName = "removeFromFavorites_" .. v[6]
						local buttonColor = tocolor(255, 148, 40, 200)

						buttons[buttonName] = {math.floor(panelPosX + panelWidth - oneButtonHeight + respc(2)), math.floor(y + respc(10)), oneButtonHeight - respc(20), oneButtonHeight - respc(20)}

						if activeButton == buttonName then
							buttonColor = tocolor(255, 148, 40, 150)
						end
						...
						if activeButton == buttonName then
							buttonColor = tocolor(255, 148, 40, 250)
						end
				...
				if expandedCategories[v[1]] then
					dxDrawImage(panelPosX + respc(5), y + oneButtonHeight / 2 - oneButtonHeight / 4, oneButtonHeight / 2, oneButtonHeight / 2, "files/arrow.png", 90, 0, 0, tocolor(255, 148, 40))
				else
					dxDrawImage(panelPosX + respc(5), y + oneButtonHeight / 2 - oneButtonHeight / 4, oneButtonHeight / 2, oneButtonHeight / 2, "files/arrow.png", 0, 0, 0, tocolor(255, 148, 40))
				end

				dxDrawText(v[1], panelPosX + respc(7) + oneButtonHeight / 2, y, panelPosX + panelWidth, y + oneButtonHeight, tocolor(255, 148, 40, 200), 0.65, panelFont, "left", "center", true)
			end
		end
	end
	...
	if #availableCategories > 14 then
		...
		dxDrawRectangle(panelPosX + panelWidth - respc(3), panelPosY + titleHeight + visibleItem * contentRatio, respc(3), contentRatio * 14, tocolor(255, 148, 40, 150))
	end
...
function renderCircle()
	if favoritesNum > 0 then
		...
		if getDistanceBetweenPoints2D(cursorX, cursorY, theX, theY) > circleSize / 2 then
			...
			dxDrawImage(theX - circleSize * 0.5 / 2, theY - circleSize * 0.5 / 2, circleSize * 0.5, circleSize * 0.5, "files/stop.png", 0, 0, 0, tocolor(255, 255, 255, 200))
		else
			hoverFavAnim = 0
		...
		for i = 1, favoritesNum do
			...
			if active then
				if hoverFavAnim == i then
					dxDrawImage(x - circleSize / 2, y - circleSize / 2, circleSize, circleSize, "files/circle.png", 0, 0, 0, tocolor(255, 148, 40, 200))
				else
					dxDrawImage(x - circleSize / 2, y - circleSize / 2, circleSize, circleSize, "files/circle.png", 0, 0, 0, tocolor(255, 148, 40, 100))
				end
			...
			if active then
				dxDrawText(v[1], x - circleSize / 2 * 0.9, y - circleSize / 2 * 0.9, x + circleSize / 2 * 0.9, y + circleSize / 2 * 0.9, tocolor(255, 255, 255, 200), 0.8, circleRoboto, "center", "center", false, true)
			else
				dxDrawText(v[1], x - circleSize / 2 * 0.9, y - circleSize / 2 * 0.9, x + circleSize / 2 * 0.9, y + circleSize / 2 * 0.9, tocolor(255, 255, 255, 200), 0.8, circleRoboto, "center", "center", false, true)
			end
		end
		...
		if key == "mouse3" then
			if not isPedInVehicle(localPlayer) then
				if not getElementData(localPlayer, "editingInterior") then
					...
					if favoritesNum == 0 then
						if press then
							exports.cosmo_hud:showInfobox("error", "Előbb adj hozzá animációt a kedvenceidhez! (/animpanel)")
						end
					else
						if press then
...
function drawButton(key, label, x, y, w, h, activeColor, postGui, theFont)
	...
	dxDrawText(label, x, y, x + w, y + h, tocolor(255, 255, 255, 200), labelScale, labelFont, "center", "center", false, false, postGui, true)
