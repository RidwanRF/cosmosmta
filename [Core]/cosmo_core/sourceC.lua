	...
	function (reason)
		if getElementData(source, "loggedIn") then
			...
			if distance <= 20 then
				...
				if quitReason == "Időtúllépés" then
					exports.cosmo_dclog:SendDiscordMessage("Kifagyott egy faszi"..playerName, "anticheat")
				end
...
function interfaceFunctions()
	return {"colorInterpolation", "drawButton", "drawInput", "drawButton2", "drawButton3"}
end
...
function getInterfaceElements()
	...
	function drawInput(key, label, x, y, sx, sy, font, fontScale, a)
		...
		local activeColor = tocolor(255, 148, 40, 200 * a)
		...
		function drawButton3(key, label, x, y, w, h, activeColor, postGui, theFont, rgbaoff, labelScale)
			local buttonColor
