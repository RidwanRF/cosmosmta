...
addEventHandler("onResourceStart", getResourceRootElement(),
	function ()
		...
		setGameType("CosmoMTA | Los Santos")
...
function sendMessageToAdmins(message, lvl)
    ...
    for k = 1, #players do
        ...
        if isElement(v) then
            ...
            if adminLevel ~= 0 and adminLevel >= lvl and not getElementData(v, "togLogs") then
                outputChatBox("#ff9428[Adminnapló]: #ffffff" .. tostring(message), v, 255, 255, 255, true)
            end
        end
    end
end

function sendMessageToAdminsPicsu(message, lvl)
    lvl = lvl or 1
    ...
    for k = 1, #players do
        ...
        if isElement(v) then
            ...
            if adminLevel ~= 0 and adminLevel >= lvl and not getElementData(v, "togLogs") then
                outputChatBox(tostring(message), v, 255, 255, 255, true)
            end
        end
    end
end
...
function toggleLogs(player)
	if getElementData(player, "acc.adminLevel") >= 1 then
		local pmBlocked = getElementData(player, "togLogs")
		if pmBlocked then
			setElementData(player, "togLogs", false)
			outputChatBox("#ff9428[CosmoMTA]: #ffffffAdminlogok mutatása #7cc576engedélyezve!", player, 255, 255, 255, true)
		else
			setElementData(player, "togLogs", true)
			outputChatBox("#ff9428[CosmoMTA]: #ffffffAdminlogok mutatása #D75959kikapcsolva!", player, 255, 255, 255, true)
		end
	end
end
addCommandHandler("toglogs", toggleLogs)

function sendMessageToAs(message, lvl)
	lvl = lvl or 1
	...
	for k = 1, #players do
		...
		if isElement(v) then
			local asLevel = getElementData(v, "acc.helperLevel") or 0
			local adminLevel = getElementData(v, "acc.adminLevel") or 0

			if asLevel ~= 0 and asLevel >= lvl or adminLevel ~= 0 and adminLevel >= lvl  then
				outputChatBox("#ffffff" .. tostring(message), v, 255, 255, 255, true)
			end
		end
	end
end
...
function outputErrorText(text, element)
	if text and isElement(element) then
		outputChatBox("#dc143c>> CosmoMTA : #ffffff" .. text, element, 0, 0, 0, true)
...
function outputInfoText(text, element)
	if text and isElement(element) then
		outputChatBox("#4aabd0>> CosmoMTA : #ffffff" .. text, element, 0, 0, 0, true)
	end
end
...
addCommandHandler("id",
	function (sourcePlayer, commandName, targetPlayer)
		if not targetPlayer then
			outputChatBox("#ff9428>> Használat: #ffffff/" .. commandName .. " [Játékos név / ID]", sourcePlayer, 0, 0, 0, true)
		else
