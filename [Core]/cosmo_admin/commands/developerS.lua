...
-- local blackjackname = "ngrp_blackjack"
...
-- setTimer(
--     function()  
--         restartResource(getResourceFromName(blackjackname))
--     end,
-- 300000, 0)
...
function getResourceList(search, orderColumn, orderDirection, startLimit, stopLimit)
	...
	for k = #availableResources, 1, -1 do
		...
		if v then
			...
			if string.find(resourceName, "cosmo_") then
...
function restartAllRes()
	restartResource(getResourceFromName("cosmo_modstarter"))
end
