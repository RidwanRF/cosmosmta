...
exports.cosmo_admin:addAdminCommand("nearbyshoppeds", 6, "Közelben lévő Bolt NPC-k")
addCommandHandler("nearbyshoppeds",
	function(commandName, maxdist)
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if not maxdist then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [ < távolság > ]", 0, 0, 0, true)
			else
				...
				if #peds > 0 then
					outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Közeledben lévő pedek (" .. maxdist .. " yard):", 0, 0, 0, true)

					for k, v in ipairs(peds) do
						outputChatBox("    * #ff9428Azonosító: #ffffff" .. v[1] .. " | #ff9428Név: #ffffff" .. utf8.gsub(v[2], "_", " ") .. " | #ff9428Távolság: #ffffff" .. v[3] .. " yard", 255, 255, 255, 230, true)
					end
				else
					outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Nincs egyetlen ped sem a közeledben.", 0, 0, 0, true)
				end
			end
		end
	end)
...
addEventHandler("onClientResourceStart", getRootElement(),
	function(res)
		if getResourceName(res) == "cosmo_inventory" then
			itemNames = exports.cosmo_inventory:getItemNameList()
			nonStackableItems = exports.cosmo_inventory:getNonStackableItems()
		elseif source == getResourceRootElement() then
			local inventory = getResourceFromName("cosmo_inventory")
		...
		if isElement(element) and not currentPedId and button == "right" and state == "up" then
			...
			if pedId then
				...
				if isElementWithinColShape(localPlayer, colshape) then
					...
					RobotoFont = dxCreateFont(":cosmo_assets/fonts/Roboto-Regular.ttf", respc(15), false, "antialiased")
					RobotoLighter = dxCreateFont(":cosmo_assets/fonts/Roboto-Light.ttf", respc(15), false, "antialiased")
...
function checkOwnership()
	...
	if tonumber(currentPedInterior) then
		if exports.cosmo_interiors:getInteriorOwner(currentPedInterior) == getElementData(localPlayer, "char.ID") or (getElementData(localPlayer, "acc.adminLevel") >= 6 and getElementData(localPlayer, "adminDuty")) then
			isOwner = true
		end
	end
...
function renderTheShop()
	...
	if promptDatas then
		...
		dxDrawRectangle(x, y, sx, sy, tocolor(0, 0, 0, 180))

		dxDrawText(promptDatas["theQuestion"], x, y + respc(20), x + sx, y + respc(20), tocolor(255, 255, 255, 230), 1, RobotoLighter, "center", "top", false, false, false, true)
	...
	elseif panelState == "setcategories" then
		...
		-- ** Háttér
		dxDrawRectangle(x, y, sx, sy, tocolor(0, 0, 0, 175))
		...
		dxDrawImage(math.floor(x + logoOffset), math.floor(y + logoOffset), logoSize, logoSize, ":cosmo_hud/files/logo.png", 0, 0, 0, tocolor(255, 255, 255, 230))
		dxDrawText("Bolt - Ped kategóriák kezelése", x + logoOffset * 2 + logoSize, y, 0, y + titleSize, tocolor(255, 255, 255, 230), 1, RobotoLighter, "left", "center")
		...
		if activeButton == "exit" then
			...
			dxDrawImageSection(math.floor(x + sx - respc(32) - logoOffset), math.floor(y + titleSize / 2 - respc(16)), respc(32), respc(32), 32, 0, 32, 32, "files/buttons.png", 0, 0, 0, tocolor(255, 255, 255, 230))
		end
		...
		dxDrawRectangle(x + respc(5), y, sx - respc(10), respc(2), tocolor(255, 255, 255, 230, 50))
		...
		for k = 1, maxVisible do
			...
			if v then
				dxDrawText(v, x2 + respc(15), y, x2 + sx2, y + sizeForRow, tocolor(255, 255, 255, 230), 1, RobotoFont, "left", "center")
				...
				if not currentCategories[k + currentOffset] then
					...
					--drawButton("addcat:" .. k + currentOffset, "Hozzáadás", x2 + sx2 - respc(160) - respc(10), y + respc(2), respc(160), sizeForRow - respc(4), 255, 148, 40, 1, RobotoLighter, 0.75, "", IconsFont, 0.75)
				else
		...
		if #mainCategories > maxVisible then
			...
			dxDrawRectangle(x + sx - respc(10), y2 + (totalSize / #mainCategories) * math.min(currentOffset, #mainCategories - maxVisible), respc(5), (totalSize / #mainCategories) * maxVisible, tocolor(255, 148, 40))
		end
		...
		if haveChanges then
			...
			if activeButton == "savecats" then
				dxDrawRectangle(x, y, sx, titleSize, tocolor(255, 148, 40, 175))
			else
				dxDrawRectangle(x, y, sx, titleSize, tocolor(255, 148, 40, 125))
			end

			dxDrawText("VÁLTOZTATÁSOK MENTÉSE", x, y, x + sx, y + titleSize, tocolor(255, 255, 255, 230), 1, RobotoLighter, "center", "center")
	...
	elseif panelState == "ownermode" then
		...
		if currentPedInterior > 0 then
			...
			dxDrawText(titleText, x + logoOffset * 2 + logoSize, y, 0, y + titleSize, tocolor(255, 255, 255, 230), 1, RobotoLighter, "left", "center")
		else
		...
		if isOwner then
			drawButton2("changeOwnerMode", "Vásárlói mód", buttonPosX, y + logoOffset, respc(230), logoSize, 255, 148, 40, 1, RobotoLighter, 1)
		...
		for k = 1, maxVisible do
			...
			if v then
				if v[1] == "category" then
					...
					if not v[3] then
						dxDrawImageSection(math.floor(x2), math.floor(y + sizeForRow / 2 - caretSize / 2), caretSize, caretSize, 0, 0, 32, 32, "files/buttons.png", 0, 0, 0, tocolor(255, 255, 255, 230))
					else
						dxDrawImageSection(math.floor(x2), math.floor(y + sizeForRow / 2 - caretSize / 2), caretSize, caretSize, 0, 0, 32, 32, "files/buttons.png", 90, 0, 0, tocolor(255, 255, 255, 230))
					end

					dxDrawText(v[2], x2 + caretSize, y, x2 + sx2, y + sizeForRow, tocolor(255, 255, 255, 230), 1, RobotoFont, "left", "center")
				...
				elseif v[1] == "noitem" and v[2] then
					...
					dxDrawImage(math.floor(x2 + respc(35)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":cosmo_inventory/files/items/" .. itemId .. ".png")
					dxDrawText(itemNames[v[2]], x2 + respc(45) + itemSize, y, x2 + sx2, y + sizeForRow, tocolor(255, 255, 255, 230), 0.8, RobotoFont, "left", "center")
				...
				elseif v[1] == "haveitem" and v[2] and currentItems[v[2]] then
					...
					if selectedActionItem == itemId then
						...
						drawButton2("applyitemaction", "OK", buttonPosX, buttonPosY, respc(50), buttonSizeY, 255, 148, 40, 1, RobotoLighter, 0.8)

						buttonPosX = buttonPosX - respc(5) - respc(65)

						if selectedItemAction == "addstock" then
							dxDrawText("Rendelés:", x2, y, x2 + (buttonPosX - x2 - respc(10)), y + sizeForRow, tocolor(255, 255, 255, 230, 255), 0.8, RobotoFont, "right", "center")
						...
						elseif selectedItemAction == "setprice" then
							dxDrawText("Beárazás:", x2, y, x2 + (buttonPosX - x2 - respc(10)), y + sizeForRow, tocolor(255, 255, 255, 230, 255), 0.8, RobotoFont, "right", "center")
						...
						if currentPedInterior > 0 then
							...
							drawButton2("addstock:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 255, 148, 40, 1, RobotoLighter, 0.8, "", IconsFont, 1)
						end

						buttonPosX = buttonPosX - respc(5) - respc(40)
						drawButton2("setprice:" .. k + currentOffset, false, buttonPosX, buttonPosY, respc(40), buttonSizeY, 255, 148, 40, 1, RobotoLighter, 0.8, "", IconsFont, 1)
		...
		if visibleCount > maxVisible then
			...
			dxDrawRectangle(x + sx - respc(10), y2, respc(5), totalSize, tocolor(0, 0, 0, 100))
			dxDrawRectangle(x + sx - respc(10), y2 + (totalSize / visibleCount) * math.min(currentOffset, visibleCount - maxVisible), respc(5), (totalSize / visibleCount) * maxVisible, tocolor(255, 148, 40))
		end
		...
		if currentPedInterior > 0 then
			...
			dxDrawText("Jelenlegi egyenleg: " .. formatNumber(currentBalance) .. " $", x + respc(10), y, 0, y + titleSize, tocolor(255, 255, 255, 230), 1, RobotoLighter, "left", "center")

			if selectedItemAction == "putInMoney" then
				local buttonPosX = x + sx - respc(10) - respc(140)

				drawButton2("putInMoneyOk", "Befizetés", buttonPosX, y, respc(140), titleSize, 255, 148, 40, 1, RobotoLighter, 1)
			...
			elseif selectedItemAction == "getOutMoney" then
				...
				buttonPosX = buttonPosX - respc(10) - respc(140)
				drawButton2("putInMoney", "Befizetés", buttonPosX, y, respc(140), titleSize, 255, 148, 40, 1, RobotoLighter, 1)
			end
		end
	-- vásárlói mód
	elseif panelState then
		...
		local logoSize = titleSize * 0.75
		local logoOffset = titleSize / 2 - logoSize / 2
		local BebasNeue = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", respc(17), false, "antialiased")
		local RobotoFont2 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 10, false, "antialiased")
		local FontAwesome = exports.cosmo_assets:loadFont("FontAwesome.ttf", 15, false, "antialiased")

		--dxDrawText("#ff9428Cosmo#ffffffMTA - Bolt", x + logoOffset * -4 + logoSize, y +1, 0, y + titleSize, tocolor(255, 255, 255, 230), 0.8, BebasNeue, "left", "center", false, false, false, true)

		local buttonPosX = x + logoOffset * 2 + logoSize + respc(10) + dxGetTextWidth("Bolt", 1, RobotoLighter)

		if isOwner then
			if currentPedInterior > 0 then
				drawButton2("changeOwnerMode", "Tulajdonosi mód", buttonPosX, y + logoOffset, respc(230), logoSize, 255, 148, 40, 1, RobotoLighter, 1)
			else
				drawButton2("changeOwnerMode", "Bolt kezelése", buttonPosX, y + logoOffset, respc(230), logoSize, 255, 148, 40, 1, RobotoLighter, 1)
			end
		...
		dxDrawText("A bolt kínálata", x + respc(5), y, x + respc(5) + holderSize, y + respc(40), tocolor(255, 255, 255, 230), 1, RobotoFont, "center", "center")

		dxDrawText("A kosarad tartalma", x + respc(10) + holderSize, y, x + respc(10) + holderSize * 2, y + respc(40), tocolor(255, 255, 255, 230), 1, RobotoFont, "center", "center")
		...
		for k = 1, maxVisible do
			...
			if k % 2 ~= currentOffset % 2 then
				dxDrawRectangle(x2, y, sx2, sizeForRow, tocolor(0, 0, 0, 150))
			else
				dxDrawRectangle(x2, y, sx2, sizeForRow, tocolor(0, 0, 0, 150))
			end
			...
			if v then
				...
				if activeButton == "addtocart:" .. k + currentOffset then
					dxDrawRectangle(x2, y, sx2, sizeForRow, tocolor(1, 1, 1))
				end

				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":cosmo_inventory/files/items/" .. itemId .. ".png")
				
				dxDrawText(itemNames[itemId], x2 + respc(10) + itemSize, y, x2 + sx2, y + sizeForRow, tocolor(255, 255, 255, 230), 0.6, BebasNeue, "left", "center")
				...
				if currentPedInterior > 0 and (not itemData[2] or itemData[2] == 0) then
					...
					dxDrawText("Ár: " .. formatNumber(itemData[1]) .. " $/db", x2, y, x2 + sx2 - respc(10), y + sizeForRow, tocolor(200, 200, 200, 200), 0.5, BebasNeue, "right", "center")
		...
		if #currentItemsEx > maxVisible then
			...
			dxDrawRectangle(x + respc(5) + holderSize - respc(5), y2 + (totalSize / #currentItemsEx) * math.min(currentOffset, #currentItemsEx - maxVisible), respc(5), (totalSize / #currentItemsEx) * maxVisible, tocolor(255, 148, 40))
		end
		...
		for k = 1, maxVisible do
			...
			if k % 2 ~= cartOffset % 2 then
				dxDrawRectangle(x2, y2, sx2, sizeForRow, tocolor(0, 0, 0, 150))
			else
				dxDrawRectangle(x2, y2, sx2, sizeForRow, tocolor(0, 0, 0, 150))
			end
			...
			if v then
				...
				if activeButton == "removefromcart:" .. k + cartOffset then
					dxDrawRectangle(x2, y2, sx2, sizeForRow, tocolor(0, 0, 0, 170))
				end

				dxDrawImage(math.floor(x2 + respc(5)), math.floor(y2 + sizeForRow / 2 - itemSize / 2), itemSize, itemSize, ":cosmo_inventory/files/items/" .. itemId .. ".png")
				
				dxDrawText(itemNames[itemId], x2 + respc(10) + itemSize, y2, x2 + sx2, y2 + sizeForRow, tocolor(255, 255, 255, 230), 0.6, BebasNeue, "left", "center")

				dxDrawText("Kosárban: " .. itemAmount .. " db | Ár: " .. formatNumber(itemAmount * currentItems[itemId][1]) .. " $", x2, y2, x2 + sx2 - respc(10), y2 + sizeForRow, tocolor(200, 200, 200, 200), 0.5, BebasNeue, "right", "center")
		...
		if #cartItems > maxVisible then
			...
			dxDrawRectangle(x + respc(15) + holderSize * 2 - respc(5), y3 + (totalSize / #cartItems) * math.min(cartOffset, #cartItems - maxVisible), respc(5), (totalSize / #cartItems) * maxVisible, tocolor(255, 148, 40))
		end

		if #cartItems > 0 then
			y3 = y3 + sizeForRow * maxVisible + respc(7)

			dxDrawRectangle(x, y3 - 6.5, sx, titleSize + respc(5), tocolor(0,0,0, 130))

			--dxDrawRectangle(x, y3, sx, 2, tocolor(255, 148, 40))
			...
			dxDrawText("Fizetendő összeg: #2ECC71" .. formatNumber(totalPrice) .. "$ ", x + respc(10), y3 - 20, 0, y3 + titleSize, tocolor(255, 255, 255, 230), 1, RobotoLighter, "left", "center", false, false, false, true)

			local buttonPosX = x + sx - respc(10) - respc(140)

			drawButton2("buyItems", "Fizetés", buttonPosX, y3 - 5, respc(140), titleSize - respc(5), 255, 148, 40, 1, RobotoLighter, 1)
		end
	end
				...
				if selectedActionItem then
					if selectedItemAction == "addstock" then
						...
						if amount and amount > 0 then
							showPrompt("Biztos vagy benne, hogy megrendeled a kiválasztott mennyiséget?\nTétel: #ff9428" .. itemNames[selectedActionItem] .. "\n#ffffffMennyiség: #ff9428" .. amount .. " db\n#ffffffTeljes ár: #ff9428" .. formatNumber(amount * itemBasePrices[selectedActionItem]) .. " $",
								function()
		...
		elseif selected[1] == "buyItems" then
			if #cartItems > 0 then
				...
				for k = 1, #cartItems do
					...
					totalWeight = totalWeight + amount * exports.cosmo_inventory:getItemWeight(itemId)
				...
				if exports.cosmo_core:getMoney(localPlayer) >= totalPrice then
					if exports.cosmo_inventory:countEmptySlots() >= totalSlot then
						if exports.cosmo_inventory:getCurrentWeight() + totalWeight <= exports.cosmo_inventory:getWeightLimit("player", localPlayer) then
							showPrompt("Biztosan megvásárolod a kosárban lévő összes terméket?\nTeljes összeg: #ff9428" .. formatNumber(totalPrice) .. " $",
								function()
									triggerServerEvent("buyItemsFromPed", localPlayer, currentPedId, cartItems, totalPrice, totalSlot, totalWeight)
									unloadShop()
								end)
						else
							exports.cosmo_hud:showInfobox("error", "Nem bírsz el ennyi tárgyat!")
						end
					else
						exports.cosmo_hud:showInfobox("error", "Nincs elég hely az inventorydban!")
					end
				else
					exports.cosmo_hud:showInfobox("error", "Nincs nálad elegendő pénz!")
				end
			end
		end
	end
