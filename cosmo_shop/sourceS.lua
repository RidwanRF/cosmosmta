	...
	function(pedId, items, totalPrice, totalSlot, totalWeight)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentMoney - totalPrice >= 0 then
				exports.cosmo_core:takeMoney(source, totalPrice, "shopBuyItems")
				...
				for k = 1, #items do
					...
					if nonStackableItems[itemId] then
						for i = 1, amount do
							exports.cosmo_inventory:addItem(source, itemId, 1)
						end
					else
						exports.cosmo_inventory:addItem(source, itemId, amount)
					end
				end

				exports.cosmo_hud:showInfobox(source, "success", "Sikeres megvásároltad a kiválasztott termékeket!")
	...
	function(pedId, amount)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentBalance - amount >= 0 then
				exports.cosmo_core:giveMoney(source, amount, "shopGetOutMoney")
				...
				dbQuery(
					function(qh, player)
						if isElement(player) then
							...
							exports.cosmo_hud:showInfobox(player, "success", "Sikeresen kifizettél a kasszából " .. amount .. " $-t.")
						end

						dbFree(qh)
					end, {source}, connection, "UPDATE shop_peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
			else
				exports.cosmo_hud:showInfobox(source, "error", "Nincs a kasszában ennyi pénz!")
			end
		end
	end)
	...
	function(pedId, amount)
		if isElement(source) and pedId and pedsTable[pedId] then
			...
			if currentMoney - amount >= 0 then
				...
				exports.cosmo_core:takeMoney(source, amount, "shopPutInMoney")
				...
				dbQuery(
					function(qh, player)
						if isElement(player) then
							...
							exports.cosmo_hud:showInfobox(player, "success", "Sikeresen befizettél a kasszába " .. amount .. " $-t.")
						end

						dbFree(qh)
					end, {source}, connection, "UPDATE shop_peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
			else
				exports.cosmo_hud:showInfobox(source, "error", "Nincs nálad ennyi pénz!")
			end
		end
	end)
		...
		if isElement(source) and pedId and pedsTable[pedId] then
			if pedsTable[pedId]["items"] and pedsTable[pedId]["items"][itemId] then
				...
				if currentBalance >= 0 then
					...
					exports.cosmo_core:takeMoney(source, amountPrice, "shopStock")
					...
					dbQuery(
						function(qh, player)
							if isElement(player) then
								...
								exports.cosmo_hud:showInfobox(player, "success", "Sikeresen megrendeltél a kiválasztott tételből " .. amount .. " darabot " .. amountPrice .. " $ értékben.")
							end

							dbFree(qh)
						end, {source}, connection, "UPDATE shop_peds SET balance = ? WHERE pedId = ?", currentBalance, pedId)
				else
					exports.cosmo_hud:showInfobox(source, "error", "Nincs elegendő pénz a kasszában! (" .. amountPrice .. " $)")
				end
			end
		end
	end)
...
exports.cosmo_admin:addAdminCommand("moveshopped", 6, "Bolt NPC átmozgatása")
addCommandHandler("moveshopped",
	function(playerSource, commandName, pedId, skin)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not pedId then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [Ped ID] [ < Skin ID > ]", playerSource, 0, 0, 0, true)
			else
				...
				if pedsTable[pedId] then
					...
					outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott ped nem létezik!", playerSource, 0, 0, 0, true)
				end
			end
		end
	end)

exports.cosmo_admin:addAdminCommand("delshopped", 6, "Bolt NPC törlése")
addCommandHandler("delshopped",
	function(playerSource, commandName, pedId)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not pedId then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [Ped ID]", playerSource, 0, 0, 0, true)
			else
				pedId = math.floor(math.abs(pedId))
					...
					dbQuery(
						function(qh, sourcePlayer, id)
							...
							if isElement(sourcePlayer) then
								outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Sikeresen törölted a kiválasztott pedet.", playerSource, 0, 0, 0, true)
							end
...
exports.cosmo_admin:addAdminCommand("createshopped", 6, "Bolt NPC létrehozása")
addCommandHandler("createshopped",
	function(playerSource, commandName, skin)
		if getElementData(playerSource, "acc.adminLevel") >= 6 then
			...
			if not skin then
				outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [Skin ID]", playerSource, 0, 0, 0, true)
				outputChatBox(exports.cosmo_core:getServerTag("info") .. "#ffff99A ped létrehozása után be kell állítanod a ped item kategóriáit.", playerSource, 0, 0, 0, true)
			else
				local x, y, z = getElementPosition(playerSource)
				local rx, ry, rz = getElementRotation(playerSource)
				local interior = getElementInterior(playerSource)
				...
				dbQuery(
					function(qh, sourcePlayer)
						...
						if result then
							...
							if isElement(sourcePlayer) then
								outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Sikeresen létrehoztál egy Bolt NPC-t. #32b3ef(PedId: " .. result["pedId"] .. ")", playerSource, 0, 0, 0, true)
							end
						end
