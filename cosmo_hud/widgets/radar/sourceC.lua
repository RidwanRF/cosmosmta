...
local show3dBlips = true

local screenSource = dxCreateScreenSource(screenX, screenY)

local mapPicture = dxCreateTexture("widgets/radar/files/map.png")
dxSetTextureEdge(mapPicture, "border", tocolor(110, 158, 206))

Raleway = dxCreateFont(":cosmo_assets/fonts/Raleway.ttf", respc(35))
Raleway2 = dxCreateFont(":cosmo_assets/fonts/Raleway.ttf", respc(12))
Raleway3 = dxCreateFont(":cosmo_assets/fonts/Raleway.ttf", respc(15))
local sotetites = dxCreateTexture("widgets/radar/files/sotet.png")
...
local gpsRoutePos = {}
local gpsLineColor = tocolor(255, 148, 40)
...
render.minimap = function (x, y)
	...
	panelWidth, panelHeight = widgets.minimap.sizeX, widgets.minimap.sizeY
	...
	if playerDimension == 0 and playerPosZ <= 10000 then
		...
		-- ** Keret
		dxDrawRectangle(x - 2, y - 2, panelWidth + 4, 2, tocolor(33, 33, 33, 200)) -- felső
		dxDrawRectangle(x - 2, y + panelHeight, panelWidth + 4, 2, tocolor(33, 33, 33, 200)) -- alsó
		dxDrawRectangle(x - 2, y, 2, panelHeight, tocolor(33, 33, 33, 200)) -- bal
		dxDrawRectangle(x + panelWidth, y, 2, panelHeight, tocolor(33, 33, 33, 200)) -- jobb

		-- ** GPS Location
		dxDrawRectangle(x, y + panelHeight - resp(30), panelWidth, resp(30), tocolor(22, 22, 22, 200))
		dxDrawText(getZoneName(playerPosX, playerPosY, playerPosZ), x, y + panelHeight - resp(30), x + panelWidth - resp(10), y + panelHeight, tocolor(255, 255, 255, 230), 0.5, Raleway, "center", "center",false,false,false,true)
		dxDrawImage(x,y,panelWidth,panelHeight,sotetites,0,0,0,tocolor(30,30,30,255))
        ...
        if show3dBlips then
            if getElementDimension(localPlayer) == 0 then
                ...
                for k = 1, #blips do
                    ...
                    if v then
                        if getElementData(v, "blipFarShow") then
                            local bx, by, bz = getElementPosition(v)
                            local screenX, screenY = getScreenFromWorldPosition(bx, by, bz + 2)

                            if screenX and screenY then
                                local px, py, pz = getElementPosition(localPlayer)

                                local distanceBetweenBlip = getDistanceBetweenPoints3D(px, py, pz, bx, by, bz)
                                local blipHalfSize = 14.5 / 2

                                dxDrawText(math.floor(distanceBetweenBlip) .. " m\n".. getElementData(v, "blipTooltipText"), screenX + 1, screenY + 1 + blipHalfSize + respc(4), screenX, 0, tocolor(0, 0, 0, 255), 0.5, Raleway3, "center", "top")
                                dxDrawText(math.floor(distanceBetweenBlip) .. " m\n".. getElementData(v, "blipTooltipText"), screenX, screenY + blipHalfSize + respc(4), screenX, 0, 0xFFFFFFFF, 0.5, Raleway3, "center", "top", false, false, false, true)
                                dxDrawImage(screenX - blipHalfSize, screenY - blipHalfSize, 14.5, 14.5, "widgets/radar/files/blips/"..getElementData(v, "blipIcon")..".png", 0, 0, 0, getElementData(v, "blipColor"))
                            end
                        end
                    end
                end
...
function drawLostSignal(x, y, sx, sy)
	...
	dxDrawImage(math.floor(x + (sx - respc(96)) / 2), math.floor(y + (sy - respc(128)) / 2), respc(96), respc(96), "widgets/radar/files/loader.png", 0, 0, 0, tocolor(184, 53, 53, 110 * alpha))

	dxDrawText("A kapcsolat megszakadt...", x, y + 150, x + sx, y + sy, tocolor(184, 53, 53, 110 * alpha), 0.7, RobotoL18, "center", "center")
end
...
function drawNavi(x, y, sx, sy)
	if gpsRoute or (not gpsRoute and waypointEndInterpolation) then
		...
		if waypointEndInterpolation then
			...
			dxDrawImage(x2, y + (resp(50) - iconSize) / 2, iconSize, iconSize, "widgets/radar/files/gps/end.png", angle, 0, 0, tocolor(255, 148, 40, 255 * alpha))
			dxDrawText("0 m", x2 + resp(8) + iconSize, y, 0, y + resp(50), tocolor(255, 148, 40, 255 * alpha), 0.75, Roboto18, "left", "center")
		...
		elseif nextWp then
			...
			if tonumber(reRouting) then
				...
				dxDrawImage(x2, y + (resp(50) - iconSize) / 2, iconSize, iconSize, "widgets/radar/files/gps/refresh.png", angle, 0, 0, tocolor(255, 148, 40, 255))
				dxDrawText("Újratervezés" .. dots, x2 + resp(8) + iconSize, y, 0, y + resp(50), tocolor(255, 148, 40, 255), 0.75, Roboto18, "left", "center")
			...
			elseif turnAround then
				...
				dxDrawImage(x2, y + (resp(50) - iconSize) / 2, iconSize, iconSize, "widgets/radar/files/gps/around.png", 0, 0, 0, tocolor(255, 148, 40, 255))
				dxDrawText("Fordulj vissza!", x2 + resp(8) + iconSize, y, 0, y + resp(50), tocolor(255, 148, 40, 255), 0.75, Roboto18, "left", "center")
			elseif not waypointInterpolation then
				...
				dxDrawImage(x2, y + (resp(50) - iconSize) / 2, iconSize, iconSize, "widgets/radar/files/gps/" .. gpsWaypoints[nextWp][2] .. ".png", 0, 0, 0, tocolor(255, 148, 40, 255))
				dxDrawText(dist, x2 + resp(8) + iconSize, y, 0, y + resp(50), tocolor(255, 148, 40, 255), 0.75, Roboto18, "left", "center")
				...
				dxDrawImage(x + currentX, y + (resp(50) - iconSize) / 2, iconSize, iconSize, "widgets/radar/files/gps/" .. gpsWaypoints[wp][2] .. ".png", 0, 0, 0, tocolor(255, 148, 40, alpha))
				dxDrawText(currentDist, x + currentX + resp(8) + iconSize, y, 0, y + resp(50), tocolor(255, 148, 40, alpha), 0.75, Roboto18, "left", "center")
...
local teleportTimer = false

function teleportToPos(x, y)
	if isTimer(teleportTimer) then
		killTimer(teleportTimer)
	end

	teleportTimer = nil

	if isElementWaitingForGroundToLoad(localPlayer) then
		teleportTimer = setTimer(teleportToPos, 100, 0, x, y)
	else
		local _, _, _, z = processLineOfSight(x, y, 3000, x, y, -3000)
		...
		z = z + getElementDistanceFromCentreOfMassToBaseOfModel(localPlayer)
...
function onBigmapClick(button, state)
	if button == "left" and state == "up" then
        ...
        elseif activeButton == "btn:show3DBlips" then
            show3dBlips = not show3dBlips
        elseif activeButton == "btn:cringeSpaceButton" then
        	mapMoveDiff = false
	...
	if button == "left" and state == "up" then
		...
		if isElement(occupiedVehicle) then
			...
			if getKeyState("lctrl") and renderData.adminLevel >= 6 and getElementDimension(localPlayer) == 0 then
				if isPedInVehicle(localPlayer) then
					exports.cosmo_hud:showInfobox("error", "Járművel nem lehet teleportálni!")
					return
				end

				local hit, _, _, z = processLineOfSight(x, y, 3000, x, y, -3000)
				...
				if not hit then
					if isTimer(teleportTimer) then
						exports.cosmo_hud:showInfobox("error", "Várj egy kevés időt!")
						return	
					end

					setCameraMatrix(x, y, z)
					teleportTimer = setTimer(teleportToPos, 50, 1, x, y)
				else
					setElementPosition(localPlayer, x, y, z + getElementDistanceFromCentreOfMassToBaseOfModel(localPlayer))
				end
			end
		end
	end
...
function renderTheBigmap()
	...
	if playerDimension == 0 and playerPosZ <= 10000 then
		...
		-- ** GPS Lokátor
		dxDrawImage(panelPosX,panelPosY,panelWidth,panelHeight,sotetites,0,0,0,tocolor(30,30,30,255))
		dxDrawImage(panelPosX + 12, panelPosY + 12, respc(70) - 24, respc(70) - 24, ":cosmo_assets/images/map/location.png")
		dxDrawText(getZoneName(playerPosX, playerPosY, playerPosZ), panelPosX + 12 + respc(70 - 12), panelPosY, panelPosX + panelWidth, panelPosY + respc(70), tocolor(255, 255, 255), 1, Raleway, "left", "center")
				...
				if isElement(activeBlip) then
					if getElementType(activeBlip) == "player" then
						if getElementData(activeBlip, "adminDuty") then
							text = "[" .. exports.cosmo_core:getPlayerAdminTitle(activeBlip) .. "] " .. getElementData(activeBlip, "visibleName"):gsub("_", " ") .. " (" .. getElementData(activeBlip, "playerID") .. ")"
						else
		...
		-- if renderData.adminLevel >= 3 then
		-- 	if showPlayers then
		-- 		drawButton("showPlayers", "Játékosok mutatása: ki", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 50 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		-- 	else
		-- 		drawButton("showPlayers", "Játékosok mutatása: be", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 50 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		-- 	end
		-- end

		-- if mapMoveDiff then
		-- 	drawButton("cringeSpaceButton", "Nézet alaphelyzetbe állítása", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 100 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		-- end
		...
		if mapMoveDiff then
			drawButton("cringeSpaceButton", "Nézet alaphelyzetbe állítása", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 150 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		end

		if show3dBlips then
			drawButton("show3DBlips", "3D Blippek mutatása: ki", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 100 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		else
			drawButton("show3DBlips", "3D Blippek mutatása: be", panelPosX + 60 + panelWidth - respc(12) - respc(320), panelPosY - 100 + panelHeight - respc(70 - 17.5), respc(220), respc(35), 200, 50, 50, 1, Raleway2, 0.75)
		end
