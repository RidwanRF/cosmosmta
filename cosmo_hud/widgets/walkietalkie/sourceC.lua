...
function setWalkieTalkieData(dbID, data1, data2)
	local channel = data1

	checkForGroups()

	activeRadioItemID = dbID

	tempChannel = data1 and tonumber(data1) or 0

	lastChannel = tempChannel

	radioChannelsMuted = data2
				...
				if getTickCount() >= antiFloodTick then
					...
					if activeRadioItemID then
						if protectedChannels[lastChannel] then
							exports.cosmo_hud:showInfobox("error", "A kiválasztott frekvencia védett!")
						else
