...
oocColorState = false
setElementData(localPlayer, "oocColorState", oocColorState)

addCommandHandler("togooccolor", 
	function()
		if getElementData(localPlayer, "acc.adminLevel") >= 6 then
			...
			if oocColorState then 
				outputChatBox(exports.cosmo_core:getServerTag("high") .. "OOC Chat szín bekapcsolva!", 255, 255, 255, true)
			else 
				outputChatBox(exports.cosmo_core:getServerTag("high") .. "OOC Chat szín kikapcsolva!", 255, 255, 255, true)
			end 
		end 
	end 
)
...
addEventHandler("onClientRecieveOOCMessage", getRootElement(),
	function (message, playerName)
		...
		local oocColorState = getElementData(source, "oocColorState")
		...
		if not adminDuty and not oocColorState then
			...
			color = exports.cosmo_core:getAdminLevelColor(adminLevel)
