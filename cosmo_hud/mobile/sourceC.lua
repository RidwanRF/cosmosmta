...
addEventHandler("onClientResourceStart", getRootElement(),
	function (res)
		if res == getThisResource() then
			local cosmo_weather = getResourceFromName("cosmo_weather")

			if cosmo_weather and getResourceState(cosmo_weather) == "running" then
				weather = getElementData(getResourceRootElement(cosmo_weather), "serverWeather")
			else
				weather = {1, 1}
			end
		else
