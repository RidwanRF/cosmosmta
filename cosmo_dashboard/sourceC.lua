...
local selectedVeh = 1
local offsetVeh = 0
local tuningName = {"#ff9428gyári", "#ff9428alap", "#ff9428profi", "#ff9428verseny", "#ff9428chip"}
tuningName[0] = "#ff9428gyári"
...
local offsetRank = 0
local groupButtonCaptions2 = {"Átnevezés", "Fizetés módosítása", "Rang létrehozása"}

local memberFirePrompt = false
local fireErrorText = ""

local memberrankUp = false
local rankUpErrorText = ""

local memberrankDown = false
local rankDownErrorText = ""
...
function fetchGroups()
	...
	for k, v in ipairs(getElementsByType("vehicle")) do
		...
		if groupVehicles[groupId] then
			...
			vehicleDatas[v].vehicleName = exports.cosmo_mods_veh:getVehicleNameFromModel(getElementModel(v))
		end
	end
end
...
function saystyleadd()
	setElementData(localPlayer, "sayAnim", true)
end
addCommandHandler("sadd", saystyleadd)
		...
		if getElementData(localPlayer, "loggedIn") then
			for k, v in pairs(renderData.loadedSettings) do
				...
				elseif string.find(k, "shader") then
					if v then
						exports.cosmo_shader:setActiveShader(k:gsub("shader_", ""), v)
					else
						exports.cosmo_shader:setActiveShader(k:gsub("shader_", ""), false)
					end
				end
			end
		end
...
function togglePanel()
	...
	if getTickCount() - renderData.openedTime <= 2000 and not panelState then
		outputChatBox("#ff9428[CosmoMTA]: #FFFFFFMaximum 2 másodpercenként nyithatod meg a dashboard-ot.", 0, 0, 0, true)
	...
	if panelState then
		...
		playerVehicles = {}
		playerInteriors = exports.cosmo_interiors:requestInteriors(localPlayer)
		...
		for k, veh in ipairs(getElementsByType("vehicle")) do
			if getElementData(veh, "vehicle.owner") == myDatas["char.ID"] then
				...
				vehicleDatas[veh].vehicleName = exports.cosmo_mods_veh:getVehicleNameFromModel(getElementModel(veh))
			end
		end

		jobNames = exports.cosmo_jobhandler:getJobNames()
		groupTypes = exports.cosmo_groups:getGroupTypes()

		for k, v in pairs(playerInteriors) do
			v.data.iconPicture = exports.cosmo_interiors:getIconByMarkerType(v.data.type)
		end
		...
		memberrankDown = false
		rankDownErrorText = ""


		renderData.clientSettings = {}
		renderData.loadedSettings.crosshairData = getElementData(localPlayer, "crosshairData") or {0, 200,200,200}

		table.insert(renderData.clientSettings, {"Célkereszt", renderData.loadedSettings.crosshairData[1] or 0, "crosshair", "crosshair"})
		...
		--table.insert(renderData.clientSettings, {"Chat típus", "nincs anyád nem lesz kurva anyád"--[[exports.cosmo_hud:getChatType(), "chatType"]], true})
		--table.insert(renderData.clientSettings, {"Custom chat háttér", "remap a kurva anyádat"--[[reMap(exports.cosmo_hud:getChatBackgroundAlpha(), 0, 255, 0, 100)]], "chatBackgroundAlpha", {0, 100, "%"}})
		--table.insert(renderData.clientSettings, {"Custom chat árnyék", "anyád", {0, 100, "%"}})
		--table.insert(renderData.clientSettings, {"Custom chat betűméret", "anyád", {50, 135, "%"}})
		table.insert(renderData.clientSettings, {"Beszéd animáció", renderData.sayStyleEx[getElementData(localPlayer, "sayAnim")] or 1, "sayStyle", true})

		renderData.chatInstance = "Nem jó valami!"--[[exports.cosmo_hud:getChatInstance()]]
		...
		Roboto = dxCreateFont("files/fonts/Roboto.ttf", respc(16.5), false, "antialiased")
		RobotoL = dxCreateFont("files/fonts/RobotoL.ttf", respc(16.5), false, "antialiased")
		RobotoB = dxCreateFont("files/fonts/RobotoB.ttf", respc(25), false, "antialiased")
		RobotoB2 = dxCreateFont("files/fonts/RobotoB.ttf", respc(16.5), false, "antialiased")

		screenSource = dxCreateScreenSource(screenX, screenY)
		--screenShader = dxCreateShader(":cosmo_hud/files/blackwhite.fx")
		--dxSetShaderValue(screenShader, "screenSource", screenSource)

		serverLogo = ":cosmo_accounts/files/logo.png"
		...
		--exports.cosmo_hud:setChatBackgroundAlpha(reMap(renderData.clientSettings[14][2], 0, 100, 0, 255))
		--exports.cosmo_hud:setChatFontBackgroundAlpha(reMap(renderData.clientSettings[15][2], 0, 100, 0, 255))
		--exports.cosmo_hud:setChatFontSize(renderData.clientSettings[16][2])
		--if renderData.chatInstance then
...
function getAdmins()
	...
	table.insert(renderData.adminSlots, {"title", "Adminsegédek"})
	...
	if #gotAdmins[1] < 1 then
		...
		for k, v in ipairs(gotAdmins[1]) do
			if getElementData(v, "adminDuty") then
				table.insert(renderData.adminSlots, {"admin", exports.cosmo_core:getAdminLevelColor(1) .. getElementData(v, "acc.adminNick") .. "#ffffff (" .. getElementData(v, "playerID") .. ")", true})
			else
				table.insert(renderData.adminSlots, {"admin", exports.cosmo_core:getAdminLevelColor(1) .. getElementData(v, "acc.adminNick"), false})
			end
		end
	end
	...
	if #gotAdmins[2] < 1 then
		...
		for k, v in ipairs(gotAdmins[2]) do
			if getElementData(v, "adminDuty") then
				table.insert(renderData.adminSlots, {"admin", "#ffffff" .. getElementData(v, "acc.adminNick") .. "#ffffff (" .. getElementData(v, "playerID") .. ")", true})
			else
				table.insert(renderData.adminSlots, {"admin", "#ffffff" .. getElementData(v, "acc.adminNick"), false})
			end
		end
	end
...
function handleInputs(character)
	...
	elseif activeFakeInput == "createrank" then
		if utf8.len(fakeInputText) <= 20 and not string.find(character, ",") then
			fakeInputText = fakeInputText .. character
		end
		...
		if hitPlayer == localPlayer and matchingDimension then
			for i = 1, #locatorElements do
				...
				if data and source == data[2] then
					...
					exports.cosmo_hud:showInfobox("success", "Sikeresen megérkeztél a járművedhez!")
					break
				end
			end
		end
...
local bincoColShape = createColSphere(207.59057617188, -129.69436645508, 1003.5078125, 4)
setElementInterior(bincoColShape, 3)
setElementDimension(bincoColShape, 219)
...
local bankCols = {
	createColSphere(582.96856689453, -1534.3472900391, 16.39599609375, 10)
}
...
addEventHandler("gpsLocalizeCar", getRootElement(),
	function ()
		...
		setElementData(blip, "blipColor", tocolor(255, 125, 0))
...
function onClickHandler(button, state)
		...
		if activeButton then
			if string.find(activeButton, "selectVehicle") then
				...
				if isElement(playerVehicles[findVehicleId]) then
					...
					exports.cosmo_hud:showInfobox("success", "Járműved lokalizálva! Keresd a NARANCSSÁRGA jelzésnél.")
				end
			end
		end
	end
	...
	if button == "left" then
		if state == "down" then
			...
			elseif activeButtonC == "rankUpp" then
				local player = loaded_company["membersCount"][selected]
				local playerRankNum = tonumber((player.rank))
				local online_player = findPlayerByName(player.name)
		...
		if activeButton then
			...
			if state == "down" then
				...
				if activeButton == "sell_vehicle" then
					exports.cosmo_hud:showInfobox("error", "Jelenleg nem elérhető.")
					-- local veh = playerVehicles[selectedVeh]

					-- if (vehicleDatas[veh]["vehicle.group"] or 0) == 0 then
					-- 	exports.cosmo_tradecontract:sellingVehicle(veh, vehicleDatas[veh])
					-- end
				end
				...
				elseif activeButton == "buySelectedPet" then
					if not renderData.buyDogProcessing then
						if utf8.len((renderData.dogName or "")) > 0 then
							...
							exports.cosmo_hud:showInfobox("error", "Név nélkül nem vehetsz kutyát!")
						end
					end
				end
				...
				if activeButton == "spawnAnimal" then
					if getTickCount() - (renderData.lastSpawnAnimal or 0) >= 5000 then
						...
						if animal and animal.animalId then
							if 0 < renderData.spawnedAnimal then
								if renderData.spawnedAnimal == animal.animalId then
									...
									exports.cosmo_hud:showInfobox("error", "Másik állat lespawnolása előtt, először despawnold az aktívat!")
								end
							else
						...
						exports.cosmo_hud:showInfobox("error", "5 másodpercenként csak egyszer használhatod ezt a gombot.")
					end
				end
					...
					if animal then
						if utfLen(renderData.newDogName) > 0 then
							if renderData.newDogName ~= animal.name then
								...
								exports.cosmo_hud:showInfobox("error", "Nem lehet ugyan az a név, mint volt.")
							end
						else
							exports.cosmo_hud:showInfobox("error", "Nem lehet üres a pet neve.")
						end
					end
				end
				...
				elseif activeButton == "buyReviveAnimal" then
					...
					if animal.health < 1 then
						...
						exports.cosmo_hud:showInfobox("error", "Ez a kutya nem halott.")
					end
				end
					...
					if buttonDetails[1] == "setting_toggle" then
						if string.find(buttonDetails[2], "shader") then
							if renderData.clientSettings[id] then
								...
								exports.cosmo_shader:setActiveShader(string.gsub(buttonDetails[2], "shader_", ""), renderData.clientSettings[id][2])
								playSound("files/sounds/switchoption.mp3")
							end
						end
					end
					...
					if buttonDetails[1] == "setting_next" then
						if string.find(buttonDetails[2], "shader") then
							if renderData.clientSettings[id] then
								...
								exports.cosmo_shader:setActiveShader(shader, renderData.clientSettings[id][2])
							...
							elseif buttonDetails[2] == "sayStyle" then
								local nextId = (renderData.sayStyleEx[getElementData(localPlayer, "sayAnim")] or 1) + 1

								if renderData.sayStyle[nextId] then
									renderData.clientSettings[id][2] = nextId
									--triggerServerEvent("sayAnimServer", localPlayer, renderData.sayStyle[nextId])
									setElementData(localPlayer, "sayStyle", renderData.sayStyle[nextId])
									setElementData(localPlayer, "sayAnim", renderData.sayStyle[nextId])
									playSound("files/sounds/menuswitch2.mp3")
								end
							elseif buttonDetails[2] == "chatType" then
								...
								exports.cosmo_hud:setChatType(chatType)

								playSound("files/sounds/menuswitch2.mp3")
							elseif buttonDetails[2] == "crosshair" then
								local currentShape = renderData.loadedSettings.crosshairData[1] or 0
								...
								renderData.loadedSettings.crosshairData[1] = currentShape

								setElementData(localPlayer, "crosshairData", renderData.loadedSettings.crosshairData, false)
							end
						end
					end
				...
				if buttonDetails[1] == "crosshair_color" then
					local r, g, b = renderData.loadedSettings.crosshairData[2], renderData.loadedSettings.crosshairData[3], renderData.loadedSettings.crosshairData[4]
					...
					renderData.loadedSettings.crosshairData[2] = r
					renderData.loadedSettings.crosshairData[3] = g
					renderData.loadedSettings.crosshairData[4] = b
					setElementData(localPlayer, "crosshairData", renderData.loadedSettings.crosshairData, false)
			...
			elseif state == "up" then
					...
					if groupId and selectedRank then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							if string.len(fakeInputText) < 1 then
								...
								triggerServerEvent("renameGroupRank", localPlayer, groupId, selectedRank, fakeInputText)
								playSound("files/sounds/promptaccept.mp3")
							end
						end
					end
				...
				if activeButton == "createrank" then
					local groupId = playerGroupsKeyed[selectedGroup]
					...
					if groupId and selectedRank then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							if string.len(fakeInputText) < 1 then
								...
								fakeInputError = "#7cc576Sikeresen létrehozva!"
								--triggerServerEvent("renameGroupRank", localPlayer, groupId, selectedRank, fakeInputText)
								triggerServerEvent("addNewGroupRank", localPlayer, groupId)
								playSound("files/sounds/promptaccept.mp3")
							end
						end
					end
					...
					if groupId and selectedRank then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							...
							elseif tonumber(fakeInputText) < 0 then
								...
								playSound("files/sounds/promptdecline.mp3")
							elseif tonumber(fakeInputText) > 1000000 then
								fakeInputError = "#d75959Sok lesz ez!"
								playSound("files/sounds/promptdecline.mp3")
							else
								fakeInputError = "#7cc576Sikeresen átállítva!"
								triggerServerEvent("setGroupRankPayment", localPlayer, groupId, selectedRank, math.floor(tonumber(fakeInputText)))
								playSound("files/sounds/promptaccept.mp3")
							end
						end
					end
				...
				elseif activeButton == "setgroupdesc" then
					...
					if groupId and selectedRank then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							...
							triggerServerEvent("rewriteGroupDescription", localPlayer, groupId, fakeInputText)
					...
					if groupId and selectedRank then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							...
							elseif tonumber(fakeInputText) < 1000 then
								...
								triggerServerEvent("setGroupBalance", localPlayer, groupId, fakeInputText)
								playSound("files/sounds/promptaccept.mp3")
							end
				...
				if buttonDetails[1] == "groupaction" then
					...
					if groupId then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							if actionNum == 1 then
								memberrankUp = true
								fireErrorText = "#d75959Biztosan előlépteted?"
								playSound("files/sounds/switchoption.mp3")
							end

							if actionNum == 2 then
								memberrankDown = true
								fireErrorText = "#d75959Biztosan lefokozod?"
								playSound("files/sounds/switchoption.mp3")
							end
				...
				if activeButton == "cancelrankUp" then
					rankUpErrorText = ""
					memberrankUp = false
					playSound("files/sounds/switchoption.mp3")
				end

				if activeButton == "cancelrankDown" then
					rankDownErrorText = ""
					memberrankDown = false
					playSound("files/sounds/switchoption.mp3")
				end
				...
				if activeButton == "fireMember" then
					...
					if groupId then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							triggerServerEvent("removePlayerFromGroup", localPlayer, groupId, playerGroups, member.charID, member.online)
							playSound("files/sounds/promptaccept.mp3")
						end
					end
				end

				if activeButton == "rankUp" then
					local groupId = playerGroupsKeyed[selectedGroup]
					...
					if groupId then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							--triggerServerEvent("removePlayerFromGroup", localPlayer, groupId, playerGroups, member.charID, member.online)
							triggerServerEvent("modifyGroupMemberRank", localPlayer, groupId, playerGroups, "up", member.charID, member.online, member.rank)
							playSound("files/sounds/promptaccept.mp3")
						end
					end
				end

				if activeButton == "rankDown" then
					local groupId = playerGroupsKeyed[selectedGroup]
					...
					if groupId then
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							--triggerServerEvent("removePlayerFromGroup", localPlayer, groupId, playerGroups, member.charID, member.online)
							triggerServerEvent("modifyGroupMemberRank", localPlayer, groupId, playerGroups, "down", member.charID, member.online, member.rank)
							playSound("files/sounds/promptaccept.mp3")
						end
					end
				end
						...
						if member and meInGroup[groupId].isLeader == "Y" then
							if string.len(fakeInputText) < 1 then
								...
								elseif isElement(found) then
									...
									triggerServerEvent("invitePlayerToGroup", localPlayer, groupId, playerGroups, getElementData(found, "char.ID"), found)
									playSound("files/sounds/promptaccept.mp3")
								else
...
function drawDataRow(startX, startY, h, caption, data, scale)
	...
	dxDrawText(caption, startX, startY, startX + w1, startY + h, rgba(255,255,255,230), scale or 1, RobotoB2, "center", "center", false, false, false, true)
	dxDrawText(data, startX + w1, startY, startX + w1 + w2, startY + h, rgba(255, 148, 40), scale or 1, Roboto, "center", "center", false, false, false, true)
...
function onRenderHandler()
	...
	if renderData.renamingPet then
		...
		-- ** Háttér
		dxDrawRectangle(x, y, sx, sy, rgba(0, 0, 0, 150))


		local y2 = y + respc(30)
		local formattedText = renderData.newDogName or ""

		dxDrawText("Név:", x + respc(15), y2, 0, y2 + respc(40), rgba(255,255,255,230), 0.5, RobotoB, "left", "center")

		dxDrawRectangle(x + respc(65), y2, sx - respc(80), respc(40), rgba(0, 0, 0, 150))

		dxDrawText(formattedText, x + respc(70), y2, 0, y2 + respc(40), rgba(255,255,255,230), 0.75, Roboto, "left", "center")
		...
		drawButton3("cancelPetRename", "Mégsem", x + respc(15), y2, buttonWidth, respc(40), {215, 89, 89}, false, Roboto)

		-- Vásárlás
		buttons["renameSelectedPet"] = {x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40)}
		drawButton3("renameSelectedPet", "Átnevezés", x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40), {255, 148, 40}, false, Roboto)
	...
	if renderData.buyingPetRevive then
		...
		-- ** Content
		dxDrawText("Tényleg szeretnéd feléleszteni a peted?\nA felélesztés ára #ff9428100 #ffffffPP", x, y, x + sx, y + sy - respc(70), rgba(255,255,255,230), 0.75, RobotoL, "center", "center", false, false, false, true)
		...
		drawButton3("cancelReviveAnimal", "Mégsem", x + respc(15), y2, buttonWidth, respc(40), {215, 89, 89}, false, Roboto)

		-- Vásárlás
		buttons["buyReviveAnimal"] = {x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40)}

		drawButton3("buyReviveAnimal", "Igen", x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40), {255, 148, 40}, false, Roboto)
	...
	if renderData.buyingPet then
		...
		if tonumber(renderData.buyDogType) then
			if renderData.buyDogProcessing then
				dxDrawText("#7cc576Feldolgozás folyamatban...", x, y, x + sx, y + sy - respc(100), rgba(255,255,255,230), 0.75, RobotoL, "center", "center", false, false, false, true)
			else
				dxDrawText("Add meg a vásárolni kívánt kutya nevét.\nA kutya ára: #ff9428" .. formatNumber(renderData.petPrices[renderData.buyDogType]) .. " PP", x, y, x + sx, y + sy - respc(100), rgba(255,255,255,230), 0.75, RobotoL, "center", "center", false, false, false, true)
			end
			...
			drawButton3("cancelPetBuy", "Mégsem", x + respc(15), y2, buttonWidth, respc(40), {215, 89, 89}, false, Roboto)
			-- Vásárlás
			buttons["buySelectedPet"] = {x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40)}
			drawButton3("buySelectedPet", "Vásárlás", x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40), {255, 148, 40}, false, Roboto)
		else
			dxDrawText("Kattints egy kutyára a vásárláshoz!", x, y + respc(20), x + sx, 0, rgba(255,255,255,230), 0.75, RobotoL, "center", "top", false, false, false, true)
			...
			for i = 1, 6 do
				...
				drawButton3("selectPetForBuy:" .. i, "", x2, y2, oneSize, oneSize, {60, 60, 60}, false, Roboto)
				...
				dxDrawText(renderData.petNameTypes[i], x2, y2 + oneSize, x2 + oneSize, y2 + oneSize + respc(20), rgba(255, 255, 255, 230), 0.65, Roboto, "center", "top")
			end

			buttons["cancelPetBuy"] = {x + respc(20), y + sy - respc(60), sx - respc(40), respc(40)}

			drawButton3("cancelPetBuy", "Mégsem", x + respc(20), y + sy - respc(60), sx - respc(40), respc(40), {215, 89, 89}, false, Roboto)
		end
	...
	if buyingVehicleSlot or buyingInteriorSlot or sellVehicleToCompanyPlayer then
		...
		-- ** Háttér
		dxDrawRectangle(x, y+respc(150), sx, respc(150), rgba(0, 0, 0, 150))
		...
		dxDrawText("Slot:", x + respc(15), y2, 0, y2 + respc(40), rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("db", 0, y2, x + sx - respc(15), y2 + respc(40), rgba(255,255,255,230), 0.5, RobotoB, "right", "center")
		...
		dxDrawText("Mégsem", x + respc(15), y2, x + respc(15) + buttonWidth, y2 + respc(40), rgba(255,255,255,230), 0.9, RobotoL, "center", "center")
		...
		if activeButton == "buy_" .. buttonPrefix .. "_slot" then
			dxDrawRectangle(x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40), rgba(255, 148, 40, 200))
		else
			dxDrawRectangle(x + sx - respc(15) - buttonWidth, y2, buttonWidth, respc(40), rgba(255, 148, 40, 150))
		end

		dxDrawText("Vásárlás", x + sx - respc(15) - buttonWidth, y2, x + sx - respc(15), y2 + respc(40), rgba(255,255,255,230), 0.9, RobotoL, "center", "center")

		-- Egyenleg
		--dxDrawText("Prémium egyenleg: " .. formatNumber(myDatas["char.PP"] or 0) .. " PP", x + 1, y + sy + 1, x + sx + 1, y + sy + respc(40) + 1, rgba(0, 0, 0), 0.6, Roboto, "center", "center")
		dxDrawText(formattedText * 100 .. " #ff9428PP", x, y + sy, x + sx, y + sy + respc(40), rgba(255,255,255,230), 0.8, Roboto, "center", "center", false, false, false, true)
	...
	-- ** Dashboard
	--wardis
	local sx, sy = screenX - respc(180), screenY - respc(180)
	local x = screenX / 2 - sx / 2
	local y = screenY / 2 - sy / 2
	...
	dxDrawRectangle(x, y, sx, sy, rgba(0, 0, 0, 200))
	dxDrawRectangle(x, y, sx, headerHeight, rgba(0, 0, 0, 150))
	...
	dxDrawImage(logoPosX, logoPosY - 5, respc(165 * 0.45), respc(165 * 0.45), serverLogo, 0, 0, 0, rgba(255,255,255))
	dxDrawText("#ff9428Cosmo#ffffffMTA", logoPosX + respc(165 * 0.45) + (logoPosX - x), y + 10, 0, y + headerHeight, rgba(255,255,255,230), 1, RobotoB, "left", "center", false, false, false, true)
	...
	for k = 1, #tabCaptions do
		...
		if v then
			--if v == 5 or v == 6 then return end
			local x2 = x + sx - oneTabSize * k
			local colorOfTab = rgba(255, 255, 255, 230)
			if v == 1 or v == 2 or v == 3 or v == 4 or v == 5 then
				x2 = x2 + oneTabSize*3
			end

			if selectedTab == v then
				dxDrawRectangle(x2, y, oneTabSize, headerHeight, rgba(255, 148, 40, 50))
				dxDrawRectangle(x2, y + headerHeight, oneTabSize, 2, rgba(255, 148, 40))

				colorOfTab = rgba(255, 148, 40)
			elseif activeButton == "selectTab:" .. v then
				colorOfTab = rgba(255,255,255,230)
			end
	...
	if selectedTab == 1 then
		...
		if mySkin == currentSkin then
			...
			drawButton3("openMyClothes", "Kiegészítők megnyitása", x2, y2, sx2, respc(40), {255, 148, 40}, false, Roboto) -- Haza értem berakom

			y2 = y2 - respc(55)

			dxDrawRectangle(x2, y2, sx2, respc(40), rgba(0, 0, 0, 150))
			dxDrawText("Skined: " .. mySkin .."", x2, y2, x + sx - respc(20), y2 + respc(40), rgba(255,255,255,230), 0.75, Roboto, "center", "center")

			local skinBcgHeight = holderSizeY - respc(110)

			dxDrawRectangle(x2, y + respc(20), sx2, skinBcgHeight, rgba(0, 0, 0, 150))
			dxDrawImage(x2 + sx2 / 2 - skinSizeX / 2, y + respc(20) + skinBcgHeight / 2 - skinSizeY / 2, skinSizeX, skinSizeY, ":cosmo_binco/files/skins2/" .. mySkin .. ".jpg", 0, 0, 0, rgba(200,200,200))
		else
			local sx2 = skinHolderSizeX * marginMul - respc(20)
			...
			dxDrawRectangle(x2, y + respc(20), skinHolderSizeX, skinBcgHeight, rgba(0, 0, 0, 150))
			dxDrawImage(x2 + skinHolderSizeX / 2 - skinSizeX / 2, y + skinBcgHeight / 2 - skinSizeY / 2, skinSizeX, skinSizeY, ":cosmo_binco/files/skins2/" .. currentSkin .. ".jpg", 0, 0, 0, rgba(200,200,200))
			
			dxDrawRectangle(x2, y2, skinHolderSizeX, respc(40), rgba(0, 0, 0, 150))
			dxDrawText("Jelenlegi skined", x2, y2, x2 + skinHolderSizeX, y2 + respc(40), rgba(255,255,255,230), 0.75, Roboto, "center", "center")
			
			x2 = x + sx - skinHolderSizeX - respc(20)

			dxDrawRectangle(x2, y + respc(20), skinHolderSizeX, skinBcgHeight, rgba(0, 0, 0, 150))
			dxDrawImage(x2 + skinHolderSizeX / 2 - skinSizeX / 2, y + skinBcgHeight / 2 - skinSizeY / 2, skinSizeX, skinSizeY, ":cosmo_binco/files/skins2/" .. mySkin .. ".jpg", 0, 0, 0, rgba(200,200,200))

			dxDrawRectangle(x2, y2, skinHolderSizeX, respc(40), rgba(0, 0, 0, 150))
			dxDrawText("Állandó skined", x2, y2, x + sx - respc(20), y2 + respc(40), rgba(255,255,255,230), 0.75, Roboto, "center", "center")
		end
		...
		dxDrawRectangle(startX, startY, oneSegmentSizeX, oneSegmentSizeY, rgba(0, 0, 0, 150))

		local oneSize = oneSegmentSizeY / 15

		for i = 1, 15 do
			if i ~= 15 then
				dxDrawRectangle(startX, startY + i * oneSize, oneSegmentSizeX, 2, rgba(0, 0, 0, 180))
			end
		end
		...
		if myDatas["char.Name"] ~= myDatas["visibleName"] then
			dxDrawText("Karakter név:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
			dxDrawText("#ff9428"..myDatas["char.Name"]:gsub("_", " "), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

			startY = startY + oneSize

			dxDrawText("Megjelenő név:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
			dxDrawText("#ff9428"..myDatas["visibleName"]:gsub("_", " "), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
		else
			dxDrawText("Karakter név:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
			dxDrawText("#ff9428"..myDatas["char.Name"]:gsub("_", " "), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
		end
		...
		dxDrawText("Felhasználó azonosító:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428"..myDatas["char.ID"], 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Karakter azonosító:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428"..myDatas["char.ID"], 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
		...
		dxDrawText("Játszott idő:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" ..formatTime, 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Idő a fizetésig:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. (myDatas["char.playTimeForPayday"] or 60) .. " #ffffffperc", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Készpénz:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. formatNumber(myDatas["char.Money"]) .. " #eaeaea$", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Banki egyenleg:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. formatNumber(myDatas["char.bankMoney"] or 0) .. " #eaeaea$", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Prémium egyenleg:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. formatNumber(myDatas["char.PP"] or 0) .. " #eaeaeaPP", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Kaszinó zseton:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. formatNumber(myDatas["char.ucoin"] or 0) .. " #eaeaeazseton", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Járműveid száma:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. #playerVehicles .. " #ffffffdb", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Ingatlanjaid száma:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. #playerInteriors .. " #ffffffdb", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

		startY = startY + oneSize

		dxDrawText("Munka:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
		dxDrawText("#ff9428" .. (jobNames[myDatas["char.Job"]] or "#ff9428Nincs"), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
	end
	...
	if selectedTab == 2 then
		...
		w3 = w3 + dxGetTextWidth(formatNumber(myDatas["char.PP"] or 0).. " PP", 1, Roboto)

		drawDataRow(startX + (oneSectionWidth - w) / 2, startY, respc(40), "Készpénz: ", "#ff9428" .. formatNumber(myDatas["char.Money"]) .. " #ffffff$")
		
		drawDataRow(startX + oneSectionWidth + (oneSectionWidth - w2) / 2, startY, respc(40), "Banki egyenleg: ", "#ff9428" .. formatNumber(myDatas["char.bankMoney"]) .. " #ffffff$")
		
		drawDataRow(startX + oneSectionWidth * 2 + (oneSectionWidth - w3) / 2, startY, respc(40), "Prémium egyenleg: ", "#ff9428"..formatNumber(myDatas["char.PP"] or 0) .. " #ffffffPP")
		...
		dxDrawText("Járműveim (#ff9428" .. maxThings .. " #ffffffdb/#ff9428" .. myDatas["char.vehicleLimit"] .. " #ffffffdb)", startX, startY - respc(50), startX + oneSegmentSizeX, startY - respc(5), rgba(255,255,255,230), 0.85, RobotoL, "center", "center", false, false, false, true)

		--if activeButton == "more_vehslot" then
		--	dxDrawRectangle(startX + oneSegmentSizeX - respc(105), startY - respc(50) + respc(45) / 2 - respc(35) / 2, respc(100), respc(35), rgba(255, 148, 40, 240))
		--else
		--	dxDrawRectangle(startX + oneSegmentSizeX - respc(105), startY - respc(50) + respc(45) / 2 - respc(35) / 2, respc(100), respc(35), rgba(255, 148, 40, 180))
		--end

		--dxDrawText("+ slot", startX + oneSegmentSizeX - respc(105), startY - respc(50), startX + oneSegmentSizeX - respc(5), startY - respc(5), rgba(255,255,255,230), 0.5, RobotoB, "center", "center")
		drawButton3("more_vehslot", "+ slot", startX + oneSegmentSizeX - respc(105), startY - respc(50) + respc(45) / 2 - respc(35) / 2, respc(100), respc(35), {255, 148, 40, 170}, true, Roboto)
		...
		for i = 1, num do
			...
			if i + offsetVeh == selectedVeh and isElement(veh) then
				dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(255, 148, 40, 150))
			elseif activeButton == "selectVehicle:" .. i + offsetVeh then
				dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(255, 148, 40, 90))
			else
				if i % 2 == 0 then
			...
			if i ~= num then
				dxDrawRectangle(startX, lineY + oneSize, oneSegmentSizeX, 2, rgba(0, 0, 0, 180))
			end
			...
			if isElement(veh) then
				...
				local colorOfText = rgba(255, 255, 255, 230)

				if i + offsetVeh == selectedVeh then
					colorOfText = rgba(255, 255, 255, 230)
				elseif activeButton == "selectVehicle:" .. i + offsetVeh then
					colorOfText = rgba(255, 255, 255, 230)
				end
				...
				dxDrawText("ID: " .. formatNumber((datas["vehicle.dbID"] or 0), ",") .. " | " .. table.concat(plateSections, "-"), 0, lineY, startX + oneSegmentSizeX - 10, lineY + oneSize, colorOfText, 0.75, Roboto, "right", "center")
		...
		if maxThings > num then
			...
			dxDrawRectangle(startX + oneSegmentSizeX, startY + offsetVeh * (trackSize / maxThings), 5, trackSize / maxThings * num, rgba(255, 148, 40, 150))
		end
		...
		if isElement(veh) then
			...
			if (datas["vehicle.group"] or 0) == 0 then
				--if activeButton == "sell_vehicle" then
				--	dxDrawRectangle(startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), respc(100), respc(35), rgba(255, 148, 40, 240))
				--else
					--dxDrawRectangle(startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), respc(100), respc(35), rgba(255, 148, 40, 180))
				--end
				...
				--drawButton3("sell_vehicle", "Eladás", startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), respc(100), respc(35), {255, 148, 40}, false, Roboto)
			...
			if kilometersToChangeOil <= 0 then
				...
				kilometersToChangeOil = "#ff9428"..formatNumber(kilometersToChangeOil) .. " #ffffffkm múlva"
			end
			...
			startY, w1 = drawDataRow(startX, startY, height, "Kilóméterszámláló állása: ", "#ff9428 0 #ffffff km", scale)
			
			s3, w2 = drawDataRow(startX + w1 + respc(10), s2, height, "Olajcsere: ", kilometersToChangeOil, scale)
			
			s3, w3 = drawDataRow(startX + w1 + w2 + respc(20), s2, height, "Állapot: ", "#ff9428"..health .. "#ffffff %", scale)
			
			drawDataRow(startX + w1 + w2 + w3 + respc(30), s2, height, "Üzemanyag: ", "#ff9428"..math.floor((datas["vehicle.fuel"] or exports.cosmo_hud:getTheFuelTankSizeOfVehicle(model)) / exports.cosmo_hud:getTheFuelTankSizeOfVehicle(model) * 100) .. " #ffffff%", scale)

			local s = 0

			s, w1 = drawDataRow(startX, startY, height, "Motor: ", (datas["vehicle.engine"] == 1 and "#ff9428elindítva" or "#ff9428leállítva"), scale)
			
			s, w2 = drawDataRow(startX + w1 + respc(10), startY, height, "Lámpa: ", (getVehicleOverrideLights(veh) == 2 and "#ff9428felkapcsolva" or "#ff9428lekapcsolva"), scale)
			
			s, w3 = drawDataRow(startX + w1 + w2 + respc(20), startY, height, "Kézifék: ", (datas["vehicle.handBrake"] and "#ff9428behúzva" or "#ff9428kiengedve"), scale)
			
			startY = drawDataRow(startX + w1 + w2 + w3 + respc(30), startY, height, "Ajtók: ", (datas["vehicle.locked"] == 1 and "#ff9428zárva" or "#ff9428nyitva"), scale)

			startY = startY + respc(10)
			dxDrawRectangle(startX, startY, oneSegmentSizeX, 2, rgba(200,200,200, 50))
			startY = startY + respc(10)

			local tuningData = datas["vehicle.tuningData"]

			s, w1 = drawDataRow(startX, startY, height, "Motor: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			s, w5 = drawDataRow(startX + w1 + respc(10), startY, height, "Fék: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			s, w2 = drawDataRow(startX + w1 + w5 + respc(20), startY, height, "Turbo: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			s, w3 = drawDataRow(startX + w1 + w5 + w2 + respc(30), startY, height, "ECU: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			startY, w4 = drawDataRow(startX + w1 + w5 + w2 + w3 + respc(40), startY, height, "Váltó: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			--s, w6 = drawDataRow(startX, startY, height, "Gumi:  ", tuningName[tuningData.gearbox or 0], scale)
			
			s, w6 = drawDataRow(startX, startY, height, "Súlycsökkentés: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
			
			startY = drawDataRow(startX + w6 + respc(20), startY, height, "Felfüggesztés: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)

			if (getElementData(veh, "danihe->tuning->airride") or 0) == 1 then
				s, w2 = drawDataRow(startX + w1 + respc(30), startY, height, "AirRide: ", "#ff9428van", scale)
			else
				s, w2 = drawDataRow(startX + w1 + respc(30), startY, height, "AirRide: ", "#ff9428nincs", scale)
			end

			if (getElementData(veh, "danihe->tuning->nitro") or 0) == 1 then
				s, w1 = drawDataRow(startX, startY, height, "Nitró: ", "#ff9428van", scale)
			else
				s, w1 = drawDataRow(startX, startY, height, "Nitró: ", "#ff9428nincs", scale)
			end

			-- if (datas["tuning.neon"] or 0) > 10000 then
			-- 	s, w3 = drawDataRow(startX + w1 + w2 + respc(20), startY, height, "Neon: ", "#ff9428van", scale)
			-- else
			-- 	s, w3 = drawDataRow(startX + w1 + w2 + respc(20), startY, height, "Neon: ", "#ff9428nincs", scale)
			-- end
		else
		...
		dxDrawRectangle(startX, startY - respc(50), oneSegmentSizeX, respc(45), rgba(0, 0, 0, 150))
		dxDrawText("Ingatlanjaim (#ff9428" .. maxThings .. " #ffffffdb/#ff9428" .. myDatas["char.interiorLimit"] .. " #ffffffdb)", startX, startY - respc(50), startX + oneSegmentSizeX, startY - respc(5), rgba(255,255,255,230), 0.85, RobotoL, "center", "center", false, false, false, true)
		...
		--dxDrawText("+ slot", startX + oneSegmentSizeX - respc(105), startY - respc(50), startX + oneSegmentSizeX - respc(5), startY - respc(5), rgba(255,255,255,230), 0.5, RobotoB, "center", "center")
		drawButton3("more_intslot", "+ slot", startX + oneSegmentSizeX - respc(105), startY - respc(50) + respc(45) / 2 - respc(35) / 2, respc(100), respc(35), {255, 148, 40, 170}, false, Roboto)
		buttons["more_intslot"] = {startX + oneSegmentSizeX - respc(105), startY - respc(50) + respc(45) / 2 - respc(35) / 2, respc(100), respc(35)}

		dxDrawRectangle(startX, startY, oneSegmentSizeX, oneSegmentSizeY, rgba(0, 0, 0, 170))
		...
		for i = 1, num do
			...
			elseif activeButton == "selectInterior:" .. i + offsetInt then
				dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(255, 148, 40, 50))
			else
				if i % 2 == 0 then
					dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(0, 0, 0, 30))
				end
		...
		if maxThings > num then
			...
			dxDrawRectangle(startX + oneSegmentSizeX, startY + offsetInt * (trackSize / maxThings), 5, trackSize / maxThings * num, rgba(255, 148, 40, 150))
		end
		...
		if int then
			...
			if datas.type ~= "rentable" then
				...
				--dxDrawText("Eladás", startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), startX + oneSegmentSizeX - respc(5), startY + sectionSize - respc(20), rgba(0, 0, 0), 0.5, RobotoB, "center", "center")
				--drawButton3("sell_interior", "Eladás", startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), respc(100), respc(35), {255, 148, 40}, false, Roboto)
				--buttons["sell_interior"] = {startX + oneSegmentSizeX - respc(105), startY + sectionSize - respc(55), respc(100), respc(35)}
			end

			if datas.iconPicture then
				dxDrawImage(math.floor(startX), math.floor(startY + height - respc(24)), respc(48), respc(48), datas.iconPicture, 0, 0, 0, rgba(255, 148, 40, 150))
			end

			dxDrawText(datas["name"], startX + respc(48), startY, 0, startY + height * 2, rgba(255, 148, 40,200), 0.75, RobotoB, "left", "center")

			startY = startY + height * 3
			startY = drawDataRow(startX, startY, height, "Típus: ", "#ff9428" .. interiorTypes[datas.type] or "#ff9428Ismeretlen", scale)
			startY = drawDataRow(startX, startY, height, "Ajtók: ", (datas.locked == "Y" and "#ff9428Zárva" or "#ff9428Nyitva"), scale)

			if datas.type == "rentable" then
				startY = drawDataRow(startX, startY, height, "Bérleti díj: ", formatNumber(datas.price) .. " $/hét", scale)
			else
				startY = drawDataRow(startX, startY, height, "Eredeti ár: ", "#ff9428"..formatNumber(datas.price) .. " #ffffff$", scale)
			end
		else
			local startY = startY + oneSegmentSizeY
			local sectionSize = (y + sy - headerHeight) - startY
	...
	if selectedTab == 3 then
		...
		for i = 1, num do
			...
			if i ~= num then
				dxDrawRectangle(startX, lineY + oneSize, oneSegmentSizeX, 2, rgba(0, 0, 0, 170))
			end

			if playerGroupsCount < 1 and i == 10 then
				dxDrawText("Nem vagy frakcióban.", startX, lineY, startX + oneSegmentSizeX, lineY + oneSize, rgba(255, 148, 40), 0.5, RobotoB, "center", "center")
			end
		...
		if playerGroupsCount > num then
			local trackSize = num * oneSize
			dxDrawRectangle(startX + oneSegmentSizeX, startY, 5, trackSize, rgba(255, 148, 40, 50))
			dxDrawRectangle(startX + oneSegmentSizeX, startY + offsetGroup * (trackSize / playerGroupsCount), 5, trackSize / playerGroupsCount * num, rgba(255, 148, 40, 150))
		end
		...
		if tonumber(groupId) and playerGroups[groupId] and groups[groupId] and meInGroup[groupId] then
			...
			dxDrawText(group.name .. " (" .. groupId .. ")", startX + oneSegmentSizeX, startY, x + sx, startY + respc(50), rgba(255, 148, 40,200), 0.85, RobotoB, "center", "center")

			startX = startX + oneSegmentSizeX + respc(25)
			startY = startY + respc(60)

			local oneSize = (x + sx - oneSegmentSizeX - respc(300)) / 3
			local scale = 0.65
			local height = respc(30) * scale
			...
			drawDataRow(startX + (oneSize - w) / 2, startY, height, "Rangod: ", "#ff9428"..group.ranks[myRank].name .. " #ffffff(" .. myRank .. ")", scale)
			
			drawDataRow(startX + oneSize + (oneSize - w2) / 2, startY, height, "Fizetésed: ", "#dddddd" .."#ff9428"..formatNumber(group.ranks[myRank].pay) .. " #ffffff$", scale)
			
			if meInGroup[groupId].dutySkin == 0 then
				drawDataRow(startX + oneSize * 2 + (oneSize - w3) / 2, startY, height, "Duty skined: ", "#ff9428nincs beállítva", scale)
			else
				drawDataRow(startX + oneSize * 2 + (oneSize - w3) / 2, startY, height, "Duty skined: ", "#ff9428" .. meInGroup[groupId].dutySkin, scale)
			end

			-- Tabok
			startY = startY + height + respc(10)

			local oneSize = (x + sx - oneSegmentSizeX - respc(300)) / 4
			...
			for i = 1, 4 do
				...
				if selectedGroupTab == i then
					dxDrawRectangle(lineX, startY, oneSize, respc(35), rgba(255, 148, 40, 150))
				else
					if activeButton == "selectGroupTab:" .. i then
						dxDrawRectangle(lineX, startY, oneSize, respc(35), rgba(255, 148, 40, 50))

						colorOfText = rgba(255, 255, 255, 230)
					else
						dxDrawRectangle(lineX, startY, oneSize, respc(35), rgba(0, 0, 0, 170))
					end
				end

				if i ~= 4 then
					dxDrawRectangle(lineX + oneSize - 1, startY, 2, respc(35), rgba(0, 0, 0, 170))
				end
			...
			if selectedGroupTab == 1 then
				dxDrawRectangle(startX, startY, oneSectionSizeX, oneSectionSizeY, rgba(0, 0, 0, 170))
				...
				for i = 1, num do
					...
					if i + offsetMember == selectedMember and member then
						if member.online then
							dxDrawRectangle(startX, lineY, oneSectionSizeX, oneSize, rgba(255, 148, 40, 150))
						else
							dxDrawRectangle(startX, lineY, oneSectionSizeX, oneSize, rgba(255, 148, 40, 150))
						end
					elseif activeButton == "selectMember:" .. i + offsetMember then
						dxDrawRectangle(startX, lineY, oneSectionSizeX, oneSize, rgba(255, 148, 40, 50))
					else
						if i % 2 == 0 then
					...
					if i ~= num then
						dxDrawRectangle(startX, lineY + oneSize, oneSectionSizeX, 2, rgba(0, 0, 0, 170))
					end
				...
				if #thisMembers > num then
					dxDrawRectangle(startX + oneSectionSizeX, startY, 5, trackSize, rgba(0, 0, 0, 150))
					dxDrawRectangle(startX + oneSectionSizeX, startY + offsetMember * (trackSize / #thisMembers), 5, trackSize / #thisMembers * num, rgba(255, 148, 40, 150))
				end

				dxDrawText(math.ceil(offsetMember / num) + 1 .. "/" .. math.ceil(#thisMembers / num) .. ". oldal (" .. selectedMember .. "/" .. #thisMembers .. "db tag)", startX, startY + trackSize, startX + oneSectionSizeX, startY + trackSize + respc(40), rgba(255, 255, 255, 230), 0.6, Roboto, "center", "center")
				...
				if member then
					...
					dxDrawText(member.characterName:gsub("_", " "), startX - 150, startY, startX + oneSectionSizeX, startY + respc(50), rgba(255, 255, 255, 230), 0.75, RobotoB, "center", "center")

					startX = startX + respc(20)
					startY = startY + respc(50)

					--dxDrawRectangle(startX, startY - respc(5), oneSectionSizeX - respc(10), 2, rgba(0, 0, 0, 170))

					local scale = 0.85
					local height = respc(45) * scale

					startY = drawDataRow(startX, startY, height, "Rang: ", "#ff9428" .. group.ranks[member.rank].name .. " #ffffff(" .. member.rank .. ")", scale)
					startY = drawDataRow(startX, startY, height, "Fizetés: ", "#dddddd" .. formatNumber(group.ranks[member.rank].pay) .. " #ff9428$", scale)
					
					if member.dutySkin == 0 then
						startY = drawDataRow(startX, startY, height, "Duty skin: ", "#ff9428nincs beállítva", scale)
					else
						startY = drawDataRow(startX, startY, height, "Duty skin: ", "#ff9428" .. member.dutySkin, scale)
					end

					local lastOnline = split(member.lastOnline or "#ff9428nem_volt_még_online", " ")

					startY = drawDataRow(startX, startY, height, "Utoljára online: ", "#ff9428" .. lastOnline[1]:gsub("_", " "), scale)

					if member.isLeader == "Y" then
						startY = drawDataRow(startX, startY, height, "Leader: ", "#ff9428igen", scale)
					else
						startY = drawDataRow(startX, startY, height, "Leader: ", "#ff9428nem", scale)
					end

					if member.online then
						startY = drawDataRow(startX, startY, height, "Online: ", "#ff9428igen", scale)
					else
						startY = drawDataRow(startX, startY, height, "Online: ", "#ff9428nem", scale)
					end
					...
					if meInGroup[groupId].isLeader == "Y" then
						...
						local buttonSizeX = oneSectionSizeX - respc(70)
						...
						for i = 1, 4 do
							...
							if i == 4 and fakeInputError then
								...
								dxDrawText(fakeInputError, startX, buttonY, 0, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.75, RobotoB2, "left", "center", false, false, false, true)
							...
							elseif i == 1 and "memberrankUp" then
								local acceptWidth = respc(272)

								--dxDrawText(fireErrorText, startX, buttonY, 0, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.75, RobotoB2, "left", "center", false, false, false, true)
								...
								dxDrawText("Előléptetés", startX + buttonSizeX - acceptWidth  + respc(282), buttonY, startX + buttonSizeX - acceptWidth - respc(10), buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
								
								buttons["rankUp"] = {startX + buttonSizeX - acceptWidth, buttonY, acceptWidth, buttonSizeY}
							elseif i == 2 and "memberrankDown" then
								local acceptWidth = respc(272)
								...
								--dxDrawText("Mégsem", startX + buttonSizeX - acceptWidth, buttonY, startX + buttonSizeX, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
								dxDrawText("Lefokozás", startX + buttonSizeX - acceptWidth + respc(282), buttonY, startX + buttonSizeX - acceptWidth - respc(10), buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
								
								--buttons["cancelrankDown"] = {startX + buttonSizeX - acceptWidth, buttonY, acceptWidth, buttonSizeY}
								buttons["rankDown"] = {startX + buttonSizeX - acceptWidth, buttonY, acceptWidth, buttonSizeY}
							elseif i == 3 and memberFirePrompt then
								...
								dxDrawText("Mégsem", startX + buttonSizeX - acceptWidth, buttonY, startX + buttonSizeX, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
								dxDrawText("Kirúgás", startX + buttonSizeX - acceptWidth * 2 - respc(10), buttonY, startX + buttonSizeX - acceptWidth - respc(10), buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
							...
							elseif i == 4 and activeFakeInput == "inviting" then
								...
								if string.len(fakeInputText) < 1 then
									...
									dxDrawText(fakeInputText, startX + respc(10), buttonY, startX + buttonSizeX - acceptWidth - respc(20), buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.75, Roboto, "left", "center")
								end
								...
								dxDrawText("Hozzáad", startX + buttonSizeX - acceptWidth, buttonY, startX + buttonSizeX, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.65, fontFamily, "center", "center")
								...
								dxDrawText(groupButtonCaptions[i], startX, buttonY, startX + buttonSizeX, buttonY + buttonSizeY, rgba(255, 255, 255, 230), 0.75, fontFamily, "center", "center")
			...
			elseif selectedGroupTab == 2 then
				...
				if #thisRanks > num then
					dxDrawRectangle(startX + oneSectionSizeX, startY, 5, trackSize, rgba(0, 0, 0, 150))
					dxDrawRectangle(startX + oneSectionSizeX, startY + offsetRank * (trackSize / #thisRanks), 5, trackSize / #thisRanks * num, rgba(255, 148, 40, 150))
				end

				dxDrawText(math.ceil(offsetRank / num) + 1 .. "/" .. math.ceil(#thisRanks / num) .. ". oldal (" .. selectedRank .. "/" .. #thisRanks .. "db rang)", startX, startY + trackSize, startX + oneSectionSizeX, startY + trackSize + respc(40), rgba(255,255,255,230), 0.6, Roboto, "center", "center")
				...
				if thisRank then
					...
					dxDrawText(thisRank.name, startX - 150, startY, startX + oneSectionSizeX, startY + respc(50), rgba(255,255,255,230), 0.75, RobotoB, "center", "center")
					...
					startY = drawDataRow(startX, startY, height, "Tagok száma ezen a rangon: ", "#ff9428" .. (rankCount[groupId][selectedRank] or 0) .. " #ffffffdb", scale)
					startY = drawDataRow(startX, startY, height, "Fizetés: ", "#dddddd" .. formatNumber(thisRank.pay) .. " #ff9428$", scale)
					...
					if meInGroup[groupId].isLeader == "Y" then
						local buttonMargin = respc(20)
						local buttonSizeX = oneSectionSizeX - respc(70)
						local buttonSizeY = ((y + sy - headerHeight) - startY - buttonMargin * 2) / 6
						...
						for i = 1, 3 do
							...
							if (i == 1 and activeFakeInput == "setrankname") or (i == 2 and activeFakeInput == "setrankpay") or (i == 3 and activeFakeInput == "createrank") then
								...
								dxDrawText(groupButtonCaptions2[i], startX, buttonY, startX + buttonSizeX, buttonY + buttonSizeY, rgba(255, 255, 255), 0.75, fontFamily, "center", "center")
			...
			elseif selectedGroupTab == 3 then
				...
				for i = 1, num do
					...
					elseif veh then
						dxDrawText("Hiányzó jármű.", startX + 10, lineY, 0, lineY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
					end
				end
				...
				if #vehicles > num then
					dxDrawRectangle(startX + oneSectionSizeX, startY, 5, trackSize, rgba(0, 0, 0, 180))
					dxDrawRectangle(startX + oneSectionSizeX, startY + offsetGroupVeh * (trackSize / #vehicles), 5, trackSize / #vehicles * num, rgba(255, 148, 40, 150))
				end

				dxDrawText(math.ceil(offsetGroupVeh / num) + 1 .. "/" .. math.ceil(#vehicles / num) .. ". oldal (" .. selectedGroupVeh .. "/" .. #vehicles .. "db jármű)", startX, startY + trackSize, startX + oneSectionSizeX, startY + trackSize + respc(40), rgba(255,255,255,230), 0.6, Roboto, "center", "center")
				...
				if isElement(veh) then
					...
					dxDrawText(datas.vehicleName, startX, startY, startX - 150 + oneSectionSizeX, startY + respc(50), rgba(255,255,255,230), 0.6, RobotoB, "center", "center")
					...
					if datas["vehicle.engine"] == 1 then
						startY = drawDataRow(startX, startY, height, "Motor: ", "#ff9428elindítva", scale)
					else
						startY = drawDataRow(startX, startY, height, "Motor: ", "#ff9428leállítva", scale)
					end

					if getVehicleOverrideLights(veh) == 2 then
						startY = drawDataRow(startX, startY, height, "Lámpa: ", "#ff9428felkapcsolva", scale)
					else
						startY = drawDataRow(startX, startY, height, "Lámpa: ", "#ff9428lekapcsolva", scale)
					end

					if datas["vehicle.handBrake"] then
						startY = drawDataRow(startX, startY, height, "Kézifék: ", "#ff94286behúzva", scale)
					else
						startY = drawDataRow(startX, startY, height, "Kézifék: ", "#ff9428kiengedve", scale)
					end

					if datas["vehicle.locked"] == 1 then
						startY = drawDataRow(startX, startY, height, "Ajtók: ", "#ff9428zárva", scale)
					else
						startY = drawDataRow(startX, startY, height, "Ajtók: ", "#ff9428nyitva", scale)
					end

					startY = startY + height

					-- if (datas["vehicle.nitroLevel"] or 0) == 0 then
					-- 	startY = drawDataRow(startX, startY, height, "Nitro: ", "#ff9428" .. math.floor(datas["vehicle.nitroLevel"] or 0) .. "%", scale)
					...
					if (tuningData.airride or 0) == 1 then
						startY = drawDataRow(startX, startY, height, "AirRide: ", "#ff9428van", scale)
					else
						startY = drawDataRow(startX, startY, height, "AirRide: ", "#ff9428nincs", scale)
					end

					if (tuningData.nitroLevel or 0) == 1 then
						startY = drawDataRow(startX, startY, height, "AirRide: ", "#ff9428van", scale)
					else
						startY = drawDataRow(startX, startY, height, "AirRide: ", "#ff9428nincs", scale)
					end

					startY = drawDataRow(startX, startY, height, "Motor: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "Fék: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "Turbo: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "ECU: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "Váltó: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "Súlycsökkentés: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					startY = drawDataRow(startX, startY, height, "Felfüggesztés: ", getElementData(veh, "danihe->tuning->airintake") or 0, scale)
					...
					startY = startY + height
					startY = drawDataRow(startX, startY, height, "Állapot: ", "#ff9428"..health .. " #ffffff%", scale)
					startY = drawDataRow(startX, startY, height, "Üzemanyag: ", "#ff9428"..math.floor((datas["vehicle.fuel"] or exports.cosmo_hud:getTheFuelTankSizeOfVehicle(model)) / exports.cosmo_hud:getTheFuelTankSizeOfVehicle(model) * 100) .. " #ffffff%", scale)
					startY = drawDataRow(startX, startY, height, "Olajcsere: ", "#ff9428"..kilometersToChangeOil, scale)
					startY = drawDataRow(startX, startY, height, "Kilóméterszámláló állása: ", "#ff9428 0 #ffffff km", scale)
				end
			-- Egyéb
			elseif selectedGroupTab == 4 then
				if (renderData.canSelectDutySkin and not renderData.dutySkinMarker) or (renderData.canSelectDutySkin and renderData.dutySkinMarker == groupId) then
					dxDrawText("Szolgálati öltözék:", startX, startY, 0, startY + respc(50), rgba(255,255,255,230), 0.75, RobotoB, "left", "center")
					...
					if activeButton == "setDutySkin" then
						dxDrawRectangle(startX, startY, oneSectionSizeX * 2, respc(40), rgba(255, 148, 40, 240))
					else
						dxDrawRectangle(startX, startY, oneSectionSizeX * 2, respc(40), rgba(255, 148, 40, 200))
					end
				...
				dxDrawText("Megjegyzés:", startX, startY, 0, startY + respc(50), rgba(255, 255, 255, 230), 0.75, RobotoB, "left", "center")

				startY = startY + respc(50)

				if meInGroup[groupId].isLeader == "N" then
					dxDrawText(group.description, startX + respc(10), startY + respc(10), startX + oneSectionSizeX * 2 - respc(10), startY + respc(10) + respc(200), rgba(255, 255, 255, 230), 0.75, Roboto, "left", "top", false, true)
				else
					dxDrawRectangle(startX, startY, oneSectionSizeX, respc(200), rgba(0, 0, 0, 150))
					...
					if activeFakeInput == "groupdesc" then
						if utf8.len(fakeInputText) < 1 then
							...
							if cursorState then
								dxDrawText("|", startX + respc(10), startY + respc(10), startX + oneSectionSizeX * 2 - respc(10), startY + respc(10) + respc(200), rgba(255,255,255,230), 0.75, Roboto, "left", "top", false, true)
							end
						elseif cursorState then
							dxDrawText(fakeInputText .. "|", startX + respc(10), startY + respc(10), startX + oneSectionSizeX * 2 - respc(10), startY + respc(10) + respc(200), rgba(255,255,255,230), 0.75, Roboto, "left", "top", false, true)
						else
							dxDrawText(fakeInputText, startX + respc(10), startY + respc(10), startX + oneSectionSizeX * 2 - respc(10), startY + respc(10) + respc(200), rgba(255,255,255,230), 0.75, Roboto, "left", "top", false, true)
						end
						...
						if activeButton == "setgroupdesc" then
							dxDrawRectangle(startX, startY, oneSectionSizeX, respc(40), rgba(255, 148, 40, 240))
						else
							dxDrawRectangle(startX, startY, oneSectionSizeX, respc(40), rgba(255, 148, 40, 200))
						end

						dxDrawText("Módosít", startX, startY, startX + oneSectionSizeX, startY + respc(40), rgba(255, 255, 255, 230), 0.75, RobotoL, "center", "center")

						buttons["setgroupdesc"] = {startX, startY, oneSectionSizeX, respc(40)}

						startY = startY + respc(50)
					else
						dxDrawText(group.description, startX + respc(10), startY + respc(10), startX + oneSectionSizeX - respc(10), startY + respc(10) + respc(200), rgba(100, 100, 100), 0.75, Roboto, "left", "top")
						
						buttons["groupdesc"] = {startX, startY, oneSectionSizeX, respc(200)}
				...
				if meInGroup[groupId].isLeader == "Y" then
					dxDrawText("Pénzügyek:", startX, startY, 0, startY + respc(50), rgba(255,255,255,230), 0.75, RobotoB, "left", "center")
					...
					local buttonSizeX = (oneSectionSizeX * 2) / 5
					local buttonSizeY = respc(40)

					if fakeInputError then
						dxDrawText(fakeInputError, startX, startY, 0, startY + buttonSizeY, rgba(255,255,255,230), 0.75, RobotoB2, "left", "center", false, false, false, true)
					...
					elseif activeFakeInput == "groupbalance" then
						...
						if string.len(inputText) > 0 then
							dxDrawText(inputText, startX + buttonSizeX, startY, startX + buttonSizeX * 2, startY + buttonSizeY, rgba(255,255,255,230), 0.75, Roboto, "center", "center")
						end
						...
						if activeButton == "putbackmoney" then
							dxDrawRectangle(startX + buttonSizeX * 2, startY, buttonSizeX, buttonSizeY, rgba(255, 148, 40, 240))
						else
							dxDrawRectangle(startX + buttonSizeX * 2, startY, buttonSizeX, buttonSizeY, rgba(255, 148, 40, 200))
						end
						...
						buttons["groupbalance"] = {startX, startY, oneSectionSizeX, buttonSizeY}

						if activeButton == "groupbalance" then
							dxDrawRectangle(startX, startY, oneSectionSizeX, buttonSizeY, rgba(255, 148, 40, 240))
						else
							dxDrawRectangle(startX, startY, oneSectionSizeX, buttonSizeY, rgba(255, 148, 40, 200))
						end

						dxDrawText("Pénzügyek kezelése", startX, startY, startX + oneSectionSizeX, startY + buttonSizeY, rgba(255, 255, 255), 0.75, RobotoL, "center", "center")
					end
				end
			end
		else
			dxDrawText("Nincs kiválasztva frakció.", startX + oneSegmentSizeX, y + headerHeight, x + sx, y + sy - headerHeight * 2, rgba(255,255,255,230), 1, RobotoB, "center", "center")
		end
	end
	...
	if selectedTab == 4 then
		...
		for i = 1, num do
			...
			if i % 2 ~= 0 then
				dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(0, 0, 0, 150))
			else
				dxDrawRectangle(startX, lineY, oneSegmentSizeX, oneSize, rgba(0, 0, 0, 180))
			end
			...
			if adminSlots[slot] then
				if adminSlots[slot][1] == "admin" or adminSlots[slot][1] == "as" then
					dxDrawText(adminSlots[slot][2], startX + respc(10), lineY, startX + oneSegmentSizeX, lineY + oneSize, rgba(255,255,255,230), 0.65, Roboto, "left", "center", false, false, false, true)

					if adminSlots[slot][1] == "admin" then
						if adminSlots[slot][3] then
							dxDrawText("Szolgálatban: #ff9428igen", 0, lineY, startX + oneSegmentSizeX - respc(10), lineY + oneSize, rgba(255,255,255,230), 0.65, Roboto, "right", "center", false, false, false, true)
						else
							dxDrawText("Szolgálatban: #d75959nem", 0, lineY, startX + oneSegmentSizeX - respc(10), lineY + oneSize, rgba(255,255,255,230), 0.65, Roboto, "right", "center", false, false, false, true)
						end
					end
				elseif adminSlots[slot][1] == "title" then
					dxDrawText(adminSlots[slot][2], startX, lineY, startX + oneSegmentSizeX, lineY + oneSize, rgba(255, 148, 40), 0.65, RobotoB2, "center", "center")
				else
					dxDrawText(adminSlots[slot][2], startX, lineY, startX + oneSegmentSizeX, lineY + oneSize, rgba(255,255,255,230), 0.65, Roboto, "center", "center")
				end
			end
		end
	...
	if selectedTab == 5 then
		...
		--if activeButton == "newAnimal" then
		--	dxDrawRectangle(startX, startY + trackSize + respc(10), oneSegmentSizeX, respc(40), rgba(255, 148, 40, 240))
		--else
		--	dxDrawRectangle(startX, startY + trackSize + respc(10), oneSegmentSizeX, respc(40), rgba(255, 148, 40, 200))
		--end
		...
		drawButton3("newAnimal", "Pet vásárlása", startX, startY + trackSize + respc(10), oneSegmentSizeX, respc(40), {255, 148, 40, 170}, false, Roboto)

		if maxThings > num then
			dxDrawRectangle(startX + oneSegmentSizeX, startY, 5, trackSize, rgba(200,200,200, 25))
			dxDrawRectangle(startX + oneSegmentSizeX, startY + renderData.offsetAnimal * (trackSize / maxThings), 5, trackSize / maxThings * num, rgba(255, 148, 40, 150))
		end
		...
		if animal then
			...
			dxDrawText(animal.name, startX, startY - respc(80), startX + oneSectionSizeX, startY, rgba(255, 148, 40), 1, RobotoB, "center", "center")
			...
			startY = drawDataRow(startX, startY, height, "Élet: ", "#ff9428"..math.floor(healthLevel) .. "#ffffff %", scale)
			...
			startY = drawDataRow(startX, startY, height, "Éhség: ", "#ff9428" .. math.floor(hungerLevel) .. "#ffffff %", scale)

			dxDrawRectangle(startX, startY, oneSectionSizeX, respc(30), rgba(200,200,200, 25))
			dxDrawRectangle(startX + 3, startY + 3, (oneSectionSizeX - 6) * hungerLevel / 100, respc(30) - 6, rgba(255, 148, 40))

			startY = startY + respc(40)

			local loveLevel = dogLove or animal.love
			startY = drawDataRow(startX, startY, height, "Szeretet: ", "#ff9428" .. math.floor(loveLevel) .. "#ffffff %", scale)
			...
			--drawButton3("sellAnimal", "Eladás", startX, startY, oneSectionSizeX, buttonSizeY, {215, 89, 89}, false, Roboto)

			-- Felélesztés
			buttons["reviveAnimal"] = {startX, startY + buttonSizeY + buttonMargin, oneSectionSizeX, buttonSizeY}
			drawButton3("reviveAnimal", "Felélesztés", startX, startY + buttonSizeY + buttonMargin, oneSectionSizeX, buttonSizeY, {50, 179, 239}, false, Roboto)

			-- Spawn/Despawn
			buttons["spawnAnimal"] = {startX, startY + buttonSizeY * 2 + buttonMargin * 2, oneSectionSizeX, buttonSizeY}

			if renderData.spawnedAnimal ~= animal.animalId then
				drawButton3("spawnAnimal", "Spawn", startX, startY + buttonSizeY * 2 + buttonMargin * 2, oneSectionSizeX, buttonSizeY, {124, 197, 118}, false, Roboto)
			elseif renderData.petDatas and renderData.spawnedPetElement then
				drawButton3("spawnAnimal", "Despawn", startX, startY + buttonSizeY * 2 + buttonMargin * 2, oneSectionSizeX, buttonSizeY, {124, 197, 118}, false, Roboto)
			end
			-- Átnevezés
			buttons["renameAnimal"] = {startX, startY + buttonSizeY * 3 + buttonMargin * 3, oneSectionSizeX, buttonSizeY}
			drawButton3("renameAnimal", "Átnevezés", startX, startY + buttonSizeY * 3 + buttonMargin * 3, oneSectionSizeX, buttonSizeY, {186, 188, 61}, false, Roboto)
		else
			dxDrawText("Nincs kiválasztva pet.", startX + oneSegmentSizeX, y + headerHeight, x + sx, y + sy - headerHeight * 2, rgba(255, 255, 255, 230, 200), 1, RobotoB, "center", "center")
		end
	end
	...
	if selectedTab == 8 then -- setting
		...
		for i = 1, tabNum do
			...
			if i % 2 == 0 then
				dxDrawRectangle(startX, lineY, oneSectionSizeX, oneSize, rgba(0, 0, 0, 150))
			end
			...
			if set then
				dxDrawText(set[1], startX + respc(20), lineY, 0, lineY + oneSize, rgba(255,255,255,230), 0.85, RobotoB2, "left", "center")
				...
				if set[4] then
					if type(set[4]) == "table" then
						...
						dxDrawRectangle(sliderX, sliderY, respc(15), sliderHeight, rgba(255, 148, 40,210))

						-- outputChatBox(sliderX .. " | " .. sliderY .. " | " .. sliderHeight)
						...
						if set[4] == "crosshair" then
							...
							drawButton3("crosshair_color", "Szín", colorX, buttonStartY, respc(128), buttonSizeY, {255, 148, 40}, false, Roboto)
						...
						drawButton3("setting_next:" .. set[3], ">",buttonStartX, buttonStartY, buttonSizeX, buttonSizeY, {255, 148, 40}, false, Roboto)
						if set[4] == "crosshair" then
							local shapeSize = oneSize * 0.35
							local shapeX = buttonStartX - buttonSizeX * 2
							local shapeY = lineY + oneSize / 2 - shapeSize / 2 - respc(8)
							local shapeId = renderData.loadedSettings.crosshairData[1] or 0
							local r, g, b = renderData.loadedSettings.crosshairData[2], renderData.loadedSettings.crosshairData[3], renderData.loadedSettings.crosshairData[4]
							...
							dxDrawImage(math.floor(shapeX), math.floor(shapeY), shapeSize, shapeSize, ":cosmo_crosshair/files/" .. shapeId .. ".png", 0, 0, 0, rgba(r, g, b))
							dxDrawImage(math.floor(shapeX + shapeSize * 2), math.floor(shapeY), -shapeSize, shapeSize, ":cosmo_crosshair/files/" .. shapeId .. ".png", 0, 0, 0, rgba(r, g, b))
							dxDrawImage(math.floor(shapeX), math.floor(shapeY + shapeSize * 2), shapeSize, -shapeSize, ":cosmo_crosshair/files/" .. shapeId .. ".png", 0, 0, 0, rgba(r, g, b))
							dxDrawImage(math.floor(shapeX + shapeSize * 2), math.floor(shapeY + shapeSize * 2), -shapeSize, -shapeSize, ":cosmo_crosshair/files/" .. shapeId .. ".png", 0, 0, 0, rgba(r, g, b))
						else
							if type(set[2]) == "boolean" or not set[2] then
								dxDrawText("Kikapcsolva", buttonStartX - buttonSizeX * 2, lineY, buttonStartX - buttonSizeX, lineY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "center", "center")
							else
								if renderData.shaderRealNames[set[3]] and renderData.shaderRealNames[set[3]][set[2]] then
									dxDrawText(renderData.shaderRealNames[set[3]][set[2]], buttonStartX - buttonSizeX * 2, lineY, buttonStartX - buttonSizeX, lineY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "center", "center")
								else
									dxDrawText(set[2], buttonStartX - buttonSizeX * 2, lineY, buttonStartX - buttonSizeX, lineY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "center", "center")
								end
							end
						end
						...
						drawButton3("setting_prev:" .. set[3], "<", buttonStartX - buttonSizeX * 4, buttonStartY, buttonSizeX, buttonSizeY, {255, 148, 40}, false, Roboto)
					...
					if not enabled then
						drawButton3("setting_toggle:" .. set[3], "Bekapcsolás", buttonStartX, buttonStartY, buttonSizeX, buttonSizeY, {255, 148, 40}, false, Roboto)
						buttons["setting_toggle:" .. set[3]] = {buttonStartX, buttonStartY, buttonSizeX, buttonSizeY}
					else
						drawButton3("setting_toggle:" .. set[3], "Kikapcsolás", buttonStartX, buttonStartY, buttonSizeX, buttonSizeY, {215, 89, 89}, false, Roboto)
						buttons["setting_toggle:" .. set[3]] = {buttonStartX, buttonStartY, buttonSizeX, buttonSizeY}
					end
				end
			end
		...
		if maxThings > tabNum then
			...
			dxDrawRectangle(startX + oneSectionSizeX, startY + renderData.offsetSettings * (trackSize / maxThings), 5, trackSize / maxThings * tabNum, rgba(255, 148, 40, 150))
		end
	...
	if selectedTab == 6 then --companyP
		for k, v in pairs(edits) do
			...
			if active then
				...
				dxDrawRectangle(x, y + h - 2, w * w2, 2, tocolor(255, 148, 40), true)
				...
				dxDrawRectangle(carretPosX + 2, y + 2.5, 2, h - 5, tocolor(255, 255, 255, 230, carretAlpha), true)
		...
		if buySlot then
			...
			if buySlot == "member" then
				drawText("Tag slot vásárlás", posX + 3, posY + 3, sizeX - 6, respc(35), rgba(255, 255, 255, 230, 200), 1, Roboto, true)
			elseif buySlot == "vehicle"	then
				drawText("Jármű slot vásárlás", posX + 3, posY + 3, sizeX - 6, respc(35), rgba(255, 255, 255, 230, 200), 1, Roboto, true)
			else
				drawText("Szint vásárlás", posX + 3, posY + 3, sizeX - 6, respc(35), rgba(255, 255, 255, 230, 200), 1, Roboto, true)
			end
			
			dxDrawRectangle(posX + respc(5), posY + respc(60), sizeX - respc(90), respc(35), tocolor(255, 255, 255, 230, 30), true)
			drawButton3("minusSlot", "-", posX + sizeX - respc(5) - respc(35), posY + respc(60), respc(35), respc(35), {188, 64, 61, 170}, true, Roboto)
			drawButton3("plusSlot", "+", posX + sizeX - respc(10) - respc(35) - respc(35), posY + respc(60), respc(35), respc(35), {255, 148, 40, 170}, true, Roboto)

			if buySlot == "member" or buySlot == "vehicle" then
				drawText(formatNumber(selectedSlot) .. " slot = " .. formatNumber(selectedSlot*slotPrice) .. "PP", posX + respc(5), posY + respc(60), sizeX - respc(90), respc(35), rgba(255, 255, 255, 230, 200), 1, Roboto, true)
			else
				drawText(formatNumber(selectedSlot) .. " szint = " .. formatNumber(selectedSlot*slotPrice) .. "PP", posX + respc(5), posY + respc(60), sizeX - respc(90), respc(35), rgba(255, 255, 255, 230, 200), 1, Roboto, true)
			end

			drawButton3("buyTheSlots", "Vásárlás", posX + 4, posY + sizeY - respc(40) - respc(5), sizeX - 8, respc(40), {255, 148, 40}, true, Roboto)
		...
		elseif panel == "company" then
			if not loaded then
				...
				for i = 1, #panels do
					...
					drawButton3(panels[i].id, panels[i].text, lineX-respc(5), y + respc(15), oneSize, respc(50), {255, 148, 40}, false, Roboto)
				end
				if page == 1 then
					local skinHolderSizeX = respc(240)
					local holderSizeY = screenY-respc(240)
					...
					dxDrawText("Vállalkozás neve:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..loaded_company["name"]:gsub("_", " "), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
					startY = startY + oneSize

					dxDrawText("Vállalkozás azonosítója:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..formatNumber(loaded_company["id"]), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Számla egyenlege:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..formatNumber(loaded_company["balance"]).." $", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Tagok:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					if loaded_company["membersCount"] then
						dxDrawText("#ff9428"..#loaded_company["membersCount"] .. "/" .. loaded_company["memberSlot"], 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)
					end
					startY = startY + oneSize

					dxDrawText("Járművek:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..#getCompanyVehicles(loaded_company["id"]) .. "/" .. loaded_company["vehicleSlot"], 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Rangod:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..loaded_company["ranks"][(getElementData(localPlayer,"char.CompanyRank"))].name, 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Szint:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..loaded_company["level"], 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					local xpToNextLevel = getEXPToNextLevel(loaded_company["level"])

					startY = startY + oneSize

					dxDrawText("EXP:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..formatNumber(loaded_company["xp"]).."/"..formatNumber(xpToNextLevel), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Részesedés:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..loaded_company["ranks"][(getElementData(localPlayer,"char.CompanyRank"))].precent.."%", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					startY = startY + oneSize

					dxDrawText("Járulék:", startX + respc(20), startY, 0, startY + oneSize, rgba(255,255,255,230), 0.5, RobotoB, "left", "center")
					dxDrawText("#ff9428"..formatNumber(taxesByRank[tonumber(getElementData(localPlayer,"char.CompanyRank"))]).. " $/Nap", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255,255,255,230), 0.75, Roboto, "right", "center", false, false, false, true)

					drawText("EXP:", x + respc(20) + oneSegmentSizeX/2 - (oneSegmentSizeX - respc(100)) / 2, screenY-respc(200) - respc(50), oneSegmentSizeX - respc(100), respc(40), rgba(255, 255, 255, 230, 200), 1, Roboto)
					dxDrawRectangle(x + respc(20) + oneSegmentSizeX/2 - (oneSegmentSizeX - respc(100)) / 2, screenY-respc(200), oneSegmentSizeX - respc(100), respc(40), rgba(0, 0, 0, 180))
					dxDrawRectangle(x + respc(20) + oneSegmentSizeX/2 - (oneSegmentSizeX - respc(100)) / 2 + 4, screenY-respc(200) + 4, (oneSegmentSizeX - respc(100) - 8), respc(40) - 8, rgba(255, 255, 255, 230, 40))
					dxDrawRectangle(x + respc(20) + oneSegmentSizeX/2 - (oneSegmentSizeX - respc(100)) / 2 + 4, screenY-respc(200) + 4, (oneSegmentSizeX - respc(100) - 8) / xpToNextLevel * loaded_company["xp"], respc(40) - 8, rgba(255, 148, 40, 125))
					...
					--local boxesSize = ((oneSegmentSizeY - (9 * 16)) / 8) 
					local boxesSize = ((oneSegmentSizeY - (8 * 10)) / 9) 
					dxDrawRectangle(x2, y, sx2, oneSegmentSizeY, rgba(0, 0, 0, 150))

					local y = y + 8
					drawButton3("buyLevel", "Szint vásárlása", x2 + 8, y, sx2 - 16, boxesSize, {188, 128, 61}, false, Roboto)
					
					local y = y + boxesSize + 8
					drawButton3("buyVehSlot", "Jármű slot vásárlása", x2 + 8, y, sx2 - 16, boxesSize, {185, 188, 61}, false, Roboto)

					local y = y + boxesSize + 8
					drawButton3("buyMemberSlot", "Tagok slot vásárlása", x2 + 8, y, sx2 - 16, boxesSize, {122, 188, 61}, false, Roboto)
					if playerHavePermission() then
						local y = y + boxesSize + 8
						dxDrawRectangle(x2 + 8, y, sx2 - 16, boxesSize, rgba(0, 0, 0, 180))
						local y = y + boxesSize + 8
						drawButton3("setMoney", "Befizetés", x2 + 8, y, sx2 - 16, boxesSize, {255, 148, 40}, false, Roboto)

						local y = y + boxesSize + 8
						drawButton3("giveMoney", "Kifizetés", x2 + 8, y, sx2 - 16, boxesSize, {188, 64, 61}, false, Roboto)
						...
						if dxGetActiveEditName() == "newPlayer" then
							...
							if thePlayerHasFind then
								drawText(getElementData(thePlayerHasFind, "char.Name"):gsub("_"," ") .. " (" .. getElementData(thePlayerHasFind, "playerID") .. ")", x2 + 8, y, sx2 - 16, boxesSize, tocolor(255, 255, 255, 230, 200), 1 ,RobotoB)
							else
								drawText("#e43232Nem található játékos",x2 + 8, y, sx2 - 16, boxesSize, tocolor(255, 255, 255, 230, 200), 1 ,RobotoB)
							end
						end
						...
						local y = y + boxesSize + 8
						drawButton3("addMemberToCompany", "Játékos felvétele", x2 + 8, y, sx2 - 16, boxesSize, {255, 148, 40}, false, Roboto)
					end
				elseif page == 2 then
					maxLine = 15
					...
					local oneSegmentSizeX = (sx - skinHolderSizeX * marginMul - respc(40))
					local oneSegmentSizeY = holderSizeY
					dxDrawRectangle(startX, startY, skinHolderSizeX * marginMul - respc(20), oneSegmentSizeY+4, rgba(0, 0, 0, 150))
					...
					for i = 1, maxLine do
						...
						if isInSlot(boxpos[1], boxpos[2], boxsize[1], boxsize[2]) then
							...
							dxBackColor = rgba(255, 255, 255, 230, 35)
						else
							if i%2 == 0 then
								dxBackColor = rgba(0, 0, 0, 180, 150)
							else
						...
						if selected == 0 then
							--dxDrawRectangle(startX, startY, skinHolderSizeX * marginMul - respc(20), oneSegmentSizeY+4, rgba(0, 0, 0, 150))
							--drawText("asd", startX, startY, skinHolderSizeX * marginMul - respc(20), oneSegmentSizeY+4, tocolor(255, 255, 255, 230, 200), 1, Roboto)
						end
						
						if player then
							if selected == i + scroll then
								textSelectedColor = rgba(255, 148, 40, 200)
							else
								textSelectedColor = rgba(255, 255, 255, 230, 200)
							end
							if loaded_company["leader"] == player.dbid then
					...
					dxDrawRectangle(skinHolderSizeX * marginMul + respc(50), startY, oneSegmentSizeX, oneSegmentSizeY+4, rgba(0, 0, 0, 150))
					...
					if selected ~= 0 then
						...
						if player then
							...
							for i = 1, 6 do
								dxDrawRectangle(respc(240) * marginMul + respc(50), startY + i * oneSize, oneSegmentSizeX, 2, rgba(0, 0, 0, 180))
							end
							local online_player = findPlayerByName(player.name)
							local startX = skinHolderSizeX * marginMul + respc(50)

							dxDrawText("Utoljára online:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							if online_player then
								dxDrawText("#dededeJelenleg elérhető", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							else
								dxDrawText(formatDate("Y-m-d","'", player.lastOnline) or "Még nem volt elérhető", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							end

							startY = startY + oneSize
							dxDrawText("Rang:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(loaded_company["ranks"][tostring(player.rank)].name, 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)

							startY = startY + oneSize
							dxDrawText("Tartózkodási hely:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							if online_player then
								dxDrawText(getZoneName(getElementPosition(online_player)), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							else
								dxDrawText("Ismeretlen", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							end
							
							startY = startY + oneSize
							dxDrawText("Járulék:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(formatNumber(taxesByRank[tonumber(player.rank)]).." $/nap", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
						
							startY = startY + oneSize
							if player.taxPayed == 0 then
								dxDrawText("Járulék befizetési állapota:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
								dxDrawText("#e43232Még nem volt befizetve!", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							else
								if player.taxPayed <= getTimestamp() then
									dxDrawText("Járulék lejárt ekkor:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
									dxDrawText(formatDate("Y-m-d","'", player.taxPayed), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
								else
									dxDrawText("Járulék Befizetve eddig:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
									dxDrawText(formatDate("Y-m-d","'", player.taxPayed), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
								end
							end
							...
							startY = startY + oneSize
							dxDrawText("Részesedés:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(loaded_company["ranks"][tostring(player.rank)].precent .. "%", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							...
							if payTaxes then
								...
								if tonumber(text) then
									text = tonumber(text)
									if text <= 31 then
										drawText("#dedede" .. formatNumber(text) .. " nap = " .. formatNumber(taxesByRank[player.rank] * text) .. "$", startX + 5, startY - (oneSize * 6) - 25, oneSegmentSizeX - 10, oneSize, tocolor(255, 255, 255, 230, 200), 1, Roboto)
									else
										drawText("#e43232Maximum 1 hónapra fizetheted ki!", startX + 5, startY - (oneSize * 6) - 25, oneSegmentSizeX - 10, oneSize, tocolor(255, 255, 255, 230, 200), 1, Roboto)
									end
								else
									drawText("#e43232Hibás összeg lett megadva!", startX + 5, startY - (oneSize * 6) - 25, oneSegmentSizeX - 10, oneSize, tocolor(255, 255, 255, 230, 200), 1, Roboto)
								end
								
								dxDrawRectangle(startX + 5, startY - (oneSize * 5) - 20, (oneSegmentSizeX / 2) - 7.5, oneSize, tocolor(0, 0, 0, 180))
								drawButton3("payPlayerTransEnter", "Befizetés", startX + 2.5 + (oneSegmentSizeX / 2), startY - (oneSize * 5) - 20 , ((oneSegmentSizeX / 2) - 10) + 2.5, oneSize, {96, 204, 99}, false, Roboto)
							end
							
							drawButton3("rankUp", "Játékos előléptetése", startX + 5, startY - (oneSize * 4) - 15, oneSegmentSizeX - 10, oneSize, {255, 148, 40}, false, Roboto)
							drawButton3("rankMinus", "Játékos lefokozása", startX + 5, startY - (oneSize * 3) - 10, oneSegmentSizeX - 10, oneSize, {230, 149, 0}, false, Roboto)

							drawButton3("payPlayerTrans", "Járulék befizetése", startX + 5, startY - (oneSize * 2) - 5, oneSegmentSizeX - 10, oneSize, {134, 197, 77}, false, Roboto)
							drawButton3("kickPlayer", "Játékos kirúgása", startX + 5, startY - oneSize, oneSegmentSizeX - 10, oneSize, {188, 64, 61}, false, Roboto)
						end
					end
				elseif page == 3 then
					maxLine = 5
					...
					local oneSegmentSizeX = (sx - skinHolderSizeX * marginMul - respc(40))
					local oneSegmentSizeY = holderSizeY
					local oneSize = oneSegmentSizeY / 5
					
					dxDrawRectangle(startX, startY, skinHolderSizeX * marginMul - respc(20), (oneSegmentSizeY / 15) * maxLine + 4, rgba(0, 0, 0, 150))
					drawText("Üzenet:", startX, startY + ((oneSegmentSizeY / 15) * maxLine) + respc(20), skinHolderSizeX * marginMul - respc(20), (oneSegmentSizeY / 15), rgba(255, 255, 255, 230, 200), 1, Roboto)
					dxDrawRectangle(startX, startY + ((oneSegmentSizeY / 15) * maxLine) + respc(20) + (oneSegmentSizeY / 15), skinHolderSizeX * marginMul - respc(20), (oneSegmentSizeY) / 3, rgba(0, 0, 0, 150))
					
					dxDrawText(loaded_company["message"], startX + respc(25), startY + ((oneSegmentSizeY / 15) * maxLine) + respc(20) + (oneSegmentSizeY / 15), skinHolderSizeX * marginMul, (oneSegmentSizeY) / 3, tocolor(255, 255, 255, 230), 1, Roboto, "left","top", false, true)
					
					dxDrawRectangle(startX, startY + ((oneSegmentSizeY / 15) * maxLine) + respc(20) + (oneSegmentSizeY / 15) + (oneSegmentSizeY) / 3 + 5, skinHolderSizeX * marginMul - respc(20), oneSize, rgba(0, 0, 0, 150))

					drawButton3("progressMessage", "Üzenet beállítása", startX, startY + ((oneSegmentSizeY / 15) * maxLine) + respc(20) + (oneSegmentSizeY / 15) + (oneSegmentSizeY) / 3 + 10 + oneSize, skinHolderSizeX * marginMul - respc(20), oneSize, {255, 148, 40}, false, Roboto)
					...
					if selected ~= 0 then
						...
						if row then
							...
							local startX = skinHolderSizeX * marginMul + respc(50)
							dxDrawText("Tagok száma ezen a rangon:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(getPlayersWithThatRank(selected).." db", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							
							startY = startY + oneSize
							dxDrawText("Rang részesedése:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(row.precent.." %", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							
							startY = startY + oneSize
							dxDrawText("Járulék:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(taxesByRank[selected].." $/Nap", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)

							startY = y + respc(80) + screenY - respc(240) - 5
							
							dxDrawRectangle(startX + 5, startY - (oneSize * 4) - 15, oneSegmentSizeX - 10, oneSize, tocolor(0, 0, 0, 180))
							drawButton3("updatePay", "Beállítás", startX + 5, startY - (oneSize * 3) - 10, oneSegmentSizeX - 10, oneSize, {255, 148, 40}, false, Roboto)
							dxDrawRectangle(startX + 5, startY - (oneSize * 2) - 5, oneSegmentSizeX - 10, oneSize, tocolor(0, 0, 0, 180))
							drawButton3("renameRankB", "Átnevezés", startX + 5, startY - oneSize, oneSegmentSizeX - 10, oneSize, {161, 210, 116}, false, Roboto)
							...
							dxDrawRectangle(startX + 5, startY - (oneSize * 6) - 25, oneSegmentSizeX - 10, oneSize, tocolor(0, 0, 0, 180))
							drawButton3("updateCompanyMessage", "Beállítás", startX + 5, startY, oneSegmentSizeX - 10, oneSize, {255, 148, 40})]]
						end
					end
				elseif page == 4 then
					maxLine = 15
					...
					if selected ~= 0 then
						...
						dxDrawText("Jármű ID:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
						dxDrawText(getElementData(vehicle, "company.vehicleID"), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
						...
						if getElementDimension(vehicle) < 30000 then
							startY = startY + oneSize
							dxDrawText("Jelenlegi sofőr:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							if driver then
								dxDrawText(getElementData(driver,"char.Name"):gsub("_"," "), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							else
								dxDrawText("Használatban", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							end

							startY = startY + oneSize
							dxDrawText("Jelenlegi tartózkodási helye:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(getZoneName(posX, posY, posZ), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)

							startY = startY + oneSize
							dxDrawText("Szállítmány típúsa:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							if getElementData(vehicle,"company.stuffName") then
								dxDrawText(getElementData(vehicle,"company.stuffName"), 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							else
								dxDrawText("Ismeretlen", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							end

							startY = startY + oneSize
							dxDrawText("Napi adó:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText("200$", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							...
							dxDrawText("Bérelt:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText(rent, 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
						else
							startY = startY + oneSize
							dxDrawText("Jelenlegi sofőr:", startX + respc(20), startY, 0, startY + oneSize, rgba(255, 255, 255, 230), 0.5, RobotoB, "left", "center")
							dxDrawText("Nincs", 0, startY, startX + oneSegmentSizeX - respc(20), startY + oneSize, rgba(255, 255, 255, 230), 0.75, Roboto, "right", "center", false, false, false, true)
							...
							--drawButton3("sellVehicle", "Jármű eladása", startX + 5, startY - oneSize, oneSegmentSizeX - 10, oneSize, {188, 64, 61}, false, Roboto)
					end
				elseif page == 5 then
					...
					dxDrawText("Időpont:", startX + respc(5), startY + oneSize / 2, nil, nil, tocolor(255, 255, 255, 230, 200), 1, Roboto, "left", "center", false, false, false, true)
					dxDrawText("Tevékenység leírása:", startX + respc(305), startY + oneSize / 2, nil, nil, tocolor(255, 255, 255, 230, 200), 1, Roboto, "left", "center", false, false, false, true)
					for i = 1, maxLine - 1 do
						...
						if row then
							dxDrawText(formatDate("Y/m/d - h:i:s","'",row.time), startX + respc(5), startY + i * (oneSize + 5) + oneSize / 2, nil, nil, tocolor(255, 255, 255, 230, 200), 1, Roboto, "left", "center", false, false, false, true)
							dxDrawText(row.text, startX + respc(305), startY + i * (oneSize + 5) + oneSize / 2, nil, nil, tocolor(255, 255, 255, 230, 200), 1, Roboto, "left", "center", false, false, false, true)
						end
					end
				end
			end
	...
	function (key, press)
		...
		if panelState then
			...
			elseif selectedTab == 3 then
				...
				if key == "backspace" and press then
					if activeFakeInput == "inviting" or activeFakeInput == "setrankname" or activeFakeInput == "setrankpay" or activeFakeInput == "groupdesc" or activeFakeInput == "groupbalance" or activeFakeInput == "createrank" then
						if utf8.len(fakeInputText) >= 1 then
...
function reMap(x, in_min, in_max, out_min, out_max)
	...
	return ((x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min or 0)
end
...
function toggleDutySkinSelect(state, groupId)
	if dutySkinSelectState ~= state then
		if state then
			...
			skinSelectFont = dxCreateFont("files/fonts/Roboto.ttf", 12, false, "antialiased")
...
function processDutySkinSelection()
	...
	exports.cosmo_hud:showInfobox("success", "Sikeresen megváltoztattad a csoporthoz tartozó szolgálati öltözékedet.")
	togglePanel()
end
...
function dutySkinRender()
	...
	dxDrawText("Lapozás: #7cc576[<-] és [->] #ffffffKiválasztás: #7cc576[ENTER]", 0, 0, screenX, screenY - 150, tocolor(255,255,255,230), 1, skinSelectFont, "center", "bottom", false, false, false, true)
end
...
function formatTime( seconds )
	if seconds then
		...
		if day > 0 and day < 10 then 
			table.insert( results, "#ff9428"..day.. " #ffffffnap" .. ( day == 1 and "" or "" ) ) 
		elseif day > 0  then 
			table.insert( results, "#ff9428"..day.." #ffffffnap" .. ( day == 1 and "" or "" ) ) end	
		if hou >= 1 and hou < 10 then 
			table.insert( results, "#ff9428".."0"..hou .." #ffffffóra" .. ( hou == 1 and "" or "" ) ) 
		elseif hou > 0  then 
			table.insert( results, "#ff9428"..hou .." #ffffffóra"..  ( hou == 1 and "" or "" ) ) end	
		if min >= 0 and min < 10 then 
			table.insert( results, "#ff9428".."0"..min .." #ffffffperc".. ( min == 1 and "" or "" ) ) 
		elseif min > 0  then 
			table.insert( results, "#ff9428"..min.." #ffffffperc" .. ( hou == 1 and "" or "" ) ) end	
