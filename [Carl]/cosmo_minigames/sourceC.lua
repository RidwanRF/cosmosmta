...
addEventHandler("igen", getRootElement(),
	function ()
		exports.cosmo_hud:showInfobox("success", "Siker :)")
	end
)
...
addEventHandler("nem", getRootElement(),
	function ()
		exports.cosmo_hud:showInfobox("error", "Elcseszted :(")
	end
)

function stopMinigame()
	if minigameData then
		if minigameState == "buttons" or minigameState == "buttons2" then
...
function startMinigame(gameType, successEvent, failEvent, ...)
	...
	if gameType == "buttons" then
		...
		minigameData.RobotoFont = dxCreateFont("files/Arrows.ttf", 12, false, "antialiased")
	...
	if minigameState == "buttons" or minigameState == "buttons2" then
		if args[1] >= 0.75 then
			if minigameData.successEvent then
				triggerEvent(minigameData.successEvent, localPlayer, localPlayer)
			end
		else
...
function spawnNextButton()
	if minigameState == "buttons" or minigameState == "buttons2" then
		...
		table.insert(minigameData.buttons, {string.char(math.random(97, 100)), 0, false})
	...
	function (key, state)
		if minigameData then
			if minigameState == "buttons" or minigameState == "buttons2" then
				...
				if key == "arrow_d" and state then
					seged = "d"
				elseif key == "arrow_u" and state then
					seged = "c"
				...
				if state and minigameData.currentBtn and not minigameData.buttons[minigameData.currentBtn][3] and (string.byte(seged) >= 97 and string.byte(seged) <= 122) then
					if minigameData.currentBtn == minigameData.btnInKey then
						if minigameData.buttons[minigameData.currentBtn][1] == seged then
...
function startMinigameButtons(gameType, successEvent, failEvent, ...)
	stopMinigame()

	local args = {...}
	minigameData = {}

	if gameType == "buttons2" then
		minigameData.buttons = {}
		minigameData.spawnedButtons = 0
...
addEventHandler("onClientPreRender", getRootElement(),
	function (timeSlice)
		if minigameData then
			if minigameState == "buttons2" then
				local sx, sy = 622, 55
				local x = (screenX - sx) / 2
				local y = screenY - sy - 125
				...
				if isElement(minigameData.renderTarget) then
					...
					dxDrawImage(-100, -30, sx+200, sy+60, "files/black.png", 0, 0, 0, tocolor(255, 255, 255, 255))
					dxDrawImage(-100, -30, sx+200, sy+60, "files/black.png", 0, 0, 0, tocolor(255, 255, 255, 255))
					dxDrawImage(-100, -30, sx+200, sy+60, "files/black.png", 0, 0, 0, tocolor(255, 255, 255, 255))
					dxDrawImage(-100, -30, sx+200, sy+60, "files/black.png", 0, 0, 0, tocolor(255, 255, 255, 255))
					...
					for i = 1, #minigameData.buttons do
						if minigameData.buttons[i] then
							...
							dxDrawImage(-30 + 712 * progress, -16, 90, 90,"files/arrow"..minigameData.buttons[i][1]..".png",0,0,0,tocolor(r, g, b, alpha))
				...
				dxDrawRectangle(x - 1, y + 1, 2, holderSize - 2, borderColor) -- bal
				dxDrawRectangle(x + holderSize - 1, y + 1, 2, holderSize - 2, borderColor) -- jobb]]
				dxDrawImage(x/1.5, y, holderSize*10, holderSize, "files/ring1.png", 0, 0, 0, tocolor(r, g, b, 255))
			end
		end
	end, true, "low-1000"
)