...
function arrestPlayer(id, reason, time, bail, bailPrice)
    local targetPlayer, targetPlayerName = exports.cosmo_core:findPlayer(source, id)
    if exports.cosmo_core:inDistance3D(source, targetPlayer, 5) then
        dropPlayerToTheCell(targetPlayer, time, reason, bail, bailPrice, getPlayerName(source):gsub("_", " "))
    else
        exports.cosmo_hud:showInfobox(source, "error", "A játékos nincs a közeledben")
    end
end
addEvent("cosmo_arrestS:arrestPlayer", true)
addEventHandler("cosmo_arrestS:arrestPlayer", root, arrestPlayer)
...
function dropPlayerToTheCell(player, time, reason, bail, bailPrice, arrestedBy)
    ...
    outputChatBox("#ff9428[Börtön infó (IC)]: #ffffffIdő: ".. time .." perc, Indok: ".. reason ..", Ő általa: ".. arrestedBy ..".", player, 255, 255, 255, true)
...
addCommandHandler("release", function(player, cmd, targetPlayer)
    if not exports.cosmo_groups:isPlayerHavePermission(player, "jail") then
        exports.cosmo_hud:showInfobox(player, "error", "Nincs engedélyed a parancs használatához")
        return
    end
    ...
    if not targetPlayer then
        ...
        local targetPlayer, targetPlayerName = exports.cosmo_core:findPlayer(player, targetPlayer)
        if arrestedPlayers[targetPlayer] then
            ...
            exports.cosmo_hud:showInfobox(player, "error", "Nincs a játékos börtönben")
        end
    end
end)
...
outputUsageText = function(text, element)
	assert(type(text) == "string", "Bad argument @ 'outputInfoText' [expected string at argument 1, got "..type(text).."]")
	outputChatBox(exports.cosmo_core:getServerTag("usage") .. text, element, 0, 0, 0, true)
end