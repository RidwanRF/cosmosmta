...
Arrest.Show = function()
    ...
    if not exports.cosmo_groups:isPlayerHavePermission(localPlayer, "jail") then
        exports.cosmo_hud:showInfobox("error", "Nincs engedélyed a parancs használatához")
        return
    end
    ...
    if dim ~= 0 then
        ...
        if not (getDistanceBetweenPoints3D(arrestPoints[dim][1], arrestPoints[dim][2], arrestPoints[dim][3], px, py, pz) <= 7) then
            exports.cosmo_hud:showInfobox("error", "Nem vagy letartóztatási hely közelében.")
            return
        end
    end
        ...
        elseif widget == widgets.arrestButton then
            ...
            if getDistanceBetweenPoints3D(arrestPoints[int][1], arrestPoints[int][2], arrestPoints[int][3], px, py, pz) <= 7 then
                ...
                if utf8.len(reason) < 6 then
                    exports.cosmo_hud:showInfobox("error", "Az indoknak hosszabbnak kell lennie mint 5 karakter.")
                    return
                end

                if canBail then
                    if utf8.len(UI:getText(widgets.bailInput)) <= 0 or tonumber(UI:getText(widgets.bailInput)) <= 0 then
                        exports.cosmo_hud:showInfobox("error", "Az óvadéknak nagyobbnak kell lennie mint 0.")
                        return
                    end
                end

                if utf8.len(UI:getText(widgets.timeInput)) <= 0 or time <= 0 then
                    exports.cosmo_hud:showInfobox("error", "Az idő nem lehet 0 vagy negatív szám.")
                    return
                end
                ...
                triggerServerEvent("cosmo_arrestS:arrestPlayer", localPlayer, player, reason, time, canBail, bailPrice)
            else
                exports.cosmo_hud:showInfobox("error", "Nem vagy letartóztatási hely közelében.")
            end
        end
        ...
        if button == "left" and state == "down" then
            if clickedElement and isElement(clickedElement) then
                if getElementData(clickedElement, "ped.type") == 3 then
                    if exports.cosmo_core:inDistance3D(getLocalPlayer(), clickedElement, 4) then
                        Arrest.Show()
                    end
                end
            end
