...
local cctvMarker = createMarker(1568.5876464844, -1691.1599121094, 5.890625 - 1, "cylinder", 0.75, 255, 148, 40, 100)
local cctvMarkerCol = createColSphere(1568.5876464844, -1691.1599121094, 5.890625, 10)
...
local Raleway = exports.cosmo_assets:loadFont("Raleway.ttf", 15, false, "antialiased")
...
local cameraPositions = {
	...
	{1465.1296386719, -1042.7807617188, 32.828125, 90, -100, 0, 100, "Bank"},
	{1185.9455566406, -1367.2734375, 21.550018310547, -300, -100, 0, 100, "Kórház"},
	{1366.2690429688, -1276.1866455078, 32.348564147949, 270, -100, 0, 65, "Fegyverbolt"},
	{1541.8026123047, -1699.9504394531, 27.3828125, 90, -100, 0, 100, "Rendőrség"},
}
...
addEventHandler("onClientPlayerWeaponFire", localPlayer,
	function (weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement, startX, startY, startZ)
		if weapon >= 22 and weapon <= 33 then
				if not exports.cosmo_groups:isPlayerHavePermission(localPlayer, "cctv") then
...
function renderTheMarkerHint()
	...
	if x and y then
		...
		dxDrawText("[CCTV]\nHasználata:\n[W][A][S][D][Q][E] - Mozgatás\n[JOBB/BAL NYÍL] - Kamera váltása\n[ESC] - Kilépés", x + 1, y + 1, x + 200 + 1, y + 20 + 1, tocolor(0, 0, 0), 1, Raleway, "center", "center")
		dxDrawText("#ff9428[CCTV]\n#ffffffHasználata:\n#ff9428[W][A][S][D][Q][E]#ffffff - Mozgatás\n#ff9428[JOBB/BAL NYÍL]#ffffff - Kamera váltása\n#ff9428[ESC]#ffffff - Kilépés", x, y, x + 200, y + 20, tocolor(255, 255, 255), 1, Raleway, "center", "center", false, false, false, true)
	end
end
...
addEventHandler("onClientMarkerHit", getResourceRootElement(),
	function (enterPlayer, matchingDimension)
		if enterPlayer == localPlayer and matchingDimension then
			if exports.cosmo_groups:isPlayerHavePermission(localPlayer, "cctv") then
			...
			exports.cosmo_hud:showInfobox("error", "Ehhez a tevékenységhez nincs jogod!")
		end
	end
end)
	...
	if key == "w" or key == "a" or key == "s" or key == "d" or key == "q" or key == "e" then
		if not press then
			...
			if not isElement(camMoveSound) then
				camMoveSound = playSound("files/cammove.mp3", true)
			end
		end
	end
...
function cctvRender()
	...
	local sx = dxGetTextWidth(currTime, 1, Raleway) + respc(15)
	local sy = dxGetFontHeight(1, Raleway) + respc(15)

	dxDrawRectangle(respc(25), screenY - respc(25) - sy, sx, sy, tocolor(25, 25, 25))
	dxDrawText(currTime, respc(25), screenY - respc(25) - sy, respc(25) + sx, screenY - respc(25), tocolor(200, 200, 200, 200), 1, Raleway, "center", "center", false, false, false, true)
	
	sx = dxGetTextWidth(cctvText, 1, Raleway) + respc(15)

	dxDrawRectangle(screenX - sx - respc(25), screenY - respc(25) - sy, sx, sy, tocolor(25, 25, 25))
	dxDrawText(cctvText, screenX - sx - respc(25), screenY - respc(25) - sy, screenX - respc(25), screenY - respc(25), tocolor(200, 200, 200, 200), 1, Raleway, "center", "center", false, false, false, true)
	...
	if cx and cy then
		...
		dxDrawImage(cx - respc(32), cy - respc(32), respc(64), respc(64), "files/pointer.png", 0, 0, 0, tocolor(23.75, 31.25, 27.5, 200))
		...
		if worldX and worldY and worldZ then
			...
			if hitElement then
				...
				if plateText then
					...
					local sx = dxGetTextWidth(str, 1, Raleway) + respc(15)
					...
					dxDrawText(str, cx + respc(40), cy - sy / 2, cx + respc(40) + sx, cy - sy / 2 + sy, tocolor(255, 255, 255), 1, Raleway, "center", "center", false, false, false, true)
				end
			end
		end
	end
