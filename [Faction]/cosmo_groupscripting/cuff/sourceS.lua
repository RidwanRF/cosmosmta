...
function grabFunction(sourcePlayer, commandName, targetPlayer)
	if not targetPlayer then
		outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [Játékos név / ID]", sourcePlayer, 0, 0, 0, true)
	else
						...
						if getElementData(targetPlayer, "player.Cuffed") then
							if not getElementData(targetPlayer, "player.Grabbed") then
								...
								if not isElement(grabbingPlayer) then
									...
									outputChatBox(exports.cosmo_core:getServerTag("error") .. "Egyszerre csak egy embert tudsz vezetni!", source, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékost már vezeti valaki!", source, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos nincs megbilincselve!", source, 0, 0, 0, true)
						end
					end
				else
					outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", source, 0, 0, 0, true)
				end
			end
		end
	end
...
function cuffFunction(sourcePlayer, commandName, targetPlayer)
	if exports.cosmo_groups:isPlayerHavePermission(sourcePlayer, "cuff") then
		if not targetPlayer then
			outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. commandName .. " [Játékos név / ID]", sourcePlayer, 0, 0, 0, true)
		else
			targetPlayer, targetName = exports.cosmo_core:findPlayer(sourcePlayer, targetPlayer)
				...
				if distance <= 5 then
					if sourceInterior == targetInterior and sourceDimension == targetDimension then
						...
						if cuffed then
							...
							exports.cosmo_controls:toggleControl(targetPlayer, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
						end
					end
				else
					outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", source, 0, 0, 0, true)
		...
		if getElementData(player, "loggedIn") then
			if exports.cosmo_groups:isPlayerHavePermission(player, "cuff") then
				if not target or utfLen(target) < 1 then
					outputChatBox(exports.cosmo_core:getServerTag("usage") .. "/" .. command .. " [Játékos név/ID]", player, 0, 0, 0, true)
				else
					...
					if target then
						...
						if getDistanceBetweenPoints3D(targetX, targetY, targetZ, localX, localY, localZ) <= 3 and getElementInterior(player) == getElementInterior(target) and getElementDimension(player) == getElementDimension(target) then
							if not getElementData(target, "player.Cuffed") then
								if exports.cosmo_inventory:hasItem(player, 77) then
									exports.cosmo_controls:toggleControl(target, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, false)
									triggerClientEvent("cuffThePlayer", player, target, true)
									setElementData(target, "player.Cuffed", true)
								else
									outputChatBox(exports.cosmo_core:getServerTag("error") .. "Nincs nálad bilincs!", player, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos már meg van bilincselve!", player, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", player, 0, 0, 0, true)
						end
					end
				end
			end
					...
					if target then
						...
						if getDistanceBetweenPoints3D(targetX, targetY, targetZ, localX, localY, localZ) <= 3 and getElementInterior(player) == getElementInterior(target) and getElementDimension(player) == getElementDimension(target) then
							if getElementData(target, "player.Cuffed") then
								if exports.cosmo_inventory:hasItem(player, 78) then
									exports.cosmo_controls:toggleControl(target, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
									triggerClientEvent("cuffThePlayer", player, target, false)
									setElementData(target, "player.Cuffed", false)
								else
									outputChatBox(exports.cosmo_core:getServerTag("error") .. "Nincs nálad bilincs kulcs!", player, 0, 0, 0, true)
								end
							else
								outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos nincs bilincselve!", player, 0, 0, 0, true)
							end
						else
							outputChatBox(exports.cosmo_core:getServerTag("error") .. "A kiválasztott játékos túl messze van tőled!", player, 0, 0, 0, true)
						end
...
exports.cosmo_admin:addAdminCommand("auncuff", 1, "Bilincs levétele a játékosról")
		...
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			if not (target) then
				...
				if target then
					...
					local targetName = (getElementData(target, "visibleName"):gsub("_", " "))
					
					setElementData(target, "player.Cuffed", false)
					exports.cosmo_controls:toggleControl(target, {"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
					
					exports.cosmo_core:sendMessageToAdmins(adminNick .." levette ".. targetName .. " játékosról a bilincset.", 7)
 				end
			end
		end
	end
...
function playerDamage_text ( attacker, weapon, bodypart, loss )
	if weapon == 0 then
		if getElementData(source, "player.Cuffed") then
			if not getElementData(source, "player.Grabbed") then
				setElementData(source, "cuffAnimation", 0)
				setElementData(source, "cuffAnimation", 3)
			end
		end
	end
end
