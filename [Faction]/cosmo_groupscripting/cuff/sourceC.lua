...
addEventHandler("onClientPlayerQuit", root, 
	function()
		if isElement(source) then
			if leftHandCuff[source] then
				destroyElement(leftHandCuff[source])
			end
			
			if rightHandCuff[source] then
	...
	function (dataName, oldValue)
		if dataName == "player.Cuffed" then
			...
			if cuffed then
				...
				exports.cosmo_boneattach:attachElementToBone(leftHandCuff[source], source, 11, 0, 0, 0, 90, -45, 0)

				rightHandCuff[source] = createObject(cuffModel, 0, 0, 0)
				setElementDoubleSided(rightHandCuff[source], true)
				exports.cosmo_boneattach:attachElementToBone(rightHandCuff[source], source, 12, 0, 0, 0, 90, -45, 0)
...
addEventHandler("onClientPreRender", getRootElement(),
	function ()
		...
		if getElementData(localPlayer, "player.Cuffed") and anim ~= "cuff_standing" and cuffAnim[localPlayer] == 0 or cuffAnim[localPlayer] == 3 and getElementData(localPlayer, "player.Cuffed") and anim ~= "cuff_standing" then
			setPedAnimation(localPlayer, "cuff_standing", "standing", -1, true, false)
		end]]
