...
function loadFonts()
	local fonts = {
		...
		Roboto16 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", resp(16), false, "cleartype"),
		...
		gtaFont2 = exports.cosmo_assets:loadFont("gtaFont2.ttf", resp(40), false, "default"),
		Themify60 = exports.cosmo_assets:loadFont("Themify.ttf", respc(60), false, "cleartype"),
		RobotoB18 = exports.cosmo_assets:loadFont("Roboto-Bold.ttf", respc(18), false, "antialiased"),
		Themify18 = exports.cosmo_assets:loadFont("Themify.ttf", respc(18), false, "cleartype"),
		RobotoL14 = exports.cosmo_assets:loadFont("Roboto-Light.ttf", respc(14), false, "cleartype"),
		RobotoB14 = exports.cosmo_assets:loadFont("Roboto-Bold.ttf", respc(14), false, "cleartype"),

		metro18 = exports.cosmo_assets:loadFont("metro.ttf", respc(18), false, "cleartype"),
		metro60 = exports.cosmo_assets:loadFont("metro.ttf", respc(60), false, "cleartype"),
	}
...
local boundColor = tocolor(255, 148, 40, 75)
	...
	function (hitElement, matchingDimension)
		if hitElement == localPlayer and matchingDimension and getElementData(source, "dutyColShape") then
			...
			if getElementData(hitElement, "groupDuty") then
				exports.cosmo_hud:showInfobox("info", "Nyomj [E] gombot a szolgálat leadásához.")
			else
				exports.cosmo_hud:showInfobox("info", "Nyomj [E] gombot a szolgálat felvételéhez.")
			end
		end
	end
)
...
bindKey("E", "down",
	function ()
		if standingColShape and isPlayerInGroup(localPlayer, standingColShape) then
			if getTickCount() >= lastDutyTick + 5000 then
				lastDutyTick = getTickCount()
				triggerServerEvent("requestDuty", localPlayer, standingColShape)
			else
				exports.cosmo_hud:showInfobox("error", "Csak öt másodpercenként dutyzhatsz.")
			end
		end
	end
)
	...
	function (dataName)
		...
		if getElementType(source) == "vehicle" and monitoredDatasForVehicle[data] and monitoredGroupVeh[source] then
			if not vehicleDatas[source] then
				vehicleDatas[source] = {}
				vehicleDatas[source].vehicleName = exports.cosmo_mods_veh:getVehicleName(source)
			end
	...
	if playerGroups then
		...
		for k, v in ipairs(getElementsByType("vehicle")) do
			...
			if groupVehicles[groupID] then
				...
				vehicleDatas[v].vehicleName = exports.cosmo_mods_veh:getVehicleName(v)
			end
		end
...
function groupsCommandHandler()
	if getElementData(localPlayer, "loggedIn") then
		...
		elseif groupsIsLoading then
			exports.cosmo_hud:showAlert("error", "Frakciók betöltése folyamatban van!", "Próbáld pár másodperc múlva.")
		elseif getTickCount() - openedTime >= 2000 then
			...
			if playerGroupsCount > 0 then
				...
				exports.cosmo_hud:showAlert("error", "Nem tartozol egy frakció tagjai közé sem.")
			end

			openedTime = getTickCount()
		else
			exports.cosmo_hud:showAlert("error", "Maximum 2 másodpercenként nyithatod meg a frakciópanelt!")
		end
	end
end
...
exports.cosmo_admin:addAdminCommand("managegroups", 6, "Frakciók kezelése")
addCommandHandler("managegroups",
	function ()
		if monitoredLocalDatas["acc.adminLevel"] >= 6 then
			if panelState or haveActiveEvents then
				togglePanel(false)
			elseif groupsIsLoading then
				exports.cosmo_hud:showAlert("error", "Frakciók betöltése folyamatban van!", "Próbáld pár másodperc múlva.")
			elseif getTickCount() - openedTime >= 2000 then
				...
				exports.cosmo_hud:showAlert("error", "Maximum 2 másodpercenként nyithatod meg a frakció kezelő panelt!")
			end
		end
	end
)
...
function getGroupMembersInDuty(groupID)
	local count = 0
	for v,k in pairs(playerCache) do
		if getPlayerGroups(v) == groupID then
			count = count+1;
		end
	end
	return count;
...
function drawGroupCreator()
	...
	dxDrawImage(math.ceil(x + marginOffset), math.ceil(y + marginOffset), logoSize, logoSize, "files/headerLogo.png", 0, 0, 0, tocolor(255, 255, 255, 255 * alphaAnim[3]))
	dxDrawText("CosmoMTA", x + logoSize + marginOffset*2, y, 0, y + headerHeight, tocolor(255, 255, 255, 255 * alphaAnim[3]), 1, Roboto14, "left", "center")
	...
	dxDrawImage(x + sx - closeSize - marginOffset, y + (headerHeight - closeSize) / 2, closeSize, closeSize, ":cosmo_assets/images/cross_x.png", 0, 0, 0, tocolor(closeColor[1], closeColor[2], closeColor[3], closeColor[4] * alphaAnim[3]))
	...
	for i = 1, #groupCreatorMenu do
		...
		if container then
			...
			if adminTabAfterInterpolation == i then
				colorOfContainer = {processColorSwitchEffect("menuTab:" .. i, {255, 148, 40, 255})}
	...
	if selectedAdminTab == 1 then
		...
		drawButton("addDutyPos", "Duty pozíció felvétele", x, y, listWidth, respc(35), {255, 148, 40, 255})
		...
		for i = 1, 8 do
			...
			if thisItem then
				...
				dxDrawImage(x + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":cosmo_assets/images/cross_x.png", 0, 0, 0, tocolor(removeIconColor[1], removeIconColor[2], removeIconColor[3], removeIconColor[4] * alphaMultipler))
		...
		drawButton("addDutySkin", "Hozzáad", x2 + listWidth - acceptWidth, y, acceptWidth, respc(35), {255, 148, 40, 255})
		...
		for i = 1, 8 do
			...
			if thisItem then
				...
				dxDrawImage(x2 + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":cosmo_assets/images/cross_x.png", 0, 0, 0, tocolor(removeIconColor[1], removeIconColor[2], removeIconColor[3], removeIconColor[4] * alphaMultipler))
	...
	if selectedAdminTab == 2 then
		...
		if not renderData.availableItems then
			renderData.availableItems = exports.cosmo_inventory:getItemList()
		...
		for i = 1, 10 do
			...
			if activeButton == "addItem:" .. i + renderData.availableItemsOffset then
				colorOfRow = tocolor(255, 148, 40, 255 * alphaMultipler)
			elseif i % 2 ~= 0 then
				colorOfRow = tocolor(53, 53, 53, 100 * alphaMultipler)
			else
				colorOfRow = tocolor(53, 53, 53, 50 * alphaMultipler)
		...
		for i = 1, 10 do
			...
			if thisItem then
				...
				dxDrawImage(x + listWidth - removeIconSize - 10, itemY + (oneItemHeight - removeIconSize) / 2, removeIconSize, removeIconSize, ":cosmo_assets/images/cross_x.png", 0, 0, 0, removeIconColor)
		...
		if renderData.selectedDutyItem then
			...
			if errorToDisplay == "acceptItemSettings" and string.len(errorText) > 0 then
				...
				drawButton("acceptItemSettings", "Mentés", x + inputWidth*2 + respc(5)*2, y, acceptWidth, respc(35), {255, 148, 40, 255})
			end
		end
	end
...
function drawGroupPanel()
	...
	dxDrawImage(x + sx - closeSize - marginOffset, y + marginOffset, closeSize, closeSize, ":cosmo_assets/images/cross_x.png", 0, 0, 0, tocolor(closeColor[1], closeColor[2], closeColor[3], closeColor[4] * alphaAnim[3]))
	...
	if selectedTab == 1 then
		...
		-- ** Rangok
		dxDrawRectangle(cardX, y, cardWidth, cardHeight, tocolor(255, 148, 40, 200 * alphaMultipler))
		...
		for i = 1, #thisRanks do
			...
			if thisRankCount[i] then
				...
				dxDrawRectangle(tileX, tileY, w, tileHeightProgress, tocolor(255, 148, 40, 150 * alphaMultipler))
	...
	if selectedTab == 2 then
		...
		if selectedRank and thisRanks[selectedRank] then
			...
			dxDrawText(rank.name .. " (" .. selectedRank .. ")", x, y, 0, 0, tocolor(255, 148, 40, 255 * alphaMultipler), 1, RobotoB18, "left", "top")

			y = y + RobotoB18H

			dxDrawText("Fizetés: ", x, y, 0, 0, tocolor(255, 255, 255, 255 * alphaMultipler), 1, Roboto16, "left", "top")
			dxDrawText("#71d024" .. (rank.pay or 0) .. " #eaeaea$", x + dxGetTextWidth("Fizetés: ", 1, Roboto16) + resp(5), y - resp(5), 0, 0, tocolor(255, 255, 255, 255 * alphaMultipler), 0.5, gtaFont2, "left", "top", false, false, false, true)

			y = y + Roboto16H

			dxDrawText("Tagok száma ezen a rangon: #71d024" .. (thisRankCount[selectedRank] or 0) .. " #eaeaeafő", x, y, 0, 0, tocolor(255, 255, 255, 255 * alphaMultipler), 1, Roboto16, "left", "top", false, false, false, true)
	...
	if selectedTab == 3 then
		...
		if selectedMember and thisMembers[selectedMember] then
			...
			y2 = drawDataRow(x2, y2, dataRowHeight, 1, "Rang: ", "#71d024" .. thisRanks[member.rank].name .. " (" .. member.rank .. ")", RobotoL14, 1, x, x + listWidthOrig - buttonWidth)
			y2 = drawDataRow(x2, y2, dataRowHeight, 1, "Fizetés: ", "#71d024" .. thisRanks[member.rank].pay .. " #eaeaea$", gtaFont2, 0.5, x, x + listWidthOrig - buttonWidth)
	...
	if selectedTab == 5 then
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			...
			y = y + respc(5)
			drawDataRow(x, y, rowHeight, 1, "Egyenleg: ", "#71d024" .. thousandsStepper(thisGroup.balance, ",") .. " #eaeaea$", gtaFont2, 0.5)
		...
		if meInGroup[selectedGroup].isLeader == "Y" then
			...
			--drawButtonSlider("swatDutyMode", thisGroup.swatDutyMode, x + listWidth - 64, y, rowHeight, {50, 50, 50, 255}, {255, 148, 40, 255})
		end
	end
...
function drawGroupSelector(isAdminMode)
	...
	for i = 1, renderData.visibleGroups do
		...
		if thisGroup and selectedGroup == thisGroup.groupID then
			colorOfRow = tocolor(255, 148, 40, 255 * alphaAnim[3])
	...
	if activeButton == "selectGroup:next" then
		buttonColor = {processColorSwitchEffect("selectGroup:next", {255, 148, 40, 175})}
	else
		buttonColor = {processColorSwitchEffect("selectGroup:next", {255, 148, 40, 125})}
	end
	...
	if button == "left" and state == "down" then
		...
		if activeButton then
			...
			if selected[1] == "setDutySkin" then
				...
				togglePanel(false)
				playSound(":cosmo_assets/audio/admin/notify.ogg")
			...
			elseif selected[1] == "deleteGroup" then
				showError("deleteGroup", "#d75959Biztos, hogy törlöd ezt a frakciót?", ":cosmo_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "acceptItemSettings" and renderData.selectedDutyItem then
				...
				showError("acceptItemSettings", "#7cc576Item adatok sikeresen megváltoztatva.", ":cosmo_assets/audio/admin/notify.ogg")
			...
			elseif selected[1] == "addItem" then
				...
				table.insert(renderData.dutyItems, {
					itemID,
					exports.cosmo_inventory:getItemName(itemID),
					1,
					""
				})
			...
			elseif selected[1] == "addDutyPos" then
				if #renderData.dutyPoses < 8 then
					...
					exports.cosmo_hud:showInfobox("error", "Elérted a maximális duty pozíció számát!")
				end
			elseif selected[1] == "addDutySkin" then
				if selected[2] == "input" then
					...
					if skinID < 0 or skinID > 312 then
						exports.cosmo_hud:showInfobox("error", "Érvénytelen skin ID! Minimum 0 és maximum 312 lehet.")
					else
			...
			elseif selected[1] == "addNewMember" then
				if selected[2] == "input" then
					...
					if string.len(inputText) < 1 then
						...
						playSound(":cosmo_assets/audio/admin/error.ogg")
					else
						local found = false
			...
			elseif selected[1] == "rankRemove" and not errorToDisplay then
				showError(2, "Biztos, hogy törlöd a rangot?", ":cosmo_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "rankRemove_promptAccept" then
				...
				hideError()
				playSound(":cosmo_assets/audio/admin/inmsg.ogg")
			elseif selected[1] == "promptDecline" then
				hideError()
				playSound(":cosmo_assets/audio/admin/outmsg.ogg")
			elseif selected[1] == "newRank" and selectedGroup and #availableGroups[selectedGroup].ranks <= maxGroupRank then
				if getTickCount() - lastNewRankTime >= 1000 then
					...
					exports.cosmo_hud:showInfobox("error", "Ne ilyen gyorsan!")
				end
			elseif selected[1] == "addLeaderToMember" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy hozzáadod a Leaderekhez?", ":cosmo_assets/audio/admin/error.ogg")
			elseif selected[1] == "removeLeaderFromMember" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy eltávolítod a Leaderek közül?", ":cosmo_assets/audio/admin/error.ogg")
			elseif selected[1] == "addLeaderToMember_promptAccept" and selectedMember and groupMembers[selectedGroup][selectedMember] then
				if getTickCount() - lastSetLeaderTick >= 2000 then
					...
					exports.cosmo_hud:showInfobox("error", "Maximum 2 másodpercenként módosíthatod!")
				end
			...
			elseif selected[1] == "removeMemberFromGroup" and not errorToDisplay then
				showError(selected[1], "Biztos, hogy eltávolítod a csoportból?", ":cosmo_assets/audio/admin/error.ogg")
			...
			elseif selected[1] == "groupDesc" then
				if selected[2] == "input" then
					...
					showError(selected[1], "Biztos, hogy felülírod a jelenlegi megjegyzést?", ":cosmo_assets/audio/admin/error.ogg")
				end
...
function onClientKey(key, press)
	if press then
		if key ~= "F3" then
			...
			elseif panelState == "selectGroup:admin" then
				if key == "mouse_wheel_down" and renderData.groupsOffset < availableGroupsCount+1 - renderData.visibleGroups then
					renderData.groupsOffset = renderData.groupsOffset + 1
				elseif key == "mouse_wheel_up" and renderData.groupsOffset > 0 then
					renderData.groupsOffset = renderData.groupsOffset - 1
				end
...
addEventHandler("setErrorDisplay", getRootElement(),
	function (soundType, display, text)
		showError(display, text, ":cosmo_assets/audio/admin/" .. soundType .. ".ogg")
...
function processGroupCreation(state)
	...
	if utf8.len(groupName) < 1 then
		exports.cosmo_hud:showInfobox("error", "A csoport nevének legalább 1 karakterből kell állnia!")
		return
	end

	if utf8.len(groupPrefix) > 10 then
		exports.cosmo_hud:showInfobox("error", "A csoport prefix nem lehet hosszabb 10 karaktertől!")
		return
	end

	if groupArmor > 100 then
		exports.cosmo_hud:showInfobox("error", "A duty armor nem lehet nagyobb 100-nál!")
		return
	end

	if not groupRadio or groupRadio <= 0 then
		exports.cosmo_hud:showInfobox("error", "A védett rádió frekvenciának nagyobbnak kell lennie nullánál!")
		return
	else
		...
		if tuneRadioExists then
			exports.cosmo_hud:showInfobox("error", "A kiválasztott rádió frekvenciát már használja egy másik frakció. (" .. tuneRadioExists.name .. ")")
			return
		end
	end
...
function setGroupCreationValues(data)
	if not data then
		...
		for i = 1, #data.duty.items do
			table.insert(renderData.dutyItems, {
				data.duty.items[i][1],
				exports.cosmo_inventory:getItemName(data.duty.items[i][1]),
				data.duty.items[i][2],
	...
	function ()
		...
		for i = 1, #dutyColShapes do
			...
			if isElement(colShape) then
				...
				local colShapeInt = getElementInterior(colShape)
				local colShapeDim = getElementDimension(colShape)
				...
				if screenX and screenY then
					if 100 >= getDistanceBetweenPoints3D(colShapeX, colShapeY, colShapeZ, localX, localY, localZ) then
						...
						dutyBlip = createPickup(colShapeX, colShapeY, colShapeZ,3, 1275)
						setElementInterior(dutyBlip, colShapeInt)
						setElementDimension(dutyBlip, colShapeDim)
		...
		if dutySkinSelectState then
			...
			dxDrawText("Lapozás: #71d024[<-] és [->] #ffffffKiválasztás: #71d024[ENTER]", 0, 0, screenX, screenY - 200, tocolor(255, 255, 255), 1, RobotoL18, "center", "bottom", false, false, false, true)
		end
	end
)