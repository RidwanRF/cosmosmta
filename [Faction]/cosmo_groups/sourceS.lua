...
addEventHandler("invitePlayerToGroup", getRootElement(),
	function (groupId, sourceGroups, charID, playerOrIsOnline)
		if groupId and sourceGroups and charID then
			iprint(groupId, sourceGroups, charID, playerOrIsOnline)
...
addEvent("setGroupBalancePicsu", true)
addEventHandler("setGroupBalancePicsu", getRootElement(),
	function (groupId, amount)
		if groupId and amount then
			local currentBalance = availableGroups[groupId].balance
	...
	function (datas, mainLeaderElement)
		...
		dbQuery(
			function (qh, creator)
				...
				exports.cosmo_hud:showInfobox(creator, "success", "Frakció sikeresen létrehozva!")
	...
	function (groupID, datas, mainLeaderElement, oldLeaderElement)
		...
		dbQuery(
			function (qh, creator)
				...
				exports.cosmo_hud:showInfobox(creator, "success", "Frakció sikeresen módosítva!")
...
addEventHandler("deleteGroup", getRootElement(),
	function (groupID)
		if groupID then
			...
			exports.cosmo_vehicles:deleteGroupVehicles(groupID)
			...
			exports.cosmo_hud:showInfobox(source, "success", "Frakció sikeresen törölve!")
		end
	end
)
	...
	function (groupId)
		if groupId and availableGroups[groupId] then
			...
			if group then
				...
				if characterId then
					exports.cosmo_inventory:removePlayerDutyItems(source)
					...
					if not getElementData(source, "groupDuty") then
						...
						for k,v in ipairs(group.duty.items) do
							...
							if v[1] == 86 then -- jelvény
								...
								if not exports.cosmo_inventory:hasItemWithData(source, v[1], "data2", badgeId) then
									data1 = group.prefix
									data2 = badgeId
								else
									give = false
							...
							if give then
								exports.cosmo_inventory:giveItem(source, v[1], v[2], data1, data2, "duty")
							end
						end

						setElementData(source, "groupDuty", true)
						exports.cosmo_hud:showInfobox(source, "success", "Sikeresen felvetted a szolgálatot.")
					else
						...
						exports.cosmo_hud:showInfobox(source, "success", "Sikeresen leadtad a szolgálatot.")
					end
				end
			end
		end
...
addEventHandler("onElementModelChange", getRootElement(),
	function (oldModel, newModel)
		if getElementType(source) == "player" and not getElementData(source, "adminDuty") then
			...
			if not modelEnabled then
				print("Ez a skint csak az adott frakció tagjai hordhatják!")
			end
		end
	end
)
...
addCommandHandler("gov", function(player, cmd, ...)
	...
	if now <= lastClickTick[player] + a * 1000 then
		exports.cosmo_hud:showAlert(player, "error", "1 percenként!")
		return
	end
	...
	if groupID then
		if isPlayerLeaderInGroup(player, groupID) then 
			...
			if #msg > 1 then 
				...
				exports.cosmo_core:sendMessageToAdmins(playerName .. " használta a /gov parancsot", 7)
...
exports.cosmo_admin:addAdminCommand("addgroupmember", 6, "Játékos meghívása egy Szervezethez.")
addCommandHandler("addgroupmember", function(sourcePlayer, commandName, targetPlayer, groupID)
    if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
        if not targetPlayer and not groupID then
            outputChatBox("#43b2e6".."Használat: #FFFFFF".."/"..commandName.." [Játékos név / ID] [Group ID]",sourcePlayer,255,255,255,true); 
        else
            targetPlayer = exports.cosmo_core:findPlayer(sourcePlayer, targetPlayer)

            if targetPlayer then
                local playerGroups = getPlayerGroups(targetPlayer)
                local groupID = tonumber(groupID)
                local adminName = getElementData(sourcePlayer, "acc.adminNick")

                if exports.cosmo_groups:isPlayerInGroup(targetPlayer, groupID) then
                    exports.cosmo_hud:showInfobox(sourcePlayer,"error", "A játékos már a Szervezet tagja!");
                    return
                end

                triggerEvent("invitePlayerToGroup", sourcePlayer, groupID, playerGroups, getElementData(targetPlayer, "char.ID"), targetPlayer)
                exports.cosmo_core:sendMessageToAdmins("Egy Adminisztrátor(#32b3ef"..getElementData(sourcePlayer, "acc.adminNick").."#FFFFFF) meghívott egy játékost(#32b3ef"..getPlayerName(targetPlayer).."#FFFFFF) az alábbi frakcióba:  "..tostring(groupID))
                exports.cosmo_logs:toLog("adminaction", "Egy Adminisztrátor(#32b3ef"..getElementData(sourcePlayer, "acc.adminNick").."#FFFFFF) meghívott egy játékost(#32b3ef"..getPlayerName(targetPlayer).."#FFFFFF) az alábbi frakcióba:  "..tostring(groupID))
                exports.cosmo_hud:showInfobox(sourcePlayer,"success", "Sikeresen meghívtad a játékost a szervezetbe!")
            end
        end
    end
end)

exports.cosmo_admin:addAdminCommand("setfactionleader", 6, "Játékos frakcióleader jog adás.")
addCommandHandler("setfactionleader", function(player, cmd, target, groupId)
	if getElementData(player, "acc.adminLevel") < 6 then return end

	if not groupId then
		outputChatBox("[CosmoMTA]: #ffffff/setfactionleader [Név / ID] [Frakció ID]", player, 255, 148, 40, true)
	else
		groupId = tonumber(groupId)
		
		target, targetName = exports.cosmo_core:findPlayer(player, target)
		local charID = getElementData(target, "char.ID")
		if target and getElementData(target, "loggedIn") then
			local qh = dbQuery(reloadGroupDatasForPlayer, connection, "UPDATE groupMembers SET isLeader= 'Y' WHERE groupID=? AND characterID=?", groupId, charID)
			loadAllGroups()
			loadPlayerGroups()
...
exports.cosmo_admin:addAdminCommand("removefactionleader", 6, "Játékos frakcióleader jog elvétel.")
addCommandHandler("removefactionleader", function(player, cmd, target, groupId)
	if getElementData(player, "acc.adminLevel") < 6 then return end

	if not groupId then
		outputChatBox("[CosmoMTA]: #ffffff/removefactionleader [Név / ID] [Frakció ID]", player, 255, 148, 40, true)
	else
		groupId = tonumber(groupId)
		...
		local charID = getElementData(target, "char.ID")
		if target and getElementData(target, "loggedIn") then
			local qh = dbQuery(reloadGroupDatasForPlayer, connection, "UPDATE groupMembers SET isLeader= 'N' WHERE groupID=? AND characterID=?", groupId, charID)
			loadAllGroups()
			loadPlayerGroups()
...
exports.cosmo_admin:addAdminCommand("removegroupmember", 6, "Játékos eltávolítása egy Szervezetből.")
addCommandHandler("removegroupmember", function(sourcePlayer, commandName, targetPlayer, groupID)
    if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
        if not targetPlayer and not groupID then
            outputChatBox("#43b2e6".."Használat: #FFFFFF".."/"..commandName.." [Játékos név / ID] [Group ID]",sourcePlayer,255,255,255,true); 
        else
            ...
            if targetPlayer then
                ...
                if exports.cosmo_groups:isPlayerInGroup(targetPlayer, groupID) then else
                    exports.cosmo_hud:showInfobox(sourcePlayer,"error", "A játékos nem a Szervezet tagja!")
                    return 
                end

                triggerEvent("removePlayerFromGroup", sourcePlayer, groupID, playerGroups, getElementData(targetPlayer, "char.ID"), targetPlayer)
                exports.cosmo_core:sendMessageToAdmins("Egy Adminisztrátor(#32b3ef"..getElementData(sourcePlayer, "acc.adminNick").."#FFFFFF) eltávolított egy játékost(#32b3ef"..getPlayerName(targetPlayer).."#FFFFFF) az alábbi frakcióból: #32b3ef "..tostring(groupID))
                exports.cosmo_logs:toLog("adminaction", "Egy Adminisztrátor(#32b3ef"..getElementData(sourcePlayer, "acc.adminNick").."#FFFFFF) eltávolított egy játékost(#32b3ef"..getPlayerName(targetPlayer).."#FFFFFF) az alábbi frakcióból: #32b3ef "..tostring(groupID))
                exports.cosmo_hud:showInfobox(sourcePlayer,"success", "Sikeresen eltávolítottad a játékost a szervezetből!")
            end
        end
    end
end)

exports.cosmo_admin:addAdminCommand("setfactionduty", 1, "Játékos frakció dutyztatás.")
addCommandHandler("setfactionduty", function(sourcePlayer, cmd, target, groupId)
	if getElementData(sourcePlayer, "acc.adminLevel") < 1 then return end

	if not groupId or not target then
		outputChatBox("[CosmoMTA]: #ffffff/setfactionduty [Név / ID] [Frakció ID]", sourcePlayer, 255, 148, 40, true)
	else
	
		local groupId = tonumber(groupId)
		target = exports.cosmo_core:findPlayer(sourcePlayer, target)
		
		 if exports.cosmo_groups:isPlayerInGroup(target, groupId) then
			triggerEvent("requestDuty", target, groupId)
			print("DUTYYYYYY")
		end
	end			
end)