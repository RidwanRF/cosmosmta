...
laserWidth = 0.7 -- width of the dx line
...
addEventHandler("onClientResourceStop", getRootElement(), function(res)
	if res == getThisResource() then
		outputChatBox("unloaded laserpointer", 124, 197, 118)
...
function ToggleLaserEnabled(cmd)
	...
	--local vane = exports.mta_item:hasItem(localPlayer, 164)
	--if vane then
	
	player = localPlayer
	if IsLaserEnabled(player) == false then	
		SetLaserEnabled(player, true)
		outputChatBox("Lézer bekapcsolva!" ,124, 197, 118)
	else
		SetLaserEnabled(player, false)
		outputChatBox("Lézer kikapcsolva!" ,124, 197, 118)
	end
	--end
end
function ChangeLaserColor(cmd, r,g,b,a)
	...
	if colorPickerInitialized == 1 and getResourceFromName("colorpicker") then
		...
		if r and g and b and a then
			...
			if r and g and b and a then
				if r < 0 or g < 0 or b < 0 or a < 0 or r > 255 or g > 255 or b > 255 or a > 40 then
					outputChatBox("Invalid colors: (0-255) usage: /" ..CMD_LASERCOLOR.. " r g b 40", 124, 197, 118)
					return false
				else
