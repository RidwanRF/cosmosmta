...
function onClientCallAdTwitter(player, ad, number)
	if player and ad and number then
		for i, v in ipairs(getElementsByType("player")) do
			if getElementData(v, "loggedIn") then
				if getElementData(v, "twitterbekapcsolva") then
					outputChatBox ("#1DA1F2[Twitter]: #ffffff" ..ad.. " #1DA1F2((".. getPlayerName(player):gsub("_", " ") .."))",v,0, 233, 58,true)
				end
			end
		end
	end
end
addEvent("onClientCallAdTwitter", true)
addEventHandler("onClientCallAdTwitter", getRootElement(), onClientCallAdTwitter)
