...
setElementData(localPlayer,"twitterbekapcsolva", false)
...
local apps = {
	...
	{"twitter.png", "changepage:twitter", "Twitter"},
}
...
function renderThePhone()
	if phoneState and forceState then
		...
		elseif currentPage == "twitter" then
			if not fakeInputs["hirdetes"] then
				fakeInputs["hirdetes"] = ""
			end
			-- háttér
			...
			-- cím
			dxDrawText("Twitter", phonePosX + resp(21), phonePosY + resp(52), 0, 0, tocolor(255, 255, 255), 0.4, RobotoFontLighter, "left", "top")
			
			local coseColor
			
			if activeButton == "twittKuldes" then
				coseColor = tocolor(29 ,161 ,242, 240)
			else
				coseColor = tocolor(29 ,161 ,242, 220)
			end
			
			local twitterkikapcsColor
			
			if activeButton == "twitterKikapcs" then
				twitterkikapcsColor = tocolor(29 ,161 ,242, 240)
			else
				twitterkikapcsColor = tocolor(29 ,161, 242, 220)
			end
			
			dxDrawRectangle(phonePosX + 20, phonePosY + 230, 240, 30, coseColor)
			buttons["twittKuldes"] = {phonePosX + 20, phonePosY + 230, 240, 30}
			dxDrawText("Tweet", phonePosX + resp(110), phonePosY + resp(230), 0, 0, tocolor(255, 255, 255), 0.4, RobotoFontLighter, "left", "top")
			
			dxDrawRectangle(phonePosX + 20, phonePosY + 230 + 40, 240, 30, twitterkikapcsColor)
			buttons["twitterKikapcs"] = {phonePosX + 20, phonePosY + 230 + 40, 240, 30}
			if getElementData(localPlayer, "twitterbekapcsolva") then
				dxDrawText("Tweetek kikapcsolása", phonePosX + resp(39), phonePosY + resp(230+40+2), 0, 0, tocolor(255, 255, 255), 0.36, RobotoFontLighter, "left", "top")
			else
				dxDrawText("Tweetek bekapcsolása", phonePosX + resp(39), phonePosY + resp(230+40+2), 0, 0, tocolor(255, 255, 255), 0.36, RobotoFontLighter, "left", "top")
			end
...
function commandHandler(command)
	...
	elseif command[1] == "twittKuldes" then
		if getTickCount() - hirdetesTick > 10000 then
			if currentPage == "twitter" then
				if fakeInputs["hirdetes"] == "" then return end
				triggerServerEvent("onClientCallAdTwitter", localPlayer, localPlayer, fakeInputs["hirdetes"], myPhoneNum)
				fakeInputs["hirdetes"] = ""
				activeButton = nil
				hirdetesTick = getTickCount()
			end
		end
	elseif command[1] == "twitterKikapcs" then
		if currentPage == "twitter" then
			if getElementData(localPlayer,"twitterbekapcsolva") then
				setElementData(localPlayer,"twitterbekapcsolva", false)
				outputChatBox("#ff9428[Telefon]:#ffffff Kikapcsoltad a Tweeteket.",0,0,0,true)
				activeButton = nil
			else 
				activeButton = nil
				setElementData(localPlayer,"twitterbekapcsolva", true)
				outputChatBox("#ff9428[Telefon]:#ffffff Bekapcsoltad a Tweeteket.",0,0,0,true)
			end
		end
