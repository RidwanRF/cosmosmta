...
local carRentPed = createPed(1, 1475.2521972656, -1741.7829589844, 13.546875)
setPedRotation(carRentPed, 0)
setElementFrozen(carRentPed, true)
setElementData(carRentPed, "invulnerable", true)
setElementData(carRentPed, "visibleName", "Kevin")
setElementData(carRentPed, "pedNameType", "Jármű Bérlés")
setElementData(carRentPed, "lossantos.carrentped", true)
...
function clickcarRentPedPed(button, state, absX, absY, wx, wy, wz, element)
	if element and getElementType(element) == "ped" and state=="down" and getElementData(element, "lossantos.carrentped") then
...
addEventHandler("onClientClick", getRootElement(), clickcarRentPedPed, true)
...
function renderCarRent()
	if showCarRent then
		...
		dxDrawText("Los Santos Járműbérlő", startX+4, startY+4, startX+30+240+140, startY+3+25, tocolor(255, 255, 255, 255), 1, font1, "center", "center")
		...
		if button == "left" and state == "down" then
				...
				elseif rajtvan(startX+376, startY+4, 20, 25) then
					...
					if kivalasztopanelnyitva then
						...
						--triggerServerEvent("createRentCar", localPlayer, localPlayer)
	...
	if kivalasztopanelnyitva then
		if button == "left" and state == "down" then
			if rajtvan(startX+2, startY+237+4+23, 194, 20) then
				...
				triggerServerEvent("createRentCar", localPlayer, localPlayer,kivalasztotJarmu)
				showCarRent = false
				showChat(true)
				kivalasztopanelnyitva = false
				removeEventHandler("onClientRender", root, kivalasztotRender)
