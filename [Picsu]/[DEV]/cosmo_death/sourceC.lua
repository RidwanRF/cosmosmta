...
local deathCol = {
	{-2.9447159767151-350, -3475.515625, -31.90625, 15000, 3000},
}
local colShape = {}
local indeathShape = false
--local lsbepatkoltel = false

function createdeathCol()
	for i, v in ipairs(deathCol) do
		colShape[i] = createColCuboid(v[1], v[2], v[3]-50, v[4], v[5], 2000)
		setElementData(colShape[i], "deathShape.id", i)
	end
end
...
addEventHandler("onClientColShapeHit", root,
	function(hitPlayer)
		...
		if not tonumber(getElementData(source, "deathShape.id")) then return end
		
		if tonumber(getElementData(source, "deathShape.id")) > 0 then
			for i, v in ipairs(deathCol) do
				if getElementData(source, "deathShape.id") == i then
					if not getElementData (source,"isPlayerDeath") then
						indeathShape = true
					end
				end
			end
...
addEventHandler("onClientColShapeLeave", root,
	function(hitPlayer)
		if hitPlayer ~= localPlayer then return end
		if not tonumber(getElementData(source, "deathShape.id")) then return end
		if getElementData(source, "deathShape.id") > 0 then
			if not getElementData (source,"isPlayerDeath") then
				indeathShape = false
			end
		end
		...
		if source == localPlayer then
			...
			elseif dataName == "loggedIn" and getElementData(localPlayer, "loggedIn") and getElementData(localPlayer, "isPlayerDeath") then
				...
				if getElementData(localPlayer, "player.Cuffed") then
					exports.cosmo_controls:toggleControl({"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
				end
				...
				elseif bloodLevel > 0 and bloodLevel <= 40 then
					if math.random(10) <= 5 then
						if not cameraShake then
							exports.cosmo_hud:showInfobox("warning", "Az alacsony vérszinted miatt elkezdtél szédülni!")
...
function startDeathCutscene(ceremonyEnd)
	...
	if playerId then
		...
		if not ceremonyEnd then
			local vehicleElement = createVehicle(416, 2034.892578125, -1423.3518066406, 16.536323547363, 0, 0, 180, "-R.I.P.-")
		...
		if deathCutscene then
			...
			elseif deathCutscene.ceremonyEnd and currentTick >= deathCutscene.ceremonyEnd then
				...
				if elapsedTime >= 10000 then
					...
					preDeathTimer = setTimer(
						function ()
							if lsbepatkoltel then
								triggerServerEvent("spawnToHospital", localPlayer)
							else
								triggerServerEvent("spawnToHospitalSF", localPlayer)
							end
						end,
					5000, 1)
				end 
	...
	function ()
		...
		if currentHealth <= 20 and currentHealth > 0 then
			...
			if not inAnim then
				...
				screenFadeEffect = {getTickCount(), 0, 160}
			end
			...
			if endAnimTimer then
				...
				if preDeath then
					...
					deathText = "" .. string.format("%02d:%02d", minute, second) .. ""
				end

				dxDrawBorderText(deathText, 0, screenY - 125, screenX, 0, tocolor(215, 89, 89), 1, Roboto, "center", "top")
			end
		elseif inAnim then
			inAnim = false
			screenFadeEffect = false
	...
	function(sourcePlayer)
		if getTickCount() - lastTryTick >= 1000 then
			...
			if getElementHealth(sourcePlayer) <= 20 and getElementHealth(sourcePlayer) > 0 then
				...
				-- van defi -> sokkolás
				if exports.cosmo_inventory:hasItem(90) then
				-- nincs defi -> mellkas kompresszió
					triggerEvent("cosmo_medicC:playerHeal", localPlayer, sourcePlayer)
				else
					outputChatBox("nincs item")

				end
			else
				exports.cosmo_hud:showAlert("error", "A kiválasztott játékos nem halott vagy már nem lehet megmenteni!")
			end
		end
	end)
			...
			if damages > 0 then
				...
				for k, v in pairs(bulletDamages) do
					...
					elseif damageType == "cut" then
						exports.cosmo_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg vágás van a testén!")
						return
					elseif tonumber(damageType) >= 25 and tonumber(damageType) <= 27 then
						exports.cosmo_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg sörétek vannak a testében!")
						return
					else
						exports.cosmo_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg golyó vannak a testében!")
						return
					end
				end

				if (getElementData(sourcePlayer, "bloodLevel") or 100) ~= 100 then
					exports.cosmo_hud:showAlert("error", "Addig nem gyógyíthatod meg amíg nem állítod el a vérzést!")
					return
				end
				]]

				-- van eset táska
				if exports.cosmo_inventory:hasItem(108) then
					
				else
					exports.cosmo_hud:showAlert("error", "Nincs nálad eset táska, ezért nem tudod ellátni a sérültet!")
				end
			else
				exports.cosmo_hud:showAlert("error", "A kiválasztott játékos nincs megsérülve!")
			end
		end
	end)
...
function abortAllStealthKills(targetPlayer)
   --[[ for i, v in ipairs(getElementsByType("ped")) do
    ...
    for i, v in ipairs(getElementsByType("player")) do
		if targetPlayer == v then
			if getElementData(v, "char.nodmg") then
				cancelEvent()
			end
		end
	end]]
	...
	if (attacker) then
		if (getElementType(attacker) == "player") then 
			if getWeaponNameFromID(weapon) == "Explosion" then
				...
				allapot = "Fegyver: " .. weaponNames(weapon)
			end
		...
		elseif (getElementType(attacker) == "vehicle") then
			...
			killog = "[" .. hours .. ":" .. minutes .. "] " .. targetName(getVehicleController(attacker)):gsub("_"," ") .. " elütötte " .. targetName(source):gsub("_"," ") .. " játékost. (Járművel: " .. allapot .. ")"
		elseif (getElementType(attacker) == "ped") then 
			allapot = "( PET )"
			killog = "[" .. hours .. ":" .. minutes .. "] ".. (getElementData(attacker, "ped:name") or "Ismeretlen") .. " "..allapot .. " megölte " .. targetName(source):gsub("_"," ") .. " játékost."
		end
	else
		killog = "[" .. hours .. ":" .. minutes .. "] " .. targetName(source):gsub("_", " ") .. " meghalt."
	end
	triggerServerEvent("killLog", localPlayer, killog)
end
