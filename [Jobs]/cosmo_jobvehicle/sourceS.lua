	...
	function (vehicle)
		...
		if vehicleId then
			...
			if spawnerName == myName then
				...
				exports.cosmo_hud:showInfobox(source, "info", "Beszálltál a munkajárművedbe.")
			end
		end
	end)
		...
		if not forcedByScript then
			...
			if vehicleId then
				...
				if spawnerName == myName then
					...
					exports.cosmo_hud:showInfobox(source, "info", "Kiszálltál a munkajárművedből. Ha nem szállsz vissza, a jármű 15 percen belül törlődik!")
				end
			end
		end
	end)
...
addEventHandler("createJobVehicle", getRootElement(),
	function (modelId, colorCode)
		if isElement(source) then
			...
			if not modelId then
				exports.cosmo_hud:showInfobox(source, "info", "Sikeresen leadtad a munkajárműved.")
			...
			elseif isElement(jobVehicles[source]) then
				exports.cosmo_hud:showInfobox(source, "error", "Már van egy munkajárműved!")
			else
				...
				if characterJob > 0 then
					...
					if isElement(vehicleElement) then
						...
						if vehicleId then
							...
							setElementData(vehicleElement, "vehicle.fuel", 100)
							setElementData(vehicleElement, "vehicle.GPS", 1)
							setElementData(vehicleElement, "vehicle.jobID", characterJob)
							characterId = getElementData(source, "char.ID")
							setElementData(vehicleElement, "vehicle.dbID", -characterId)
							...
							outputChatBox("#ff9428[CosmoMTA]: #ffffffSikeresen lehívtál egy munkajárművet. Amennyiben le szeretnéd adni, hozd ide.", source, 0, 0, 0, true)
