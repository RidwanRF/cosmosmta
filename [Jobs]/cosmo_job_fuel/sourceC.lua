		...
		if activeJob then
			...
			if startDestination then
				if finalDestination then
					...
					setElementData(startBlip, "blipIcon", "cp")
					setElementData(startBlip, "blipColor", tocolor(255, 0, 140))
					...
					setElementData(finalBlip, "blipIcon", "cp")
					setElementData(finalBlip, "blipColor", tocolor(193, 11, 11))
			...
			outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffElkezdted a munkát! Töltsd fel a tartályt a " .. startDestination.name .." telepen, majd vidd el a megjelölt helyre.", 255, 255, 255, true)
			--outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffJelenlegi úticél: #ff9428" .. finalDestination.name, 255, 255, 255, true)
		end
	end
)
...
function startTheJob()
	...
	if not theVehicle then
		exports.cosmo_hud:showInfobox("error", "A munkát csak " .. exports.cosmo_mods_veh:getVehicleNameFromModel(truckModelId) .. " típusú kocsival tudod elkezdeni.")
		return
	end
	...
	if getElementData(theVehicle, "jobWithCar") then
		exports.cosmo_hud:showInfobox("error", "Ezzel a kocsival már valaki dolgozik.")
		return
	end
...
function endJob(isRestore, isStartRequest)
	...
	if not isStartRequest then
		for i, v in ipairs(jobStartPositions) do
			...
			setElementData(blipElement, "blipColor", tocolor(193, 11, 11))
							...
							if getDistanceBetweenPoints3D(playerX, playerY, playerZ, offsetX, offsetY, offsetZ) <= 1.15 then
								...
								if guiX and guiY then
									...
									if isInSlot(guiX, guiY, guiSize, guiSize) and getKeyState("mouse1") then
										...
										if boundingBoxCheck then
											if getElementData(localPlayer,"hasLine") then
												if exports.cosmo_boneattach:isElementAttachedToBone(handOBJ) then
													setElementData(localPlayer,"hasLine", false)
													if isElement(handOBJ) then
														destroyElement(handOBJ)
													end
												else
													outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffAmíg töltődik a tartály, addig nem tudod visszarakni!", 255, 255, 255, true)
												end
											else
												handOBJ = createObject(1974, 0, 0, 0)
												setElementAlpha(handOBJ,0)
												setObjectScale(handOBJ,0.001)
												exports.cosmo_boneattach:attachElementToBone(handOBJ, localPlayer, 12, -0.03, 0.02, 0.08, 0, 240, 0)
												setElementData(localPlayer,"hasLine", true)
											end
										else
											outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffNem vagy a kijelölt helyen, ezért nem tudod levenni a csövet.", 255, 255, 255, true)										
										end
									end
								end
							end
					...
					if isElement(handOBJ) then
						if exports.cosmo_boneattach:isElementAttachedToBone(handOBJ) then
							if getElementData(myActiveJob.vehicle,"fuelTank") < 100 then
								exports.cosmo_boneattach:detachElementFromBone(handOBJ,localPlayer)
								...
								setTimer(function()
									if getElementData(myActiveJob.vehicle,"fuelTank") < 75 then
										...
										exports.cosmo_boneattach:attachElementToBone(handOBJ, localPlayer, 12, -0.03, 0.02, 0.08, 0, 240, 0)
										outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffSikeresen feltöltötted a tartályt. Tedd vissza a csövet, majd vidd le a fuvart..", 255, 255, 255, true)
									end
								end,10000,4);
							else
								outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffA tartály már tele van.", 255, 255, 255, true)
							end
						end
					end
							...
							if getElementData(myActiveJob.vehicle,"fuelTank") >= 25 then
								...
								setElementData(myActiveJob.vehicle,"fuelTank",0)
									setTimer(function()
										...
										local finalmoney = (math.random(1000000,1500000)*3)
										outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffSikeresen feltöltötted a tartályt. Jutalmad: '".. finalmoney .."$'", 255, 255, 255, true)
										exports.cosmo_core:giveMoney(localPlayer,finalmoney)
										endJob(false,false)
										outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffHa szeretnél még dolgozni, akkor menj vissza a telepre egy új fuvarért.", 255, 255, 255, true)
									end,45000,1);
							else
								outputChatBox("#ff9428[CosmoMTA - Üzemanyag szállító]: #ffffffNincs elég üzemanyag a tartályban.", 255, 255, 255, true)
							end
						end
					end
				end
...
function isInSlot(posX, posY, sizeX, sizeY)
    return exports.cosmo_core:isInSlot(posX, posY, sizeX, sizeY)
end