...
local maleSkins = exports.cosmo_binco:getSkinsByType("Férfi")
local femaleSkins = exports.cosmo_binco:getSkinsByType("Női")

local jobPedElement = createPed(67, 2336.0998535156, -1996.294921875, 13.88413143158, 180)
...
setPedAnimation(jobPedElement, "COP_AMBIENT", "Coplook_think", -1, true, false, false)
				...
				if getElementModel(source) == 498 then
					if getElementData(localPlayer, "char.Job") == jobID then
						if not deliveryPackages then
							outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffMenj #ff9428Frank#ffffff-hez, és vedd fel a rendeléseket. (Jobbklikk az NPC-n)", 255, 255, 255, true)
							
							if not isElement(jobPedBlip) then
								local x, y, z = getElementPosition(jobPedElement)
								jobPedBlip = createBlip(x, y, z)
								setElementData(jobPedBlip, "blipIcon", "cp")
								setElementData(jobPedBlip, "blipTooltipText", "Frank")
								setElementData(jobPedBlip, "blipColor", tocolor(255, 148, 40))
							end
						end
					end
				end
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) <= 3 then
							if not deliveryPackages and not requestJobStart then
								...
								if isPedInVehicle(localPlayer) then
									outputChatBox("#d75959[CosmoMTA - Postás]: #ffffffElőbb szállj ki a járművedből.", 255, 255, 255, true)
									return
								end

								if not jobVehicle or getElementModel(jobVehicle) ~= 498 then
									outputChatBox("#d75959[CosmoMTA - Postás]: #ffffffElőbb kérj egy furgont a #ff9428munkajármű#ffffff ikonnál.", 255, 255, 255, true)
									return
								end
...
addEventHandler("onPostmanJobStarted", localPlayer,
	function (recipientsNumber, deliveryPositions, packageType, packageIdentifier)
		...
		for i = 1, recipientsNumber do
			...
			local markerElement = createMarker(position[1], position[2], position[3], "checkpoint", 2, 255, 148, 40)
			local zoneName = getZoneName(position[1], position[2], position[3])

			local blipElement = createBlip(position[1], position[2], position[3])

			setElementData(blipElement, "blipIcon", "cp")
			setElementData(blipElement, "blipColor", tocolor(255, 148, 40))
		...
		exports.cosmo_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, false)

		pdaFont = dxCreateFont("files/pda.ttf", respc(20), false, "antialiased")
		addEventHandler("onClientRender", getRootElement(), renderPDA)

		outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffFelvetted a küldeményeket. Tedd be a kocsiba a szállítmányt. (Jobbklikk az autó hátuljára)", 255, 255, 255, true)
		outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffA #ff9428címzetteket#ffffff a térképen és a #ff9428PDA-n#ffffff látod.", 255, 255, 255, true)
		outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffA PDA-n a címzettek között a nyilakkal tudsz navigálni.", 255, 255, 255, true)
		outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffAz autód csomagtartóját a jobbklikk segitségével nyithatod meg.", 255, 255, 255, true)
	end
)
...
addEventHandler("postDeliveredSuccessfully", getRootElement(),
	function (packageId, remainingPackages)
		...
		exports.cosmo_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, true)
...
function endTheJob(forced)
	if deliveryPackages then
		...
		if not forced then
			...
			outputChatBox("#ff9428[CosmoMTA - Postás]: #ffffffLeszállítottad az összes küldeményt. Új kör megkezdéséhez menj #ff9428Frank#ffffff-hez.", 255, 255, 255, true)
		else
			if isElement(jobPedBlip) then
				destroyElement(jobPedBlip)
			end
			...
			if string.find(activeButton, "get_out:") then
				...
				if packageIdentifier then
					if postCarInventory[packageIdentifier] then
						...
						exports.cosmo_chat:sendLocalMeAction(localPlayer, "kivett egy " .. (packageType == 1 and "levelet" or "csomagot") .. " a kocsiból.")
					else
						return
					end
				end
...
function renderInventory()
	...
	-- ** Cím
	dxDrawRectangle(x + 3, y + 3, sx - 6, respc(32) - 6, tocolor(0, 0, 0, 150))
	dxDrawText("#ff9428CosmoMTA#ffffffMTA - Postakocsi", x + 4, y + 3 + (respc(32) - 6) / 2, nil, nil, tocolor(200, 200, 200, 200), 0.85, Roboto, "left", "center", false, false, false, true)
	...
	if empty then
		...
		for k in pairs(postCarInventory) do
			...
			drawButton3(buttonName, "Kivesz", buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], {255, 148, 40}, false, Roboto)
			y = y + respc(32)
		end
	end
				...
				if #dataValue == 1 then
					if dataValue[1][1] == 1 then
						...
						if isElement(objectElement) then
							...
							exports.cosmo_boneattach:attachElementToBone(objectElement, source, 12, 0, 0.05, 0, 270, 0, 0)
						...
						if isElement(objectElement) then
							...
							exports.cosmo_boneattach:attachElementToBone(objectElement, source, 12, 0.2, 0.1, 0.15, 0, 90, 70)
							table.insert(carriedObjects[source], objectElement)
						end
					end
				else
					...
					if isElement(objectElement) then
						...
						exports.cosmo_boneattach:attachElementToBone(objectElement, source, 12, 0.25, 0.3, 0.15, -100, 0, -20)
						table.insert(carriedObjects[source], objectElement)
					end
					...
					if isElement(objectElement) then
						...
						exports.cosmo_boneattach:attachElementToBone(objectElement, source, 3, 0, -0.3, 0, 0, 0, 180)
						table.insert(carriedObjects[source], objectElement)
					end
				end
			else
