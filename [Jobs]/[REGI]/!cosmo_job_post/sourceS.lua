								...
								if getElementData(thePlayer, "char.Job") == jobID then
									...
									if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) <= 3 then
										...
										if getElementData(source, "vehicle.jobID") ~= jobID then
											outputChatBox("#d75959[CosmoMTA - Postás]: #ffffffEz nem munkajármű.", thePlayer, 255, 255, 255, true)
											return
										end

										if isPedInVehicle(thePlayer) then
											outputChatBox("#d75959[CosmoMTA - Postás]: #ffffffElőbb szállj ki a járművedből.", thePlayer, 255, 255, 255, true)
											return
										end

										if jobVehicle ~= source then
											outputChatBox("#d75959[CosmoMTA - Postás]: #ffffffEz nem a te munkajárműved.", thePlayer, 255, 255, 255, true)
											return
										end
										...
										if carryingMail then
											...
											if postCarInventory then
												...
												if #carryingMail == 1 then
													...
													exports.cosmo_chat:localAction(thePlayer, "berakott egy küldeményt az autóba.")
												else
													setPedAnimation(thePlayer, "carry", "putdwn", -1, false, false, false, false)
													exports.cosmo_controls:toggleControl(thePlayer, {"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, true)
													exports.cosmo_chat:sendLocalMeAction(thePlayer, "berakott néhány küldeményt az autóba.")
												end
											end
										else
	...
	function (postCarElement, packageIdentifier, packageType)
		if isElement(source) then
			...
			if postCarInventory then
				...
				exports.cosmo_controls:toggleControl(thePlayer, {"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, false)
			end
		end
	end
)
						...
						if colData and colData[1] == thePlayer then
							...
							if carryingMail[1] then
								...
								if carryingMail[1][2] == currentJob.packageIdentifiers[deliveryId] then
									...
									exports.cosmo_core:giveMoney(thePlayer, 125000, "postDelivered")
									...
									exports.cosmo_hud:showInfobox(thePlayer, "success", "Sikeresen leszállítottad a küldeményt és kerestél 1250$-t!")
								else
									exports.cosmo_hud:showInfobox(thePlayer, "error", "Ez a küldemény másnak van címezve!")
								end
							end
						end
					end
