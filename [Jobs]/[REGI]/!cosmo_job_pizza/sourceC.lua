...
local jobPedElement = createPed(155, 2109.0607910156, -1790.6820068359, 13.56875038147, 0)
				...
				if getElementModel(source) == 448 then
					if getElementData(localPlayer, "char.Job") == jobID then
						if not pizzaOrders then
							outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffMenj #ff9428Joe#ffffff-hoz, és vedd fel a rendeléseket. (Jobbklikk az NPC-n)", 255, 255, 255, true)
						...
						if animControls then
							animControls = false
							exports.cosmo_controls:toggleControl({"steer_forward", "steer_back"}, animControls)
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) <= 3 then
							if not pizzaOrders and not requestJobStart then
								...
								if isPedInVehicle(localPlayer) then
									outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffElőbb szállj ki a járművedből.", 255, 255, 255, true)
									return
								end

								if not jobVehicle or getElementModel(jobVehicle) ~= 448 then
									outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffElőbb kérj egy robogót a #F0E76Cmunkajármű#ffffff ikonnál.", 255, 255, 255, true)
									return
								end
...
addEventHandler("onPizzaJobStarted", localPlayer,
	function (recipients, positions, pizzas)
		...
		for i = 1, recipients do
			...
			local markerElement = createMarker(position[1], position[2], position[3] + 1.5, "arrow", 0.3, 255, 148, 40)
		...
		outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffFelvetted a rendeléseket. Tedd be a motorba a pizzákat. (Jobbklikk a motoron)", 255, 255, 255, true)
		outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffA #ff9428rendeléseket#ffffff a térképen látod.", 255, 255, 255, true)
		outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffVidd a #ff9428megrendelőhöz#ffffff a pizzát.", 255, 255, 255, true)
		outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffA robogó csomagtartóját a jobbklikk segitségével nyithatod meg.", 255, 255, 255, true)
	end
)
			...
			if string.find(activeButton, "get_out:") then
				...
				if pizzaType then
					if pizzaBoyInventory[pizzaType] and pizzaBoyInventory[pizzaType] > 0 then
						...
						exports.cosmo_chat:sendLocalMeAction(localPlayer, "kivett egy " .. pizzaTypes[pizzaType] .. "-t a robogóból.")
					else
						exports.cosmo_hud:showInfobox("error", "Ebből a fajta pizzából nincs a motorodban!")
						return
					end
				end
			end
...
function renderInventory()
	...
	--dxDrawImage(x, y + 32, sx, sy - 32, ":cosmo_hud/files/images/vin.png")

	-- ** Cím
	dxDrawRectangle(x, y, sx, 32, tocolor(0, 0, 0, 200))
	dxDrawText("Pizzaboy", x + 4, y, x + sx, y + 32, tocolor(255, 148, 40), 1, Roboto, "left", "center")
	...
	for i = 1, #pizzaTypes do
		...
		if activeButton == buttonName then
			dxDrawRectangle(buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], tocolor(255, 148, 40))
		else
			dxDrawRectangle(buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], tocolor(255, 148, 40, 165))
		end
...
function endTheJob(forced)
	if pizzaOrders then
		...
		if not forced then
			...
			outputChatBox("#d75959[CosmoMTA - Pizzafutár]: #ffffffLeszállítottad az összes pizzát. Új rendelések felvételéhez menj #ff9428Joe#ffffff-hoz.", 255, 255, 255, true)
		else
			if isElement(jobPedBlip) then
				destroyElement(jobPedBlip)
			end
				...
				if #carriedObjects[source] < #dataValue then
					for i = #carriedObjects[source] + 1, #dataValue do
						...
						if isElement(objectElement) then
							...
							exports.cosmo_boneattach:attachElementToBone(objectElement, source, 12, 0.15 + 0.015 * (i - 1), 0 + 0.055 * (i - 1), 0.15, -100, 0, -20)
