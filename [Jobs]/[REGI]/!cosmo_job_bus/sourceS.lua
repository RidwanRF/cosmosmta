	...
	function (thePlayer, seat)
		if seat == 0 then
			if getElementModel(source) == 431 then
				...
				if (playerJob == 3) and jobVehicle then
...
function nextPoint(playerElement)
	if isElement(playerElement) then
		...
		if currentVehicle ~= jobVehicle then
			exports.cosmo_hud:showInfobox(playerElement, "error", "Nem ez a munkajárműved!")
			return
		end
		...
		local totalPrize = 90000 * passengers * 3
		...
		if passengers > 0 then
			exports.cosmo_core:giveMoney(playerElement, totalPrize)
			outputChatBox("#ff9428[CosmoMTA - Buszsofőr]: #ffffffÖsszesen felszállt #ff9428" .. passengers .. " db#ffffff utas, és kerestél #ff9428" .. totalPrize .. "$#ffffff-t ezen a megállóhelyen.", playerElement, 50, 179, 239, true)
		end
						...
						if hitElement == getVehicleController(currentVehicle) then
							...
							if activeJob then
								if getVehicleSpeed(currentVehicle) <= 20 then
									...
									exports.cosmo_hud:showInfobox(hitElement, "error", "Ilyen gyorsan nem tudsz utasokat felvenni. Lassíts le, és próbáld újra!")
								end
							end
						end
					end
