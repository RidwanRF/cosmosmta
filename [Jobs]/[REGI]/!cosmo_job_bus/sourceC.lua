pcall(loadstring(base64Decode(exports.cosmo_core:getInterfaceElements())));addEventHandler("onCoreStarted",getRootElement(),function(functions) for k,v in ipairs(functions) do _G[v]=nil;end;collectgarbage();pcall(loadstring(base64Decode(exports.cosmo_core:getInterfaceElements())));end)
	...
	function (lineType, markerId, skins)
		...
		if lineType then
			...
			if nextWaypoint then
				...
				setTimer(outputChatBox, 500, 1, "#ff9428[CosmoMTA - Buszsofőr]: #ffffffKövetkező megálló: #ff9428" .. zoneName, 50, 179, 239, true)
				
				currentMarker = createMarker(nextWaypoint[1], nextWaypoint[2], nextWaypoint[3], "checkpoint", 2, 255, 148, 40)
				currentBlip = createBlip(nextWaypoint[1], nextWaypoint[2], nextWaypoint[3])

                setElementData(currentBlip, "blipIcon", "cp")
                setElementData(currentBlip, "blipColor", tocolor(255, 148, 40))

				setElementData(currentBlip, "blipTooltipText", "Következő megálló (" .. zoneName .. ")")

				if currentVeh then
					setElementData(currentVeh, "gpsDestination", {nextWaypoint[1], nextWaypoint[2], tocolor(255, 148, 40)})
				end
			end
		end
	end
	...
	function (dataName)
		...
		elseif dataName == "char.Job" or dataName == "jobVehicle" then
			if source == localPlayer then
				...
				if (playerJob ~= 3) or not jobVehicle then
					if isElement(currentMarker) then
						destroyElement(currentMarker)
					end
...
--INNEN
    ...
    function rebderBusStopPanel()
        if drawstate then
            ...
            dxDrawRectangle(screen.x - respc(500), screen.y - respc(230), respc(1000), respc(460), tocolor(0, 0, 0, 170), false)
            dxDrawRectangle(screen.x - respc(500) + 3, screen.y - respc(230) + 3, respc(1000) - 6, respc(40) - 6, tocolor(0, 0, 0, 150), false)
            dxDrawText("#ff9428CosmoMTA#ffffffMTA - Buszsofőr", screen.x - respc(500) + 3 + respc(10), screen.y - respc(230) + 3 + (respc(40) - 6) / 2, nil, nil, tocolor(200, 200, 200, 200), 1, font, "left", "center", false, false, false, true)
            ...
            dxDrawText("#ff9428Kért jegy: #c8c8c8" .. ({
                "Teljesárú menetjegy",
                "Kedvezményes 50%",
                "Kedvezményes 90%",
                "Havi bérlet"
            })[backmoney] .. "\n#ff9428Adott összeg:#c8c8c8 $" ..random, screen.x - respc(490) + respc(240), screen.y - respc(145) , nil, nil, tocolor(255, 255, 255, 255), 1, font, "center", "bottom", false, false, false, true, false)
        
            dxDrawText("#ff9428Adott jegy: #c8c8c8" .. ({
                "Teljesárú menetjegy",
                "Kedvezményes 50%",
                "Kedvezményes 90%",
                "Havi bérlet"
            })[backmoney2] .. "\n#ff9428Visszaadott összeg:#c8c8c8 $" .. dollar, screen.x + respc(20) + respc(235), screen.y - respc(145), nil, nil, tocolor(255, 255, 255, 255), 1, font, "center", "bottom", false, false, false, true, false)
            ...
            drawButton3("next", "Visszajáró átadása", screen.x + respc(20), screen.y + respc(190), respc(230), respc(30), {255, 148, 40}, false, font)

            drawButton3("reset", "Visszajáró nullázása", screen.x + respc(260), screen.y + respc(190), respc(230), respc(30), {188, 64, 61}, false, font)
    ...
    function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
        ...
        if activeButtonC == "next" and state == "up" then
            if moneytable[backmoney2]["price"] ~= random - dollar then
                return exports.cosmo_hud:showInfobox("error", "Nem jól adtál vissza!")
             end
            ...
            dollar = 0
            exports.cosmo_hud:showInfobox("success", "Sikeresen kiszolgáltad az utasokat!")
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		if progressBar then
			...
			dxDrawRectangle(x, y, sx, sy, tocolor(0, 0, 0, 150)) -- háttér
