...
function()
	txd = engineLoadTXD( "files/model/stick.txd" )
	dff = engineLoadDFF( "files/model/stick.dff" )
										...
										if waterHit then
											if isLineOfSightClear(cameraPosX, cameraPosY, cameraPosZ, waterPosX, waterPosY, waterPosZ, true, false, false, true, false) then
												...
												elseif distanceBetweenPoints <= 5 then
													exports.cosmo_hud:showInfobox("error", "Ilyen közelre nem tudod eldobni!")
												else
													exports.cosmo_hud:showInfobox("error", "Ilyen messzire nem tudod eldobni!")
												end
											end
										else
											exports.cosmo_hud:showInfobox("error", "Itt nem tudod használni!")
										end
									end
								end
							end
	...
	if rodThrowed then
		if not minigameTick and not balanceMinigame.preStart then
			if math.random(5) == 2 then
				...
				exports.cosmo_hud:showInfobox("info", "Készülj, mert kapásod van!")
			end
		end
	end
end
	...
	function (waterPosX, waterPosY, waterPosZ)
		if isElement(source) then
			...
			if isElement(rodObjectElement) then
				...
				playerFloaters[source] = createObject(1974, rodEndPosX, rodEndPosY, rodEndPosZ)
					...
					elseif isElementStreamedIn(objectElement) then
						if playerElement == localPlayer then
							...
							if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, throwPosition[1], throwPosition[2], throwPosition[3]) >= 20 then
								exports.cosmo_hud:showInfobox("error", "Nem kéne ilyen messzire menni!")
...
addEventHandler("onClientRender", getRootElement(),
	function ()
					...
					if bonePosX and bonePosY and bonePosZ then
						...
						if distanceBetweenPoints <= 15 then
							...
							if screenPosX and screenPosY then
								...
								if v.itemId then
									dxDrawImage(screenPosX, screenPosY - 85, imageWidth, imageHeight, ":cosmo_inventory/files/items/" .. v.itemId .. ".png", 0, 0, 0, tocolor(255, 255, 255, imageAlpha))
								else
									dxDrawImage(screenPosX, screenPosY - 80, imageWidth, imageHeight, "files/images/notification.png", 0, 0, 0, tocolor(0, 0, 0, imageAlpha))
								end
							end
						end
					end
		...
		if minigameInProgress then
			dxDrawText("Állítsd meg a jobb oldali, mozgó csíkot a #ff9428zöld#ffffff mezőben, a #ff9428SPACE#ffffff gomb megnyomásával!", 0, screenY - 200, screenX, screenY, tocolor(255, 255, 255), 1, Roboto, "center", "top", false, false, false, true)
		elseif balanceMinigame.preStart then
			dxDrawText("Tartsd a felső #ff9428zöld#ffffff nyílt középen, a #598ed7balra#ffffff és #598ed7jobbra nyíl#ffffff gombok nyomogatásával!", 0, screenY - 200, screenX, screenY, tocolor(255, 255, 255), 1, Roboto, "center", "top", false, false, false, true)
		end
	end
)
...
function stopMinigame()
	if minigameTick then
		...
		if math.abs(minigameProgress - 0.5) * 2 <= 0.25 then
			...
			exports.cosmo_hud:showInfobox("success", "Siker! Kezdd el a hal kifárasztását!")
			outputChatBox("#ff9428[CosmoMTA - Horgászat]: #FFFFFFSiker! Kezdd el a hal kifárasztását!", 255, 255, 255, true)
			...
			exports.cosmo_hud:showInfobox("error", "Sajnos a hal leakadt a horogról!")
			outputChatBox("#ff9428[CosmoMTA - Horgászat]: #FFFFFFSajnos a hal #d75959leakadt#ffffff a horogról!", 255, 255, 255, true)
		end
	end
end
...
function balanceMinigameFail()
	...
	if math.random(10) < 5 then
		...
		outputChatBox("#ff9428[CosmoMTA - Horgászat]: #FFFFFFA horgászbotodon #d75959elszakadt#ffffff a damil!", 255, 255, 255, true)
	else
		outputChatBox("#ff9428[CosmoMTA - Horgászat]: #FFFFFFA hal #d75959meglógott#ffffff!", 255, 255, 255, true)
	end

	triggerServerEvent("endFishing", localPlayer, false)

	exports.cosmo_hud:showInfobox("error", "Sajnos nem sikerült kifognod a halat.")
	outputChatBox("#ff9428[CosmoMTA - Horgászat]: #FFFFFFSajnos nem sikerült kifognod a halat.", 255, 255, 255, true)
end
...
function balanceMinigameSuccess()
	...
	if itemId then
		triggerServerEvent("endFishing", localPlayer, itemId, exports.cosmo_inventory:getItemName(itemId))
	else
		triggerServerEvent("endFishing", localPlayer, false)
	end
end
	...
	for key, value in ipairs(fishIds) do
		if hasPlayerFish(value[1]) then
			if hasFish(value[1]) >= 1 then
				...
				triggerServerEvent("giveMoneyFish",localPlayer,localPlayer,cost)
			end
		end
	end
end
