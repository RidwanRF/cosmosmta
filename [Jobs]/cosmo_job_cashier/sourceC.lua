...
function createFont()
    lcd15 = dxCreateFont("files/fonts/digital7.ttf", respc(15), false, "antialiased")
    sans17 = dxCreateFont("files/fonts/opensans.ttf", respc(17), false, "antialiased")
end

function destroyFont()
    if isElement(lcd15) then
        destroyElement(lcd15)
        lcd15 = false
    end

    if isElement(sans17) then
        destroyElement(sans17)
        sans17 = false
    end
end
...
function renderJobPanel()
    ...
    if renderButtons then 
        ...
        dxDrawRectangle(sx / 2 - 300 / 2, sy / 2 - 150 / 2, 300, 150, tocolor(0, 0, 0, 170))
        dxDrawRectangle(sx / 2 - 300 / 2 + 3, sy / 2 - 150 / 2 + 3, 300 - 6, 30, tocolor(0, 0, 0, 180))
        dxDrawText("CosmoMTA - Pénztáros", sx / 2 - 300 / 2 + 3 + 5, sy / 2 - 150 / 2 + 3 + 30 / 2, nil, nil, tocolor(255, 255, 255, 255), 0.8, sans17, "left", "center")

        nextColor = {60, 60, 60, 40}

        if activeButtonC == "nextCustomer" then
            nextColor = {255, 148, 40, 140}
        end

        drawButton3("nextCustomer", "Tovább", sx / 2 - 300 / 2 + 10, sy / 2 - 150 / 2 + 36 + 10, 300 - 20, 40, nextColor, false, sans17, true)
        ...
        drawButton3("exitPanel", "Kilépés", sx / 2 - 300 / 2 + 10, sy / 2 - 150 / 2 + 36 + 10 + 40 + 10, 300 - 20, 40, closeColor, false, sans17, true)
        --dxDrawRectangle(, tocolor(0, 0, 0))
    ...
    dxDrawText(buyedItemPrice .. " $", sx*0.873, sy*0.44, sx*0.873+sx*0.09, sy*0.44+sy*0.045, tocolor(255, 255, 255, 255*alpha), 0.9/myX*sx, lcd15, "center", "center")
        ...
        for k, v in ipairs(shopedItems) do 
            if isInSlot(grid[5][1], grid[5][2], grid[5][3], grid[5][4]) then 
                if isInSlot(lastItemPosition + ((k-1)*250/myX*sx) + buyableItems[v[1]][1] /myX*sx, sy*0.715 + buyableItems[v[1]][2] /myX*sx, sx*0.03, sy*0.03) then 
                    if v[6] then 
                        buyedItemPrice = buyedItemPrice + buyableItems[v[1]][3] * 3
...
function startPedMovement(state)
    if state == 1 then 
        ...
        setElementDimension(movePed, 999 + getElementData(localPlayer, "char.ID"))
...
--     dxDrawShadowedText("Nyomd le az "..color.."[E] #ffffffbillentyűt a belépéshez.", 0, sy*0.89, sx, sy*0.89+sy*0.05, tocolor(255, 255, 255, 255), tocolor(0, 0, 0, 255), 0.9/myX*sx, sans17, "center", "center", false, false, false, true)
-- end
...
function warpToShop()
    ...
    setElementDimension(localPlayer, 999 + getElementData(localPlayer, "char.ID"))
    ...
    setElementDimension(outMarker, 999 + getElementData(localPlayer, "char.ID"))
    ...
    setElementDimension(jobStartMarker, 999 + getElementData(localPlayer, "char.ID"))

    pedStopCol = createColTube(-21.196887969971, -138.61964416504, 1002.5468755, 1.3, 3)
    setElementInterior(pedStopCol, 16)
    setElementDimension(pedStopCol, 999 + getElementData(localPlayer, "char.ID"))

    pedDelCol = createColTube(-14.912612915039, -138.51348876953, 1002.546875, 1, 3)
    setElementInterior(pedDelCol, 16)
    setElementDimension(pedDelCol, 999 + getElementData(localPlayer, "char.ID"))
...
addEventHandler("onClientMarkerHit", root, function(player, mdim)
    if (player == localPlayer and mdim) then 
        ...
        if getElementData(source, "isCashierJobMarker") then 
            ...
            exports.cosmo_hud:showInfobox("info", "Nyomd le az [E] billentyűt a belépéshez.")
...
function createJobElements()
    for k, v in ipairs(markerPositions) do 
        ...
        setElementData(blip, "blipIcon", "shop_h")
...
addEventHandler("onPlayerQuitTakeFromInterior", getRootElement(),
    function()
        if inInterior then 
            warpOutFromInterior()
