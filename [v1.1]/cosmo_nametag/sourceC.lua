...
local playerCanSeeYourself = false -- DEBUG ONLY
...
function createFonts()
	RobotoFont = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 15, false, "antialiased")
	RobotoFont2 = exports.cosmo_assets:loadFont("Roboto-Regular.ttf", 10, false, "antialiased")
	FontAwesome = exports.cosmo_assets:loadFont("FontAwesome.ttf", 18, false, "antialiased")
end
	...
	function (dataName, oldValue)
		if getElementType(source) == "player" then
			...
			if source == localPlayer then
				if dataName == "char.painted" then
					if not isTimer(paintProcessTimer) then
						paintProcessTimer = setTimer(paintProcess, 60000, 0)
					end	
...
addCommandHandler("togname", function(cmd)
    if playerCanSeeYourself then
        playerCanSeeYourself = false
        if not playerCanSeeYourself then exports.cosmo_hud:showInfobox("info", "Kikapcsoltad a saját neved megjelenítését.") end
    else
        playerCanSeeYourself = true
        if playerCanSeeYourself then exports.cosmo_hud:showInfobox("success", "Bekapcsoltad a saját neved megjelenítését.") end
    end

    for k,v in ipairs(getElementsByType("player", getRootElement(), true)) do
        streamedPlayers[v] = nil
        if v == localPlayer and not playerCanSeeYourself then
        elseif not streamedPlayers[v] then
            processPlayerNametag(v)
        end
    end
end)
...
function paintProcess()
	if getElementData(localPlayer, "loggedIn") then
		local painted = getElementData(localPlayer, "char.painted") or "N"
		local paintTime = getElementData(localPlayer, "char.paintTime") or 0
		
		if painted ~= "N" then
			if paintTime - 1 <= 0 then
				setElementData(localPlayer, "char.painted", "N")
				outputChatBox("#32b3ef[CosmoMTA - ATM]: #FFFFFFLekopott a #32b3effesték #ffffffaz arcodról.", 255, 255, 255, true)

			else
				setElementData(localPlayer, "char.paintTime", paintTime - 1)
			end
		end
	end
end
...
addEventHandler("onClientElementDataChange", getResourceRootElement(),
	function (dataName, oldValue, newValue)
		if dataName == "adminDuty" then
			if not newValue or newValue == true then
				adminNames = false
			else
				adminNames = true
			end
...
function processPlayerNametag(player, dataName, dataValue)
	if not dataName and not dataValue then
		streamedPlayers[player] = {
			...
			adminNick = getElementData(player, "acc.adminNick") or (getElementData(player, "visibleName") or getPlayerName(player)):gsub("_", " "),
			adminDuty = getElementData(player, "adminDuty"),
			helperLevel = getElementData(player, "acc.helperLevel") or 0,
			helperDuty = getElementData(player, "helperDuty"),
			...
			inAnimTime = getElementData(player, "inAnimTime") or 300000,
			iPlayer = player
		}

		if streamedPlayers[player].adminDuty then
			streamedPlayers[player].adminBadgeColor = exports.cosmo_core:getAdminLevelColor(streamedPlayers[player].adminLevel)
			streamedPlayers[player].adminRankName = exports.cosmo_core:getPlayerAdminTitleByLevel(streamedPlayers[player].adminLevel)
		end

		if streamedPlayers[player].helperLevel then
			streamedPlayers[player].helperBadgeColor = exports.cosmo_core:getHelperLevelColor(streamedPlayers[player].helperLevel)
			streamedPlayers[player].helperRankName = exports.cosmo_core:getPlayerHelperTitleByLevel(streamedPlayers[player].helperLevel)
		end
	else
		...
		elseif dataName == "acc.helperLevel" then
			streamedPlayers[player].helperLevel = dataValue
			streamedPlayers[player].helperBadgeColor = exports.cosmo_core:getHelperLevelColor(streamedPlayers[player].helperLevel)
			streamedPlayers[player].helperRankName = exports.cosmo_core:getPlayerHelperTitleByLevel(streamedPlayers[player].helperLevel)
		elseif dataName == "acc.adminNick" then
			streamedPlayers[player].adminNick = dataValue or (getElementData(player, "visibleName") or getPlayerName(player)):gsub("_", " ")
...
--	function ()
--		playerCanSeeYourself = not playerCanSeeYourself
--		outputChatBox(tostring(playerCanSeeYourself))
--		--processPlayerNametag(localPlayer)
--	end
--)


--exports.cosmo_admin:addAdminCommand("anames", 1, "Adminisztrátori nevek be/ki kapcsolása")
	...
	--function ()
		--if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			--if not adminNames then
				...
				--outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Sikeresen #ffff99bekapcsoltad #ffffffaz adminisztrátori neveket.", 255, 255, 255, true)
			--else
				--adminNames = false
				--setElementData(localPlayer, "adminNames", tostring(adminNames), false)
				--outputChatBox(exports.cosmo_core:getServerTag("admin") .. "Sikeresen #ff4646kikapcsoltad #ffffffaz adminisztrátori neveket.", 255, 255, 255, true)
			--end
		--end
	--end)
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		if adminNames and getElementData(localPlayer, "acc.adminLevel") < 1 then 
			adminNames = false
			setElementData(localPlayer, "adminNames", tostring(adminNames), false)
		end 
					...
					if headPosX and headPosY then
						...
						if distance <= visibleDistance then
							...
							if progress < 1 then
								...
								local isPlayerHelper = getElementData(k, "acc.helperLevel") or 0

								if (isPlayerHelper >= 1) then
									local helperBadge = v.helperBadgeColor .. "(" .. v.helperRankName .. ")"
									
									text = helperBadge .. " " .. text
								end

							

								local glueData = getElementData(v.iPlayer,"gluetxt") or ""

								if (getElementData(v.iPlayer, "char.painted") or "N") == "N" then
									text = text .. adminNameColor .. " (" .. v.playerId .. ")"..glueData
								else
									text = text .. adminNameColor .. " (" .. v.playerId .. ")"..glueData.."\n#bfff00*Festékes az arca*#FFFFFF"
								end
								...
								if textWidth then
									...
									elseif playerHealth <= 20 and playerHealth > 0 then
										local timeLeft = (300000 - (v.inAnimTime[1] or 0)) / 1000
									...
									elseif v.badgeData then
										...
										--textWidth = dxGetTextWidth(utf8.gsub(text, "#%x%x%x%x%x%x", ""), fontScale, RobotoFont)
										textPosX = headPosX - textWidth * 0.5

										--dxDrawText(utf8.gsub(text, "#%x%x%x%x%x%x", ""), textPosX + 1, headPosY + 1 + fontHeight, 100, 100, blackColor, fontScale, RobotoFont, "left", "top", false, false, false, false, true)
								...
								if v.isAfk and v.startAfk and v.startAfk > 0 then
									...
									dxDrawText(text, headPosX - textWidth * 0.5 + 1, headPosY - fontHeight + 1, 100, 100, blackColor, fontScale, RobotoFont, "left", "top", false, false, false, false, true)
									dxDrawText(text, headPosX - textWidth * 0.5, headPosY - fontHeight, 100, 100, tocolor(175, 175, 175), fontScale, RobotoFont, "left", "top", false, false, false, false, true)
								end
								...
								if v.isTyping then
									dxDrawImage(iconPosX, iconPosY - 25, iconSize, iconSize, "files/typing.png")
									iconPosX = iconPosX + iconSize
								elseif v.isConsoling then
									dxDrawImage(iconPosX, iconPosY - 25, iconSize, iconSize, "files/consoling.png")
									iconPosX = iconPosX + iconSize
								end

								if v.cuffed then
									dxDrawImage(iconPosX, iconPosY - 25, iconSize, iconSize, "files/cuffed.png")
									iconPosX = iconPosX + iconSize
								end

								if v.tazed then
									dxDrawImage(iconPosX, iconPosY - 25, iconSize, iconSize, "files/tazed.png")
									iconPosX = iconPosX + iconSize
								end
								...
								if distance <= 30 then
									...
									if sizeForIcons > 0 then
										picturePosY = picturePosY - iconSize * 0.55
									end

									if v.adminLevel > 0 and v.adminDuty then
										if v.adminLevel == 9 then
										...
										elseif v.adminLevel == 8 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/cmlogo.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 11 then
										...
										elseif v.adminLevel == 10 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/rendszergazda.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										...
										elseif v.adminLevel == 1 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/kisadmin.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 2 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/kisadmin.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 3 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/kisadmin.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 4 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/kisadmin.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 5 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/kisadmin.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										elseif v.adminLevel == 6 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/adminlogo.png", 0, 0, 0, tocolor(255, 255, 255, alpha))
										end
									elseif v.helperDuty then
										if v.helperLevel == 1 or v.helperLevel == 2 then
											dxDrawImage(picturePosX, picturePosY, pictureSizeX, pictureSizeY, "files/helperlogo.png", 0, 0, 0, tocolor(255, 255, 255, alpha))	
										end
									end
								end
								...
								if messageBubbles[k] and distance <= 10 then
									...
									for k2, v2 in pairs(messageBubbles[k]) do
										...
										roundedRectangle(textPosX - 5, textPosY - 5, textWidth + 10, fontHeight + 10, tocolor(0, 0, 0, 90 * interpolate), false, true)
										--dxDrawRectangle(textPosX - 5, textPosY + fontHeight + 4, textWidth + 10, 1, tocolor(255, 148, 40, 150 * interpolate), false, true)
										dxDrawText(prefix .. v2[1], textPosX, textPosY, textPosX + textWidth, textPosY + fontHeight, tocolor(255, 255, 255, 255), fontScale, RobotoFont2, "center", "center", false, false, false, false, true)
