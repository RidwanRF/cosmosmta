...
addEventHandler("onClientResourceStart",root,function(res)
    if getResourceName(res) == "cosmo_gate_engine" or getThisResource(res) then 
...
function preRender(dt)
    if isElement(pObject) then 
        ...
        e:shadowedText("Mozgatás",sx/2-130,sy-220,sx/2-130+210,140,tocolor(255,255,255),1,font2,"center");
        ...
        e:shadowedText("Fel/Le",sx/2+90,sy-220,sx/2+90+60,140,tocolor(255,255,255),1,font2,"center");
        ...
        if e:isInSlot(sx-210,sy-90,200,30) then 
            ...
            if getKeyState("mouse1") and clickTick+700 < getTickCount() then 
                ...
                outputChatBox(e:getServerSyntax("Gate","servercolor").."Sikeresen beállítottad a nyitási pozíciót.",255,255,255,true);
                clickTick = getTickCount();
            end
        end
        dxDrawRectangle(sx-210,sy-90,200,30,openColor);
        dxDrawBorder(sx-210,sy-90,200,30,3,tocolor(0,0,0,200));
        e:shadowedText("Nyitási Pozíció",sx-210,sy-90,sx-210+200,sy-90+30,tocolor(255,255,255),1,font3,"center","center");
        ...
        if e:isInSlot(sx-210,sy-50,200,30) then 
            ...
            if getKeyState("mouse1") and clickTick+700 < getTickCount() then 
                ...
                outputChatBox(e:getServerSyntax("Gate","servercolor").."Sikeresen beállítottad a zárási pozíciót.",255,255,255,true);
                clickTick = getTickCount();
            end
        end
        dxDrawRectangle(sx-210,sy-50,200,30,closeColor);
        dxDrawBorder(sx-210,sy-50,200,30,3,tocolor(0,0,0,180));
        e:shadowedText("Zárási Pozíció",sx-210,sy-50,sx-210+200,sy-50+30,tocolor(255,255,255),1,font3,"center","center");
        -----------------
        ...
        if e:isInSlot(sx-210,sy-130,200,30) then 
            ...
            if getKeyState("mouse1") and clickTick+700 < getTickCount() then 
                if not pos["open"] or not pos["close"] then 
                    outputChatBox(e:getServerSyntax("Gate","red").."Nem állíottad be a nyitási/zárási pozíciót.",255,255,255,true);
                else 
        ...
        e:shadowedText("Mentés",sx-210,sy-130,sx-210+200,sy-130+30,tocolor(255,255,255),1,font3,"center","center");
...
addCommandHandler("creategate",function()
    if getElementData(localPlayer,"acc.adminLevel") >= 6 then 
        if editing then 
            ...
            pos = {};
            outputChatBox(e:getServerSyntax("Gate","servercolor").."Kapu lerakás bekapcsolva!",255,255,255,true);
        end
    end
end);
...
function closeGateCreate()
    ...
    outputChatBox(e:getServerSyntax("Gate","red").."Kapu lerakás kikapcsolva!",255,255,255,true);
end
...
if getElementData(localPlayer,"acc.adminLevel") >= 6 then 
    ...
    if nearbyGate then 
        ...
        outputChatBox(e:getServerSyntax("Gate","servercolor").."Közeli kapuk bekapcsolva.",255,255,255,true);
    else 
        removeEventHandler("onClientRender",root,renderNearbyGate);
        outputChatBox(e:getServerSyntax("Gate","red").."Közeli kapuk kikapcsolva.",255,255,255,true);
    end
end
end
