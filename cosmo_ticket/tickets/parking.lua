	...
	if isEdit then
		...
		if signStart then
			...
			if progress > 1.25 then
				...
				exports.cosmo_chat:sendLocalMeAction(localPlayer, "kitépi a bírságot a füzetből.")	
			end
		end
	else
		...
		if inEdit then
			if cursorInBox(ticketX + respc(188), ticketY + ticketH - inputH - respc(30), respc(152), inputH) then
				...
				if utf8.len(inputValues["location|-|35"]) < 4 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Név\" rész kevesebb, mint 4 karakter")
					return
				end

				if utf8.len(inputValues["reason|-|35"]) < 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Indok\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["type|-|35"]) < 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Jármű Típusa\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["numberplate|-|8"]) < 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Rendszám\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["agency|-|35"]) < 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Hatóság\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["money|num-only|6"]) < 1 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Bírság Összege\" rész kevesebb, mint 1 karakter")
					return
				end

				if utf8.len(inputValues["date|-|16"]) < 7 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Büntetés Dátuma\" rész kevesebb, mint 7 karakter")
					return
				end
				...
				if not theVehicle then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A jármű nem található vagy nincs a közeledben.")
					return
				end

				exports.cosmo_chat:sendLocalMeAction(localPlayer, "elkezd megírni egy csekket.")
				...
				if isElement(theVehicle) then
					...
					exports.cosmo_hud:showAlert("error", "A megadott rendszámhoz nincs társított jármű!")
				end
			end
		end
	end
