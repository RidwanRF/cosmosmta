		...
		if inEdit then
			if cursorInBox(ticketX + respc(355 + 7 + 147), ticketY + respc(291), respc(180), inputH) then
				...
				if utf8.len(inputValues["reason|-|40"]) <= 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Szabálysértés/Vétség\" rész kevesebb, mint 5 karakter")
					return
				end

				if utf8.len(inputValues["officer|-|28"]) <= 5 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Kiállító személy\" rész kevesebb, mint 5 karakter")
					return
				end
				...
				if utf8.len(inputValues["date|-|28"]) <= 7 then
					exports.cosmo_hud:showAlert("error", "Nem megfelelő kitöltés", "A \"Kiállítás időpontja\" rész kevesebb, mint 7 karakter")
					return
				end
				...
				if targetPlayer == localPlayer then
					exports.cosmo_hud:showAlert("error", "Magadnak nem állíthatsz ki csekket!")
					return
				end

				if not targetPlayer then
					exports.cosmo_hud:showInfobox("error", "A \"Név\" résznél beírt személy nincs a közeledben!")
					return
				end
